#!/usr/bin/env python3
"""Build and run the FOR-364 independent arc Rec.2020 F16 Skia reference source."""
...
SKIA_ROOT = Path(os.environ.get("KANVAS_FOR364_UPSTREAM_SKIA_ROOT", DEFAULT_SKIA_ROOT))
SKIA_OUT = Path(os.environ.get("KANVAS_FOR364_UPSTREAM_SKIA_OUT", SKIA_ROOT / "out/Release"))
...
def ensure_inputs() -> None:
    require_file(SOURCE, "FOR-364 source")
...
def normalize_command(command: list[str], binary: Path) -> list[str]:
    binary_text = str(binary)
    return ["<temporary-binary>" if item == binary_text else item for item in command]
...
def write_provenance(
    ...
    provenance: dict[str, Any] = {
        ...
        "sceneId": SCENE_ID,
        "sourceType": "isolated-skia-independent-arc-rec2020-f16-for364-butt-cap",
        "sourceImplementation": rel(SOURCE),
        "sourceSha256": sha256(SOURCE),
        "dimensions": reference["dimensions"],
        "command": " ".join(command),
        "commandArgv": command,
        "buildCommand": normalize_command(build_command, binary),
        "executeCommand": normalize_command(execute_command, binary),
        "headless": True,
        ...
        "sourceTypeProof": {
            ...
            "executedBinary": True,
            "isolatedIndependentArcOutput": True,
        ...
        "sceneCoordinateBranchAccepted": False,
        "fixtureBranchAccepted": False,
...
def main() -> None:
    ...
    try:
        ensure_inputs()
        with tempfile.TemporaryDirectory(prefix="kanvas-for364-skia-") as tmp:
            binary = Path(tmp) / "f16_independent_comparable_arc_evidence"
            build_command = compile_command(binary)
            build = run(build_command)
            if build.returncode != 0:
                raise BuildMissing("FOR-364 source compile failed against upstream Skia:\n" + build.stdout + build.stderr)

            execute_command = [str(binary), str(REFERENCE_JSON), str(REFERENCE_PNG)]
            execution = run(execute_command)
            if execution.returncode != 0:
                raise BuildMissing(
                    "FOR-364 source execution failed against upstream Skia:\n"
                    + execution.stdout
                    + execution.stderr
                )
            ...
            write_provenance(command, build_command, execute_command, binary)
    except BuildMissing as exc:
        REFERENCE_JSON.unlink(missing_ok=True)
        REFERENCE_PNG.unlink(missing_ok=True)
        PROVENANCE.unlink(missing_ok=True)
        print(f"F16_INDEPENDENT_COMPARABLE_ARC_EVIDENCE_PARTIAL: {exc}", file=sys.stderr)
        raise SystemExit(2)

    print("F16_INDEPENDENT_COMPARABLE_ARC_EVIDENCE_SKIA_REFERENCE_CAPTURED")
