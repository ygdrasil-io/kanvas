// Repo-owned upstream Skia non-arc Rec.2020 F16 reference source for FOR-358.
...
static constexpr Sample kSamples[] = {
        {"for358_background_top_left", 0, 0},
        {"for358_rect_center", 14, 14},
        {"for358_rect_left_inside", 5, 14},
        {"for358_rect_bottom_inside", 14, 20},
};
...
static bool write_sample_json(const char* outputPath, const SkPixmap& pixmap) {
    ...
    output.writeText("{\n");
    output.writeText("  \"sceneId\": \"f16-real-additional-non-arc-row-for358\",\n");
    output.writeText("  \"sourceType\": \"isolated-skia-non-arc-rec2020-f16-for358-src-over-rect\",\n");
    output.writeText("  \"dimensions\": {\"width\": 40, \"height\": 28},\n");
...
static bool write_reference(const char* jsonPath, const char* pngPath) {
    ...
    green.setStyle(SkPaint::kFill_Style);
    green.setBlendMode(SkBlendMode::kSrcOver);
    green.setColor(SkColorSetARGB(160, 0, 192, 64));
...
int main(int argc, char** argv) {
    const char* jsonPath = argc > 1 ? argv[1] : "f16-real-additional-non-arc-row-for358-skia.json";
    const char* pngPath = argc > 2 ? argv[2] : "f16-real-additional-non-arc-row-for358-skia.png";
