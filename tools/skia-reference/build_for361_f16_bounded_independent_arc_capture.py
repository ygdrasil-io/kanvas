#!/usr/bin/env python3
"""Build and run the FOR-361 bounded independent arc Rec.2020 F16 Skia reference source."""
...
SKIA_ROOT = Path(os.environ.get("KANVAS_FOR361_UPSTREAM_SKIA_ROOT", DEFAULT_SKIA_ROOT))
SKIA_OUT = Path(os.environ.get("KANVAS_FOR361_UPSTREAM_SKIA_OUT", SKIA_ROOT / "out/Release"))
...
def ensure_inputs() -> None:
    require_file(SOURCE, "FOR-361 source")
...
def write_provenance(
    ...
    provenance: dict[str, Any] = {
        ...
        "sceneId": SCENE_ID,
        "sourceType": "isolated-skia-bounded-independent-arc-rec2020-f16-for361-round-cap",
...
def main() -> None:
    ...
    try:
        ensure_inputs()
        with tempfile.TemporaryDirectory(prefix="kanvas-for361-skia-") as tmp:
            binary = Path(tmp) / "f16_bounded_independent_arc_capture"
            build_command = compile_command(binary)
            build = run(build_command)
            if build.returncode != 0:
                raise BuildMissing("FOR-361 source compile failed against upstream Skia:\n" + build.stdout + build.stderr)

            execute_command = [str(binary), str(REFERENCE_JSON), str(REFERENCE_PNG)]
            execution = run(execute_command)
            if execution.returncode != 0:
                raise BuildMissing(
                    "FOR-361 source execution failed against upstream Skia:\n"
                    + execution.stdout
                    + execution.stderr
                )
        ...
        REFERENCE_PNG.unlink(missing_ok=True)
        PROVENANCE.unlink(missing_ok=True)
        print(f"F16_BOUNDED_INDEPENDENT_ARC_CAPTURE_PARTIAL: {exc}", file=sys.stderr)
        raise SystemExit(2)

    print("F16_BOUNDED_INDEPENDENT_ARC_CAPTURE_SKIA_REFERENCE_CAPTURED")
