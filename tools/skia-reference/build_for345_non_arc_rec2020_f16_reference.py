#!/usr/bin/env python3
"""Build and run the FOR-345 non-arc Rec.2020 F16 Skia reference source."""
...
SKIA_ROOT = Path(os.environ.get("KANVAS_FOR345_UPSTREAM_SKIA_ROOT", DEFAULT_SKIA_ROOT))
SKIA_OUT = Path(os.environ.get("KANVAS_FOR345_UPSTREAM_SKIA_OUT", SKIA_ROOT / "out/Release"))
...
SOURCE_FINDING = (
    ...
    "for-344-broader-non-arc-f16-color-evidence-partial-finding"
)
...
def ensure_inputs() -> None:
    require_file(SOURCE, "FOR-345 source")
...
def write_provenance(command: list[str], build_command: list[str], execute_command: list[str]) -> None:
    ...
    provenance: dict[str, Any] = {
        ...
        "sceneId": SCENE_ID,
        "sourceType": "isolated-skia-non-arc-rec2020-f16-src-over-rect",
...
def main() -> None:
    ...
    try:
        ensure_inputs()
        with tempfile.TemporaryDirectory(prefix="kanvas-for345-skia-") as tmp:
            binary = Path(tmp) / "non_arc_rec2020_f16_reference"
            build_command = compile_command(binary)
            build = run(build_command)
            if build.returncode != 0:
                raise BuildMissing("FOR-345 source compile failed against upstream Skia:\n" + build.stdout + build.stderr)

            execute_command = [str(binary), str(REFERENCE_JSON), str(REFERENCE_PNG)]
            execution = run(execute_command)
            if execution.returncode != 0:
                raise BuildMissing(
                    "FOR-345 source execution failed against upstream Skia:\n"
                    + execution.stdout
                    + execution.stderr
                )
        ...
        REFERENCE_PNG.unlink(missing_ok=True)
        PROVENANCE.unlink(missing_ok=True)
        print(f"F16_NON_ARC_REC2020_REFERENCE_ROW_PARTIAL_REQUIRES_SKIA_REFERENCE_SOURCE: {exc}", file=sys.stderr)
        raise SystemExit(2)

    print("F16_NON_ARC_REC2020_REFERENCE_ROW_SKIA_REFERENCE_CAPTURED")
