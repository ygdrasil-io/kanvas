// Repo-owned upstream Skia selected-cell renderer source for FOR-326.
// Intended to be built in a real upstream Skia checkout by the follow-up ticket.

#include "include/core/SkCanvas.h"
#include "include/core/SkColor.h"
#include "include/core/SkColorSpace.h"
#include "include/core/SkImageInfo.h"
#include "include/core/SkPaint.h"
#include "include/core/SkPixmap.h"
#include "include/core/SkRefCnt.h"
#include "include/core/SkRect.h"
#include "include/core/SkStream.h"
#include "include/core/SkSurface.h"
#include "include/encode/SkPngEncoder.h"

int main(int argc, char** argv) {
    const char* outputPath = argc > 1 ? argv[1] : "skia.png";

    const SkImageInfo imageInfo =
            SkImageInfo::MakeN32Premul(80, 80, SkColorSpace::MakeSRGB());
    sk_sp<SkSurface> surface = SkSurfaces::Raster(imageInfo);
    if (!surface) {
        return 1;
    }

    SkCanvas* canvas = surface->getCanvas();
    canvas->clear(SK_ColorTRANSPARENT);

    const SkRect arcRect = SkRect::MakeLTRB(20, 20, 60, 60);

    SkPaint red;
    red.setAntiAlias(true);
    red.setStyle(SkPaint::kStroke_Style);
    red.setStrokeWidth(15);
    red.setStrokeCap(SkPaint::kButt_Cap);
    red.setColor(SkColorSetARGB(100, 255, 0, 0));

    SkPaint blue = red;
    blue.setColor(SkColorSetARGB(100, 0, 0, 255));

    canvas->drawArc(arcRect, 0, 90, false, red);
    canvas->drawArc(arcRect, 0, -270, false, blue);

    SkPixmap pixmap;
    if (!surface->peekPixels(&pixmap)) {
        return 1;
    }

    SkFILEWStream output(outputPath);
    if (!output.isValid()) {
        return 1;
    }

    SkPngEncoder::Options options;
    return SkPngEncoder::Encode(&output, pixmap, options) ? 0 : 1;
}
