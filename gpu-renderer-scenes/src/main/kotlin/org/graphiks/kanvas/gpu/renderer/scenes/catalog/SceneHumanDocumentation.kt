...
object GPURendererSceneHumanDocs {
    val docs: List<SceneHumanDocs> = listOf(
        ...
        doc("custom-runtime-effect-valid-tile", "Enregistre un shader WGSL personnalise produisant une couleur unie, affiche un pave avec ce shader et verifie la sortie GPU.", "Valide CustomRuntimeEffectTile avec wgslSource et uniformSchema personnalises.", "Ne revendique pas les runtime effects enregistres ni la compilation SkSL dynamique.", "Preuve WebGPU offscreen avec sortie GPU verifiee."),
        doc("custom-runtime-effect-unregistered-refusal", "Tente d'executer un effet personnalise jamais enregistre. Doit refuser avec un diagnostic stable.", "Valide le refus stable CustomRuntimeEffectTile avec customEffectId null.", "Ne revendique pas l'execution runtime effect non enregistree.", "Preuve WebGPU offscreen avec diagnostic de refus."),
        ...
        doc("path-star-gradient", "Remplir une etoile non-convexe via stencil-cover avec degrade lineaire.", "Valide PathFillGradient avec chemin etoile et degrade ambre vers vert.", "Ne revendique pas stencil-cover GPU natif ni activation produit.", "Preuve WebGPU offscreen avec sortie PNG pour la scene."),
        ...
        doc("savelayer-isolated", "Rendre un groupe translucide via offscreen saveLayer.", "Valide SaveLayer avec cible isolee, rendu enfant et composite srcOver.", "Ne revendique pas saveLayer general, filtres, destination-read ou activation produit.", "Preuve WebGPU offscreen avec execution saveLayer."),
        doc("savelayer-group-alpha", "Rendre un saveLayer a group alpha 0.5 avec deux enfants opaques qui se chevauchent.", "Valide l isolation de couche reelle: la region de chevauchement se compose a 50% uniforme comme le reste.", "Ne revendique pas saveLayer general, filtres, destination-read ou activation produit.", "Preuve WebGPU offscreen avec parite GPU/CPU sur le chevauchement."),
        doc("dst-read-strategy", "Verifier une strategie destination-read par copie ou intermediate.", "Valide FillRect avec lecture de destination via copie et bind intermediate.", "Ne revendique pas framebuffer fetch, input attachment ou fallback CPU.", "Preuve WebGPU offscreen avec strategie dst-read."),
        doc("blur-radius-ladder", "Verifier un echelon de rayons de flou gaussien.", "Valide quatre rectangles avec blur radii croissants via separable gaussian.", "Ne revendique pas blur texture output ni kernel CPU.", "Preuve WebGPU offscreen avec scene blur."),
        doc("color-matrix-filter", "Verifier trois transformations couleur via matrice 4x5 WGSL.", "Valide identity, saturation et hue-rotate via color matrix filter.", "Ne revendique pas color matrix filter produit ni pipeline GPU separe.", "Preuve WebGPU offscreen avec scene color matrix."),
        doc("glyph-atlas-strip", "Verifier un echantillonnage atlas glyph A8.", "Valide TextRun avec glyph route a8-atlas et diagnostic rect.", "Ne revendique pas atlas glyph A8 executable ni rendu glyph texture.", "Preuve WebGPU offscreen avec scene text."),
        doc("sdf-glyph-scale", "Verifier trois tailles de glyphe SDF avec smoothstep.", "Valide TextRun avec glyph route sdf-atlas pour 3 tailles de fonte.", "Ne revendique pas atlas SDF executable ni rendu glyph texture.", "Preuve WebGPU offscreen avec scene text."),
        doc("runtime-effect-uniform", "Verifier quatre tuiles SimpleRT avec uniforms gColor differents.", "Valide RuntimeEffectTile SimpleRT avec variation d uniforms et route registered descriptor.", "Ne revendique pas compilation SkSL dynamique, SpiralRT ou runtime effects enfants.", "Preuve WebGPU offscreen et Kadre windowed."),
        doc("runtime-effect-child", "Verifier une tuile SimpleRT avec reference a un shader enfant.", "Valide RuntimeEffectTile avec label child-effect-tile et descriptor registered simple_rt.", "Ne revendique pas enfant runtime effect actif ni pipeline child compose.", "Preuve WebGPU offscreen et Kadre windowed."),
        doc("stroke-cap-join", "Verifier quatre styles de cap et join d expansion stroke.", "Valide StrokeExpander avec butt, square, miter et bevel via FillRect proxy.", "Ne revendique pas stroke expansion GPU native ni activation produit.", "Preuve WebGPU offscreen avec scene stroke."),
        doc("dash-pattern-ladder", "Verifier quatre motifs dash via decomposition d intervalles.", "Valide DashPathEffect avec shorts, mediums, longs et alternes via FillRect proxy.", "Ne revendique pas dash path effect GPU natif ni activation produit.", "Preuve WebGPU offscreen avec scene dash."),
        doc("tile-mode-strip", "Verifier quatre bandes avec modes tile clamp, repeat, mirror et decal.", "Valide BitmapRect avec intentions de tile mode et echantillonnage nearest/linear.", "Ne revendique pas tile modes GPU natifs ni echantillonnage texture executable.", "Preuve WebGPU offscreen avec scene tile-mode."),
        doc("vertices-color-mesh", "Rendre un maillage colore via per-vertex colors.", "Valide VerticesExecutor avec triangle list, vertex colors et MeshRibbon scene.", "Ne revendique pas DrawVertices GPU natif ni activation produit.", "Preuve WebGPU offscreen avec scene vertices."),
        doc("mesh-ribbon-depth", "Rendre trois rubans qui se chevauchent avec ordre de profondeur.", "Valide MeshRibbon avec trois couches superposees et ordre paintOrder.", "Ne revendique pas mesh batching GPU natif ni activation produit.", "Preuve WebGPU offscreen avec scene mesh."),
        doc("performance-budget-review", "Verifier les budgets performance par famille de draw.", "Valide PerformanceBudgetEvaluator avec seuils pass, warning et fail.", "Ne revendique pas release-blocking ni activation produit.", "Preuve WebGPU offscreen avec scene budget."),
        doc("pipeline-cache-telemetry-review", "Verifier la telemetrie cache pipeline par scene.", "Valide GPUPipelineCacheTelemetry avec hit rate, eviction et module count.", "Ne revendique pas cache readiness movement ni activation produit.", "Preuve WebGPU offscreen avec scene telemetry."),
        doc("frame-gate-m23-baseline", "Verifier la politique frame gate M23 60fps/30fps.", "Valide M23 baseline avec cibles 60fps et avertissement 30fps.", "Ne revendique pas release-blocking gate ni activation produit.", "Preuve WebGPU offscreen avec scene frame-gate."),
        doc("pm-evidence-m23-bundle", "Verifier le bundle evidence PM final M23.", "Valide M23PMEvidenceBundle avec familles activees et gates verts.", "Ne revendique pas readiness movement ni activation produit.", "Preuve WebGPU offscreen avec scene PM bundle."),
        doc("performance-gates-product-flag", "Verifier le flag produit performanceGatesEnabled.", "Valide performanceGatesEnabled flag et sa propriete systeme disable.", "Ne revendique pas release-blocking ni activation produit.", "Preuve WebGPU offscreen avec scene product flag."),
        doc("stroke-rect-outline", "Verifier un contour de rectangle stroke bleu simple.", "Valide Stroke avec strokeColor, strokeWidth et pathKind bounded-rect-path.", "Ne revendique pas path AA ni joins/caps complexes.", "Preuve WebGPU offscreen avec scene stroke."),
        doc("gaussian-blur-photo", "Verifier un flou gaussien applique a un rectangle colore.", "Valide GaussianBlur via passes gaussiennes separables sur un FillRect.", "Ne revendique pas blur texture output ni kernel CPU.", "Preuve WebGPU offscreen avec scene blur."),
        doc("color-matrix-tint", "Verifier une teinte couleur via matrice 4x5 WGSL.", "Valide ColorMatrix filter avec transformation sur un FillRect vert.", "Ne revendique pas color matrix filter produit ni pipeline GPU separe.", "Preuve WebGPU offscreen avec scene color matrix."),
