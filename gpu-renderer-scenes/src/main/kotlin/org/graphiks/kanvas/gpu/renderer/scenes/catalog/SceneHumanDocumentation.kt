...
object GPURendererSceneHumanDocs {
    val docs: List<SceneHumanDocs> = listOf(
        ...
        doc("path-stencil-cover-gate-board", "Rendre visible les blockers du stencil-cover path natif.", "Valide les diagnostics de capacite depth-stencil, ordering et readback manquant.", "Ne revendique pas la route native stencil-cover ni activation produit.", "Preuve WebGPU offscreen avec statut de gate bloque."),
    ...
    val candidateScenes: List<CandidateScene> = listOf(
        candidate(
            id = "product-route-smoke-lanes",
            title = "Product Route Smoke Lanes",
            milestones = listOf("M0", "M1"),
            ...
            validationTarget = "Comparer lanes legacy, route candidate et refus sans activer le produit.",
            nonClaims = "Ne revendique pas activation par defaut ni decision release.",
            rationale = "Couvre le pont M0/M1 entre inventaire de routes et premier rollout controle.",
        ),
        candidate(
            id = "gradient-tile-mode-boundary",
            title = "Gradient Tile Mode Boundary",
            milestones = listOf("M2"),
            tags = setOf(SceneTag.RRect, SceneTag.Gradient, SceneTag.Clip),
            status = CandidateSceneStatus.RunnerGap,
            intention = "Rendre visible les variations tile mode et transform autour des gradients.",
            validationTarget = "Verifier que les tile modes non supportes refusent avec raison stable.",
            nonClaims = "Ne revendique pas repeat, mirror, decal ou matrices locales completes.",
            rationale = "Complete les scenes M2 actuelles qui couvrent surtout le degrade lineaire simple.",
        ),
        candidate(
            id = "path-aa-stroke-join-board",
            title = "Path AA Stroke Join Board",
            milestones = listOf("M3"),
            tags = setOf(SceneTag.Path, SceneTag.Stroke, SceneTag.Clip),
            status = CandidateSceneStatus.RunnerGap,
            intention = "Preparer une revue des joins, caps et AA path avant support natif.",
            validationTarget = "Montrer les routes path/stroke attendues et les refus coverage manquants.",
            nonClaims = "Ne revendique pas couverture AA, joins/caps reels ou stencil-cover natif.",
            rationale = "Couvre un trou M3 entre proxy rectangulaire et vraie couverture path.",
        ),
        candidate(
            id = "bitmap-sampler-matrix",
            title = "Bitmap Sampler Matrix",
            milestones = listOf("M4"),
            tags = setOf(SceneTag.Image, SceneTag.Clip, SceneTag.RRect),
            status = CandidateSceneStatus.FixtureReady,
            intention = "Comparer plusieurs fixtures bitmap avec nearest et linear dans une matrice compacte.",
            validationTarget = "Valider l'extension de scenes bitmap fixture-backed sans codec reel.",
            nonClaims = "Ne revendique pas mipmap, tile mode, anisotropic ou decode color-managed.",
            rationale = "Renforce M4 avec une candidate implementable par le runner actuel.",
        ),
        candidate(
            id = "layer-filter-chain-board",
            title = "Layer Filter Chain Board",
            milestones = listOf("M5"),
            tags = setOf(SceneTag.Layer, SceneTag.Filter),
            status = CandidateSceneStatus.RunnerGap,
            intention = "Preparer une scene de chainage layer/filter plus ambitieuse.",
            validationTarget = "Distinguer bounded fixtures supportees et DAG image-filter non supporte.",
            nonClaims = "Ne revendique pas saveLayer general, intermediate textures ou DAG arbitraire.",
            rationale = "Couvre les limites M5 autour de SaveLayer, FilterNode et destination-read.",
        ),
        candidate(
        ...
        candidate(
            id = "runtime-effect-uniform-ladder",
            title = "Runtime Effect Uniform Ladder",
            milestones = listOf("M7"),
            ...
            validationTarget = "Valider descriptor SimpleRT, layout gColor et pipeline key stable.",
            nonClaims = "Ne revendique pas SkSL dynamique, SpiralRT ou effets enfants.",
            rationale = "Exerce M7 avec une scene implementable par le runner actuel.",
        ),
        candidate(
            id = "mesh-ribbon-depth-stack",
            title = "Mesh Ribbon Depth Stack",
            milestones = listOf("M8"),
            ...
            validationTarget = "Valider bounded-ribbon-strip et diagnostics de non-support vertices general.",
            nonClaims = "Ne revendique pas DrawVertices general ni vertex/index buffer upload.",
            rationale = "Etend M8 avec une scene fixture-backed proche du runner actuel.",
        ),
        candidate(
        ...
        candidate(
            id = "legacy-parity-snapshot-board",
            title = "Legacy Parity Snapshot Board",
            milestones = listOf("M10"),
            ...
            validationTarget = "Afficher parite attendue, evidence manquante et refus de retirement.",
            nonClaims = "Ne revendique pas remplacement accepte ni retrait de la route legacy.",
            rationale = "Couvre M10 avec une candidate orientee revue de migration.",
        ),
    )
