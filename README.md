...
Post-MVP Big Target readiness for MEP: 99%.
...
and missing metrics visible as non-claims. The platform is now one step from
the MEP readiness target: the measured-lane gate is real, but remaining
not-measured performance rows still need measured payloads before a broad 100%
PM readiness claim is defensible. Broad Skia parity is not claimed, and
dependency-gated text/font/codec gaps remain visible outside the selected
evidence rows.
...
M58 performance release gate evidence is complete for the same seven selected
rows: 4 measured rows now have release-blocking CPU and GPU/cache thresholds,
3 rows remain `not-measured`, and the generated gate reports 0 blocking
failures. The gate is generated by `pipelinePerformanceReleaseGate`, writes
JSON/Markdown under `build/reports/wgsl-pipeline-performance-release-gate/`,
and is exposed through `pipelinePmBundle`. Because three selected rows remain
not measured, M58 raises readiness to 99%, not 100%.
...
| CI and release gates | 20% | Release-blocking measured performance gate | 100% | `wgsl_scene_dashboard_release_gate` runs dashboard gate, performance warnings, PM bundle, M54 metadata checks, M55 candidate output, M56/M57 generated evidence, and M58 measured-row release gate |
| Performance readiness | 15% | Blocking for selected measured rows | 95% | Seven M58 rows have decisions: 4 measured pass rows, 3 not-measured rows, 8 blocking measured lanes, 0 blocking failures; estimated/missing metrics are non-claims and remain the last gap |
| PM demo and reporting workflow | 15% | PM bundle + front QA + M58 counters | 100% | `pipelinePmBundle` includes manifest, dashboard, artifacts, front QA, gate, performance warnings, inventory reports, M52/M53/M54 counters, M55 candidate counters, M56/M57 evidence, and M58 release-gate counters |

Weighted PM readiness: 99% after rounding.
...
| Performance release gate | Done for measured lanes | 95% | M58 turns selected measured M55 rows into `pipelinePerformanceReleaseGate`, blocks measured lanes with missing required metadata or threshold breaches, and keeps estimated/missing metrics non-blocking; remaining not-measured rows prevent a full 100% readiness claim |
