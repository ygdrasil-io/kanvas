...
class For345NonArcRec2020F16CurrentCaptureTest {

    @Test
    fun `FOR-345 exports current Kanvas non-arc Rec2020 F16 samples`() {
        val rawOutput = configValue(OUTPUT_PROPERTY, OUTPUT_ENV).orEmpty()
        assumeTrue(
            rawOutput.isNotBlank(),
            "FOR-345 current Kanvas capture is driven by its validator script",
        )
    ...
    private fun renderCurrentKanvas(): SkBitmap {
        ...
        val bitmap = SkBitmap(32, 32, rec2020, SkColorType.kRGBA_F16Norm)
    ...
    private fun toJson(bitmap: SkBitmap): String {
        ...
        return """
            {
              "linear": "FOR-345",
              "sceneId": "non-arc-rec2020-f16-reference-row-for345",
              "sourceType": "current-kanvas-non-arc-rec2020-f16-src-over-rect",
    ...
    private companion object {
        const val OUTPUT_PROPERTY = "kanvas.for345.currentCapture.output"
        const val OUTPUT_ENV = "KANVAS_FOR345_CURRENT_CAPTURE_OUTPUT"

        val samples = listOf(
            Sample("background_top_left", 0, 0),
            Sample("rect_center", 16, 16),
            Sample("rect_left_inside", 8, 16),
            Sample("rect_right_inside", 23, 16),
        )
