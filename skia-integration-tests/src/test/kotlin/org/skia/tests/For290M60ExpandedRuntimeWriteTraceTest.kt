...
class For290M60ExpandedRuntimeWriteTraceTest {

    @Test
    fun `FOR-290 exports M60 expanded runtime write trace`() {
        ...
        assumeTrue(
            ...
            "FOR-290 expanded runtime write trace export is driven by its validator script",
        )
        val targetPixels = parseTargets(rawTargets)
        val output = File(rawOutput)
        assertEquals(59, targetPixels.size, "FOR-290 must trace exactly the 59 outer-boundary target pixels")
        ...
        assertTrue(
            ...
            "every FOR-290 target pixel must have at least one traced runtime write",
        )
    ...
    private fun parseTargets(raw: String): Set<SkCpuWriteChronologyTrace.Target> {
        ...
        return raw.split(";")
            ...
            .map { item ->
                ...
                require(parts.size == 2) { "Invalid FOR-290 target coordinate: `$item`" }
    ...
    private fun toJson(
        ...
        return """
            {
              "linear": "FOR-290",
              "probe": "m60-expanded-runtime-write-trace-raw",
    ...
    private companion object {
        const val TARGETS_PROPERTY = "kanvas.for290.targets"
        const val OUTPUT_PROPERTY = "kanvas.for290.output"
        const val TARGETS_ENV = "KANVAS_FOR290_TARGETS"
        const val OUTPUT_ENV = "KANVAS_FOR290_OUTPUT"
