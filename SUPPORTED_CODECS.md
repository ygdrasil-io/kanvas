...
| BMP | Supported through `SkBmpEncoder`. | Not supported. Output uses sRGB interpretation. | Not applicable. | Supported when writing 32 bpp output; otherwise format variants are opaque. | Not supported. | Not applicable. | The encoder writes the implemented uncompressed BMP path. RLE encode and V4/V5 ICC output are not official support. |
...
- Unsupported variants should fail explicitly by returning `null`,
  `kUnimplemented`, or a specific `SkCodec.Result` error rather than silently
