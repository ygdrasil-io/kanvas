...
| JPEG-LS | Supported through `codec/jpeg-ls` for SOF55 8-bit grayscale LOCO-I with `NEAR=0..127`, plus RGB lossless scans with component IDs 1/2/3, unit sampling and `ILV=1` (line interleave). Both routes use implicit defaults or an explicit default LSE (`MAXVAL=255`), and regular/run/run-interruption coding. | Not applicable. | APP and COM segments are retained by the immutable document, but ICC color management is not exposed in this profile. | Not applicable. | JPEG-LS profile has no alpha in this implementation. Output is opaque RGBA. | Not applicable; output is normalized from 8-bit samples. | Not applicable. | Non-default LSE (including a different `MAXVAL`), RGB `NEAR>0`, sample/non-interleaved colour scans, non-RGB component IDs/sampling/mapping tables, DRI/RST, point transforms and other JPEG-LS profiles return stable diagnostics; `NEAR>127` is rejected for the 8-bit `MAXVAL=255` grayscale profile. Malformed SOF55/SOS/entropy/EOI streams are refused by JPEG-LS without fallback to JPEG classic. |
...
| JPEG XL | No pixel decode is supported. `codec/jpegxl` owns raw `FF 0A` and `JXL `/`ftyp`/`jxlc` signatures, validates a bounded SizeHeader, then returns `kUnimplemented` before metadata, frame, Modular/VardCT, ANS, animation, or JPEG reconstruction processing. | Animation is not decoded. | ICC/color metadata is not decoded. | EXIF/orientation is not decoded. | Alpha is not decoded. | HDR/F16 is not decoded. | Not applicable. | There is no JPEG XL encoder or reconstruction path. `jxlp` fragmentation is explicitly `kUnimplemented`; no JXL input may silently fall back to AWT/ImageIO/JNI or another codec. |
...
| JPEG-LS | Supported through `JpegLsEncoder` for grayscale 8-bit SOF55 with `NEAR=0..127`, and lossless RGB 8-bit SOF55 with `ILV=1` (line interleave), both with implicit default `MAXVAL=255` parameters. | APP/COM emission is not exposed by the initial encoder API. | Not applicable. | The encoder requires fully opaque input and emits opaque output; any translucent pixel is refused (`null`). | Input dimensions must be within `1..65535`. Pixel-wise grayscale inputs emit grayscale; other opaque pixels emit RGB components exactly, without a colour transform. | Not applicable. | RGB `NEAR>0`, sample/non-interleaved colour scans, custom LSE, DRI/RST and point transforms are refused until independently evidenced. |
