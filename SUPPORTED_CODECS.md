...
| JPEG-LS | Supported through `codec/jpeg-ls` for the SOF55 grayscale 8-bit lossless LOCO-I profile with `NEAR=0`, default LSE parameters, and regular/run/run-interruption coding. | Not applicable. | APP and COM segments are retained by the immutable document, but ICC color management is not exposed in this profile. | Not applicable. | JPEG-LS profile has no alpha in this implementation. Output is opaque RGBA. | Not applicable; output is normalized from 8-bit samples. | Not applicable. | Near-lossless (`NEAR>0`), multi-component/interleaved scans, non-default LSE, DRI/RST, point transforms and other JPEG-LS profiles return stable diagnostics; malformed SOF55/SOS/entropy/EOI streams are refused by JPEG-LS without fallback to JPEG classic. |
...
| JPEG-LS | Supported through `JpegLsEncoder` for grayscale 8-bit lossless SOF55 with `NEAR=0` and default LSE parameters. | APP/COM emission is not exposed by the initial encoder API. | Not applicable. | The encoder requires fully opaque input and emits opaque output; any translucent pixel is refused (`null`). | The public source must be pixel-wise grayscale (`r == g == b`) and dimensions must be within `1..65535`; color input is refused (`null`). | Not applicable. | Near-lossless, color/interleaved scans, custom LSE, DRI/RST and point transforms are refused until independently evidenced. |
