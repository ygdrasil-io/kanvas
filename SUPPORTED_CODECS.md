...
| WebP | Supported through `WebpEncoder` for lossless VP8L. | ICC/EXIF/XMP writing is not supported. | EXIF orientation writing is not supported. | Supported for lossless VP8L. | Not supported; input pixels are projected to 8-bit samples. | Not applicable. | Lossy VP8 encode intentionally returns `null` until a pure Kotlin lossy encoder is implemented or explicitly kept out of scope. |
| ICO / CUR | Supported through `IcoEncoder` in `codec/ico/`. Encodes single-image ICO with PNG payload. | Encoder writes sRGB 8-bit RGBA truecolor via embedded PNG; does not currently serialize ICC metadata on the ICO envelope. | Not applicable. | Alpha supported through the embedded PNG payload. | Not supported. | Not applicable. | Multi-image, CUR (cursor), and BMP-payload ICO encode are not supported. Public encode API returns `null` or a documented stub behavior for unsupported variants. |
| GIF | Supported through `GifEncoder` in `codec/gif/` for single-frame indexed GIF87a output with LZW compression and optional Netscape loop extension. | Not applicable. GIF output uses the global color table and is indexed. | Not applicable. | Not supported by the encode path; output is opaque indexed. | Not applicable. Output is indexed with up to 256 colors. | Not applicable. | Multi-frame (animated) encode is not supported. Loop count control is exposed but only writes the Netscape extension on single-frame output. Encode API returns `null` for degenerate bitmaps. |
