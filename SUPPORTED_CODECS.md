...
| JPEG-LS | Supported through `JpegLsEncoder` for grayscale 8-bit SOF55 with `NEAR=0..127` and implicit default `MAXVAL=255` parameters. | APP/COM emission is not exposed by the initial encoder API. | Not applicable. | The encoder requires fully opaque input and emits opaque output; any translucent pixel is refused (`null`). | The public source must be pixel-wise grayscale (`r == g == b`) and dimensions must be within `1..65535`; color input is refused (`null`). | Not applicable. | Color/interleaved scans, custom LSE, DRI/RST and point transforms are refused until independently evidenced. |
