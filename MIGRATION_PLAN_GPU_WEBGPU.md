...
- [x] G-suivi (round 26) : 9 cross-tests neufs (ClipDrawDrawGM 100.00 %, Crbug938592GM 99.80 %, SmallArcGM 99.80 %, ArcOfZorroGM 99.73 %, ShallowAnglePathArcToGM 99.88 %, FillTypesGM 99.55 %, PathInteriorGM 98.70 %, Bug6987GM 99.77 %, ScaledStrokesGM 96.49 %) avg GPU **99.30 %** (cross-backend avg raster **99.28 %**) — zéro changement device. Tous via `runCrossBackendTest`. Harvest sur les GMs qui avaient un per-backend GPU test (G7 single-backend) + un per-backend raster test mais n'avaient pas encore de cross-backend test consolidé. Profil dominant : pure AA-fill / AA-stroke / hardstop-gradient / arcTo flatten, tous unlock-stable depuis G3.3b + G3.4.1 + G4.x. Byte-exact (or quasi-byte-exact) : `ClipDrawDrawGM` (crbug.com/423834 repro -- clipRect + drawRect axis-aligned non-AA, both backends byte-exact 100 % sur le rect fast-path). Gradients hardstop : `Crbug938592GM` (3-stop hardstop linear gradient 150 x 30, both 99.80 %). Strokers sur courbes : `SmallArcGM` (cubic stroked width 120 sous scale(8, 8), GPU 99.80 % / raster 99.81 %), `ArcOfZorroGM` (4-arc "Z" stroker pattern, both 99.73 %), `ShallowAnglePathArcToGM` (arcTo sub-degree sweep, both 99.88 %), `Bug6987GM` (skbug 6987 stroker-corner near-degenerate scale, GPU 99.77 % / raster 99.56 %), `ScaledStrokesGM` (rect/circle/line/path stroked sous 0.1x..10x scale matrix, GPU 96.49 % / raster 96.41 % -- 96 % floor dominé par l'AA stroke edge convention sur les sub-1x scales). AA polygon fill : `FillTypesGM` (4 SkPathFillType x non-convex, GPU 99.55 % / raster 99.63 %), `PathInteriorGM` (multi-contour winding/even-odd fill, GPU 98.70 % / raster 98.68 %). Tous deltas raster-vs-GPU sous 0.25 pt sauf ScaledStrokesGM (0.08 pt) — cross-backend short-circuit consistant. Profil "lift G7 single-backend -> cross-backend", pas de nouveau capability unlock requis.
- [x] G-suivi (round 27) : 12 cross-tests neufs (Bug7792GM 99.99 %, BitmapRectRoundingGM 100.00 %, DrawBitmapRectSkbug4734GM 100.00 %, FiddleGM 100.00 %, ShallowGradientLinearGM 100.00 %, ShallowGradientLinearNoditherGM 100.00 %, ShallowGradientRadialGM 100.00 %, ShallowGradientRadialNoditherGM 100.00 %, AnalyticAntialiasInverseGM 99.98 %, AnalyticAntialiasConvexGM 99.90 %, PathSkbug11859GM 99.95 %, QuadCapGM 99.80 %) avg GPU **99.97 %** (cross-backend avg raster **99.95 %**) — zéro changement device. Tous via `runCrossBackendTest`. Harvest sur les GMs qui avaient un per-backend GPU test mais pas encore de cross-backend test consolidé, post-J7 heap bump (#604) + J6 self-intersecting winding fix (#602) + saveLayer backdrop ImageFilter Blur (#603) merges. Profil dominant : pure AA-fill / drawBitmapRect rounding / shallow-gradient near-grey ramp / analytic-AA convex+inverse / stroker degree-2 curve -- tous unlock-stable depuis G3.3b + G3.4.1 + G4.1 + G5.1 / G5.2. Byte-exact : `BitmapRectRoundingGM` + `DrawBitmapRectSkbug4734GM` (drawBitmapRect sub-pixel rounding fast-path), `FiddleGM` (2 filled rects), `ShallowGradient{Linear,LinearNodither,Radial,RadialNodither}GM` (4 shallow-gradient variants 800 x 800 -- l'isDither = true sub-LSB delta sur near-grey ramps reste byte-exact contre la dithered upstream reference). Stencil-and-cover : `Bug7792GM` (16 line-only paths multi-contour kWinding, GPU 99.99 % / raster 100.00 %). Analytic AA polygon : `AnalyticAntialiasInverseGM` (kInverseWinding cover, GPU 99.98 % / raster 99.97 %), `AnalyticAntialiasConvexGM` (convex-AA fast-path, GPU 99.90 % / raster 99.84 %). Path AA + stroker : `PathSkbug11859GM` (skbug 11859 near-degenerate quad regression, GPU 99.95 % / raster 99.80 %), `QuadCapGM` (quad-Bezier stroked avec butt/round/square caps, GPU 99.80 % / raster 99.82 %). Tous deltas raster-vs-GPU sous 0.20 pt — cross-backend short-circuit byte-stable. Profil "lift G7 single-backend -> cross-backend", pas de nouveau capability unlock requis.
- [x] K8 — RCA `OverStrokeGM` drift vs reference : confirmation que le 15-pt gap vs `original-888/OverStroke.png` **n'est pas** un drift raster ↔ GPU. Mesure cross-backend : **GPU 84.81 % vs raster 84.87 %** (Δ ≈ 0.06 pt, well inside la warning band 2 %) — les deux backends produisent un output visuellement identique. Root cause : quand `strokeWidth >> rayon de courbure` (e.g. `OVERSTROKE_WIDTH = 500` sur quad/oval/cubic `~100 x 50`), l'algo "offset extérieur / offset intérieur" de `SkStroker` émet des offsets intérieurs qui se croisent eux-mêmes (artefact classique de la butterfly/bowtie overstroke). L'upstream Skia C++ original (2016) avait **le même** bug — voir le commentaire en tête de `gm/overstroke.cpp` : *"we offset each part of the curve the request amount even if it makes the offsets overlap and create holes. There is not a really great algorithm for this and several other 2D graphics engines have the same bug."*. La reference `original-888/OverStroke.png` a été rendue par une version upstream plus récente qui ajoute un band-aid `fCusper` pour les cusps cubic (`SkPathStroker::cubicTo` → `fCusper.addCircle(cuspLoc, fRadius)` dans `src/core/SkStroke.cpp:1363-1367`). Le port kanvas (`kanvas-skia/src/main/kotlin/org/skia/foundation/SkStroker.kt`) n'implémente pas ce band-aid ; même implémenté, il ne ré-aligne que la cellule cubic (3 cellules / 6 sont des quad/oval closed-contour où l'artefact persiste). **Pas de fix device-side** : ré-aligner kanvas sur la reference demande une réécriture de `SkStroker` (passer à l'algorithme upstream récent qui détecte les inversions et fill-shell). Hors scope K8. Cross-backend test ajouté à 84.5 % floor (`OverStrokeCrossBackendTest`) pour capturer la stabilité GPU ↔ raster sans bloquer sur le 15-pt gap intrinsèque. Note prompt : la formulation "GPU drift vs raster 84.81 %" mélangeait le score (GPU-vs-reference) avec la métrique (GPU-vs-raster) ; après mesure, GPU-vs-raster est < 0.1 pt.
...
- [x] G-suivi (round 28) : ré-investigation des skip lists historiques (rounds 21-25) post-merges Wave J/K. Deux unlocks identifiés et ratchetés : `RepeatedBitmapGM` (round 21 skip @ GPU 65.42 % -- post-PR #582 bitmap-shader rotated CTM composition fix : GPU 99.98 % / raster 99.98 %, 3 px delta cross-backend ; floors `RepeatedBitmapCrossBackendTest` montés `95.0 -> 99.93` sur les deux backends), `Crbug892988GM` (round 23 skip @ GPU 13.98 % -- post-PR #599 kSrc non-opaque sous AA-clipRect routing : GPU 100.00 % / raster 100.00 % byte-exact ; floors `Crbug892988CrossBackendTest` montés `99.0 -> 99.95`). Probed-but-still-skip (mesures inchangées) : `BlurDrawImageGM` (GPU 78.60 % / raster 78.46 %, mandrill jpg drift inchangé par K2 axis-aligned drawImageRect+maskFilter), `B340982297GM` (GPU 52.93 % / raster 96.23 %, le J6 fix reste raster-only -- self-intersection winding GPU à creuser séparément), `BlurredClippedCircleGM` + `Skbug9319GM` (clipPath kDifference panic toujours présent), `Crbug996140GM` (GPU 79.66 % / raster 74.82 %), `SimpleBlurRoundRectGM` (GPU 79.65 % / raster 79.66 %), `AlphaImageGM` (GPU 34.44 % / raster 32.53 %), `BlurLargeRRectsGM` (GPU 49.43 % / raster 47.11 %). Zéro changement device, zéro nouveau cross-test (les deux unlocks réutilisent les tests existants en montant juste les floors). La pool est officiellement sèche pour ce profil ; round 29 doit attendre un nouvel unlock device-side avant de re-balayer.
...
- [x] K9 -- stroke + linear-gradient shader on non-AA path (RCA + fix). `StrokeRectShaderGM` was scoring 91.88 % on GPU because the AA-off row was painted entirely solid black : `SkCanvas.drawRect` with `paint.shader != null` routes to `drawPath(SkPath.Rect(rect))`, the kStroke recursion runs `SkStroker` and re-dispatches with a fill-style copy carrying the gradient, but the non-rect outline path landed at the solid-colour `StencilCoverPolygonDraw` arm because the `linearGradForAaPath` gate required `paint.isAntiAlias`. Fix : drop the `paint.isAntiAlias` requirement from the gate and reuse the AA stencil-cover linear-gradient pipeline with `edgeCount = 0` (sentinel : `minSegmentDistance` returns `1e9`, collapsing the inside cover to coverage `= 1.0` and the outside cover to `0.0` -- sharp stencil-bound fill, no AA falloff). Mirror byte-for-byte of the G5.2.3 bitmap-shader fix. Touches multi-contour + single-contour-concave/inverse arms in [`SkWebGpuDevice.drawPath`](gpu-raster/src/main/kotlin/org/skia/gpu/webgpu/SkWebGpuDevice.kt). Lifts `StrokeRectShaderGM` from **91.88 %** to **99.54 %** (+7.66 pp). `StrokeCircleGM` (all-AA, no shader) unchanged at 91.81 % -- its residual drift is documented as conic flattening at 20x CTM scale + 565-quantised random colours, intrinsic to the upstream reference convention and not addressed by this fix.
- [x] SkStroker -- inner-offset reversal heuristic on engulfed closed contours. Mirrors the upstream `SkPathStroker::cubicTo` per-segment "inner-vs-outer" reversal logic (when `halfW` exceeds the local inradius the upstream emits to `fInner` separately and the close-contour assembly reverses-or-drops the inner band) at the **contour level** : after the standard offset-polyline pass, compute `signedArea` of the source polyline, the outer offset polyline, and the inner offset polyline ; when (a) `halfW > 10` source units, (b) `halfW > 1.5 × bboxMaxDim(src)`, AND (c) the outer / inner areas are **same-signed with ratio > 0.7** → the contour is "engulfed" by the offset (both polylines ended up on the same side of the source), and we emit both as **same-direction** filled regions (winding UNION) instead of the standard "outer + reversed-inner" band. The full upstream `cubicStroke` + `STROKER_RESULT` machinery would handle this per-segment ; our polyline approach catches it at contour level. Touches [`SkStroker.strokeContour`](kanvas-skia/src/main/kotlin/org/skia/foundation/SkStroker.kt) only -- adds `signedArea` (Shoelace) + `contourMaxDimension` helpers ; the standard band path is preserved byte-exact (`right` = outer, `left` = inner reversed) for all non-engulfed contours so text-glyph + oval-donut renderings stay pixel-stable. Score deltas (raster, zero regressions across `:skia-integration-tests:test` 585 GMs + `:gpu-raster:test --tests "*Stroke*"` 18 cross / per-backend stroke tests) : **OverStrokeGM 83.48 % -> 85.87 % (+2.39 pp)** (the quad cell engulfed-symmetric pattern now renders both offsets as union ; the cubic and oval cells fall outside the heuristic and stay unchanged). `OverStrokeCrossBackendTest` floors bumped 84.5 -> 85.5 on both backends ; `OverStrokeTest` raster floor bumped 83.4 -> 85.5. Note : the cubic cell (`cubicTo(25, 75, 75, -50, 100, 0)`) does **not** trigger the heuristic (its outer / inner area ratio is below 0.7) -- closing the remaining ~14 pt gap vs the upstream reference requires the full upstream `cubicStroke` rewrite documented in K8, which remains scoped as future work.
