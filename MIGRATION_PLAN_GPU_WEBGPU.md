...
- [x] N? — `SkStroker` cubic-cusp band-aid port (port ciblé `fCusper`). Ajoute le port byte-for-byte des helpers upstream `SkFindCubicCusp` + `SkFindCubicMaxCurvature` + `solve_cubic_poly` + `formulate_F1DotF2` + `on_same_side` (`/Users/chaos/workspace/kanvas-forge/skia-main/src/core/SkGeometry.cpp:961-1149`) dans `kanvas-skia/src/main/kotlin/org/skia/foundation/SkStroker.kt`. Le `flattenContoursWithCusps` accumule un `FloatArrayList` de cusps détectés (un `(x, y)` par cubic cusp) ; `stroke()` appelle `out.addCircle(cuspX, cuspY, halfW)` pour chaque cusp après les contours stroked, exactement comme `fCusper.detach() -> fOuter.addPath(...)` upstream (`SkStroke.cpp:386-387`). Effet mesuré sur la suite GM : `TrickyCubicStrokesButtMiterGM` **97.8684 -> 97.9443** (+0.08 pt) ; **zéro régression** sur les 18 stroke tests (raster + GPU) du suite `*Stroke*`. `OverStrokeGM` reste inchangé : son cubic `cubicTo(25, 75, 75, -50, 100, 0)` ne déclenche pas la détection upstream (`on_same_side` retourne `true` pour les deux paires de legs car les segments P0-P1 et P2-P3 ne se croisent pas, x-ranges disjoints), conforme à l'analyse upstream (le band-aid était optimiste dans K8). Test unitaire ajouté `SkStrokerTest.cubic with a true cusp emits an extra disc sub-contour vs cusp-free cubic` (verbe-stream check : +1 moveTo entre une cubique cusp-y `(0,0)→(10,10)→(0,10)→(10,0)` et sa version smooth). Follow-up à investiguer si on veut combler le gap OverStroke : (a) heuristique kanvas-spécifique "cubic max-curvature t où derivative magnitude < strokeWidth" pour fill-disc agressif, (b) port complet du `cubicStroke`/`STROKER_RESULT` machinery (rewrite, scope ~jour).
