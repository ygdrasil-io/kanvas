...
Cette fixture documente le premier profil JPEG XL dont le décodeur Kotlin doit
restituer les pixels, sans dépendance native au runtime : un codestream brut
`FF 0A`, image fixe grayscale 8-bit, lossless, Modular, sans alpha, palette,
transform, animation ni conteneur ISO-BMFF.
...
djxl flower-510x532-8bit-lossless.jxl oracle.pgm --num_threads=0 -v
```
...
rtk ./gradlew :codec:jpegxl:test \
  --tests '*JpegXlModularDecodeTest.opt in djxl oracle retains pinned modular lossless pixels exactly' \
