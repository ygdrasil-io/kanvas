...
class JpegLsCodecTest {
    ...
    @Test
    fun `decodes a CharLS custom LSE fixture exactly`() {
        val codec = Codec.MakeFromData(CHARLS_CUSTOM_LSE_FIXTURE)
        val (bitmap, result) = requireNotNull(codec).getImage()

        assertEquals(Codec.Result.kSuccess, result, decodeDiagnostic(CHARLS_CUSTOM_LSE_FIXTURE))
        assertNotNull(bitmap)
        val expected = intArrayOf(
            0x41, 0x71, 0x41, 0x71, 0x41, 0x71, 0x41, 0x71,
            0x41, 0x71, 0x41, 0x71, 0x41, 0x71, 0x41, 0x71,
    ...
    @Test
    fun `decodes a CharLS custom LSE RESET fixture exactly`() {
        val codec = Codec.MakeFromData(CHARLS_CUSTOM_LSE_RESET_FIXTURE)
        val (bitmap, result) = requireNotNull(codec).getImage()

        assertEquals(Codec.Result.kSuccess, result, decodeDiagnostic(CHARLS_CUSTOM_LSE_RESET_FIXTURE))
        assertNotNull(bitmap)
        for (y in 0 until 4) {
            for (x in 0 until 64) {
    ...
    @Test
    fun `parses zero-valued LSE entries as defaults`() {
        val zeroDefaults = DEFAULT_LSE.copyOf().also { lse ->
            for (index in 5 until lse.size) lse[index] = 0
        }
        val withPreset = CHARLS_REGULAR_FIXTURE.copyOfRange(0, 2) + zeroDefaults +
            CHARLS_REGULAR_FIXTURE.copyOfRange(2, CHARLS_REGULAR_FIXTURE.size)

        val (bitmap, result) = requireNotNull(Codec.MakeFromData(withPreset)).getImage()
    ...
    @Test
    fun `refuses an LSE maximum sample value outside the 8-bit profile`() {
        val unsupportedLse = DEFAULT_LSE.copyOf().also { it[6] = 0xFE.toByte() }
        val withPreset = CHARLS_REGULAR_FIXTURE.copyOfRange(0, 2) + unsupportedLse +
            CHARLS_REGULAR_FIXTURE.copyOfRange(2, CHARLS_REGULAR_FIXTURE.size)
    ...
    @Test
    fun `refuses an LSE threshold smaller than the NEAR bound`() {
        val invalidLse = NEAR_1_DEFAULT_LSE.copyOf().also { it[8] = 0x01 }
        val withCustomLse = CHARLS_NEAR_1_FIXTURE.copyOfRange(0, 2) + invalidLse +
            CHARLS_NEAR_1_FIXTURE.copyOfRange(2, CHARLS_NEAR_1_FIXTURE.size)

        val opened = JpegLsDocument.open(withCustomLse)

        assertNull(opened.document)
        assertEquals("jpeg-ls.lse.invalid", opened.diagnostic?.code)
        assertEquals(Codec.Result.kErrorInInput, opened.diagnostic?.result)
        assertEquals(2, opened.diagnostic?.offset)
    }
    ...
    private companion object {
        ...
        /**
         * Generated by CharLS 2.4.4 (`9930a2a2fa75f516c4a08708180c9907fa501a97`), BSD-3-Clause,
         * from the regular fixture samples using the explicit valid LSE tuple
         * `MAXVAL=255, T1=6, T2=14, T3=42, RESET=40`:
         * SHA-256 `0d19ccedc86bc7103f2958adc2c452fdb9cd72e281a60d80657a09c344f0d7e7`.
         */
        val CHARLS_CUSTOM_LSE_FIXTURE: ByteArray = Base64.getDecoder().decode(
            "/9j/9wALCAAEAAgBAREA//gADQEA/wAGAA4AKgAo/9oACAEBAAAAAAAAAYAAAAFeES+xL7EvyV4AAACvgAAAr58uLSwrAAAAr0v+/9k=",
        )

        /**
         * Generated by CharLS 2.4.4 (`9930a2a2fa75f516c4a08708180c9907fa501a97`), BSD-3-Clause,
         * from a project-owned 64x4 alternating PGM using `MAXVAL=255`,
         * `T1=6`, `T2=14`, `T3=42`, and `RESET=3`. The repeating regular-mode
         * context crosses the custom reset boundary. SHA-256
         * `72e2cf68a71f5343c56808fcf5a1b3dd17e80bb3d8ca7f9846f90edec3df778c`.
         */
        val CHARLS_CUSTOM_LSE_RESET_FIXTURE: ByteArray = Base64.getDecoder().decode(
            "",
        )
