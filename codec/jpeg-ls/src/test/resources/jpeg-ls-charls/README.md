...
The `line*` fixtures are 8-bit RGB, component IDs 1/2/3, unit sampling, `ILV=1`, with no
...
`sample-lossless-6x4` and `sample-run-lossless-6x4` are project-owned 8-bit
RGB, unit-sampling, `ILV=2`, `NEAR=0` streams generated by CharLS 2.4.4
(`9930a2a2fa75f516c4a08708180c9907fa501a97`), BSD-3-Clause. The first uses
alternating R plus varying G/B to enter regular mode; the second is a constant
triplet field with one `(0x70, 0x60, 0x50)` interruption at `(4, 2)`, exercising
sample-interleaved run and run interruption. Their SHA-256 values are
