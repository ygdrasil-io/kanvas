 ...
 * 4x4 grid of rotated cells, each with a grey rounded background rect
 * and scaled `images/randPixels.png` (8x8 native, upscaled to 128x128).
...
class RepeatedBitmapGm : SkiaGm {
    ...
    override fun onOnceBeforeDraw(canvas: GmCanvas) {
        val bytes = this::class.java.classLoader?.getResourceAsStream("images/randPixels.png")?.readBytes()
        if (bytes != null) {
    ...
    override fun draw(canvas: GmCanvas, width: Int, height: Int) {
        ...
        for (j in 0 until 4) {
            for (i in 0 until 4) {
                ...
                val dst = RectF32.ofOriginSize(-img.width / 2f, -img.height / 2f, img.width.toFloat(), img.height.toFloat())
    ...
    private fun drawCheckerboard(canvas: GmCanvas, color1: Color, color2: Color, size: Int) {
        for (y in 0 until 576 step size) {
            for (x in 0 until 576 step size) {
                ...
                canvas.drawRect(RectF32(x.toFloat(), y.toFloat(), (x + size).toFloat(), (y + size).toFloat()), Paint(color = if (isEven) color1 else color2))
            }
        }
    }
}
