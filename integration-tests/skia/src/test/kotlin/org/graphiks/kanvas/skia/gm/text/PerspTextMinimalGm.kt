 ...
 * Draws "Hamburgefons" under perspective matrices with a 32×
 * reduction factor for perspX/perspY, producing very subtle
 * perspective.
    ...
    override fun draw(canvas: GmCanvas, width: Int, height: Int) {
        ...
        for (pm in PerspMode.entries) {
            for (step in 0 until kSteps) {
                val persp0 = when (pm) {
                    PerspMode.X -> step * 0.0005f / kSteps / kMinimalFactor
                    PerspMode.XY -> step * -0.00025f / kSteps / kMinimalFactor
                    PerspMode.Y -> 0f
                }
                val persp1 = when (pm) {
                    PerspMode.X -> 0f
