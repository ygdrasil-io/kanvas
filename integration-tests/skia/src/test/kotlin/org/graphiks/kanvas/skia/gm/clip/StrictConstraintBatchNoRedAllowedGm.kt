...
/**
 * Tests strict constraint in batch mode: multiple guarded-image draw
 * calls with clip enabled, verifying that red guard pixels are excluded
 * when strict constraint batching is used.
 *
 * @see org.graphiks.kanvas.skia.gm.image.SrcRectConstraintGm
 */
class StrictConstraintBatchNoRedAllowedGm : SkiaGm {
    override val name = "strict_constraint_batch_no_red_allowed"
    ...
    override fun draw(canvas: GmCanvas, width: Int, height: Int) {
        ...
        // Batch of strict-clipped draws, all using the same guarded image
        // but different source rects — strict mode means no red guard bleeds
        val draws = listOf(
