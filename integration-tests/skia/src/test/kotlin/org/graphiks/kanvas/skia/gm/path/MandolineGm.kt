...
class MandolineGm : SkiaGm {
    ...
    override fun draw(canvas: GmCanvas, width: Int, height: Int) {
        ...
        val mandoline = MandolineSlicer(Point2F32(41f, 43f))
        mandoline.sliceCubic(Point2F32(5f, 277f), Point2F32(381f, -74f), Point2F32(243f, 162f))
        mandoline.sliceLine(Point2F32(41f, 43f))
        canvas.drawPath(mandoline.path(), paint)

        mandoline.reset(Point2F32(357.049988f, 446.049988f))
        mandoline.sliceCubic(
            Point2F32(472.750000f, -71.950012f),
            Point2F32(639.750000f, 531.950012f),
            Point2F32(309.049988f, 347.950012f),
        )
        mandoline.sliceLine(Point2F32(309.049988f, 419f))
        mandoline.sliceLine(Point2F32(357.049988f, 446.049988f))
        canvas.drawPath(mandoline.path(), paint)

        canvas.save()
        canvas.translate(421f, 105f)
        canvas.scale(100f, 81f)
        mandoline.reset(Point2F32(-cosD(-60f), sinD(-60f)))
        mandoline.sliceConic(
            Point2F32(-2f, 0f),
            Point2F32(-cosD(60f), sinD(60f)),
            0.5f,
        )
        mandoline.sliceConic(
            Point2F32(-cosD(120f) * 2f, sinD(120f) * 2f),
            Point2F32(1f, 0f),
            0.5f,
        )
        mandoline.sliceLine(Point2F32(0f, 0f))
        mandoline.sliceLine(Point2F32(-cosD(-60f), sinD(-60f)))
        canvas.drawPath(mandoline.path(), paint)
        canvas.restore()
    ...
    private class MandolineSlicer(anchorPt: Point2F32) {
        private var path: Path = Path { }
        private var anchorPt: Point2F32 = anchorPt
        private var lastPt: Point2F32 = anchorPt
        private val rand = Random(1)

        init { reset(anchorPt) }

        fun reset(anchor: Point2F32) {
            path = Path { }
            this.anchorPt = anchor
            this.lastPt = anchor
        }

        fun sliceLine(pt: Point2F32, numSubdivisions: Int = K_DEFAULT_SUBDIVISIONS) {
        ...
        fun sliceQuadratic(p1: Point2F32, p2: Point2F32, numSubdivisions: Int = K_DEFAULT_SUBDIVISIONS) {
            if (numSubdivisions <= 0) {
                path.moveTo(anchorPt.x, anchorPt.y)
                path.lineTo(lastPt.x, lastPt.y)
        ...
        fun sliceCubic(p1: Point2F32, p2: Point2F32, p3: Point2F32, numSubdivisions: Int = K_DEFAULT_SUBDIVISIONS) {
            if (numSubdivisions <= 0) {
                path.moveTo(anchorPt.x, anchorPt.y)
                path.lineTo(lastPt.x, lastPt.y)
        ...
        fun sliceConic(p1: Point2F32, p2: Point2F32, w: Float, numSubdivisions: Int = K_DEFAULT_SUBDIVISIONS) {
            if (numSubdivisions <= 0) {
                path.moveTo(anchorPt.x, anchorPt.y)
                path.lineTo(lastPt.x, lastPt.y)
                path.quadTo(p1.x, p1.y, p2.x, p2.y)
...
private fun chopQuadAt(p: Array<Point2F32>, t: Float): Array<Point2F32> {
...
private fun chopCubicAt(p: Array<Point2F32>, t: Float): Array<Point2F32> {
    val a = lerp(p[0], p[1], t)
    val b = lerp(p[1], p[2], t)
...
private fun chopConicAt(
    ...
    val newP1L = Point2F32(ax / aw, ay / aw)
    val newP1R = Point2F32(bx / bw, by / bw)
    val mid = Point2F32(mx / mw, my / mw)
...
private data class ConicHalf(val p1: Point2F32, val p2: Point2F32, val w: Float)

private fun lerp(a: Point2F32, b: Point2F32, t: Float): Point2F32 =
    Point2F32(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t)
