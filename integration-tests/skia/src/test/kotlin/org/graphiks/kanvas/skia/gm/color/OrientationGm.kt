...
class OrientationGm : SkiaGm {
    ...
    override fun draw(canvas: GmCanvas, width: Int, height: Int) {
        val w2 = width / 2f
        val h2 = height / 2f
        canvas.drawRect(RectF32(0f, 0f, w2, h2), Paint(color = ColorARGB.Red))
        canvas.drawRect(RectF32(w2, 0f, width.toFloat(), h2), Paint(color = ColorARGB.Green))
        canvas.drawRect(RectF32(0f, h2, w2, height.toFloat()), Paint(color = ColorARGB.Blue))
        canvas.drawRect(RectF32(w2, h2, width.toFloat(), height.toFloat()), Paint(color = ColorARGB.fromRGBA(1f, 1f, 0f, 1f)))
    }
}
