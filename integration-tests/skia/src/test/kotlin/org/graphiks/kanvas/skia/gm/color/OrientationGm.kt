...
class OrientationGm : SkiaGm {
    ...
    override fun draw(canvas: GmCanvas, width: Int, height: Int) {
        val w2 = width / 2f
        val h2 = height / 2f
        canvas.drawRect(RectF32(0f, 0f, w2, h2), Paint(color = Color.RED))
        canvas.drawRect(RectF32(w2, 0f, width.toFloat(), h2), Paint(color = Color.GREEN))
        canvas.drawRect(RectF32(0f, h2, w2, height.toFloat()), Paint(color = Color.BLUE))
        canvas.drawRect(RectF32(w2, h2, width.toFloat(), height.toFloat()), Paint(color = Color.fromRGBA(1f, 1f, 0f, 1f)))
    }
}
