...
class MeshWithPaintImageGm : SkiaGm {
    ...
    override fun onOnceBeforeDraw(canvas: GmCanvas) {
        val bytes = loadResource("images/color_wheel.png")
            ?: error("Resource not found: images/color_wheel.png")
        val image = Image.decode(bytes)
        colorWheelShader = image.makeShader(sampling = SamplingOptions.LINEAR)
        updateVertices()
    }
    ...
    override fun draw(canvas: GmCanvas, width: Int, height: Int) {
        ...
        canvas.drawVertices(verts, Paint(shader = colorWheelShader))
    }
