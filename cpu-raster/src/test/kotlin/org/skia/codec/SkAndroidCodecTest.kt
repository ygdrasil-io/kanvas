...
class SkAndroidCodecTest {
    ...
    fun `MakeFromData returns non-null for a small PNG`() {
        ...
        val codec = SkAndroidCodec.MakeFromData(bytes)
        assertNotNull(codec, "synthetic 16x12 PNG must be decodable through SkAndroidCodec")
    ...
    fun `MakeFromData returns null for non-image bytes`() {
        assertNull(SkAndroidCodec.MakeFromData(ByteArray(0)))
        assertNull(SkAndroidCodec.MakeFromData("not an image".toByteArray()))
    }

    @Test
    fun `MakeFromStream is equivalent to MakeFromData`() {
        val bytes = synthPng(width = 8, height = 8)
        val viaData = SkAndroidCodec.MakeFromData(bytes)!!
        val viaStream = SkAndroidCodec.MakeFromStream(ByteArrayInputStream(bytes))!!
    ...
    fun `MakeFromData accepts SkData overload`() {
        ...
        val codec = SkAndroidCodec.MakeFromData(data)
        assertNotNull(codec)
    ...
    fun `MakeFromCodec wraps an existing codec`() {
        ...
        val base = SkCodec.MakeFromData(bytes)!!
        val android = SkAndroidCodec.MakeFromCodec(base)
    ...
    fun `getSampledDimensions returns half at sampleSize=2`() {
        val codec = SkAndroidCodec.MakeFromData(synthPng(64, 32))!!
    ...
    fun `MakeFromData unused warning suppressor`() {
        ...
        val opts = SkAndroidCodec.AndroidOptions()
        assertEquals(1, opts.sampleSize)
        assertNull(opts.subset)
        assertEquals(SkAndroidCodec.ZeroInitialized.kNo, opts.zeroInitialized)
        // ExifOrientationBehavior enum reachable.
        assertFalse(SkAndroidCodec.ExifOrientationBehavior.kIgnore == SkAndroidCodec.ExifOrientationBehavior.kRespect)
    }
}
