package org.skia.modules

import kotlin.Boolean
import kotlin.Int
import kotlin.UInt
import kotlin.ULong
import kotlin.collections.List
import org.skia.core.SkCanvas
import org.skia.core.SkTextBlob
import org.skia.foundation.SkPaint
import org.skia.foundation.SkPath
import org.skia.foundation.SkSp
import org.skia.math.SkScalar

/**
 * C++ original:
 * ```cpp
 * class Paragraph {
 *
 * public:
 *     Paragraph(ParagraphStyle style, sk_sp<FontCollection> fonts);
 *
 *     virtual ~Paragraph() = default;
 *
 *     SkScalar getMaxWidth() { return fWidth; }
 *
 *     SkScalar getHeight() { return fHeight; }
 *
 *     SkScalar getMinIntrinsicWidth() { return fMinIntrinsicWidth; }
 *
 *     SkScalar getMaxIntrinsicWidth() { return fMaxIntrinsicWidth; }
 *
 *     SkScalar getAlphabeticBaseline() { return fAlphabeticBaseline; }
 *
 *     SkScalar getIdeographicBaseline() { return fIdeographicBaseline; }
 *
 *     SkScalar getLongestLine() { return fLongestLine; }
 *
 *     bool didExceedMaxLines() { return fExceededMaxLines; }
 *
 *     virtual void layout(SkScalar width) = 0;
 *
 *     virtual void paint(SkCanvas* canvas, SkScalar x, SkScalar y) = 0;
 *
 *     virtual void paint(ParagraphPainter* painter, SkScalar x, SkScalar y) = 0;
 *
 *     // Returns a vector of bounding boxes that enclose all text between
 *     // start and end glyph indexes, including start and excluding end
 *     virtual std::vector<TextBox> getRectsForRange(unsigned start,
 *                                                   unsigned end,
 *                                                   RectHeightStyle rectHeightStyle,
 *                                                   RectWidthStyle rectWidthStyle) = 0;
 *
 *     virtual std::vector<TextBox> getRectsForPlaceholders() = 0;
 *
 *     // Returns the index of the glyph that corresponds to the provided coordinate,
 *     // with the top left corner as the origin, and +y direction as down
 *     virtual PositionWithAffinity getGlyphPositionAtCoordinate(SkScalar dx, SkScalar dy) = 0;
 *
 *     // Finds the first and last glyphs that define a word containing
 *     // the glyph at index offset
 *     virtual SkRange<size_t> getWordBoundary(unsigned offset) = 0;
 *
 *     virtual void getLineMetrics(std::vector<LineMetrics>&) = 0;
 *
 *     virtual size_t lineNumber() = 0;
 *
 *     virtual void markDirty() = 0;
 *
 *     // This function will return the number of unresolved glyphs or
 *     // -1 if not applicable (has not been shaped yet - valid case)
 *     virtual int32_t unresolvedGlyphs() = 0;
 *     virtual std::unordered_set<SkUnichar> unresolvedCodepoints() = 0;
 *
 *     // Experimental API that allows fast way to update some of "immutable" paragraph attributes
 *     // but not the text itself
 *     virtual void updateTextAlign(TextAlign textAlign) = 0;
 *     virtual void updateFontSize(size_t from, size_t to, SkScalar fontSize) = 0;
 *     virtual void updateForegroundPaint(size_t from, size_t to, SkPaint paint) = 0;
 *     virtual void updateBackgroundPaint(size_t from, size_t to, SkPaint paint) = 0;
 *
 *     enum VisitorFlags {
 *         kWhiteSpace_VisitorFlag = 1 << 0,
 *     };
 *     struct VisitorInfo {
 *         const SkFont&   font;
 *         SkPoint         origin;
 *         SkScalar        advanceX;
 *         int             count;
 *         const SkGlyphID* glyphs;     // count values
 *         const SkPoint*  positions;  // count values
 *         const uint32_t* utf8Starts; // count+1 values
 *         unsigned        flags;
 *     };
 *
 *     // lineNumber begins at 0. If info is null, this signals the end of that line.
 *     using Visitor = std::function<void(int lineNumber, const VisitorInfo*)>;
 *     virtual void visit(const Visitor&) = 0;
 *
 *     struct ExtendedVisitorInfo {
 *         const SkFont&   font;
 *         SkPoint         origin;
 *         SkSize          advance;
 *         int             count;
 *         const SkGlyphID* glyphs;     // count values
 *         SkPoint*        positions;  // count values
 *         const SkRect*   bounds;     // count values
 *         const uint32_t* utf8Starts; // count+1 values
 *         unsigned        flags;
 *     };
 *     using ExtendedVisitor = std::function<void(int lineNumber, const ExtendedVisitorInfo*)>;
 *     virtual void extendedVisit(const ExtendedVisitor&) = 0;
 *
 *     /* Returns path for a given line
 *      *
 *      * @param lineNumber  a line number
 *      * @param dest        a resulting path
 *      * @return            a number glyphs that could not be converted to path
 *      */
 *     virtual int getPath(int lineNumber, SkPath* dest) = 0;
 *
 *     /* Returns path for a text blob
 *      *
 *      * @param textBlob    a text blob
 *      * @return            a path
 *      */
 *     static SkPath GetPath(SkTextBlob* textBlob);
 *
 *     /* Checks if a given text blob contains
 *      * glyph with emoji
 *      *
 *      * @param textBlob    a text blob
 *      * @return            true if there is such a glyph
 *      */
 *     virtual bool containsEmoji(SkTextBlob* textBlob) = 0;
 *
 *     /* Checks if a given text blob contains colored font or bitmap
 *      *
 *      * @param textBlob    a text blob
 *      * @return            true if there is such a glyph
 *      */
 *     virtual bool containsColorFontOrBitmap(SkTextBlob* textBlob) = 0;
 *
 *     // Editing API
 *
 *     /* Finds the line number of the line that contains the given UTF-8 index.
 *     *
 *     * @param index         a UTF-8 TextIndex into the paragraph
 *     * @return              the line number the glyph that corresponds to the
 *     *                      given codeUnitIndex is in, or -1 if the codeUnitIndex
 *     *                      is out of bounds, or when the glyph is truncated or
 *     *                      ellipsized away.
 *     */
 *     virtual int getLineNumberAt(TextIndex codeUnitIndex) const = 0;
 *
 *     /* Finds the line number of the line that contains the given UTF-16 index.
 *     *
 *     * @param index         a UTF-16 offset into the paragraph
 *     * @return              the line number the glyph that corresponds to the
 *     *                      given codeUnitIndex is in, or -1 if the codeUnitIndex
 *     *                      is out of bounds, or when the glyph is truncated or
 *     *                      ellipsized away.
 *     */
 *     virtual int getLineNumberAtUTF16Offset(size_t codeUnitIndex) = 0;
 *
 *     /* Returns line metrics info for the line
 *      *
 *      * @param lineNumber    a line number
 *      * @param lineMetrics   an address to return the info (in case of null just skipped)
 *      * @return              true if the line is found; false if not
 *      */
 *     virtual bool getLineMetricsAt(int lineNumber, LineMetrics* lineMetrics) const = 0;
 *
 *     /* Returns the visible text on the line (excluding a possible ellipsis)
 *      *
 *      * @param lineNumber    a line number
 *      * @param includeSpaces indicates if the whitespaces should be included
 *      * @return              the range of the text that is shown in the line
 *      */
 *     virtual TextRange getActualTextRange(int lineNumber, bool includeSpaces) const = 0;
 *
 *     struct GlyphClusterInfo {
 *         SkRect fBounds;
 *         TextRange fClusterTextRange;
 *         TextDirection fGlyphClusterPosition;
 *     };
 *
 *     /** Finds a glyph cluster for text index
 *      *
 *      * @param codeUnitIndex   a text index
 *      * @param glyphInfo       a glyph cluster info filled if not null
 *      * @return                true if glyph cluster was found; false if not
 *      */
 *     virtual bool getGlyphClusterAt(TextIndex codeUnitIndex, GlyphClusterInfo* glyphInfo) = 0;
 *
 *     /** Finds the closest glyph cluster for a visual text position
 *      *
 *      * @param dx              x coordinate
 *      * @param dy              y coordinate
 *      * @param glyphInfo       a glyph cluster info filled if not null
 *      * @return                true if glyph cluster was found; false if not
 *      *                        (which usually means the paragraph is empty)
 *      */
 *     virtual bool getClosestGlyphClusterAt(SkScalar dx,
 *                                           SkScalar dy,
 *                                           GlyphClusterInfo* glyphInfo) = 0;
 *
 *     // The glyph and grapheme cluster information assoicated with a unicode
 *     // codepoint in the paragraph.
 *     struct GlyphInfo {
 *         SkRect fGraphemeLayoutBounds;
 *         TextRange fGraphemeClusterTextRange;
 *         TextDirection fDirection;
 *         bool fIsEllipsis;
 *     };
 *
 *     /** Retrives the information associated with the glyph located at the given
 *      *  codeUnitIndex.
 *      *
 *      * @param codeUnitIndex   a UTF-16 offset into the paragraph
 *      * @param glyphInfo       an optional GlyphInfo struct to hold the
 *      *                        information associated with the glyph found at the
 *      *                        given index
 *      * @return                false only if the offset is out of bounds
 *      */
 *     virtual bool getGlyphInfoAtUTF16Offset(size_t codeUnitIndex, GlyphInfo* glyphInfo) = 0;
 *
 *     /** Finds the information associated with the closest glyph to the given
 *      *  paragraph coordinates.
 *      *
 *      * @param dx              x coordinate
 *      * @param dy              y coordinate
 *      * @param glyphInfo       an optional GlyphInfo struct to hold the
 *      *                        information associated with the glyph found. The
 *      *                        text indices and text ranges are described using
 *      *                        UTF-16 offsets
 *      * @return                true if a graphme cluster was found; false if not
 *      *                        (which usually means the paragraph is empty)
 *      */
 *     virtual bool getClosestUTF16GlyphInfoAt(SkScalar dx, SkScalar dy, GlyphInfo* glyphInfo) = 0;
 *
 *     struct FontInfo {
 *         FontInfo(const SkFont& font, const TextRange& textRange)
 *                 : fFont(font), fTextRange(textRange) {}
 *         virtual ~FontInfo() = default;
 *         FontInfo(const FontInfo& ) = default;
 *         SkFont fFont;
 *         TextRange fTextRange;
 *     };
 *
 *     /** Returns the font that is used to shape the text at the position
 *      *
 *      * @param codeUnitIndex   text index
 *      * @return                font info or an empty font info if the text is not found
 *      */
 *     virtual SkFont getFontAt(TextIndex codeUnitIndex) const = 0;
 *
 *     /** Returns the font used to shape the text at the given UTF-16 offset.
 *      *
 *      * @param codeUnitIndex   a UTF-16 offset in the paragraph
 *      * @return                font info or an empty font info if the text is not found
 *      */
 *     virtual SkFont getFontAtUTF16Offset(size_t codeUnitIndex) = 0;
 *
 *     /** Returns the information about all the fonts used to shape the paragraph text
 *      *
 *      * @return                a list of fonts and text ranges
 *      */
 *     virtual std::vector<FontInfo> getFonts() const = 0;
 *
 * protected:
 *     sk_sp<FontCollection> fFontCollection;
 *     ParagraphStyle fParagraphStyle;
 *
 *     // Things for Flutter
 *     SkScalar fAlphabeticBaseline;
 *     SkScalar fIdeographicBaseline;
 *     SkScalar fHeight;
 *     SkScalar fWidth;
 *     SkScalar fMaxIntrinsicWidth;
 *     SkScalar fMinIntrinsicWidth;
 *     SkScalar fLongestLine;
 *     bool fExceededMaxLines;
 * }
 * ```
 */
public abstract class Paragraph public constructor(
  style: ParagraphStyle,
  fonts: SkSp<FontCollection>,
) {
  /**
   * C++ original:
   * ```cpp
   * sk_sp<FontCollection> fFontCollection
   * ```
   */
  protected var fFontCollection: Int = TODO("Initialize fFontCollection")

  /**
   * C++ original:
   * ```cpp
   * ParagraphStyle fParagraphStyle
   * ```
   */
  protected var fParagraphStyle: Int = TODO("Initialize fParagraphStyle")

  /**
   * C++ original:
   * ```cpp
   * SkScalar fAlphabeticBaseline
   * ```
   */
  protected var fAlphabeticBaseline: Int = TODO("Initialize fAlphabeticBaseline")

  /**
   * C++ original:
   * ```cpp
   * SkScalar fIdeographicBaseline
   * ```
   */
  protected var fIdeographicBaseline: Int = TODO("Initialize fIdeographicBaseline")

  /**
   * C++ original:
   * ```cpp
   * SkScalar fHeight
   * ```
   */
  protected var fHeight: Int = TODO("Initialize fHeight")

  /**
   * C++ original:
   * ```cpp
   * SkScalar fWidth
   * ```
   */
  protected var fWidth: Int = TODO("Initialize fWidth")

  /**
   * C++ original:
   * ```cpp
   * SkScalar fMaxIntrinsicWidth
   * ```
   */
  protected var fMaxIntrinsicWidth: Int = TODO("Initialize fMaxIntrinsicWidth")

  /**
   * C++ original:
   * ```cpp
   * SkScalar fMinIntrinsicWidth
   * ```
   */
  protected var fMinIntrinsicWidth: Int = TODO("Initialize fMinIntrinsicWidth")

  /**
   * C++ original:
   * ```cpp
   * SkScalar fLongestLine
   * ```
   */
  protected var fLongestLine: Int = TODO("Initialize fLongestLine")

  /**
   * C++ original:
   * ```cpp
   * bool fExceededMaxLines
   * ```
   */
  protected var fExceededMaxLines: Boolean = TODO("Initialize fExceededMaxLines")

  /**
   * C++ original:
   * ```cpp
   * SkScalar getMaxWidth() { return fWidth; }
   * ```
   */
  public fun getMaxWidth(): Int {
    TODO("Implement getMaxWidth")
  }

  /**
   * C++ original:
   * ```cpp
   * SkScalar getHeight() { return fHeight; }
   * ```
   */
  public fun getHeight(): Int {
    TODO("Implement getHeight")
  }

  /**
   * C++ original:
   * ```cpp
   * SkScalar getMinIntrinsicWidth() { return fMinIntrinsicWidth; }
   * ```
   */
  public fun getMinIntrinsicWidth(): Int {
    TODO("Implement getMinIntrinsicWidth")
  }

  /**
   * C++ original:
   * ```cpp
   * SkScalar getMaxIntrinsicWidth() { return fMaxIntrinsicWidth; }
   * ```
   */
  public fun getMaxIntrinsicWidth(): Int {
    TODO("Implement getMaxIntrinsicWidth")
  }

  /**
   * C++ original:
   * ```cpp
   * SkScalar getAlphabeticBaseline() { return fAlphabeticBaseline; }
   * ```
   */
  public fun getAlphabeticBaseline(): Int {
    TODO("Implement getAlphabeticBaseline")
  }

  /**
   * C++ original:
   * ```cpp
   * SkScalar getIdeographicBaseline() { return fIdeographicBaseline; }
   * ```
   */
  public fun getIdeographicBaseline(): Int {
    TODO("Implement getIdeographicBaseline")
  }

  /**
   * C++ original:
   * ```cpp
   * SkScalar getLongestLine() { return fLongestLine; }
   * ```
   */
  public fun getLongestLine(): Int {
    TODO("Implement getLongestLine")
  }

  /**
   * C++ original:
   * ```cpp
   * bool didExceedMaxLines() { return fExceededMaxLines; }
   * ```
   */
  public fun didExceedMaxLines(): Boolean {
    TODO("Implement didExceedMaxLines")
  }

  /**
   * C++ original:
   * ```cpp
   * virtual void layout(SkScalar width) = 0
   * ```
   */
  public abstract fun layout(width: SkScalar)

  /**
   * C++ original:
   * ```cpp
   * virtual void paint(SkCanvas* canvas, SkScalar x, SkScalar y) = 0
   * ```
   */
  public abstract fun paint(
    canvas: SkCanvas?,
    x: SkScalar,
    y: SkScalar,
  )

  /**
   * C++ original:
   * ```cpp
   * virtual void paint(ParagraphPainter* painter, SkScalar x, SkScalar y) = 0
   * ```
   */
  public abstract fun paint(
    painter: ParagraphPainter?,
    x: SkScalar,
    y: SkScalar,
  )

  /**
   * C++ original:
   * ```cpp
   * virtual std::vector<TextBox> getRectsForRange(unsigned start,
   *                                                   unsigned end,
   *                                                   RectHeightStyle rectHeightStyle,
   *                                                   RectWidthStyle rectWidthStyle) = 0
   * ```
   */
  public abstract fun getRectsForRange(
    start: UInt,
    end: UInt,
    rectHeightStyle: RectHeightStyle,
    rectWidthStyle: RectWidthStyle,
  ): Int

  /**
   * C++ original:
   * ```cpp
   * virtual std::vector<TextBox> getRectsForPlaceholders() = 0
   * ```
   */
  public abstract fun getRectsForPlaceholders(): Int

  /**
   * C++ original:
   * ```cpp
   * virtual PositionWithAffinity getGlyphPositionAtCoordinate(SkScalar dx, SkScalar dy) = 0
   * ```
   */
  public abstract fun getGlyphPositionAtCoordinate(dx: SkScalar, dy: SkScalar): Int

  /**
   * C++ original:
   * ```cpp
   * virtual SkRange<size_t> getWordBoundary(unsigned offset) = 0
   * ```
   */
  public abstract fun getWordBoundary(offset: UInt): Int

  /**
   * C++ original:
   * ```cpp
   * virtual void getLineMetrics(std::vector<LineMetrics>&) = 0
   * ```
   */
  public abstract fun getLineMetrics(param0: List<LineMetrics>)

  /**
   * C++ original:
   * ```cpp
   * virtual size_t lineNumber() = 0
   * ```
   */
  public abstract fun lineNumber(): ULong

  /**
   * C++ original:
   * ```cpp
   * virtual void markDirty() = 0
   * ```
   */
  public abstract fun markDirty()

  /**
   * C++ original:
   * ```cpp
   * virtual int32_t unresolvedGlyphs() = 0
   * ```
   */
  public abstract fun unresolvedGlyphs(): Int

  /**
   * C++ original:
   * ```cpp
   * virtual std::unordered_set<SkUnichar> unresolvedCodepoints() = 0
   * ```
   */
  public abstract fun unresolvedCodepoints(): Int

  /**
   * C++ original:
   * ```cpp
   * virtual void updateTextAlign(TextAlign textAlign) = 0
   * ```
   */
  public abstract fun updateTextAlign(textAlign: TextAlign)

  /**
   * C++ original:
   * ```cpp
   * virtual void updateFontSize(size_t from, size_t to, SkScalar fontSize) = 0
   * ```
   */
  public abstract fun updateFontSize(
    from: ULong,
    to: ULong,
    fontSize: SkScalar,
  )

  /**
   * C++ original:
   * ```cpp
   * virtual void updateForegroundPaint(size_t from, size_t to, SkPaint paint) = 0
   * ```
   */
  public abstract fun updateForegroundPaint(
    from: ULong,
    to: ULong,
    paint: SkPaint,
  )

  /**
   * C++ original:
   * ```cpp
   * virtual void updateBackgroundPaint(size_t from, size_t to, SkPaint paint) = 0
   * ```
   */
  public abstract fun updateBackgroundPaint(
    from: ULong,
    to: ULong,
    paint: SkPaint,
  )

  /**
   * C++ original:
   * ```cpp
   * virtual void visit(const Visitor&) = 0
   * ```
   */
  public abstract fun visit(param0: ParagraphVisitor)

  /**
   * C++ original:
   * ```cpp
   * virtual void extendedVisit(const ExtendedVisitor&) = 0
   * ```
   */
  public abstract fun extendedVisit(param0: ParagraphExtendedVisitor)

  /**
   * C++ original:
   * ```cpp
   * virtual int getPath(int lineNumber, SkPath* dest) = 0
   * ```
   */
  public abstract fun getPath(lineNumber: Int, dest: SkPath?): Int

  /**
   * C++ original:
   * ```cpp
   * virtual bool containsEmoji(SkTextBlob* textBlob) = 0
   * ```
   */
  public abstract fun containsEmoji(textBlob: SkTextBlob?): Boolean

  /**
   * C++ original:
   * ```cpp
   * virtual bool containsColorFontOrBitmap(SkTextBlob* textBlob) = 0
   * ```
   */
  public abstract fun containsColorFontOrBitmap(textBlob: SkTextBlob?): Boolean

  /**
   * C++ original:
   * ```cpp
   * virtual int getLineNumberAt(TextIndex codeUnitIndex) const = 0
   * ```
   */
  public abstract fun getLineNumberAt(codeUnitIndex: TextIndex): Int

  /**
   * C++ original:
   * ```cpp
   * virtual int getLineNumberAtUTF16Offset(size_t codeUnitIndex) = 0
   * ```
   */
  public abstract fun getLineNumberAtUTF16Offset(codeUnitIndex: ULong): Int

  /**
   * C++ original:
   * ```cpp
   * virtual bool getLineMetricsAt(int lineNumber, LineMetrics* lineMetrics) const = 0
   * ```
   */
  public abstract fun getLineMetricsAt(lineNumber: Int, lineMetrics: LineMetrics?): Boolean

  /**
   * C++ original:
   * ```cpp
   * virtual TextRange getActualTextRange(int lineNumber, bool includeSpaces) const = 0
   * ```
   */
  public abstract fun getActualTextRange(lineNumber: Int, includeSpaces: Boolean): Int

  /**
   * C++ original:
   * ```cpp
   * virtual bool getGlyphClusterAt(TextIndex codeUnitIndex, GlyphClusterInfo* glyphInfo) = 0
   * ```
   */
  public abstract fun getGlyphClusterAt(codeUnitIndex: TextIndex, glyphInfo: GlyphClusterInfo?): Boolean

  /**
   * C++ original:
   * ```cpp
   * virtual bool getClosestGlyphClusterAt(SkScalar dx,
   *                                           SkScalar dy,
   *                                           GlyphClusterInfo* glyphInfo) = 0
   * ```
   */
  public abstract fun getClosestGlyphClusterAt(
    dx: SkScalar,
    dy: SkScalar,
    glyphInfo: GlyphClusterInfo?,
  ): Boolean

  /**
   * C++ original:
   * ```cpp
   * virtual bool getGlyphInfoAtUTF16Offset(size_t codeUnitIndex, GlyphInfo* glyphInfo) = 0
   * ```
   */
  public abstract fun getGlyphInfoAtUTF16Offset(codeUnitIndex: ULong, glyphInfo: GlyphInfo?): Boolean

  /**
   * C++ original:
   * ```cpp
   * virtual bool getClosestUTF16GlyphInfoAt(SkScalar dx, SkScalar dy, GlyphInfo* glyphInfo) = 0
   * ```
   */
  public abstract fun getClosestUTF16GlyphInfoAt(
    dx: SkScalar,
    dy: SkScalar,
    glyphInfo: GlyphInfo?,
  ): Boolean

  /**
   * C++ original:
   * ```cpp
   * virtual SkFont getFontAt(TextIndex codeUnitIndex) const = 0
   * ```
   */
  public abstract fun getFontAt(codeUnitIndex: TextIndex): Int

  /**
   * C++ original:
   * ```cpp
   * virtual SkFont getFontAtUTF16Offset(size_t codeUnitIndex) = 0
   * ```
   */
  public abstract fun getFontAtUTF16Offset(codeUnitIndex: ULong): Int

  /**
   * C++ original:
   * ```cpp
   * virtual std::vector<FontInfo> getFonts() const = 0
   * ```
   */
  public abstract fun getFonts(): Int

  public data class VisitorInfo public constructor(
    public val font: Int,
    public var origin: Int,
    public var advanceX: Int,
    public var count: Int,
    public val glyphs: Int?,
    public val positions: Int?,
    public val utf8Starts: UInt?,
    public var flags: UInt,
  )

  public data class ExtendedVisitorInfo public constructor(
    public val font: Int,
    public var origin: Int,
    public var advance: Int,
    public var count: Int,
    public val glyphs: Int?,
    public var positions: Int?,
    public val bounds: Int?,
    public val utf8Starts: UInt?,
    public var flags: UInt,
  )

  public data class GlyphClusterInfo public constructor(
    public var fBounds: Int,
    public var fClusterTextRange: Int,
    public var fGlyphClusterPosition: Int,
  )

  public data class GlyphInfo public constructor(
    public var fGraphemeLayoutBounds: Int,
    public var fGraphemeClusterTextRange: Int,
    public var fDirection: Int,
    public var fIsEllipsis: Boolean,
  )

  public data class FontInfo public constructor(
    public var fFont: Int,
    public var fTextRange: Int,
  )

  public enum class VisitorFlags {
    kWhiteSpace_VisitorFlag,
  }

  public companion object {
    /**
     * C++ original:
     * ```cpp
     * SkPath Paragraph::GetPath(SkTextBlob* textBlob) {
     *     SkPathBuilder builder;
     *     SkTextBlobRunIterator iter(textBlob);
     *     while (!iter.done()) {
     *         SkFont font = iter.font();
     *         struct Rec { SkPathBuilder* fBuilder; SkPoint fOffset; const SkPoint* fPos; } rec =
     *             {&builder, {textBlob->bounds().left(), textBlob->bounds().top()},
     *              iter.points()};
     *         font.getPaths({iter.glyphs(), iter.glyphCount()},
     *             [](const SkPath* src, const SkMatrix& mx, void* ctx) {
     *                 Rec* rec = (Rec*)ctx;
     *                 if (src) {
     *                     SkMatrix tmp(mx);
     *                     tmp.postTranslate(rec->fPos->fX - rec->fOffset.fX,
     *                                       rec->fPos->fY - rec->fOffset.fY);
     *                     rec->fBuilder->addPath(*src, tmp);
     *                 }
     *                 rec->fPos += 1;
     *             },
     *             &rec);
     *         iter.next();
     *     }
     *     return builder.detach();
     * }
     * ```
     */
    public fun getPath(textBlob: SkTextBlob?): Int {
      TODO("Implement getPath")
    }
  }
}
