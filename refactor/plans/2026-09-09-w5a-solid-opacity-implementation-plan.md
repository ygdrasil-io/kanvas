...
Le composite ajoute les lanes stroke/hairline W4d authentiques et refuse plus de 512 runs avant toute copie de scène par lane. La preuve 512/513 utilise le même runtime/backend avec une autre Surface pour le refus; seule la preuve gradient valide→refus→valide est réellement same-Surface. Le public `DrawMesh` sans programme normalise vers les prepared vertices; `MeshProgram` conserve son contrat distinct hors promotion W5a. Les deux reviews globales ont demandé cette correction et restent à reprendre, non validées implicitement par les tests.

Continuation numérique autorisée dans cette même vague : le RED public bleu 18 hors singleton 17 a révélé que la conversion d'attachement fixed-function devait être bornée séparément du `pow` WGSL. L'oracle conserve les bornes officielles Metal/D3D encode/decode/UNORM, propage l'état d'attachement et la corrélation affine de la destination, sans seuil empirique. Le cas original reste une régression à `{17, 18}`. Treize anciennes fixtures multi-draw devenues `Unbounded` ont des entrées reformulées non triviales qui préservent ordre/mutation et satisfont à nouveau le singleton ou deux codes adjacents; un ordre inversé rendu publiquement est rejeté. Les scènes arbitraires peuvent rester `Unbounded` avec ces bornes officielles : aucune assertion n'est élargie et aucun pipeline destination-read n'est ajouté. Dérivation, sources et limites sont consignées dans [le status W05](../waves/W05-material-graph/status.md).

Vérification de la continuation `7dbaf8cdf672e836f6ec6d77b1734cb68b6669db` : compile JVM GREEN, W5a 49 tests (48 passés, 1 skip) et GPUPlan 71 tests (69 passés, 2 skips), soit 120 tests publics, 117 passés, 0 failure/error, 3 skips AA4 authentiques. Les deux re-reviews Task 8 restent pending.
