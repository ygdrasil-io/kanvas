...
Valider l'ensemble des contours projetés avec le broad phase budgété : chaque arête projetée conserve son pont source `F64`, classifié avant l'arête `F32`; un point projeté doit avoir un point/overlap source correspondant et un overlap projeté un overlap source correspondant. Les contacts de lattice voisins d'une tangence source prouvée entre la même paire de cycles restent permis, mais une jonction, un croisement ou un overlap partiel nouveau entre cycles source-disjoints échoue sauf cycle projeté structurellement identique soumis à l'agrégation globale. Pour un contact nouveau non-adjacent au sein d'un cycle, comparer la modification exacte d'aire normalisée. Grouper tous les cycles par clé structurelle `F32` indépendante de la rotation/orientation, avec zéros signés normalisés et sommets colinéaires exacts supprimés; conserver multiplicité/orientation et agréger par expansion la modification signée ainsi que l'étendue cumulée des doubles-aires source. L'une ou l'autre strictement supérieure à `2^-45` échoue, ce qui couvre outer/hole et un groupe `+,-,+` sans analyse seulement paire-à-paire.
