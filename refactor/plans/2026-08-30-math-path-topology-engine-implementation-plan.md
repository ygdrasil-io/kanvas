...
Valider l'ensemble des contours projetés avec le broad phase budgété : chaque arête projetée conserve son pont source `F64`, classifié avant l'arête `F32`. L'ordre partiel est obligatoire : `Point F32 <- Point F64 | Overlap F64`, mais `Overlap F32 <- Overlap F64` seulement. Un witness `Overlap` doit couvrir les endpoints du contact projeté ainsi que les deux ponts source localement mappés dans son intervalle source exact et son image F32. Un witness `Point` ne conserve jamais un intervalle `Overlap F32` : une tangence arrondie peut seulement normaliser sa relation en `Point F32` lorsqu'un certificat direct relie deux branches sources opposées et non parallèles autour du point exact à la même coordonnée normale de lattice; deux rails parallèles opposés restent un long overlap nouveau et échouent. Aucun cache par paire de contours, BFS, marche par hops ni fermeture transitive ne peut autoriser un contact distant; une preuve locale ambiguë échoue par `path-f32-projection-collapse`, et chaque prédicat est débité d'avance. Une jonction, un croisement ou un overlap partiel nouveau entre cycles source-disjoints échoue sauf cycle projeté structurellement identique soumis à l'agrégation globale. Pour un contact nouveau non-adjacent au sein d'un cycle, dériver des préimages source exactes; additionner sans signe les doubles-aires des deux arcs fermés au Point ou de la bande locale aux endpoints d'un Overlap, et échouer conservativement si la préimage est ambiguë. Ainsi une compensation d'aire distante ne peut jamais autoriser la perte locale. Grouper tous les cycles par clé structurelle `F32` indépendante de la rotation/orientation, avec zéros signés normalisés et sommets colinéaires exacts supprimés; choisir la rotation minimale sur la séquence cyclique complète dans les deux sens (Booth), y compris avec minimum répété. Booth débite `maxCandidateProbes` juste avant chaque comparaison réelle de points, plutôt qu'une réservation fixe `2n`; cela garde le coût `O(n)` et la même frontière JVM/JS. Conserver multiplicité/orientation et agréger par expansion la modification signée ainsi que l'étendue cumulée des doubles-aires source. L'une ou l'autre strictement supérieure à `2^-45` échoue, ce qui couvre outer/hole et un groupe `+,-,+` sans analyse seulement paire-à-paire.
