...
Pendant la projection, arrondir explicitement sur la grille `F32` commune JVM/JS, supprimer les sommets adjacents devenus identiques puis recalculer aire et orientation. Supprimer un cycle dégénéré seulement si son aire absolue `F64` normalisée était au plus `tolerance² = 2^-46` (donc comparer sa double-aire exacte à `2^-45`); sinon échouer par `IllegalStateException("path-f32-projection-collapse")`, car une frontière topologiquement significative a disparu.

Valider l'ensemble des contours projetés avec le broad phase budgété : chaque arête projetée conserve son pont source `F64`, classifié avant l'arête `F32`. L'ordre partiel est obligatoire : `Point F32 <- Point F64 | Overlap F64`, mais `Overlap F32 <- Overlap F64` seulement. Un witness `Overlap` doit couvrir les endpoints du contact projeté ainsi que les deux ponts source localement mappés dans son intervalle source exact et son image F32.

Un witness `Point` ne conserve jamais un intervalle `Overlap F32`, ni par requalification. Pour préserver une tangence dont les subdivisions d'une branche courbe sont devenues colinéaires en `F32`, une normalisation amont peut seulement compacter une branche locale non-cyclique qui part de ce witness : chaque pont source de son run doit être robuste non parallèle au carrier F32 ramené par la normalisation, et chaque pont conservé doit avoir le witness exact comme endpoint. Un rail source réel, même très proche, ne satisfait donc pas cette preuve. Après cela, le certificat direct exige que les deux ponts candidats et les deux ponts witness contiennent le point source exact, que les candidats soient le pont witness ou son voisin unique sans franchir le seam, et que le locus F32 contienne l'ancre exacte. Aucun cache par paire de contours, BFS, marche par hops, wrap cyclic ou agrégation de witnesses ne peut autoriser un contact distant; une preuve locale ambiguë échoue par `path-f32-projection-collapse`, et chaque prédicat est débité d'avance.

Une jonction, un croisement ou un overlap partiel nouveau entre cycles source-disjoints échoue sauf cycle projeté structurellement identique soumis à l'agrégation globale. Pour un contact nouveau non-adjacent au sein d'un cycle, dériver des préimages source exactes; additionner sans signe les doubles-aires des deux arcs fermés au Point ou de la bande locale aux endpoints d'un Overlap, et échouer conservativement si la préimage est ambiguë. Ainsi une compensation d'aire distante ne peut jamais autoriser la perte locale. Grouper tous les cycles par clé structurelle `F32` indépendante de la rotation/orientation, avec zéros signés normalisés et sommets colinéaires exacts supprimés; choisir la rotation minimale sur la séquence cyclique complète dans les deux sens (Booth), y compris avec minimum répété.

Booth débite `maxCandidateProbes` juste avant chaque comparaison réelle de points. Une passe vérifie d'abord que son plafond canonique `3n` est disponible, puis débite seulement son padding prouvé après les comparaisons réelles : les phases conclues par mismatch sont couvertes par l'avance monotone d'au moins `offset + 1` de l'un des deux indices (moins de `2n` au total), plus une seule phase finale d'égalités de longueur au plus `n`. C'est une borne sûre, calculée en `Long`, et non l'ancienne réservation insuffisante `2n`; elle donne la même frontière JVM/JS sous rotation et réversion. Conserver multiplicité/orientation et agréger par expansion la modification signée ainsi que l'étendue cumulée des doubles-aires source. L'une ou l'autre strictement supérieure à `2^-45` échoue, ce qui couvre outer/hole et un groupe `+,-,+` sans analyse seulement paire-à-paire.
