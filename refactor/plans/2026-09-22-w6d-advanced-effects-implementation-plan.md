...
- [x] **Step 7: Request whole-branch Sol review and run the one bounded correction policy.** Review `codex/w6c-spatial-dag..HEAD` for spec coverage, all eleven families, bounds/mappings, save order, immutable Picture/wire, runtime ABI/hash isolation, resources/lifetimes/budgets, atomic visibility, public evidence, and legacy fallback. The single Terra correction wave (`536d0e58e`, `79c1fa86d`) and single scoped Sol re-review were completed. **Gate not green:** two Important Magnifier variants remain: `0 < zoom < 1` under clipped backdrop/previous, and a transformed direct draw whose reverse demand omits the draw transform; see durable W06 status. No second review loop is authorized by this plan.
