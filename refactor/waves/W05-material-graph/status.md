...
## W5h — correction Sol R1 ; publication Draft pending final Sol review

La review Task7 Sol a trouvé **0 Critical / 1 Important**, accepté : le layout Core global par taille répétait tous les uniforms dans chaque segment physique séparé par Text/Vertices ou une copie destination. La correction locale retire ce layout commun global et fait porter aux layouts, sizing, bytes, resources, seals et offsets la liste exacte des commandes de chaque `HostRunBoundaryPlan` segment. Le bind V6 conserve les ranges calculés avant publication ; direct/Path, root, witness et native vérifient la même liste et l'identité du plan lié, sans re-layout/repack, second planner, changement de budget ou géométrie. Les routes legacy gardent leurs owners historiques. Coût si frontière erronée : corruption des offsets ou faux refus/admission de budget ; containment par listes ordonnées et identities exactes à chaque frontière.

Le témoin public est une extension de `W5hConvergenceSurfacePixelTest.sameCapturedRuntimeOwnerAcrossEveryLane` (toujours exactement cinq méthodes). À400000 octets, les mêmes512 RRects et un Vertices passent groupés mais refusent quand Vertices sépare CoreA/CoreB sur HEAD9ef ; après correction, les deux ordres rendent leurs pixels,1024 RRects refusent, puis la même Surface récupère. RED XML1/0PASS/1failure, GREEN convergence5/5, aucun error/skip. Gradle1 et native133 UNKNOWN sont séparés des résultats JUnit. Custody `/private/tmp/w5h-task7-sol-uniform-run-{red-400k,green-initial}.{log,xml}`. Correctif committé `70c70d344edddab1873acea399ab6fc18d8dcd58`, implementation-addressed ; même Sol re-review puis whole-branch Sol pending contrôleur. L'ancien covering ci-dessous appartient exclusivement à d88, pas à la nouvelle correction. Aucun verdict final ni URL W5h n'est inventé.
