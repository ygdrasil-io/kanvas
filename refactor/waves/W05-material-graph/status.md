...
## W5h — Task7 R1 et whole-branch Sol APPROVED ; Draft PR #2402 publiée

La re-review Task7 R1 par le même Sol est **APPROVED — 0 Critical / 0 Important / 0 Minor** sur `9ef62acd7..b5a7fd312`. Elle clôt l'Important initial : le layout Core global par taille répétait tous les uniforms dans chaque segment physique séparé par Text/Vertices ou une copie destination. La correction locale retire ce layout commun global et fait porter aux layouts, sizing, bytes, resources, seals et offsets la liste exacte des commandes de chaque `HostRunBoundaryPlan` segment. Le bind V6 conserve les ranges calculés avant publication ; direct/Path, root, witness et native vérifient la même liste et l'identité du plan lié, sans re-layout/repack, second planner, changement de budget ou géométrie. Les routes legacy gardent leurs owners historiques. Coût si frontière erronée : corruption des offsets ou faux refus/admission de budget ; containment par listes ordonnées et identities exactes à chaque frontière.

Le témoin public est une extension de `W5hConvergenceSurfacePixelTest.sameCapturedRuntimeOwnerAcrossEveryLane` (toujours exactement cinq méthodes). À400000 octets, les mêmes512 RRects et un Vertices passent groupés mais refusent quand Vertices sépare CoreA/CoreB sur HEAD9ef ; après correction, les deux ordres rendent leurs pixels,1024 RRects refusent, puis la même Surface récupère. RED XML1/0PASS/1failure, GREEN convergence5/5, aucun error/skip. Gradle1 et native133 UNKNOWN sont séparés des résultats JUnit. Custody `/private/tmp/w5h-task7-sol-uniform-run-{red-400k,green-initial}.{log,xml}`. Correctif committé `70c70d344edddab1873acea399ab6fc18d8dcd58`, approuvé par la re-review Task7 R1. La re-review whole-branch par le même Sol sur `fff9f90171ebbdaa6642482dd757d153c7e909c3` est **APPROVED — C0/I0/M0** après correction documentaire I1. Les Steps9–10 sont closes ; R1 code appartenait à la review Task7, pas à la correction whole-branch. L'ancien covering ci-dessous appartient exclusivement à d88, pas à la nouvelle correction. Step11 close : [Draft PR W5h #2402](https://github.com/ygdrasil-io/kanvas/pull/2402) OPEN/Draft, empilée sur [W5g #2401](https://github.com/ygdrasil-io/kanvas/pull/2401) / `codex/w5g-composed-procedural-materials`. Aucun merge ni claim ISO/global ; les limites de preuve restent inchangées.

Vérification R1 sur1062 hashes inchangés : H G4=598/598, G5=444/444, G6=84/84 (1021 H +105 contrôles), selector public683=1/1 ; Point matrix=1 JUnit rouge dont le tuple nom/type/message est strictement identique à `/private/tmp/w5h-task4-historical-base-d9bc.xml` (42 cas internes PASS,3 SOFT_LIGHT historiques). Total préservation1128 registered/1127PASS/1failure/0error/0skip ; avec convergence5,1133/1132/1/0/0. Commande exacte : `rtk proxy ./gradlew :kanvas:test --tests org.graphiks.kanvas.surface.W5hGeometryHLaneSurfacePixelTest --tests org.graphiks.kanvas.surface.W5hTextVerticesSurfacePixelTest --tests org.graphiks.kanvas.surface.W5hImageOriginSurfacePixelTest --tests 'org.graphiks.kanvas.surface.W5aMaterialSurfacePixelTest.683 distinct three-entry point materials refuse the real table bound publicly' --tests org.graphiks.kanvas.surface.gpu.GPUAllApiBlendSurfaceTest.drawPointHistoricalW5bMatrix --rerun-tasks --no-parallel --console=plain`. Gradle1/9m26s pour l'assertion historique, aucun native133/OOM dans ce run. Logs/XML `...sol-uniform-run-h-point-683*`, manifeste exact `/private/tmp/w5h-task7-sol-uniform-run-manifest.json` SHA256 `64ee304e3335891051736f9bbfd5112fe4d92c9e19529b835a5f89777d064210`. L'ancien skip AA4 n'est pas sélectionné ni recompté ici.

Trois compiles séparées `rtk proxy ./gradlew :gpu-renderer:compileKotlin`, `:kanvas:compileKotlin`, `:kanvas:compileTestKotlin`, chacune avec `--no-parallel --console=plain`, exit0 (944ms/622ms/644ms). Contrôle supplémentaire renderer compileTest exit1/7s, exactement16 erreurs baseline,0new/0missing, aucun test/shim adapté. Logs `/private/tmp/w5h-task7-sol-uniform-run-compile-{renderer,kanvas,kanvas-test,renderer-test-baseline}.log`. Manifest source `/private/tmp/w5h-task7-sol-uniform-run-all-production.sha256` SHA256 `5a316676884be3b5c32cee2d0303360381e27100fd77f26278f8f0849b6a8859`, convergence SHA256 `4bfbb70dafe90110f5948e1328595b9880c42bf9aabc2932764bca68cf84e8d3` ; vérification finale1062/1062. Six fichiers production et une méthode publique modifiés, aucun fichier supprimé. Les33 cellules H restent14+12+7, sans nouvelle cellule ni claim de covering monolithique sur70c. Tous les gaps W8/v0/device-loss/Picture/baselines ci-dessous restent ouverts.

### Checkpoint antérieur d88/9ef — custody historique

Source du covering Task7 figée `d88d8f49132fc7a4461df9945145dfc0feadb5e9` après implémentation `9a25200b427fcd9e9fd1a6382d37d870a4102a45` et correction Point/SCISSOR `e9c9ff1cbd4a6a5bb4fcbb6b387ffa35540906ac`, base Task6 `343dfafcb831c81e2ed2a49746ce096664811906`, branche `codex/w5h-registered-runtime-effects`, cible empilée `codex/w5g-composed-procedural-materials`. Les trois diagnostics image/mask-filter sont corrigés ; covering attribué et six compiles achevés sur ce freeze, avec les dettes/exclusions explicites ci-dessous. Les Steps1–8 locaux étaient clos par le commit tracking-only `9ef62acd751e4416484a80c5961c5856a742f256` avant la review et la correction R1 ci-dessus. Pas de push ni merge dans ce lot.
...
| Task7 convergence/owner frame-wide | `9a25200b4`, `e9c9ff1cb` (Point domain/SCISSOR), `d88d8f491` (image/mask-filter domain), `70c70d344` (R1 slabs par segment), tracking `b5a7fd312`, `fff9f9017` (I1 documentaire whole-branch) | Task7 R1 Sol **APPROVED — 0 Critical / 0 Important / 0 Minor** ; whole-branch **APPROVED — C0/I0/M0** après I1 documentaire ; [Draft PR W5h #2402](https://github.com/ygdrasil-io/kanvas/pull/2402) publiée (Step11), sur [W5g #2401](https://github.com/ygdrasil-io/kanvas/pull/2401) |
...
R41 partage upload/request/accounting/lease uniquement pour la même identité capturée `ImageResourceSnapshot.Pixels`, après vérification des faits physiques et des octets. Deux owners indépendants restent deux réservations même si le contenu canonique est égal. Les deux ordres ordinary/V5, le budget causal 549432/1073720 avec fenêtre 800000, le refus distinct-owner et la recovery sont prouvés par pixels publics. `Picture.playback(Canvas)` perd toujours `SetClip` et le composite refuse toujours les clips complexes ; ces gaps préexistants ne sont ni aggravés ni masqués.

Epoch 7 ciblée : 8/9 PASS en 27 s ; compositions6/6, Stroke/recovery et mixed-lane passent, seule la mutation Picture Fractal8 refuse. R40 change uniquement ce child incident à Fractal2 tout en gardant matrice mutable, deux familles/seeds, stitch, bounded/disjoint, capture et replays ; octave8/255 reste couvert sur les deux géométries et en stitch dans la matrice complète. Aucun guard/proof/production/tolérance/wire change. Gradle1/native133 reste `UNKNOWN`, sources pre/post identiques.

R39 est limité au proof Noise existant : corrélation du même `f` dans le smooth et du même `a` dans le lerp original, après validation ordinaire complète, avec extrema dirigés et erreur gamma/DAZ/FTZ explicite dans des contextes d'identité frais. Intersection vide = refus ; aucun WGSL/opcode/clamp/epsilon/alpha/domain/tail/tolérance ne change. Risque suivi : erreur d'amplification, d'identité ou de branche négative/DAZ. Scope inchangé39 Modify/5 Kotlin Create/21 assets/2 producteurs éphémères.

Epoch 6 nonfinale : 13/22 PASS, 9 FAIL, 0 erreur/skip. Les domaines Noise 2/8/255, l'accounting exact sur trois routes, les ressources Noise/gradient/image et le budget 32 KB passent. Les sept refus numériques de filtre restent ouverts. R37 corrige seulement l'attente Stroke vers la frontière existante `composed.slice`; R38 corrige seulement le clip mixed-lane vers `antiAlias=false`, le défaut AA=true étant explicitement hors scope W4d. Aucun owner production/promotion/fallback/test d'infrastructure nouveau. Gradle1/native133 reste `UNKNOWN`. Scope inchangé39 Modify/5 Kotlin Create/21 assets/2 producteurs éphémères.

R36 conserve la même preuve mais remplace après custody la matérialisation quadratique des couples d'adresses par une union `BooleanArray` de 256 entrées et le raccourci exact y=all256 ; aucun ensemble d'adresses ni garde numérique ne change.

R36 resserre uniquement les opérations originales déjà prouvées : phase `q-floor(q)` corrélée lorsque FLOOR est singleton, puis adresses exactes floor+corner/période/permutation vers les U16 du slab authentifié. Le floor négatif BigInteger est vers −∞ ; les domaines larges conservent all256. Aucun changement du graph/WGSL/guards/tolérance. Risque suivi : oubli DAZ ou erreur signed/corner/period/permutation ; négatifs/transforms/stitch/seeds/Matrix/mutation/final4/Sol restent obligatoires. Scope inchangé39 Modify/5 Kotlin Create/21 assets/2 producteurs éphémères.

R35 ajoute uniquement l’enveloppe exacte W3 du preflight : la ligne Noise déclarée par R34 doit correspondre au même slab authentifié packet-stage et à ses bytes/limits/lifetime/peak, sans nouvelle preparation. Les guards geometry/clip/blend/readback/Gradient restent intacts. Risque suivi : refus direct/composite, ressource contrefaite ou double charge. Scope Task4 : 39 Modify, 5 Kotlin Create, 21 assets, 2 producteurs historiques éphémères ; aucun test d’infrastructure/harness.

R34 étend Task4 à cinq owners de lowering/custody strictement nécessaires : W3/W5b conserve le même slab Noise authentifié dans le peak, la catégorie et l'allocation exacts, le validateur packet W3 accepte uniquement cette identité, et le composite la déduplique une seule fois entre lanes. Les comptes geometry, les gates capability/AA/clip, les lifetimes et les refus historiques restent inchangés. Risque suivi : refus direct-W3, sous-comptage, ressource contrefaite ou charge multipliée par lane. Les preuves publiques work/storage/recovery et final4 restent obligatoires ; aucun test d'infrastructure ni changement de harness. Scope Task4 : 38 Modify, 5 Kotlin Create, 21 assets, 2 producteurs historiques éphémères.
...
Task4 nonfinal : première compilation ciblée exit0/1m11, console initiale tronquée conservée honnêtement (pas cinq gates ni compilation clean). Les12 assertions publiques initiales passent ; console/XML complets conservés dans le rapport2399lignes/SHAa7250e8c, mais commande FAILED11s/Gradle1/native226exit133, causeUNKNOWN. Lors du contrôle ROOT, le fichier XML réel avait déjà avancé :12tests/9PASS/3échecs `FixtureUnbounded` dans les contre-factuels Perlin stitched avant Surface,0error/skip, timestamp23:50:26.589Z/SHA2614cf91, intégralement lu. Les cas nonzero2/8/255 et Fractal stitched passent dans cette epoch nonfinale. Le précédent XML12PASS n'est donc pas revendiqué byte-exact à un fichier réel contrôlé ROOT ;52/54empreintes de la première inventory sont encore identiques, seules les extensions autorisées oracle/NoiseTest ont évolué. Même Astra poursuit les corrections sans élargir la borne singleton/deux codes adjacents ; archives/work/resources/recovery/final4/source gelée/commit/Sol et convergence/final19 restent ouverts, aucune acceptance Noise/nativeGREEN.
...
Task4 : dérivation numérique corrigée102lignes validée MAIN (R28), puis genuineRED12cas/12refusproduction/0error-skip, aucun échec oracle. RapportRED902lignes intégralement lu, XML réel534lignes byte-exact/SHAa1a41071 et29blobs production identiques BASEd1/preRED à cette gate,3tests pre/post/current vérifiés. FAILED9s/Gradle1,aucun133observé/nativeGREEN ; premier échec bootstrappermission ne compte pas commeRED. Même Astra/high implémente dans32Modify/5KotlinCreate/21assets/2R29ephemeral owners après R30/R31, toute extension exacte avant edit. Captures réelles8/9/10 exit0 en49/9/12s ; rapport1663FULL MAIN/SHA3a9da8,9exports==blobs historiques et15assets décodés exacts aux consoles/SHA/longueurs, producer/initSHA vérifiés ROOT. Ce n'est pas un replay/PASS. Six refus publics historiques NaN/infini conservés ; R32 autorise seulement des constructions old-wire explicitement invalides. Archives publiques/work/storage/final4/commit/review Task4 restent ouverts ; pas d'acceptance Noise ni fix production/test ROOT.
...
Final06 :690méthodesréelles/688PASS/0failure-error/2AA4skipsconnus, commande BUILD FAILED15m03s/Gradle1/native174exit133 ; cinq compilesSÉPARÉES0 incrémentalesUP-TO-DATE, custody réelle1225INDEX/HEADsources identiques au code testé. Ready to merge:NO ; aucune conclusion nativeGREEN/ISOglobal/tousGM/domainesillimités. MinorM1 castColorSourceProof471:60 etMinorM2inventairewarningshorsM1 (PreparedGPUFrame346:25 héritée342+4imports, ImageNumericAuthority270:21) restent suivis sans polish :321Kotlin+8JVMforcés/4JVMparcompile.

Tous les coûts et réserves du plan restent ouverts : copiesV3préinventaireR40, scans128axes/bytes/Atlas/BigInteger etreissuedproofs, rétentionprudente après handoffpartielR49, échecs historiques93/09/04 et05nonfinal/ordresJUnitdifférents, ancien136helperSHAabsent, AA4/admissiongenerale filteredW4e complexclip-inverse/Pictureclip/close-rollbacktargetlevel/cachehit-handle/injectedquarantine nonprouvés par pixels. La revue statique des ownersR49–50 estdistincte depreuvesd'injection. W5g/h/W6–8 etobjectifGMglobal nonclos ; font/codec/jpg-color-cube/GM/dashboard/renders/refs/scores/baseline exclus. PRstackDraftsurW5e#2399seulement, jamaisready/merge/parentmutation.

Les paragraphes suivants sont l'historique des checkpoints, pas l'état courant.

État courant autoritaire : Task8 soleSolR2 clôt les cinq findings Important, tous ADDRESSED, zéro nouveau Critical/Important/Minor; conformitéfonctionnelle etqualitéApproved sur85d8c2d16. ROOTlitFULLrapport858SHAf4185164…/verbatimR2SHA1f70d7eb…, vérifiefinal06exact14/690XMLfrais/688PASS/0FAIL/2AA4known/0errors, sourceactual1225INDEX/HEAD342fe…/diff9afdad…identiqueslotfinal/worktree0; cinqcompiles séparées0/targetsUP893/682/620/707/689ms. La commande resteFAILED15m03/Native174133/Gradle1,329warningsforcés+4JVMparcompile visibles. Sixbudgets/refuscausaux/recoveriesfermés surcesources, sansrequalifieranciens93/09/04niantériorité. Task7checkboxesstalesréconciliées àsa clôtureexistante; Task8fonctionnel1–4checked, étapesROOT5–7pascochées. Une reviewglobaleSol7aparentW5e→postclôtureROOTHEAD, éventuellevagueAstraCOMPLETE+uniqueSolciblée, puisdocsfinales/PRDraftempiléeW5e#2399 restentàfaire. CopiesV3prébudget/proofcosts/warnings/native133/AA4/W4ecomplexclip/Pictureclose-target/136helperSHAgap/GMexclus explicitementconservés; pixels≠handles/cache/quarantineproof, pasclaimISOglobal. Paragraphesantérieursci-dessous=historiquedesessais, pasétatcourant.

Fixround1 livréprovisoirement85d8c2d16/9sources296+/54−, aucunROOTdoc. ROOTlitFULL858rapportSHAf4185164…, vérifieactual1225blobsINDEX/HEAD342fe…/diff9afdad…identiquesfinal06/worktree0, cinqcompiles07–11FULL0/targetsUP et20comparaisonschaîne0. ONEre-reviewSolR2 ciblée lancéerevieweroriginal surcinqfindingsverbatim+nouvellecasseFIXDIFF045→85; pasnouvelreviewlarge. Verdicts5OPEN, final688PASS0FAIL2AA4 conservésdistinctsFAILEDNative174133. ROOTclosure/reviewglobale/PRDraftattendentverdicts; warnings321+8et4JVMparcompilevisibles.

Finale06ROOTvérifiée : exact14classes/690XMLfrais/688PASS/0FAIL/2skipsAA4exactconnus/0errors,13:57:41→14:12:44Z. Six contrôles auparavantbloqués exécutent refuscausaux/récupérationssaines aprèsl'historiqueW5a–e de06. CommandeFAILED15m03/Native174133/Gradle1/55tasks55EXEC distincte, source342fe…/diff9afdad…prépost2cmp0. Les12échecs09 sont absents de ce covering, pasencoreclôturecode/review. Cinqcompiles séparées/custodyactual1225HEAD/index/fixreport/commit/re-reviewSol restentouverts; pasGREENnatif/globalISO/GM.

05 terminé et ROOTvérifié :14classes/690XMLfrais/688PASS/0FAIL/2AA4skips/0errors, commandeFAILED15m15s/Native173133/Gradle1/55tasks55EXEC, prépostSHA/binairecmp0. OrdreJUnit différent04 explicitement conservé. R50 appliqué uniquement3lignes invalidationolder scopes, ROOTvoitprédicatexact. 05NONFINAL après ce patch; nouvelle finale06 forcée noninstrumentée lancée13:57:41Z sur1225sources342fe1ea…/diff9afdadc3…, argv exact14 vérifié. XML complet06/cinqcompiles/custody/fixcommit/re-reviewSol/finale/PR restent ouverts; aucune clôtureNative/globalISO.

R50 réel classifié avant édition : seul invalidatePhysicalPoolsBefore du même provider devra retirer aussi les scopes de préparation de génération strictement antérieure, comme son journal/pools existants. Sinon la nouvelle table R49 peut laisser passer un owner invalidé comme NoScratch; les seals d'exécution ne remplacent pas cet invariant local. Agent ne modifie rien avant fin05. Ce patch tardif crée ensuite un nouvel epoch :05 conservé comme candidat nonfinal, nouveau lot forcé14classes non instrumenté puis cinqcompiles séparées/custody/fixcommit/re-review. Coût supplémentaire ~13–15min, raison/prédicat/garanties exacts au plan. Aucun nouveau reset/invalidation/budget/cache/autorité ni testinfra/injection, aucune clôture ou nouvelle boucle de design.

Gate final05 lancé13:40:34Z : exact14classes/--rerun-tasks, sources figées1225SHA6256d009…/diffFIXBASE0456cad62f…, commande réelle vérifiéeROOT. R49 écrit uniquement dans ses trois chemins production; six tests classifiés conservés. Aucun stderr diagnostic/temporaire. Gradle exclusif agentTask8; résultatXML final/cinqcompiles/custody/commit/re-review à venir, aucune clôture annoncée.

R49 classifié avant édition : raccord scratch réel limité aux facades du journal GPUConcreteResourceProvider, transitions GPUFrameRollback et fermeture de completion GPUFrameExecutor même sans payload natif. Reçu fondé sur acquisitionToken existant de la première lease du même scope/génération, conservé avec le ticket EXACT; ni hash/parse/String→Long ni compteur nouveau. Transitions de pool existantes seulement. Refus scratch laisse rollback avant submit intact; refus natif après handoff scratch met les owners transférés/incertains en quarantaine et nettoie uniquement les owners non soumis, sans succès de rollback fictif. Raison/coût et garanties exacts au plan : réutilisation GPU prématurée, owner perdu/double transition, rétention conservatrice possible en quarantaine. Aucun changement pool/budget/cache/purge/reset/ABI/geometry/testinfra. Agent autorisé à corriger, puis lot final forcé14classes non instrumenté/cinq compiles/custody/fixcommit/re-reviewSol obligatoires; aucune clôtureTask8/nativeGREEN.

Le raccord causal scratch manquant est vérifié dans les owners réels : réservations sous le même provider/scope, ticket de completion déjà adopté, mais hooks de soumission/fin/quarantaine limités au payload natif. Aucune production n'appelle les transitions scratch existantes. Les textures temporaires restent Reserved; leur backing physique dépasse seul les six budgets dans04. Ce n'est pas une justification pour relever le budget/purger/réinitialiser. ROOT attend le mapping exact d'identité de soumission et l'ordre des refus partiels avant R49 et édition des trois chemins proposés. Gate final forcé non instrumenté/cinq compiles/re-review restent ouverts.

Lotobservéfix1-04 terminé et vérifiéROOT :690méthodes/682PASS/6FAIL/2AA4skips/0errors, FAILED13m21s/Native172133/Gradle1. Les13diagnosticsFULL écartent l'hypothèse de réutilisable évincé : requestedNew0 etinitial/final identiques. Les6refus ont readback79,644octets entièrementreclaimable, sesétatsreserved/inFlight/mappingOwned/quarantined0 ; ReusableScratch1,933,312/1,941,504/2,277,376 dépasseàlui-seullesbudgets1,585,000/1,600,000. Cause decesrésidences/owner pasencoreadjugée. ROOT vérifie restaurationproductionexact0debSHA/diff0 ; aucune instrumentationcommittée ni patchpool/lifecycle/budget. Correctioncausale nécessitejoinsréels/classificationpréalable ; gatefinalforcénoninstrumenté/cinqcompiles/custody/re-review toujoursrequis.

Observationfix1-03 déclarée :28publicsPASS(27ImageShader+contrôleTask8), aucune ligne de refusR46 ; commandeFAILED1m50s/Native171133/Gradle1. Aucun second historique partiel ne reproduit. R48 autorise une seule observation des14classes publiques originales, incrémentale, avec les deux diagnostics existants et restauration stricte ; pas de nouveau sous-groupe arbitraire ni correction d'owner avant preuve. Ce lot observé n'est pas le gate final forcé non instrumenté, qui reste obligatoire après correction classifiée. Coût supplémentaire du lot complet et risque de perturbation explicités au plan.

Fixespublicsfix1-02 vérifiésROOT :21méthodesPASS/0failure/error/skip, XML frais12:59:20–30Z ;15Task8+3profondeur+2Conical+wrapper. CommandeFAILED20s/Native170133/Gradle1,55tasks7executed1FROM-CACHE47UP. Les corrections restent des candidats avant re-reviewSol et covering complet ; aucune cellule de budget agrégé n'est fermée par ce sous-groupe. Pré/postSHA/binaire identiques, source réindentée ensuite crée un nouvel epoch à valider séparément. ObservationR47 en cours, pas de patchproduction supplémentaire.

Observationfix1-01 :2témoins publicsPASS mais commandeFAILED25s/Native169133/Gradle1 ; aucun refus de staging reproduit. ROOT vérifie hashlog/source etprépostSHA/binaire0, production exactementrestaurée. R47 autorise une seule observation supplémentaire avec classe publique W5eImageShader existante puis contrôleTask8, historique antérieur vérifié dans09 mais pas cause établie ; ses Surfaces sont petites, pas de « grand readback » prouvé. Aucun contournement ni clôture agrégée ; raison/coût au plan.

R46 autorise deux impressions stderr temporaires des diagnostics de staging déjà calculés, sur deux contrôles publics existants, sans lecture supplémentaire du pool ni changement de réserve/purge/budget/runtime. L'agent conserve un epoch source distinct et restaure le hash de production exact avant validation finale ou commit. Ce diagnostic ne ferme pas l'agrégat et n'autorise aucune correction d'owner ; raison/coût au plan.

Fixround1 : R44 autorise le seul témoin public wrapper SRGB à vérifier le diagnostic exact filter.slice et une récupération native bornée répétée, sans promotion. R45 autorise uniquement l'oracle Conical indépendant à conserver l'incertitude des intermédiaires eager (évalués immédiatement) puis à l'écarter lorsqu'un opérateur prouve son absence d'influence. gradientSqrt, ses bornes et toutes les fixtures restent inchangés ; toute incertitude utile demeure un refus DomainUnbounded. Les valeurs attendues précèdent Surface et le résultat exige Native Render/Readback. Aucun changement de production supplémentaire ; causes/coûts exacts au plan. Les six refus de staging restent investigués, hypothèse d'éviction non prouvée, pas de correction ni contournement autorisé.

ReviewSolTask8 refuse conformité/clôture etqualité :0Critical/5Important/1Minor. Fixround1Astra lancé sur touslesfindings; aucun parking/waiver. R43 vérifiéROOT autorise seulementles trois témoins publics deprofondeur W5a/c/d à attendre SceneRecordingValidationException/graph-depth-limit à la premièrecapture, avantcopies, avecmêmes graphesprofonds etrécupérationpubliquebornée. Aucun report devalidation ni modificationproduction. Lesrefusstaging, oraclesConical, ownerwrapper etdiscriminantsmutation sont investigués séparément; touteextraModify avantédition validéeROOT, pasnewplan/geometry/nativeworkaround. Coûts etraison exactsR43 sontauplan.

Dernier contrôle Task8 vérifié par ROOT : lot forcé09, exactement14classes/690méthodespubliques,676PASS/12FAIL/2skipsAA4/0errors; XML tous frais entre12:17:40–12:32:48Z. Commande FAILED15m8s/Gradle1/native168133,55tasks55executed. Sixrefusphysiques de staging cumulés touchent le budgetTask8, Task2 et quatre cellulesTask5; trois anciens témoins de profondeur attendent un refus plus tardif, deux oracles Conical BareSqrt sont non bornés, un ancien test attend un diagnosticw5a.kind au lieu defilter.slice. Aucun défaut n'est déclaré antérieur sans preuve, aucun skip/tolérance/reset modifié. Le gate agrégé et la clôture fonctionnelleTask8 NE SONT PAS atteints;15PASSdu lot isolé08 ne les ferment pas.

Cinqcompilationsséparées10–14 réussissent exit0 (922/583/603/756/677ms), targets tousUP-TO-DATE/incrémentaux. Pré/post et08→09→cinqcompiles identiques aa6a0f87…/diff547e77da…,1225sources contrôléesSHA0. Les321warningsKotlin et8JVM restent visibles. Rapportfinal/custody/commitprovisoire puis revueSolTask8 et revueglobale/éventuellevagueuniquedefix/PR restent ouverts; aucun GREENnatif/globalISO.

R42 est appliqué : seul hunk production projectGeometry6ajouts, après contrôle public06 NoPaintRED/explicitPaint(false)PASS puis07les deuxPASS. Run07 n'était pas vert : Picture8PASS, Picture9FAIL sur une hypothèse de viewport2×2 non documentée. Essai conservé comme erreur de fixture, non comme bugcodec; test corrigé au viewport public commun1×1, archives inchangées, playback8/9PASS08. Aucune autre correction de production ni changement capture/IR/version/codec/géométrie/AA4/native/cache/lease/authority.

Les paragraphes antérieurs ci-dessous conservent les checkpoints précédents; ils ne remplacent pas l'état courant ci-dessus.

R42Task8 ciblé : vraiREDpublicNoPaintDrawImage03 confirméROOT (1failure,XML12:12:12.834Z, commandeFAILED3s/native162133). Captureimagepaintnull→DEFAULT, projectiongéométriqueneutralefabriquepaintsansAAmaislaisseDEFAULThorscoverageW3/W4a. SeulModifyautoriséW5eImagePlanCompiler.projectGeometry : nullPaint+DEFAULTdesoriginesimage→HARD_EDGEpourconstructionneutre, originaux/clip/paint/ownersinchangés. MatchingcontrolpublicexplicitPaint(false) avantfixpuisNoPaint/hist8/9GREEN etfullgates/review requis. Pasrefontecapture/codec/version/géométrie/clip/AA4/authority/cache/native; risqueprojection/semantics/authenticité conservéauplan.

Premiers témoinsTask8 :11méthodespubliques positives,2échecsdeplaybackhistorique8/9 (w5a.composite.unsupported),0error/skip; ROOT vérifieXML/hashpré-post/UTC. CommandeFAILED19s/Gradle1/native161133, pasGREEN. Aucuneproductionmodifiée : diagnosticde scènehistorique/admission en coursavant toutecorrection, sansinférerbugdecoder. Run01bloquéavantGradle/XMLpériméexclu. Task8/reviewsfinales/PRrestentouverts.

Task8 lancée séquentiellement surBASEpropre e386964299db7855c8e1a5b0b2bd3fb8ac5e8a7e avecnouvelAstra/high. Périmètreapprouvé : convergencepublique/Picture/capture/budgets/récupération, unlotfinalforcé14classespubliques,cinqcompilationsséparées, commitprovisoire+reviewSolTask8puisreviewglobale/vagueuniquedefix/reviewciblée/PRDraftempilée. Aucun résultatTask8encore, aucuneextensiongéométrie/codec/native/harness/testinfrastructure. Solreviewonly, ROOTseulpropriétairedocs/reviews/PR.
