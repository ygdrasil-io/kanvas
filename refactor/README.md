...
| W5 | Material graph | Gates d'implémentation W5a Solid/Opacity vertes : table material commune avec chaînes Opacity distinctes préservées, autorités exclusives, frames natives Rect/RRect/Path direct ou stencil avec ordre/AA/mutation, et W5a valide → refus authentifié → récupération. Une RRect hard-edge non admise conserve legacy. 104 tests publics : 101 passés, 3 skips AA4 authentiques, aucune failure/error. Reviews finales Task 8 restantes; W5b (blends communs) ensuite ([status](waves/W05-material-graph/status.md)) |
