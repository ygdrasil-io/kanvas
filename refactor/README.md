...
- [Plan W6b — blur, masks et shadows](plans/2026-09-22-w6b-blur-masks-shadows-implementation-plan.md) — Tasks 1–6 et la correction whole-branch bornée depuis `2bf3d52` ferment le périmètre W6b : table/replay Picture, X/Y image blur, mask blur/shader/table et `DropShadow` `COMPOSITE`/`SHADOW_ONLY` gelés par le plan. Les sept shards W6b totalisent 80 assertions JUnit XML PASS; les compilations ciblées `gpu-plan`, `gpu-renderer` et `kanvas` sortent 0. B/B−1, nested budget, refus sibling tardif, invalid-bounds et recovery Surface sont publics. Les exits natifs133 restent `UNKNOWN`; backdrop, filtered previous, F16/HDR, W6c/W6d, device-loss, fonts/codecs, GMs/dashboard/renders/baselines/scores, Skia global et `jpg-color-cube` restent exclus, sans claim ISO/global.
