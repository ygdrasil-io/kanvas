...
| W5 | Material graph | Gates d'implémentation W5a Solid/Opacity vertes à `7782151` : table material commune avec chaînes Opacity distinctes préservées, autorités exclusives, frames natives Rect/RRect/Path direct ou stencil avec ordre/AA/mutation, et W5a valide → refus authentifié → récupération. Une RRect hard-edge non admise conserve legacy. 104 tests publics : 101 passés, 3 skips AA4 authentiques, aucune failure/error. Les reviews spec et qualité Task 7 sont `READY`; les deux reviews globales Task 8 vont suivre. Le gap legacy `uniform slab` reste non bloquant; W5b (blends communs) ensuite ([status](waves/W05-material-graph/status.md)) |
