...
| W5 | Material graph | Correctif global W5a de `7dbaf8c` complété par les recoveries `64e6429c`, `582606d7`, `cbd8ab5e`, `33c54c09` et `9891e11` : DAG fragment avec raw bindings/ABI V2, registre material scellé après admission réelle Core/A8/Vertices/Mesh sans programme, diagnostics géométriques préservés. Ordre/capture, refus gradient puis récupération et bornes 512/513 sur des Surfaces distinctes du même runtime/backend vérifiés. L'oracle place la couverture dans le fragment puis ferme le blend fixed-function à la précision RGBA8 minimale autorisée et F32, avec destination corrélée; les fixtures multi-draw ne passent que sur un singleton ou deux codes adjacents. 119 tests publics : 116 passés, 3 skips AA4 authentiques, aucune failure/error. Les deux re-reviews globales Task 8 restent en attente; pas de clôture implicite. MeshProgram et le gap legacy Rect+hard-RRect `uniform slab` restent hors promotion; W5b ensuite ([status](waves/W05-material-graph/status.md)) |
