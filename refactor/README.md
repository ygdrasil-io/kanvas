...
- [Plan W6b — blur, masks et shadows](plans/2026-09-22-w6b-blur-masks-shadows-implementation-plan.md) — Tasks 1–6, la correction whole-branch `2bf3d52`, R21 (`a1aabe6`) puis R22 (`6453b06`) bornent le périmètre W6b : table/replay Picture, X/Y image blur, mask blur/shader/table et `DropShadow` `COMPOSITE`/`SHADOW_ONLY` gelés par le plan. Les `FilterComposite` publient offset/scissor I32 target-local consommés verbatim; l'aggregate Picture porte séparément l'admission du scissor terminal (optionnel, vide/non-vide) et la validation compare ce fait au terminal et au pass. Les sept shards W6b totalisent 80 assertions JUnit XML PASS, les 100 contrats graph passent et les compilations ciblées `gpu-plan`, `gpu-renderer` et `kanvas` sortent 0. B/B−1, nested budget, refus sibling tardif, invalid-bounds, origins élevés, archives Picture oversize et recovery Surface sont publics; aucun shard public W6b n'importe `GraphLimits` ou `SceneCaptureLimits`. Les exits natifs133 restent `UNKNOWN`; backdrop, filtered previous, F16/HDR, W6c/W6d, device-loss, fonts/codecs, GMs/dashboard/renders/baselines/scores, Skia global et `jpg-color-cube` restent exclus, sans claim ISO/global.
