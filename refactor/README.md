...
## W5h — reviews approuvées ; Draft PR #2402 publiée

La re-review Task7 R1 par le même Sol est **APPROVED — 0 Critical / 0 Important / 0 Minor** sur `9ef62acd7..b5a7fd312`. L'Important initial concernait les uniforms Core dupliqués entre segments physiques. La correction R1 `70c70d344edddab1873acea399ab6fc18d8dcd58` garde un layout/seal/footprint exact par segment et son identité jusqu'au native. Le témoin public CoreA→Vertices→CoreB à budget400000 est RED sur9ef puis GREEN, avec recovery ; convergence5/5, classes H1126/1126 (1021 invocations H +105 contrôles) et test683 PASS. Le seul échec du gate1128 est le tuple Point/SOFT_LIGHT historique exact. Trois compiles GREEN ; renderer compileTest reste baseline16 exacte. Native133 des runs causaux reste UNKNOWN. La re-review whole-branch par le même Sol sur `fff9f90171ebbdaa6642482dd757d153c7e909c3` est **APPROVED — C0/I0/M0** après la seule correction documentaire I1 ; la correction de code R1 relevait de la review Task7. Les Steps9–11 sont closes : [Draft PR W5h #2402](https://github.com/ygdrasil-io/kanvas/pull/2402) OPEN/Draft, empilée sur [W5g #2401](https://github.com/ygdrasil-io/kanvas/pull/2401), sans merge ni claim ISO/global. Les comptes d88 suivants restent une custody historique, pas un nouveau covering de R1.

Avant review, la branche `codex/w5h-registered-runtime-effects` avait son covering figé sur `d88d8f49132fc7a4461df9945145dfc0feadb5e9` :33/33 cellules H et cinq comportements de convergence vérifiés ;2084 invocations qualifiées,2019PASS,63 failures attribuées (61 dettes historiques et2 stale refusals de promotion),0error,2skips AA4. Deux selectors restent `EXCLUDED — historical proof heap exhaustion` ; le total XML brut2085 inclut une interruption OOM, jamais un PASS. Cinq compiles GREEN et renderer compileTest baseline16 appartiennent à ce freeze. La Draft cible directement `codex/w5g-composed-procedural-materials`. [Suivi W05](waves/W05-material-graph/status.md) et [plan W5h](plans/2026-09-15-w5h-registered-runtime-effects-implementation-plan.md) conservent comptes, commits, limites NoClip/Scissor et gaps W8/intégration.
...
- [Plan W6b — blur, masks et shadows](plans/2026-09-22-w6b-blur-masks-shadows-implementation-plan.md) — Tasks 1–6, la correction whole-branch `2bf3d52`, R21 (`a1aabe6`), R22 (`6453b06`) et R23 (base `b37bf29`) bornent le périmètre W6b : table/replay Picture, X/Y image blur, mask blur/shader/table et `DropShadow` `COMPOSITE`/`SHADOW_ONLY` gelés par le plan. L'autorité `PictureTerminalScissorAuthorityV1` est produite directement depuis les inputs d'admission avant tout terminal/operands/pass ; le terminal et `FilterComposite` en reçoivent chacun une copie et la validation les compare à ce fait antérieur. Les contrats incluent baseline non-vide, contained-wrong, forged-null, true-empty-null et une Picture filtrée produite par le vrai compilateur. Les sept shards W6b totalisent 80 assertions JUnit XML PASS, les 102 contrats graph passent et les compilations ciblées `gpu-plan`, `gpu-renderer` et `kanvas` sortent 0. B/B−1, nested budget, refus sibling tardif, invalid-bounds, origins élevés, archives Picture oversize et recovery Surface sont publics; aucun shard public W6b n'importe `GraphLimits` ou `SceneCaptureLimits`. Les exits natifs133 restent `UNKNOWN`; backdrop, filtered previous, F16/HDR, W6c/W6d, device-loss, fonts/codecs, GMs/dashboard/renders/baselines/scores, Skia global et `jpg-color-cube` restent exclus, sans claim ISO/global.
- [Plan W6c — DAG spatial principal](plans/2026-09-22-w6c-spatial-dag-implementation-plan.md) — correction whole-branch `2ffdae4` sur `cb93adb28` : reverse demand externe des quatre racines non spatiales, no-op `Layer` à clip terminal vide, bornage Crop/Tile par le consumer, préflight W5f des uniforms `ColorFilter` et cache spatial `RenderAttachment + Sampled` sans `CopySrc` non consommé. Les 15 assertions publiques ciblées sont XML PASS; native133 reste `UNKNOWN`, sans claim ISO/globale.
- [Plan W6d — effets avancés et backdrop](plans/2026-09-22-w6d-advanced-effects-implementation-plan.md) — Tasks 1–6 et la tranche Terra Task 7 (budget atomique, refus F16, recovery et stack gate) sont implémentées sur `87b3b580a`. Le B public 1×1 vaut exactement `4 + 4 + 4 + 4 + 4 + 16 + 16 + 4096 + 256 = 4404` octets ; B−1 terminalise avec `w6d.layer.frame_budget_exceeded`, sentinel intact et recovery sur la même `Surface`. Les 4096 octets sont le lease logique gelé par programme W6d (owner, génération, descriptor, usages, slot et lifetime), chargé à `max(4096, descriptor encodé + payload checked-I64)` même sur cache-hit ; ils ne prétendent pas mesurer les allocations opaques du driver. `RGBA16_FLOAT` est un token public refusal-only : les graphes W6d refusent avant capture/plan/native avec `w6d.layer.unsupported_target_format`, sans conversion ni backend F16 ; RGBA8 reste positif. Les 14 selectors ciblés donnent 132/0/0/0 dans les XML de classe, mais chaque worker termine en 133 : native **UNKNOWN**, sans claim ISO/globale. Les sept compiles ciblés sortent 0. La review whole-branch Sol et la Draft PR empilée sont les seuls Steps 7–8 restants, réservés au contrôleur ; W6e suivra.
...
| W6 | Layers et effets | W6a/W6b/W6c et W6d bornés : layers, budgets, W4/W5, blur/masks/shadows, les neuf familles spatiales et les onze familles avancées/backdrop/filtered previous ont une custody publique ciblée dans le [status](waves/W06-layers-effects/status.md). Task 7 ajoute B/B−1 checked-I64, refus F16 pré-publication, visibilité atomique et recovery même `Surface`; son correctif R1 réserve aussi par owner un lease programme logique gelé (owner/génération/descriptor/usages/slot/lifetime, `max(4096,descriptor+payload)`), sans prétendre mesurer les bytes opaques du driver. Le témoin onze-familles utilise des régions/oracles/mutations publics sensibles. Les 132 assertions ciblées XML passent, mais native133/134 reste `UNKNOWN`. Les Steps contrôleur (review Sol whole-branch et Draft PR) puis W6e restent ouverts ; aucune claim ISO/globale. |
