...
- `font/text` now exposes a pinned Script_Extensions itemization route for this
  bounded evidence wave so Devanagari runs can exercise the reviewed `deva` /
  `dev2` feature-policy path without changing the engine defaults used by
  existing shaping tickets.
- `DevanagariShapingFixtureTest` now proves bounded vendored-font evidence for
  ...
  and mark placement on `NotoSansDevanagari-Regular.ttf`.
...
- `BasicOpenTypeShapingEngine` now emits `Deva` on the vendored `कि` case when
  it is given the pinned Script_Extensions itemizer route used by this wave,
  proving bounded Devanagari evidence no longer relies on the legacy
  `BasicUnicodeData` script classifier.
