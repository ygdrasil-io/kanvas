...
| PKT-07 GSUB/GPOS simple script shaping | Current-supported | Latin/Greek/Cyrillic/Hebrew defaults, features, clusters, fallback runs. | `font/text`, `font/sfnt`. | Parsed layout table fixtures, feature ordering evidence, per-script fixture families (M6-007/008/009 done). |
...
| PKT-16 `:kanvas-skia` facade migration adapters | Partially implementable; M13 route slices (SkTypeface, SkShaper, SkTextBlob) are done, GPU handoff readiness remains a separate gap | Delegate `SkFontMgr`, `SkTypeface`, `SkShaper`, `SkTextBlob` toward pure Kotlin dumps. | `kanvas-skia/src/main/kotlin/org/skia/foundation`. | M13-002/003/004 done; GPU handoff gates remain separate. |
...
Status: historical coordination-only blocker audit; superseded by later M6 fixture closeout and runtime-adoption waves.
...
- The audit also keeps the critical non-claim explicit: no reviewed
  ...
  is present in-repo yet, so `KFONT-M6-009` and `KFONT-M6-010` remain
  `blocked` and must not be treated as actionable.
...
- `scripts/validate_kfont_m13_001_facade_inventory.py`
...
- `facade-adapter-inventory.json` now covers all scoped façade routes:
  `SkFontMgr`, `SkTypeface`, `SkFont`, simple deterministic
  `SkCanvas.drawString`, explicit `SkShaper`, `SkTextBlob`, and
  paragraph-compatible APIs.
- The inventory maps every durable legacy gate and keeps `pdf_never_embed`
  visible as an explicit adjacent/out-of-scope gate rather than assigning it a
  false runtime facade owner.
- The dashboard now includes the `skia-facade-adapter-inventory` row as
  `tracked-gap`, with the inventory JSON/markdown plus the diagnostic taxonomy
  linked as route/refusal evidence.
- `dump-evidence-index.json` now tracks the facade inventory artifact as a
  producer-only coordination dump with PM bundle/dashboard wiring validations.
- The inventory preserves all non-claims: no route support promotion, no GPU
  route readiness claim, no legacy-gate retirement, no hidden native-engine
  dependency, and no implicit complex `drawString` broadening.
...
Remaining gate: route-specific façade promotions still belong to
`KFONT-M13-002`, `KFONT-M13-003`, and `KFONT-M13-004`; the inventory itself is
coordination evidence, not a support claim.
...
- The same bundle now emits six dedicated `fallback-fixture` assets so later
  ...
  aggregate dumps.
- `defaultFallbackShapedGlyphRunEvidenceJson()` adds a shaping-owned
  `fallback-shaped-glyph-run.json` golden that links each bounded shaping case
  back to `fallback-decision-trace`, `resolved-font-runs`, and the dedicated
  `fallback-fixture` asset while preserving selected/rejected typeface facts
...
- `BasicOpenTypeShapingEngine` now resolves per-run feature policy before GSUB,
  ...
  fallback.
- The Arabic policy row now includes `curs`, so bounded cursive attachment
  support no longer depends on the raw request-feature fallback that previously
  bypassed the resolved policy.
...
- `reports/pure-kotlin-text/font-fixture-inventory.json`
- `reports/pure-kotlin-text/coverage-ticket-matrix.md`
- `reports/pure-kotlin-text/2026-06-17-kfont-m10-002-colrv1-solid-glyph-colr-glyph.md`
...
- `reports/pure-kotlin-text/font-fixture-inventory.json`
- `reports/pure-kotlin-text/coverage-ticket-matrix.md`
- `reports/pure-kotlin-text/2026-06-17-kfont-m10-004-colrv1-transform-composite-clip.md`

Evidence:

- `COLRV1ColorGlyphPlanner` now emits bounded transform/composite/clip planning facts for translate, generic transform, classified scale/rotate/skew, and clip-wrapped composite operations, with deterministic bounds, determinant facts, destination-read hints, and layer-isolation hints.
...
- `reports/font/fixtures/expected/color/color-glyph-plan.json` and `reports/font/fixtures/expected/color/colrv1-paint-graph.json` now carry accepted transform/composite/clip cases plus the new refusal diagnostics, while `reports/font/fixtures/expected/color/color-glyph-composite-plan.json` records the ordered composite-plan handoff facts for `colrv1-composite-clip`.
...
Remaining gate: this ticket closes only the bounded COLRv1 transform/composite/clip planning slice. It does not claim GPU composite execution, dedicated `PaintScale` / `PaintRotate` / `PaintSkew` parse-format support beyond classified generic transforms, bitmap/SVG route support, emoji sequence planning, GPU artifact registration, or platform/native color fallback behavior. The legacy gate `coloremoji_blendmodes` remains open until CPU oracle and GPU route evidence are linked.
...
- `reports/pure-kotlin-text/font-fixture-inventory.json`
- `reports/pure-kotlin-text/coverage-ticket-matrix.md`
- `reports/pure-kotlin-text/2026-06-17-kfont-m10-005-colrv1-recursion-cycle-bounds-fixtures.md`
...
- `ColorGlyphSurfaceTest` serializes the fixture manifest deterministically from synthetic provenance and stable refusal snapshots, including `text.color.COLRv1-cycle-detected` and `text.color.COLRv1-budget-exceeded` without promoting new runtime operation support.
- `reports/pure-kotlin-text/dump-evidence-index.json`, `reports/pure-kotlin-text/fixture-evidence-manifest.json`, and `reports/pure-kotlin-text/font-fixture-inventory.json` attach the new dump to `color-glyphs` while keeping `coloremoji_blendmodes` open.
...
- `KFONT-M11-006` is now done on deterministic subrun split evidence.
...
- `reports/pure-kotlin-text/2026-06-23-kfont-m11-007-resource-plans.md`
- `.upstream/specs/pure-kotlin-text/tickets/M11-gpu-handoff/KFONT-M11-007-add-resource-upload-instance-binding-plan-contracts.md`
- `.upstream/specs/pure-kotlin-text/tickets/M11-gpu-handoff/KFONT-M11-008-add-upload-before-sample-ordering-validation.md`
- `.upstream/specs/pure-kotlin-text/tickets/M11-gpu-handoff/KFONT-M11-009-add-wgsl-parser-reflection-validation-for-text-routes.md`
- `.upstream/specs/pure-kotlin-text/tickets/M11-gpu-handoff/KFONT-M11-010-add-materialkey-leakage-tests.md`
...
- `gpu-text-resource-plan.json` records texture ownership, atlas descriptor/
  page/entry refs, upload/instance/binding plan IDs, lifetime scope, and
  `resourceHandlesMaterialized=false` for the accepted A8 atlas subrun.
- `gpu-text-upload-plan.json` records source artifact key, destination texture
  plan, byte ranges, row stride, page region, staging plan, and upload-before-
  sample dependency while keeping `uploadExecution=not-executed`.
- `gpu-text-instance-layout.json` records stride, alignment, attributes,
  target/source/UV rects, page index, atlas generation, representation flags,
  and instance-upload-before-draw dependency.
- `gpu-text-binding-plan.json` records resource slots, binding layout hash,
  atlas generation facts, material plan ref, and explicit `MaterialKey`
  exclusions for glyph IDs, atlas rects, atlas generations, upload tokens, and
...
- `reports/pure-kotlin-text/2026-06-23-kfont-m11-010-material-key-leakage.md`
- `.upstream/specs/pure-kotlin-text/tickets/M11-gpu-handoff/KFONT-M11-010-add-materialkey-leakage-tests.md`
- `.upstream/specs/pure-kotlin-text/tickets/M11-gpu-handoff/README.md`
- `.upstream/specs/pure-kotlin-text/tickets/STATUS.md`
...
- `text-material-key-leakage-report.json` records 12 leakage cases: 7 clean
  ...
  variance, live-handle variance, and legitimate color changes) and 5
  `leak-detected` negative fixtures.
- All 5 required excluded fields (`glyphId`, `atlasRect`, `atlasGeneration`,
  `uploadToken`, `liveTextureHandle`) are validated across variance and
  negative scenarios.
...
- `GPUTextResourcePlanTest.binding plan keeps atlas resource facts outside material identity`
  continues to pass.
...
rtk ./gradlew --no-daemon :font:gpu-api:test --tests '*GPUTextResourcePlanTest*binding plan keeps atlas*'
rtk git diff --check
```

Remaining gate: no ticket-local `MaterialKey` leakage gate remains for
KFONT-M11-010. All 10 M11 tickets are now `done`. The milestone still does
not claim executed GPU uploads, SDF route support, broad GPU text support,
route promotion, product activation, or `dftext` retirement.
