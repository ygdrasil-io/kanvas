...
- `font/text` now applies bounded mark-to-base, mark-to-mark, and cursive
  attachment offsets while preserving stable refusal diagnostics for missing
  GDEF, malformed lookup data, and ambiguous multi-component ligature matches
  without a unique runtime component choice; the reviewed mono-codepoint
  ligature fixture stays refusal-only until unique runtime component-mapping
  evidence exists.
...
- Runtime tests now prove bounded advances, offsets, glyph classes, stable
  zero-advance cursive matches, cluster preservation for unrelated GSUB runs,
  and refusal diagnostics for the checked-in mark/cursive fixtures, including
  the ambiguous mono-codepoint ligature case.
- `gpos-trace.json` now records mark-to-base, refusal-only mark-to-ligature,
  mark-to-mark, cursive attachment, missing-GDEF, and malformed-anchor cases.
- `shaped-glyph-run.json` now links the bounded mark/cursive shaped-run cases
  and refusal diagnostics into the shared GSUB/GPOS dump without inventing a
  ligature-component choice the fixture cannot prove.
