...
- `validate_gpu_renderer_m9_readiness_pm_evidence_bundle.py` validates the
  summary, dump-line non-claims, artifact hash, and module-owned sidecar.
- The module-owned evidence bundle is generated by
  `:gpu-renderer:gpuRendererM9ReadinessPmEvidenceBundle` without a root
  packaging task.
- `pm-readiness-freeze-board` diagnostics now expose
  `missingGate=none`, `standalonePmEvidenceBundleWritten=true`, and
...
all dashboard-line readiness and release/product movement tokens, and verifies
the module-owned sidecar against the generated evidence.
...
- standalone sidecars are rebuilt from the generated dashboard summary and
  validated without a root manifest.
...
- dashboard rows separate correctness, activation, performance, cache, and
  release state;
- the module-owned evidence sidecar is deterministic and does not overwrite R6
  evidence;
