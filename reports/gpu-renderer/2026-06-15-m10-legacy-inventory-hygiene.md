...
| saveLayer, destination read, and filter DAGs | `SkWebGpuDevice` layer allocation/composite/filter helpers and intermediate resources | KGPU-M5-001, KGPU-M5-002, KGPU-M5-003, and KGPU-M5-004 are `done` as bounded route/refusal evidence; M11 live materialization evidence is also `done` for destination-read and saveLayer resource lanes | No saveLayer, destination-read, or filter route can be retired until route-specific promoted replacement, activation, rollback, and old-path usage evidence is accepted |
...
| Runtime effects, color filters, blends, and color management | Legacy runtime shader/color-filter/blend paths in `SkWebGpuDevice` | KGPU-M7-001, KGPU-M7-002, KGPU-M7-003, and KGPU-M7-004 are `done` as registered-descriptor, refusal, fixed-function blend, destination-read refusal, and bounded SDR color evidence | No arbitrary SkSL, dynamic shader, destination-read blend, HDR/profile, or all-blend support is implied |
| Vertices, points, and mesh-like draws | Legacy vertices/points test coverage and `SkWebGpuDevice` draw-family handling | KGPU-M8-001, KGPU-M8-002, and KGPU-M8-003 are `done` as contract-only descriptor, payload/resource, batching, sort, and refusal evidence | No `DrawVertices` route, vertex/index upload, batching, or CPU-rasterized mesh texture fallback is promoted |
...
| Performance, cache, and release gates | Legacy and renderer PM evidence/reporting lanes | KGPU-M9-001, KGPU-M9-002, and KGPU-M9-003 are `done` as reporting-only cache telemetry, frame-gate policy, and PM readiness dashboard evidence | Cache/source-map and frame-gate evidence remain reporting-only; no release-blocking frame gate or readiness movement |
...
- `KGPU-M10-003` was closed on 2026-06-17 by
  `reports/gpu-renderer/2026-06-17-m10-003-legacy-retirement-gates.md`, which
  adds a per-family retirement checklist and stable refusals for missing,
  duplicate, shared, partially shared, generic, archived-only, broad deletion,
  activated, release-blocking, or readiness-moving evidence while keeping
  legacy routes active in this ticket. Independent review
  `019ed5fb-8292-7931-b494-9034a88e15e0` accepted the final gate.
