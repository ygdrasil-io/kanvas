...
| saveLayer, destination read, and filter DAGs | `SkWebGpuDevice` layer allocation/composite/filter helpers and intermediate resources | KGPU-M5-001, KGPU-M5-002, and KGPU-M5-004 are `done`; KGPU-M5-003 remains `blocked` | No saveLayer, destination-read, or filter route can be retired until native intermediate/copy/filter evidence lands |
...
| Runtime effects, color filters, blends, and color management | Legacy runtime shader/color-filter/blend paths in `SkWebGpuDevice` | KGPU-M7-004 is `done`; KGPU-M7-001 is in `review` with descriptor-route contract evidence; KGPU-M7-002 and KGPU-M7-003 remain `blocked` | No arbitrary SkSL, dynamic shader, destination-read blend, HDR/profile, or all-blend support is implied |
