...
| Device scissor and simple clips | Legacy scissor/active clip state in `SkWebGpuDevice`, plus `WebGpuCoveragePlanSelector` diagnostics | KGPU-M2-003 is `done`; M3 prepared clip evidence and atlas refusals are `done`; native stencil cover remains KGPU-M3-002 `proposed` | No arbitrary clip-stack support; native/path clip migration waits on adapter-backed stencil/pass/readback evidence |
| Path fill and path stroke | `SkWebGpuDevice.drawPath`, path-effect dispatch, mask-filter helpers, and `WebGpuCoveragePlanSelector` | KGPU-M3-001, KGPU-M3-003, KGPU-M3-004, and KGPU-M3-005 are `done`; KGPU-M3-002 remains `proposed` | Prepared artifacts are evidence only; legacy path behavior remains until native/adapter evidence is accepted |
| Images, bitmap shaders, codecs, and uploads | `SkWebGpuDevice.drawImageRect`, bitmap shader dispatch, texture upload/cache helpers | KGPU-M4-001, KGPU-M4-002, and KGPU-M4-003 are `done`; KGPU-M4-004 remains `proposed` | No broad codec, animation, mipmap, perspective sampling, or CPU-rendered compatibility texture claim |
