# W130 — SweepGradient clamp + stroke square/miter sous clip EvenOdd Difference avec trou
...
d’un rectangle extérieur et d’un trou intérieur, avec `ClipOp.DIFFERENCE`.
...
- Clip `EvenOdd` : rectangle extérieur `(3.25,3.25)-(28.75,28.75)` et trou
  intérieur `(10.25,10.25)-(21.75,21.75)`, `ClipOp.DIFFERENCE`,
...
L’oracle CPU indépendant rejette les centres de pixels dans la coque EvenOdd,
c’est-à-dire lorsque `inOuter XOR inInner` est vrai. Il conserve donc le
complément du clip (`DIFFERENCE`), vérifie la distance au segment square étendu,
puis évalue le SweepGradient avec `atan2` et une interpolation linear-light
avant encodage sRGB. La comparaison couvre l’intégralité du buffer RGBA avec
une tolérance maximale de 1 LSB par canal pour la différence CPU/GPU en `f32`.

## Preuve native

Le test
`clamp sweep gradient square miter stroke under even odd difference hole clip renders natively`
vérifie :
...
./gradlew --no-daemon :kanvas:test --no-parallel \
  --tests 'org.graphiks.kanvas.surface.gpu.GPUFramePathApiInventoryNativeSmokeTest.clamp sweep gradient square miter stroke under even odd difference hole clip renders natively'
```

Résultat initial : l’oracle contenait une condition inversée ; le test a
signalé un pixel attendu dans la coque mais absent du rendu. Après correction
de la condition en `if (inOuter.xor(inInner)) continue`, le même test ciblé est
