...
> **Scope-extension flag (Task 7):** the roadmap's FP-11 entry lists only the 7 FP-09 transfers + 1 FP-10 transfer, and its "Résolus" note explicitly kept coverage-mask and analytic clips over the blur composite terminal (`unsupported.native-mask-blur.clip`). Task 7's analytic device-rect clip coverage is a DELIBERATE scope addition beyond the roadmap transfer list: it closes a real residual (the AA-rect clip blur case) that the FP-09 Task 11 lane left refused. It is flagged here and in the evidence (fp-11 evidence §0) as such; the stencil/coverage-mask/complex-clip blur cases stay terminal exactly as the roadmap stated.
...
| `GPUCorePrimitivePreparedFrameTaskListBuilderTest.kt:1191, 1408, 1432, 1465, 2418` | `core_primitive_mixed_uniform_layouts` (1191/1408/1432/1465 = the four refusal-pin `assertEquals` lines; **2418 = the positive `uniform32 and uniform80 split into two direct passes with per layout slabs` model** — the best existing split reference) | Task 6: the 4 refusal pins re-point per the executed split; the :2418 model was ADDED by Task 6. (Pre-execution plan cited :1190/1407/1431/1464/2445 — the +1 shift and the 2445→2418 replacement are the Task-6 re-points; verified at closure HEAD.) |
| `GPUPreparedSurfaceFrameBuilderTest.kt:686` | post-Task-6 split model: `hard rect and aa clear rect split into two direct passes with per layout slabs` (the pre-execution `mixed_uniform_layouts` refusal row at :669 was re-pointed by Task 6 — that pin no longer exists at HEAD; verified by `rg` over the file) | Task 6: flip to Ready + split-pass assertion |
...
- Modify: `GPUAllApiBlendSurfaceTest.kt` (DrawRRect rows + DrawRect/DrawColor ALPHA_MASK rows), `GPUPathClipRegressionTest.kt:23-89, 117-146`, `GPUClipAdvancedBlendSurfaceTest.kt:28-100`, `GPUCorePrimitivePreparedFrameTaskListBuilderTest.kt:1191, 1408, 1432, 1465` (refusal pins) + `:2418` (the split model to mirror), `GPUPreparedSurfaceFrameBuilderTest.kt:686` (the split Ready model; the pre-execution :669 refusal row was re-pointed by this task), `GPUClipCoverageSurfaceTest.kt:59`, `GPUPreparedSurfaceProductRouterTest.kt:466`
...
Flip `GPUPathClipRegressionTest.kt:23-56` (`device rect clip path frame refuses with the mixed uniform layouts code`) to assert `Ready` + pixels (the clip rect clips the path; CPU reference = the blended clip outcome). Flip `GPUClipAdvancedBlendSurfaceTest.kt:28-100` (AA-clip dst-read + scissor dst-read) similarly. In `GPUCorePrimitivePreparedFrameTaskListBuilderTest.kt`, add a case modeled on the split test at `:2418` (`uniform32 and uniform80 split into two direct passes with per layout slabs`): a frame with an unclipped `uniform32` rect AND an analytic-shape `uniform80` rect plans `Ready` with TWO direct render tasks (one per layout), each with its own slab (today it plans `Refused`). Note the executed scope: the shipped split admits uniform32+uniform80 mixes; the analytic-clip uniform64/160 mixes stay refused (verify-gate, Task 8 B-row) — do NOT flip a uniform64 case to Ready.
...
10. **Post-execution citation corrections (review finding, 2026-08-13 — applied to this plan at closure HEAD):** (a) `GPUPreparedSurfaceFrameBuilderTest.kt:669` was the pre-execution `mixed_uniform_layouts` refusal row; Task 6 re-pointed it, so the plan now cites the post-Task-6 split model at `:686` (verified by `rg`: `mixed_uniform_layouts` no longer appears in that file). (b) `GPUCorePrimitivePreparedFrameTaskListBuilderTest` pins moved +1 (1190/1407/1431/1464 → 1191/1408/1432/1465 — the `assertEquals` lines) and the old :2445 mention is replaced by the positive split model at `:2418` (`uniform32 and uniform80 split into two direct passes with per layout slabs`), re-cited as the best existing split reference. (c) Task 7's analytic-rect clip coverage is flagged as a deliberate scope extension beyond the roadmap transfer list (Context §1 note) — the roadmap had kept `unsupported.native-mask-blur.clip` terminal; stencil/coverage-mask/complex-clip blur stay terminal.
