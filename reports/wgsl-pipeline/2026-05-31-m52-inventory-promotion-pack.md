...
- generated provenance copied from a verified base generated dashboard row,
  plus `derivedFromGeneratedScene`, `derivationTask`, `linearIssue`, and
  `artifactRoot`;
...
- generated route diagnostics and stats artifacts under
  `build/reports/wgsl-pipeline-m52-generated/artifacts/<scene-id>/`;
- tags in the required namespaces, plus `source.inventory` and either
  `risk.none` or `risk.expected-unsupported`.

## Generation Path

M52 evidence is generated by `pipelineM52InventoryPromotionPack` from the
declarative contract at
`reports/wgsl-pipeline/scenes/generated/m52-inventory-promotion-pack.json`.
For each selected inventory candidate, the task verifies the named base row in
`reports/wgsl-pipeline/scenes/generated/results.json`, carries forward that
row's real `sourceTask`, `sourceTest`, and `sourceReport`, and records
`derivedFromGeneratedScene` plus the M52 derivation contract.

The task writes the M52 manifest fragment and artifacts under
`build/reports/wgsl-pipeline-m52-generated/`; `pipelineGeneratedSceneExport`
then merges that fragment with the base generated scene manifest before
dashboard validation.

The checked-in dashboard source does not store `m52-*` artifact directories or
M52 rows in `reports/wgsl-pipeline/scenes/generated/results.json`. Regeneration
is required for dashboard export and PM bundle creation.
