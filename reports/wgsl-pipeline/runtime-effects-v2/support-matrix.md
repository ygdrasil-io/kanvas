...
Status counts: total=8; descriptor-backed=6; CPU-only=2; GPU-backed=4; dependency-gated=0; expected-unsupported=2.
...
| runtime.invert_blender | kBlender | descriptor-backed | cpu-only | kotlin/invert_blender | - | runtime-effect.blender-dst-read-unsupported | This registered runtime blender has Kotlin/CPU behavior, but its result depends on destination color. WebGPU remains refused until an explicit shader/layer composite BlendPlan exists; this does not support all blend modes. |
