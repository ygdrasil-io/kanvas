...
`M90-PAA-3B-REF` is the row-specific evidence gate for `StrokeRectGM` /
`skia-gm-strokerect`.
...
Historical StrokeRect tests, historical similarity floors, and the
`path-aa-stroke-primitive` representative artifacts remain provenance only.
They do not replace the `skia-gm-strokerect` artifact bundle and do not update
registry, dashboard, threshold, edge-budget, or readiness state.
...
```text
reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerect/
```
...
```text
rtk python3 scripts/validate_m90_strokerect_evidence_gate.py --check-worktree-scope
rtk ./gradlew --no-daemon pipelineM90PathAaStrokeRectEvidenceGate
