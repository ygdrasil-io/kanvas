...
private const val WEBGPU_M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_FLAG: String =
    "kanvas.webgpu.m60F16AaStencilCoverContributionIsolation.enabled"
private const val WEBGPU_M60_F16_AA_STENCIL_COVER_PREDRAW_DST_READBACK_FLAG: String =
    "kanvas.webgpu.m60F16AaStencilCoverPredrawDstReadback.enabled"
private const val WEBGPU_M60_F16_AA_STENCIL_COVER_SHADER_RETURN_DIAGNOSTIC_FLAG: String =
...
public class SkWebGpuDevice(
    ...
    public data class M60F16AaStencilCoverPredrawDstReadbackSnapshot(
        val propertyName: String,
        val enabled: Boolean,
        val requestedBoundary: String,
        val observedBoundary: String,
        ...
        val intermediateFormat: String,
        val sampleLimit: Int,
        val events: List<M60F16AaStencilCoverPredrawDstReadbackEvent>,
    )

    public data class M60F16AaStencilCoverPredrawDstReadbackEvent(
        val drawIndex: Int,
        val pipelineFamily: String,
        val fillType: String,
        val blendMode: String,
        ...
        val copySucceeded: Boolean,
        val copyFailureReason: String?,
        val samples: List<M60F16AaStencilCoverPredrawDstReadbackSample>,
    )

    public data class M60F16AaStencilCoverPredrawDstReadbackSample(
        val x: Int,
        val y: Int,
        val targetWithinScissor: Boolean,
        val readbackAttempted: Boolean,
    ...
    public data class M60F16AaStencilCoverContributionIsolationSnapshot(
        ...
        val pipelineLayout: String,
        val sampleLimit: Int,
        val events: List<M60F16AaStencilCoverContributionIsolationEvent>,
    )

    public data class M60F16AaStencilCoverContributionIsolationEvent(
        val drawIndex: Int,
        val pipelineFamily: String,
        val fillType: String,
        val blendMode: String,
        ...
        val edgeCount: Int,
        val coverVertexCount: Int,
        val samples: List<M60F16AaStencilCoverContributionIsolationSample>,
    )

    public data class M60F16AaStencilCoverContributionIsolationSample(
        val x: Int,
        val y: Int,
        val subdrawOrdinal: Int,
        val subdrawRole: String,
        val targetWithinScissor: Boolean,
        val shaderObserved: Boolean,
        val dstBeforeRgbaFloat: FloatArray?,
        val sourceColorPremulRgbaFloat: FloatArray?,
        val coverageOrAaAlpha: Float?,
        val blendMode: String,
        val expectedSourceOverRgbaFloat: FloatArray?,
        val dstAfterRgbaFloat: FloatArray?,
        val postPassDstAfterRgbaFloat: FloatArray?,
        val missingFields: List<String>,
        val classification: String,
        val reason: String,
    )

    public data class M60F16AaStencilCoverShaderReturnDiagnosticSnapshot(
        val propertyName: String,
        val enabled: Boolean,
        val requestedBoundary: String,
        val observedBoundary: String,
        ...
        val pipelineLayout: String,
        val sampleLimit: Int,
        val events: List<M60F16AaStencilCoverShaderReturnDiagnosticEvent>,
    )

    public data class M60F16AaStencilCoverShaderReturnDiagnosticEvent(
        val drawIndex: Int,
        val pipelineFamily: String,
        val fillType: String,
        val blendMode: String,
        ...
        val edgeCount: Int,
        val coverVertexCount: Int,
        val samples: List<M60F16AaStencilCoverShaderReturnDiagnosticSample>,
    )

    public data class M60F16AaStencilCoverShaderReturnDiagnosticSample(
        val x: Int,
        val y: Int,
        val subdrawOrdinal: Int,
        val subdrawRole: String,
        ...
        val colorAfterColorFilter: FloatArray?,
        val colorAfterTargetColorspaceIfNeeded: FloatArray?,
        val correctedColorBeforeCoverage: FloatArray?,
        val coverageOrAaAlpha: Float?,
        val sourceAlphaAfterCoverage: Float?,
        val sourceColorBeforeQuantization: FloatArray?,
    ...
    private val m60F16AaStencilCoverPredrawDstReadbackEvents:
    ...
    public fun m60F16AaStencilCoverPredrawDstReadbackSnapshot():
        M60F16AaStencilCoverPredrawDstReadbackSnapshot =
        M60F16AaStencilCoverPredrawDstReadbackSnapshot(
            propertyName = WEBGPU_M60_F16_AA_STENCIL_COVER_PREDRAW_DST_READBACK_FLAG,
            enabled = m60F16AaStencilCoverPredrawDstReadbackDiagnosticsEnabled,
            requestedBoundary = "immediately before each selected M60 F16 StencilCoverAaPolygonDraw",
            observedBoundary =
                "compute textureLoad from intermediate RGBA16Float before StencilCoverAaPolygonDraw render pass",
            diagnosticShader = M60_F16_AA_STENCIL_COVER_POST_PASS_READBACK_SHADER,
            pipelineLayout = M60_F16_AA_STENCIL_COVER_POST_PASS_READBACK_LAYOUT,
            intermediateFormat = intermediateFormat.name,
            sampleLimit = M60_F16_DIRECT_PASS_WRITE_HOOK_POINTS.size,
    ...
    public fun m60F16AaStencilCoverContributionIsolationSnapshot():
        ...
        M60F16AaStencilCoverContributionIsolationSnapshot(
            propertyName = WEBGPU_M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_FLAG,
            enabled = m60F16AaStencilCoverContributionIsolationDiagnosticsEnabled,
            requestedBoundary = "per StencilCoverAaPolygonDraw inside/outside cover sub-draw",
            observedBoundary = "fragment shader source/coverage fields plus post-pass RGBA16Float readback",
            diagnosticShader = M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_SHADER,
            pipelineLayout = M60_F16_FRAGMENT_LANE_DIAGNOSTIC_LAYOUT,
            sampleLimit = M60_F16_DIRECT_PASS_WRITE_HOOK_POINTS.size,
    ...
    public fun m60F16AaStencilCoverShaderReturnDiagnosticSnapshot():
        ...
        M60F16AaStencilCoverShaderReturnDiagnosticSnapshot(
            propertyName = WEBGPU_M60_F16_AA_STENCIL_COVER_SHADER_RETURN_DIAGNOSTIC_FLAG,
            enabled = m60F16AaStencilCoverShaderReturnDiagnosticsEnabled,
            requestedBoundary = "per StencilCoverAaPolygonDraw fragment return before fixed-function blend",
            observedBoundary =
                "fragment shader storage side-channel written immediately before returning @location(0)",
    ...
    private suspend fun recordM60F16FragmentLaneDiagnostics(perDrawResources: List<DrawResources>) {
        ...
        m60F16AaStencilCoverShaderReturnDiagnosticEvents.clear()
        val readbacks = perDrawResources.mapIndexedNotNull { drawIndex, res ->
            res.m60F16FragmentLaneDiagnosticStaging?.let { staging ->
                M60F16FragmentLaneDiagnosticReadback(
                    drawIndex = drawIndex,
                    metadata = res.m60F16AaStencilCoverDiagnosticMetadata,
                    staging = staging,
        ...
        readbacks.forEach { readback ->
            ...
            } else if (contributionIsolationFormat) {
                M60_F16_DIRECT_PASS_WRITE_HOOK_POINTS.flatMap { point -> listOf(point, point) }
            } else {
            ...
            readback.staging.unmap()
            val buffer = ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN)
            val contributionIsolationSamples =
                mutableListOf<M60F16AaStencilCoverContributionIsolationSample>()
            val shaderReturnDiagnosticSamples =
            ...
            points.forEachIndexed { index, point ->
                ...
                val lane: Int
                val side: Int
                if (applicationPointFormat || coverageStencilMapFormat || contributionIsolationFormat) {
                ...
                if ((applicationPointFormat || contributionIsolationFormat) &&
                ...
                if (contributionIsolationFormat) {
                    ...
                    val missing = listOf(
                        "dstBeforeRgbaFloat",
                        "expectedSourceOverRgbaFloat",
                        "dstAfterRgbaFloat",
                    )
                    contributionIsolationSamples +=
                        M60F16AaStencilCoverContributionIsolationSample(
                            x = sampleX,
                            y = sampleY,
                            subdrawOrdinal =
                                index % M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_SUBDRAW_COUNT,
                            subdrawRole = role,
                            targetWithinScissor = readback.metadata?.scissor?.let {
                                m60F16PointWithinScissor(sampleX, sampleY, it)
                            } ?: false,
                            shaderObserved = observed,
                            dstBeforeRgbaFloat = null,
                            sourceColorPremulRgbaFloat = if (observed) {
                                floatArrayOf(
                                    target[0] * target[3],
                                    target[1] * target[3],
                                    target[2] * target[3],
                            ...
                            blendMode = readback.metadata?.blendMode ?: "unknown",
                            expectedSourceOverRgbaFloat = null,
                            dstAfterRgbaFloat = null,
                            postPassDstAfterRgbaFloat = null,
                            missingFields = missing,
                            classification = if (observed) {
                                "per-subdraw-framebuffer-state-unavailable"
                            } else {
                                "per-subdraw-sample-not-observed"
                            },
                            reason = if (observed) {
                                "FOR-408 captured shader-side source color and coverage for the subdraw, but WebGPU fixed-function blending does not expose framebuffer dstBefore/dstAfter to this fragment shader."
                            } else {
                                "The bounded FOR-408 shader-side hook did not observe this coordinate for this subdraw."
                            },
                        )
                }
                if (shaderReturnDiagnosticFormat) {
                    val observed = lane != 0 || x != 0 || y != 0 || side != 0
                    val sampleX = if (observed) x else point.first
                    val sampleY = if (observed) y else point.second
                    val filtered = FloatArray(4) { channel ->
                    ...
                    shaderReturnDiagnosticSamples +=
                        M60F16AaStencilCoverShaderReturnDiagnosticSample(
                            ...
                            reason = if (observed) {
                                "FOR-412 captured the vec4f returned by m60_f16_application_point_output immediately before the fragment shader returns @location(0) to fixed-function blend."
                            } else {
                                "The bounded FOR-412 shader-return hook did not observe this coordinate for this subdraw."
                            },
                        )
                }
            ...
            if (contributionIsolationFormat) {
                m60F16AaStencilCoverContributionIsolationEvents +=
                    M60F16AaStencilCoverContributionIsolationEvent(
                        drawIndex = readback.drawIndex,
                        pipelineFamily = "StencilCoverAaPolygonDraw",
                        fillType = readback.metadata?.fillType ?: "unknown",
                        blendMode = readback.metadata?.blendMode ?: "unknown",
                        scissor = readback.metadata?.scissor?.copyOf() ?: intArrayOf(),
                        edgeCount = readback.metadata?.edgeCount ?: -1,
                        coverVertexCount = readback.metadata?.coverVertexCount ?: -1,
                        samples = contributionIsolationSamples,
                    )
            }
            if (shaderReturnDiagnosticFormat) {
                m60F16AaStencilCoverShaderReturnDiagnosticEvents +=
    ...
    private fun recordM60F16AaStencilCoverPredrawDstUnavailable(
        drawIndex: Int,
        d: StencilCoverAaPolygonDraw,
        copyFailureReason: String,
        reasonForTargetedPixel: String,
    ) {
        ...
        val samples = M60_F16_DIRECT_PASS_WRITE_HOOK_POINTS.map { (x, y) ->
            val targetWithinScissor = m60F16PointWithinScissor(x, y, d.scissor)
            M60F16AaStencilCoverPredrawDstReadbackSample(
                x = x,
                y = y,
                targetWithinScissor = targetWithinScissor,
                readbackAttempted = false,
                ...
                classification = if (targetWithinScissor) {
                    ...
                    "predraw-dst-not-targeted-by-draw"
                },
                reason = if (targetWithinScissor) {
                    reasonForTargetedPixel
                } else {
                    "The coordinate is outside this StencilCoverAaPolygonDraw scissor, so this draw cannot provide the predraw destination for it."
                },
            )
        }
        m60F16AaStencilCoverPredrawDstReadbackEvents +=
            m60F16PredrawDstReadbackEvent(
                drawIndex = drawIndex,
                d = d,
                copyAttempted = false,
                copySucceeded = false,
    ...
    private suspend fun recordM60F16AaStencilCoverPredrawDstReadbacks() {
        if (!m60F16AaStencilCoverPredrawDstReadbackDiagnosticsEnabled) return
        val readbacks = m60F16AaStencilCoverPredrawDstPendingReadbacks.toList()
            ...
            try {
                ...
                val samples = M60_F16_DIRECT_PASS_WRITE_HOOK_POINTS.mapIndexed { index, (x, y) ->
                    ...
                    M60F16AaStencilCoverPredrawDstReadbackSample(
                        ...
                        dstBeforeRgbaFloat = if (valid) observed else null,
                        dstBeforeRgba8 = if (valid) {
                            IntArray(4) { channel ->
                                ((observed[channel] * 255f) + 0.5f).toInt().coerceIn(0, 255)
                            }
                        } else {
                        ...
                        reason = if (valid) {
                            "FOR-410 compute diagnostic sampled the intermediate texture immediately before this StencilCoverAaPolygonDraw render pass."
                        } else if (targetWithinScissor) {
                            "The predraw compute diagnostic ran, but this coordinate did not produce an in-bounds destination sample for this draw."
                        } else {
                            "The coordinate is outside this StencilCoverAaPolygonDraw scissor, so this draw cannot provide the predraw destination for it."
                        },
                    )
                ...
                val samples = M60_F16_DIRECT_PASS_WRITE_HOOK_POINTS.map { (x, y) ->
                    ...
                    M60F16AaStencilCoverPredrawDstReadbackSample(
                        ...
                        reason = "FOR-410 predraw compute/copy readback failed: ${t::class.simpleName}: ${t.message}",
                    )
                }
                m60F16AaStencilCoverPredrawDstReadbackEvents +=
                    m60F16PredrawDstReadbackEvent(
    ...
    private fun m60F16PredrawDstReadbackEvent(
        drawIndex: Int,
        d: StencilCoverAaPolygonDraw,
        copyAttempted: Boolean,
        copySucceeded: Boolean,
        copyFailureReason: String?,
        samples: List<M60F16AaStencilCoverPredrawDstReadbackSample>,
    ): M60F16AaStencilCoverPredrawDstReadbackEvent =
        m60F16PredrawDstReadbackEvent(
            metadata = M60F16AaStencilCoverPostPassReadbackMetadata(
                drawIndex = drawIndex,
                fillType = d.fillType.name,
    ...
    private data class StencilCoverAaPolygonDraw(
        ...
        /**
         * FOR-398 opt-in correction probe. Unlike the older FOR-380
         * draw-wide probe, this uses the runtime M60 F16 lane predicate
         * inside `aa_stencil_cover.wgsl` and leaves all other fragments on
         * the current target-colour blend path.
         */
        val m60F16BoundedRuntimeCorrectionProbe: Boolean = false,
        /**
    ...
    private data class LayerCompositeDraw(
        ...
        // L1b -- materialize-to-scratch mode (MatrixTransform inside a
        // Compose tree). When [materializeTargetTexture] is non-null, the
        // dispatch loop redirects this draw's render pass to write INTO
        // that scratch instead of the parent's `colorView`. The scratch's
        // view is then handed to the downstream Blur / ColorFilter /
        // LayerComposite stages as their `sourceLayerView`, so they
        // consume the matrix-transformed pixels as if they were the raw
        // layer source.
        //
        // Semantics when materialising :
        //  - the dispatch FORCES `loadOp = Clear` on the render pass (the
        //    scratch starts transparent ; the MatrixTransform writes its
        //    full footprint into it).
        //  - the blend mode is forced to `kSrc` regardless of `mode` (the
        //    scratch starts blank, the MatrixTransform IS the only
        //    contribution).
        //  - `scissor` is in LAYER-LOCAL coords (the scratch is layer-sized
        //    so the parent clip is irrelevant here -- the outer composite
        //    of the materialised scratch carries the parent clip).
        //  - `dstOriginX/Y` are layer-local (0, 0) ; the matrix payload's
        //    `c / f` translation slots don't bake any parent origin shift
        //    (the layer-local case has `originX = originY = 0`).
        //
        // Mirrors the [DropShadowLayerCompositeDraw.materializeTargetView]
        // / [DropShadowLayerCompositeDraw.materializeTargetTexture] pair
        // (L1a) ; the scratch texture is OWNED by this draw and released
        // in [closeDrawResources] after submission.
    ...
    private data class DropShadowLayerCompositeDraw(
        ...
        // L1a -- materialize-to-scratch mode. When [materializeTargetTexture]
        // is non-null, both composite sub-passes (shadow + original) write
        // INTO that scratch instead of the parent's `colorView`. Used by
        // [resolveLayerImageFilterPlan] + [compositeFrom] when a DropShadow
        // is nested inside a Compose tree : the materialize pass turns the
        // (shadow + original) composite into a layer-local texture that the
        // remaining chain (Blur, ColorFilter, Offset) consumes as if it were
        // the raw layer source. The scratch texture is OWNED by this draw
        // (mirrors [scratchHTexture] / [scratchVTexture]) and released in
        // [closeDrawResources] after submission.
        //
        // Semantics when materialising :
        //  - both sub-passes use `loadOp = Clear` (the scratch starts
        //    transparent ; the shadow pass populates it, the original pass
        //    then runs on TOP via the dispatch loop's natural `Load`
        //    sequencing).
        //  - the dispatch FORCES `mode = kSrc` on BOTH sub-passes : the
        //    first writes (shadow), the second blends on top (still kSrc
        //    after the first Clear because the dispatch promotes the second
        //    pass to `Load` automatically -- so kSrc here means "REPLACE
        //    the destination pixel with the source pixel", which for the
        //    original-on-top-of-shadow combination is wrong... we use
        //    kSrcOver on the second sub-pass).
        //  - `originalScissor` / `shadowScissor` are translated to layer-
        //    local coords (no parent clip, no parent viewport intersection
        //    -- the scratch is layer-sized).
        //  - `dstOriginX/Y` and `shadowDstOriginX/Y` are layer-local : the
        //    original sits at (0, 0) inside the scratch, the shadow at
        //    (sdx, sdy).
        val materializeTargetTexture: GPUTexture? = null,
        val materializeTargetView: GPUTextureView? = null,
        // PendingDraw interface : r/g/b/a placeholders ; mode honoured
    ...
    private fun loadM60F16AaStencilCoverProbeShader(
        cacheKey: String,
        diagnostic: Boolean,
        ...
        if (diagnostic) {
            wgsl = wgsl.replace(
                ...
                if (applicationPointDiagnostic ||
                    coverageStencilContributionMapDiagnostic ||
                    contributionIsolationDiagnostic
                ) {
...
""".trimIndent() + "\n"
        ...
        val insertedHelpers = buildString {
            if (boundedRuntimeCorrection) {
                append(
                    """
fn m60_f16_bounded_runtime_corrected_color(pixel: vec2f) -> vec4f {
    let filtered = apply_color_filter(uniforms.color);
    if (m60_f16_candidate_lane(pixel)) {
        return filtered;
    }
    return apply_target_colorspace_if_needed(filtered);
}

fn m60_f16_quantize_after_bounded_runtime_correction(pixel: vec2f, c: vec4f) -> vec4f {
    if (m60_f16_candidate_lane(pixel)) {
        return c;
    }
    return quantize_rgba8_if_target_blend(c);
}
""".trimIndent(),
                )
                append("\n\n")
            ...
            if (diagnostic) {
                append(
                    if (contributionIsolationDiagnostic) {
                        """
fn m60_f16_application_point_slot(pixel: vec2f) -> i32 {
    let px = u32(floor(pixel.x));
    let py = u32(floor(pixel.y));
    if (px == 92u && py == 75u) { return 0; }
    if (px == 91u && py == 76u) { return 1; }
    if (px == 90u && py == 77u) { return 2; }
    if (px == 89u && py == 78u) { return 3; }
    if (px == 88u && py == 79u) { return 4; }
    if (px == 87u && py == 80u) { return 5; }
    if (px == 101u && py == 37u) { return 6; }
    if (px == 102u && py == 37u) { return 7; }
    if (px == 99u && py == 38u) { return 8; }
    if (px == 100u && py == 38u) { return 9; }
    if (px == 101u && py == 38u) { return 10; }
    if (px == 102u && py == 38u) { return 11; }
    if (px == 103u && py == 38u) { return 12; }
    if (px == 104u && py == 38u) { return 13; }
    if (px == 98u && py == 39u) { return 14; }
    if (px == 99u && py == 39u) { return 15; }
    return -1;
}
...
""".trimIndent()
                    } else if (coverageStencilContributionMapDiagnostic) {
                        """
fn m60_f16_application_point_slot(pixel: vec2f) -> i32 {
    let px = u32(floor(pixel.x));
    let py = u32(floor(pixel.y));
...
""".trimIndent()
        ...
        if (insertedHelpers.isNotEmpty()) {
            wgsl = wgsl.replace("@fragment\nfn fs_inside", "$insertedHelpers\n\n@fragment\nfn fs_inside")
        }
        if (diagnostic) {
            wgsl = wgsl
                .replace(
...
""".trimIndent()
    ...
    private fun loadM60F16FragmentLaneDiagnosticShader(): GPUShaderModule =
        loadM60F16AaStencilCoverProbeShader(
            cacheKey = "diagnostic://m60-f16-aa-stencil-cover-fragment-lane",
            diagnostic = true,
            boundedRuntimeCorrection = false,
        )

    private fun loadM60F16BoundedRuntimeCorrectionShader(): GPUShaderModule =
        loadM60F16AaStencilCoverProbeShader(
    ...
    private fun loadM60F16BoundedRuntimeCorrectionDiagnosticShader(): GPUShaderModule =
        loadM60F16AaStencilCoverProbeShader(
    ...
    private fun loadM60F16AaStencilCoverContributionIsolationDiagnosticShader(): GPUShaderModule =
        loadM60F16AaStencilCoverProbeShader(
            cacheKey = "diagnostic://m60-f16-aa-stencil-cover-contribution-isolation-for408",
            diagnostic = true,
            boundedRuntimeCorrection = true,
    ...
    private val m60F16BoundedRuntimeCorrectionShaderLazy: Lazy<GPUShaderModule> = lazy {
    ...
    private val m60F16AaStencilCoverContributionIsolationDiagnosticShaderLazy: Lazy<GPUShaderModule> = lazy {
        loadM60F16AaStencilCoverContributionIsolationDiagnosticShader()
    }
    private val m60F16AaStencilCoverShaderReturnDiagnosticShaderLazy: Lazy<GPUShaderModule> = lazy {
        loadM60F16AaStencilCoverShaderReturnDiagnosticShader()
    }

    private val m60F16FragmentLaneDiagnosticShader: GPUShaderModule
        get() = m60F16FragmentLaneDiagnosticShaderLazy.value

    private val m60F16BoundedRuntimeCorrectionShader: GPUShaderModule
        get() = m60F16BoundedRuntimeCorrectionShaderLazy.value
    ...
    private val m60F16AaStencilCoverContributionIsolationDiagnosticShader: GPUShaderModule
        get() = m60F16AaStencilCoverContributionIsolationDiagnosticShaderLazy.value

    private val m60F16AaStencilCoverShaderReturnDiagnosticShader: GPUShaderModule
        get() = m60F16AaStencilCoverShaderReturnDiagnosticShaderLazy.value
    ...
    private val m60F16BoundedRuntimeCorrectionPipelineCache:
        MutableMap<Pair<Triple<SkBlendMode, SkPathFillType, CoverageSide>, Boolean>, GPURenderPipeline> =
            mutableMapOf()
    ...
    private fun m60F16BoundedRuntimeCorrectionPipelineFor(
        ...
        m60F16BoundedRuntimeCorrectionPipelineCache.getOrPut(Triple(mode, fillType, side) to diagnostic) {
            val readMask: UInt = if (fillType.isEvenOdd()) 0x01u else 0xFFu
            val insideCompare =
                if (fillType.isInverse()) GPUCompareFunction.Equal else GPUCompareFunction.NotEqual
            val compare = when (side) {
            ...
            val shaderModule = if (diagnostic) {
                when {
                    ...
                    m60F16AaStencilCoverContributionIsolationDiagnosticsEnabled ->
                        m60F16AaStencilCoverContributionIsolationDiagnosticShader
                    m60F16CoverageStencilContributionMapDiagnosticsEnabled ->
    ...
    override fun compositeFrom(
        ...
        if (blurParams != null &&
            ...
            enqueueBlurredLayerComposite(
                ...
                mode = mode,
                // L1a -- when a DropShadow leaf was materialised above,
                // route the blur's H pass to read from the materialised
                // scratch instead of the raw layer view.
                sourceLayerView = innerSourceView,
            )
            return
        }
        ...
        if (nonNativeBlend) {
            ...
            pending.add(
                NonNativeBlendLayerCompositeDraw(
                    // L1a -- source the materialised scratch when an
                    // inner DropShadow ran above ; else the raw layer view.
                    layerView = innerSourceView,
                    layerWidth = w, layerHeight = h,
        ...
        pending.add(
            LayerCompositeDraw(
                // L1a -- source the materialised scratch when an inner
                // DropShadow ran above ; else the raw layer view.
                layerView = innerSourceView,
                layerWidth = w, layerHeight = h,
                dstOriginX = composeShiftedOriginX, dstOriginY = composeShiftedOriginY,
                scissor = intArrayOf(ix0, iy0, ix1 - ix0, iy1 - iy0),
    ...
    private fun enqueueBlurredLayerComposite(
        ...
        mode: SkBlendMode,
        // L1a -- the texture the blur's H pass samples from. Default is
        // `gpuSrc.intermediateView` (raw layer pixels). When a DropShadow
        // materialise pre-pass ran (Phase L1a), the caller substitutes the
        // materialised scratch's view here so the blur consumes the
        // shadowed result instead.
    ...
    /**
     * L1a -- render-to-texture pre-pass that materialises a `DropShadow`
     * filter into a layer-sized scratch texture, used by [compositeFrom]
     * when the filter sits INSIDE a Compose tree (rather than at the top
     * level where it dispatches directly onto the parent's intermediate).
     *
     * Allocates the scratch + the two blur H/V scratches, enqueues a
     * [DropShadowLayerCompositeDraw] in **materialise mode** :
     *  - `materializeTargetView` = the new scratch's view
     *  - `dstOriginX/Y` = (0, 0) (the original sits at the top-left of
     *    the scratch -- layer-local)
     *  - `shadowDstOriginX/Y` = (sdx, sdy) (the shadow shifted by the
     *    integer-rounded displacement, layer-local)
     *  - `originalScissor` = (0, 0, w, h) (full layer rect)
     *  - `shadowScissor` = clipped against (0, 0, w, h) (the shadow's
     *    offset rect intersected with the layer footprint -- matches the
     *    upstream `SkDropShadowImageFilter` semantic when running inside
     *    saveLayer : the layer rect bounds the output, the shadow outside
     *    that rect is clipped)
     *  - The dispatch loop forces `loadOp = Clear` + `mode = kSrc` on the
     *    shadow sub-pass and `loadOp = Load` + `mode = kSrcOver` on the
     *    original sub-pass when `materializeTargetView` is non-null.
     *
     * Returns the (texture, view) pair so the caller can :
     *  - Pass the view to downstream Blur / ColorFilter / LayerComposite
     *    enqueue helpers as their `sourceLayerView`
     *  - Keep the texture alive until the consumer draws are submitted
     *    (the texture is registered on the materialise DrawResources and
     *    closed in [closeDrawResources] after the entire `pending` batch
     *    is flushed)
     *
     * Sigma / colour / displacement constraints mirror
     * [enqueueDropShadowLayerComposite] (sigmas must be finite + non-
     * negative, displacement integer-rounded per upstream Skia's
     * `(d + 0.5).toInt()` convention).
     */
    private fun enqueueMaterializeDropShadowToScratch(
        gpuSrc: SkWebGpuDevice,
        w: Int, h: Int,
        params: org.skia.foundation.SkDropShadowImageFilterParams,
        ...
        if (!params.sigmaX.isFinite() || !params.sigmaY.isFinite() ||
            ...
            error(
                "SkWebGpuDevice.compositeFrom : a Compose-nested DropShadow's " +
                    "sigma must be finite and non-negative ; got (sigmaX = " +
                    "${params.sigmaX}, sigmaY = ${params.sigmaY})."
            )
        }
        ...
        // Blur H/V scratches : same shape as the top-level DropShadow path.
        val scratchH = context.device.createTexture(
            TextureDescriptor(
                size = Extent3D(width = w.toUInt(), height = h.toUInt()),
                format = intermediateFormat,
                usage = GPUTextureUsage.RenderAttachment or GPUTextureUsage.TextureBinding,
                label = "SkWebGpuDevice.materializeDropShadowScratchH",
            ),
        )
        val scratchHView = scratchH.createView()
        val scratchV = context.device.createTexture(
            TextureDescriptor(
                ...
                format = intermediateFormat,
                usage = GPUTextureUsage.RenderAttachment or GPUTextureUsage.TextureBinding,
                label = "SkWebGpuDevice.materializeDropShadowScratchV",
            ),
        )
        val scratchVView = scratchV.createView()

        // The materialised target : holds the (shadow + original)
        // composite in layer-local coords. Sampled by downstream Blur /
        // ColorFilter / LayerComposite draws via the returned view.
        val materializeTarget = context.device.createTexture(
            TextureDescriptor(
                size = Extent3D(width = w.toUInt(), height = h.toUInt()),
                format = intermediateFormat,
                usage = GPUTextureUsage.RenderAttachment or GPUTextureUsage.TextureBinding,
                label = "SkWebGpuDevice.materializeDropShadowTarget",
            ),
        )
        val materializeTargetView = materializeTarget.createView()

        // Integer-round the shadow displacement, layer-local (no parent
        // origin shift here -- the materialised scratch IS layer space).
        val sdx = floor(params.dx + 0.5f).toInt()
        val sdy = floor(params.dy + 0.5f).toInt()

        // Original-pass scissor : full layer rect (layer-local).
        val originalScissor = intArrayOf(0, 0, w, h)
        // Shadow-pass scissor : the shadow's (sdx, sdy)..(sdx+w, sdy+h)
        // intersected with the layer rect (0, 0, w, h). Pixels outside
        // the layer are clipped -- matches the upstream
        // SkDropShadowImageFilter semantic when running inside saveLayer
        // (the layer rect bounds the output ; the shadow that would land
        // outside the layer rect is dropped).
        val shadIx0 = sdx.coerceAtLeast(0)
        val shadIy0 = sdy.coerceAtLeast(0)
        val shadIx1 = (sdx + w).coerceAtMost(w)
        val shadIy1 = (sdy + h).coerceAtMost(h)
        val shadowScissor = if (shadIx0 >= shadIx1 || shadIy0 >= shadIy1) {
            intArrayOf(0, 0, 0, 0)
        } else {
            intArrayOf(shadIx0, shadIy0, shadIx1 - shadIx0, shadIy1 - shadIy0)
        ...
        // Shadow colour filter : `Blend(shadowColor, kSrcIn)`. Same
        // packing as [enqueueDropShadowLayerComposite]. The paintAlpha
        // term collapses to 1 here because the OUTER plan's paintAlpha
        // applies on the FINAL composite (when the materialised scratch
        // is consumed) ; the materialised pixels stay at their natural
        // alpha so the outer Blur / ColorFilter operates on the same
        // pixels the CPU raster would see at the DropShadow boundary.
        val shadowColor = params.color
        val ca = ((shadowColor ushr 24) and 0xFF) / 255f
        val crNonPre = ((shadowColor ushr 16) and 0xFF) / 255f
        val cgNonPre = ((shadowColor ushr 8) and 0xFF) / 255f
        val cbNonPre = (shadowColor and 0xFF) / 255f
        val shadowColorFilterPacked = FloatArray(24)
        shadowColorFilterPacked[0] = 1f
        shadowColorFilterPacked[1] = SkBlendMode.kSrcIn.ordinal.toFloat()
        shadowColorFilterPacked[4] = crNonPre * ca
        shadowColorFilterPacked[5] = cgNonPre * ca
        shadowColorFilterPacked[6] = cbNonPre * ca
        shadowColorFilterPacked[7] = ca

        // No colour filter on the original sub-pass -- the outer plan's
        // [effectiveColorFilter] applies on the FINAL composite, not on
        // the materialised pixels.
        val noColorFilter = FloatArray(24)
        ...
        pending.add(
            DropShadowLayerCompositeDraw(
                ...
                scratchHTexture = scratchH, scratchHView = scratchHView,
                scratchVTexture = scratchV, scratchVView = scratchVView,
                // Layer-local origins : the original sits at (0, 0)
                // inside the scratch ; the shadow at (sdx, sdy).
                dstOriginX = 0, dstOriginY = 0,
                shadowDstOriginX = sdx, shadowDstOriginY = sdy,
                originalScissor = originalScissor,
                shadowScissor = shadowScissor,
                kernelX = kernelX, radiusX = radiusX,
                kernelY = kernelY, radiusY = radiusY,
                tileModeOrdinal = SkTileMode.kDecal.ordinal,
                shadowColorFilterPacked = shadowColorFilterPacked,
                // paintAlpha = 1 on the materialised original -- the
                // outer plan handles the layer paint's alpha + colour
                // filter on the FINAL composite (where the materialised
                // scratch is the source).
                paintR = 1f, paintG = 1f, paintB = 1f, paintA = 1f,
                originalColorFilterPacked = noColorFilter,
                // Materialise mode : the dispatch will redirect both
                // sub-passes to [materializeTargetView] instead of the
                // parent's colorView, and force the blend modes /
                // load ops described in the [DropShadowLayerCompositeDraw]
                // kdoc.
    ...
    /**
     * L1b -- render-to-texture pre-pass that materialises a `MatrixTransform`
     * filter into a layer-sized scratch texture, used by [compositeFrom]
     * when the filter sits INSIDE a Compose tree (rather than at the top
     * level where it dispatches directly onto the parent's intermediate
     * via [enqueueMatrixTransformLayerComposite]).
     *
     * Allocates the scratch, enqueues a [LayerCompositeDraw] in
     * **materialise mode** :
     *  - `materializeTargetView` = the new scratch's view
     *  - `dstOriginX/Y` = (0, 0) (the matrix-transformed image sits at the
     *    top-left of the scratch -- layer-local)
     *  - matrix payload : the inverse 2x3 of the user matrix, with NO
     *    parent-origin shift baked in (the layer-local case has `originX
     *    = originY = 0`, so the `c / f` translation slots are just the
     *    bare inverse's `tx / ty`)
     *  - scissor : the axis-aligned bbox of `M([0..w] × [0..h])`,
     *    clamped to the layer rect (layer-local)
     *  - mode : kSrc (the scratch starts blank, the matrix-transformed
     *    image IS the only contribution -- the dispatch forces this)
     *
     * Returns the (texture, view) pair so the caller can :
     *  - Pass the view to downstream Blur / ColorFilter / LayerComposite
     *    enqueue helpers as their `sourceLayerView`
     ...
     *    is flushed)
     *
     * Bail conditions mirror [enqueueMatrixTransformLayerComposite] :
     *  - Non-null child filter -- throws (input == null required).
     *  - Perspective matrix -- throws (2x3 affine only).
     *  - Cubic sampling -- throws (kNearest / kLinear only).
     *  - Non-invertible matrix -- returns null (the CPU raster's
     *    `MatrixTransform` returns the input image unchanged in that
     *    case ; mapping that semantic onto a materialise scratch is
     *    a no-op, so we return null and the caller falls back to the
     *    raw layer view as its source).
     */
    private fun enqueueMaterializeMatrixTransformToScratch(
        gpuSrc: SkWebGpuDevice,
        w: Int, h: Int,
        params: org.skia.foundation.SkMatrixTransformImageFilterParams,
        ...
        if (params.input != null) {
            error(
                "SkWebGpuDevice.compositeFrom : a Compose-nested MatrixTransform " +
                    "leaf has a non-null child filter. The Compose support " +
                    "flattens the tree, so leaves must have input == null -- " +
                    "nest the child via a Compose node instead (e.g. " +
                    "SkImageFilters.Compose(MatrixTransform(m, sampling), child) " +
                    "rather than MatrixTransform(m, sampling, input = child))."
            )
        }
        if (params.matrix.hasPerspective()) {
            error(
                "SkWebGpuDevice.compositeFrom : a Compose-nested MatrixTransform " +
                    "leaf has a perspective matrix (non-trivial bottom row). " +
                    "Only 2x3 affine is supported in the L1b slice (Phase " +
                    "G-saveLayer-imageFilter-matrixTransform-compose) -- " +
                    "perspective needs a homogeneous divide in the fragment " +
                    "shader (the current shader uses a vec3 dot-product affine " +
                    "path) and is deferred to a follow-up slice."
            )
        }
        if (params.sampling.cubic != null) {
            error(
                "SkWebGpuDevice.compositeFrom : a Compose-nested MatrixTransform " +
                    "leaf has a cubic resampler. Only kNearest and kLinear are " +
                    "supported in the L1b slice (Phase G-saveLayer-imageFilter-" +
                    "matrixTransform-compose) -- cubic resampling needs a " +
                    "16-tap kernel in the composite shader and is deferred."
            )
        }
        val samplingModeOrdinal = when (params.sampling.filter) {
            SkFilterMode.kNearest -> 1
        ...
        // Layer-local inverse : no parent origin shift to bake in (the
        // materialised scratch IS layer space, so `originX = originY = 0`
        // and `c / f` collapse to the bare inverse's `tx / ty`).
        val invMatrix = params.matrix.invert()
            ?: return null  // Non-invertible -- fall back to raw layer view.
        val matrixPacked = FloatArray(12)
        matrixPacked[0] = invMatrix.sx
        matrixPacked[1] = invMatrix.kx
        matrixPacked[2] = invMatrix.tx
        matrixPacked[4] = invMatrix.ky
        matrixPacked[5] = invMatrix.sy
        matrixPacked[6] = invMatrix.ty
        matrixPacked[8] = samplingModeOrdinal.toFloat()

        // Materialised target : holds the matrix-transformed image in
        // layer-local coords. Sampled by downstream Blur / ColorFilter /
        // LayerComposite draws via the returned view.
        val materializeTarget = context.device.createTexture(
            TextureDescriptor(
                size = Extent3D(width = w.toUInt(), height = h.toUInt()),
                format = intermediateFormat,
        ...
        // Cover-quad scissor in LAYER-LOCAL pixel coords : the axis-
        // aligned bbox of the four corners of the layer rect mapped
        // through the user matrix, clamped to the layer rect (the
        // scratch is layer-sized -- pixels outside the layer rect are
        // dropped, matching the upstream SkMatrixTransformImageFilter
        // semantic when running inside saveLayer).
        val mappedRect = params.matrix.mapRect(
            org.graphiks.math.SkRect.MakeLTRB(0f, 0f, w.toFloat(), h.toFloat()),
        )
        val ix0 = floor(mappedRect.left).toInt().coerceAtLeast(0)
        val iy0 = floor(mappedRect.top).toInt().coerceAtLeast(0)
        val ix1 = ceil(mappedRect.right).toInt().coerceAtMost(w)
        val iy1 = ceil(mappedRect.bottom).toInt().coerceAtMost(h)
        if (ix0 >= ix1 || iy0 >= iy1) {
            // Mapped footprint lies entirely outside the layer rect.
            // The downstream consumers should see a fully-transparent
            // source : return the scratch view as-is (the dispatch's
            // `loadOp = Clear` will leave it transparent). We still
            // need a draw entry on `pending` so the scratch is owned by
            // a [DrawResources] and gets cleaned up after submit ; we
        ...
        pending.add(
            LayerCompositeDraw(
                ...
                dstOriginX = 0, dstOriginY = 0,
                scissor = intArrayOf(ix0, iy0, ix1 - ix0, iy1 - iy0),
                // paintAlpha = 1 on the materialised image -- the outer
                // plan handles the layer paint's alpha + colour filter
                // on the FINAL composite (where the materialised scratch
                // is the source).
                paintR = 1f, paintG = 1f, paintB = 1f, paintA = 1f,
                r = 1f, g = 1f, b = 1f, a = 1f,
                // Materialise mode forces `kSrc` at the dispatch site
                // regardless of `mode` ; we still ship a sentinel.
                mode = SkBlendMode.kSrc,
                colorFilterPacked = FloatArray(24),
                matrixPacked = matrixPacked,
                imageFilterPacked = FloatArray(12),
                // Materialise mode : the dispatch redirects this draw's
                // render pass to write into the scratch (with `loadOp =
        ...
        fun walk(node: org.skia.foundation.SkImageFilter) {
            ...
            if (dropShadowLeaf != null) {
                if (dropShadowLeaf.input != null) {
                    error(
                        "SkWebGpuDevice.compositeFrom : a DropShadow leaf inside " +
                            "the imageFilter tree has a non-null child filter. The " +
                            "Compose support flattens the tree, so leaves must have " +
                            "input == null -- nest the child via a Compose node " +
                            "instead (e.g. SkImageFilters.Compose(DropShadow(...), " +
            ...
            if (matrixTransformLeaf != null) {
                if (matrixTransformLeaf.input != null) {
                    error(
                        "SkWebGpuDevice.compositeFrom : a MatrixTransform leaf " +
                            "inside the imageFilter tree has a non-null child " +
                            "filter. The Compose support flattens the tree, so " +
                            "leaves must have input == null -- nest the child via " +
                            "a Compose node instead (e.g. SkImageFilters.Compose(" +
                            "MatrixTransform(m, sampling), child) rather than " +
    ...
    */
    ...
    private fun m60F16BoundedRuntimeCorrectionProbeEnabled(paint: SkPaint): Boolean {
        val style = activeStrokeStyleForPathAaDiagnostics ?: return false
        if (!targetColorSpaceBlend) return false
        if (intermediateFormat != GPUTextureFormat.RGBA16Float) return false
        if (!System.getProperty(WEBGPU_STROKE_CAP_JOIN_EXPERIMENTAL_RENDER_FLAG, "false").toBoolean()) return false
        if (!System.getProperty(WEBGPU_M60_F16_BOUNDED_RUNTIME_CORRECTION_PROBE_FLAG, "false").toBoolean()) {
            return false
        }
        if (paint.blendMode != SkBlendMode.kSrcOver) return false
        if (paint.shader != null || paint.colorFilter != null || paint.maskFilter != null || paint.pathEffect != null) {
            ...
            if (paint.isAntiAlias && totalVerts <= MAX_AA_EDGES) {
                ...
                pending.add(
                    StencilCoverAaPolygonDraw(
                        ...
                        m60F16BoundedRuntimeCorrectionProbe =
                            m60F16BoundedRuntimeCorrectionProbeEnabled(paint),
    ...
    private fun encodePendingDrawsToIntermediate(
        ...
        if (m60F16AaStencilCoverPredrawDstReadbackDiagnosticsEnabled) {
        ...
        if (m60F16AaStencilCoverShaderReturnDiagnosticsEnabled) {
            m60F16AaStencilCoverShaderReturnDiagnosticEvents.clear()
        }
        ...
        // L1a -- track whether the parent's `colorView` has actually been
        // written by some pass yet. The materialise DropShadow pre-pass
        // (Phase L1a) writes to its own scratch texture, NOT to the
        // colorView ; the next pass that DOES target colorView must
        // therefore still see `loadOp = Clear` (with the background
        // colour) instead of `Load` on its first colorView write. Pre-
        // L1a, the "first draw clears colorView" invariant was a simple
        // `i == 0` check ; with materialise passes interleaved, we need
        // to track the colorView access flag separately.
        var colorViewWritten = intermediateInitialized
        pending.forEachIndexed { i, d ->
            ...
            // unchanged.
            //
            // L1a -- a DropShadow materialise pass writes to its own
            // scratch (the [DropShadowLayerCompositeDraw.materializeTargetView]
            // override) and does NOT touch colorView, so it does NOT
            // consume the "first colorView access clears" slot. The
            // [colorViewWritten] flag is flipped only when a pass that
            // targets colorView is encoded below (see the materialise
            // branch's `colorViewWritten` guard).
            //
            // L1b -- same reasoning for a MatrixTransform materialise
            // pass via [LayerCompositeDraw.materializeTargetView] : the
            // single-pass affine remap writes to the layer-sized scratch,
            // never to colorView, so it doesn't consume the clear slot
            // either.
            ...
            if (d is StencilCoverAaPolygonDraw) {
                ...
                if (m60F16AaStencilCoverPredrawDstReadbackDiagnosticsEnabled && d.m60F16BandMetadata != null) {
                    if (intermediateFormat != GPUTextureFormat.RGBA16Float) {
                        recordM60F16AaStencilCoverPredrawDstUnavailable(
                            drawIndex = i,
                            d = d,
                            copyFailureReason = "intermediate format ${intermediateFormat.name} is not RGBA16Float",
                            reasonForTargetedPixel =
                                "FOR-410 predraw destination readback is scoped to the M60 F16 intermediate; current intermediate format is ${intermediateFormat.name}.",
                        )
                    } else if (loadOp == GPULoadOp.Clear && !intermediateInitialized) {
                        recordM60F16AaStencilCoverPredrawDstUnavailable(
                            drawIndex = i,
                            d = d,
                            copyFailureReason = "intermediate texture not initialized before this draw",
                            reasonForTargetedPixel =
                                "The predraw boundary is before the first render pass that materializes the clear color; reading it without changing rendering would sample an uninitialized intermediate.",
                        )
                    } else if (res.m60F16AaStencilCoverPredrawDstReadbackBindGroup != null) {
                        encoder.clearBuffer(
                            res.m60F16AaStencilCoverPredrawDstReadbackStorage!!,
                            0uL,
                            res.m60F16AaStencilCoverPredrawDstReadbackBufferSize,
                        )
                        val pass = encoder.beginComputePass()
                        pass.setPipeline(m60F16AaStencilCoverPostPassReadbackPipeline)
                        pass.setBindGroup(0u, res.m60F16AaStencilCoverPredrawDstReadbackBindGroup)
                        pass.dispatchWorkgroups(M60_F16_AA_STENCIL_COVER_POST_PASS_READBACK_SAMPLE_COUNT)
                        pass.end()
                        encoder.copyBufferToBuffer(
                            source = res.m60F16AaStencilCoverPredrawDstReadbackStorage,
                            sourceOffset = 0uL,
                            destination = res.m60F16AaStencilCoverPredrawDstReadbackStaging!!,
                            destinationOffset = 0uL,
                            size = res.m60F16AaStencilCoverPredrawDstReadbackBufferSize,
                        )
                        m60F16AaStencilCoverPredrawDstPendingReadbacks +=
                            M60F16AaStencilCoverPredrawDstReadback(
                                metadata = M60F16AaStencilCoverPostPassReadbackMetadata(
                                    drawIndex = i,
                                    fillType = d.fillType.name,
                                    blendMode = d.mode.name,
                                ...
                                staging = res.m60F16AaStencilCoverPredrawDstReadbackStaging,
                                bufferSize = res.m60F16AaStencilCoverPredrawDstReadbackBufferSize,
                            )
                    }
                }
                encoder.beginRenderPass(
                    ...
                    setPipeline(
                        ...
                        } else if (diagnosticBindGroup != null) {
            ...
            if (d is DropShadowLayerCompositeDraw) {
                ...
                // L1a -- when materialising, both sub-passes target the
                // scratch texture instead of the parent's `colorView`. The
                // first sub-pass uses `loadOp = Clear` to wipe the scratch
                // (replaces the parent's `loadOp` which would carry the
                // intermediate-initialised state). The second sub-pass uses
                // `loadOp = Load` to compose on top. The blend mode for the
                // shadow sub-pass is forced to `kSrc` (the scratch starts
                // blank, the shadow IS the first contribution) ; the
                // original sub-pass keeps `kSrcOver` (lands on top of the
                // shadow). When NOT materialising, the existing flow runs
                // unchanged : both sub-passes target the parent's
                // `colorView` with `d.mode` (= kSrcOver, gated at the
                // dispatch site).
                val dropShadowOutView = d.materializeTargetView ?: colorView
                val dropShadowFirstLoadOp =
                    if (d.materializeTargetView != null) GPULoadOp.Clear
                    else loadOp
                val dropShadowShadowMode =
                    if (d.materializeTargetView != null) SkBlendMode.kSrc
                    else d.mode
                ...
                if (d.shadowScissor[2] > 0 && d.shadowScissor[3] > 0) {
                    encoder.beginRenderPass(
                        RenderPassDescriptor(
                            colorAttachments = listOf(
                                RenderPassColorAttachment(
                                    view = dropShadowOutView,
                                    loadOp = dropShadowFirstLoadOp,
                                    clearValue = background,
                                    storeOp = GPUStoreOp.Store,
                                ),
                            ),
                        ...
                        setPipeline(layerCompositePipelineFor(dropShadowShadowMode))
                        setBindGroup(0u, res.tertiaryBindGroup!!)
                        setScissorRect(
                ...
                val originalLoadOp = when {
                    d.materializeTargetView != null && d.shadowScissor[2] > 0 && d.shadowScissor[3] > 0 ->
                        GPULoadOp.Load
                    d.materializeTargetView != null ->
                        // Materialise + shadow skipped : Clear the scratch
                        // so the original pass starts from a transparent
                        // background (the shadow pass would have done it
                        // otherwise).
                        GPULoadOp.Clear
                    d.shadowScissor[2] > 0 && d.shadowScissor[3] > 0 ->
                        GPULoadOp.Load
                    else ->
                        loadOp
                }
                // L1a -- when materialising, the original sub-pass uses
                // kSrcOver (always lands on TOP of the shadow). The non-
                // materialise path keeps `d.mode` (gated to kSrcOver at the
                // dispatch site, so this is bit-iso).
                val originalMode =
                    if (d.materializeTargetView != null) SkBlendMode.kSrcOver
                    else d.mode
                if (d.originalScissor[2] > 0 && d.originalScissor[3] > 0) {
                    encoder.beginRenderPass(
                        ...
                        setPipeline(layerCompositePipelineFor(originalMode))
                        setBindGroup(0u, res.quaternaryBindGroup!!)
                        setScissorRect(
            ...
            if (d is LayerCompositeDraw && d.materializeTargetView != null) {
                // L1b -- MatrixTransform-inside-Compose materialise pass.
                // Single render pass that writes the matrix-transformed
                // image into the layer-sized scratch (the
                // [LayerCompositeDraw.materializeTargetView]) with
                // `loadOp = Clear` and `mode = kSrc`. The downstream
                // Blur / ColorFilter / LayerComposite draws then sample
                // the scratch as if it were the raw layer source.
                encoder.beginRenderPass(
                    RenderPassDescriptor(
                        colorAttachments = listOf(
                            RenderPassColorAttachment(
                                view = d.materializeTargetView,
                                loadOp = GPULoadOp.Clear,
                                clearValue = Color(0.0, 0.0, 0.0, 0.0),
                                storeOp = GPUStoreOp.Store,
                            ),
                    ...
                    // Zero-area scissor (out-of-layer matrix footprint) :
                    // the Clear loadOp above already wipes the scratch to
                    // transparent, which is the correct semantic for the
                    // CPU MatrixTransform on a fully-out-of-bounds map.
                    // Skip the draw call so wgpu doesn't reject the
                    // zero-size scissor rect.
    ...
    private fun closeDrawResources(perDrawResources: List<DrawResources>) {
        perDrawResources.forEach {
            ...
            // L1a -- materialise target texture (DropShadow inside Compose).
    ...
    private data class DrawResources(
        ...
        val m60F16AaStencilCoverPredrawDstReadbackStorage: GPUBuffer? = null,
        ...
        // after the command buffer that consumed it has been submitted.
        // The texture's view is referenced by the [DropShadowLayerCompositeDraw]
        // dispatch as `materializeTargetView` AND by the downstream Blur /
        // ColorFilter / LayerComposite draws as their `layerView` -- those
        // views (re-derived from the same texture) outlive the materialise
        // draw because each draw owns its own per-draw resources, but the
        // underlying texture must remain alive until the LAST consumer's
        // command buffer is submitted. All these draws live in the same
        // [pending] batch and share a single `flush` cycle, so closing
    ...
    private data class M60F16AaStencilCoverPredrawDstReadback(
        val metadata: M60F16AaStencilCoverPostPassReadbackMetadata,
        val staging: GPUBuffer,
        val bufferSize: ULong,
    )
    ...
    private fun buildLayerCompositeDrawResources(d: LayerCompositeDraw): DrawResources {
        ...
        return DrawResources(
            uniform = uniform,
            bindGroup = bindGroup,
            // L1b -- transfer ownership of the materialise scratch
            // (allocated by [enqueueMaterializeMatrixTransformToScratch])
            // from the PendingDraw to the DrawResources so it's cleaned
            // up in [closeDrawResources] after the command buffer that
            // sampled it (the downstream Blur / ColorFilter /
            // LayerComposite draws) has been submitted. `null` on every
            // non-materialise draw so the fast path is bit-iso.
    ...
    private fun buildDropShadowLayerCompositeDrawResources(
        ...
        return DrawResources(
            ...
            // is owned by SkCanvas, not by this draw (mirrors
            // [BlurredLayerCompositeDraw]).
            // L1a -- transfer ownership of the materialise scratch
            // (allocated by [enqueueMaterializeDropShadowToScratch]) from
            // the PendingDraw to the DrawResources so it gets cleaned up
            // in [closeDrawResources] after the command buffer that
            // sampled it (the downstream Blur / ColorFilter / LayerComposite
            // draws) has been submitted.
            materializeTargetTexture = d.materializeTargetTexture,
            materializeTargetView = d.materializeTargetView,
        )
    }
    ...
    private fun buildStencilCoverAaDrawResources(d: StencilCoverAaPolygonDraw): DrawResources {
        ...
        val anyDiagnosticEnabled =
            shaderReturnDiagnosticForDraw ||
                contributionIsolationDiagnosticForDraw ||
                diagnosticEnabled ||
        ...
        val predrawDstReadbackEnabled =
            m60F16AaStencilCoverPredrawDstReadbackDiagnosticsEnabled && d.m60F16BandMetadata != null &&
                intermediateFormat == GPUTextureFormat.RGBA16Float
        val predrawDstReadbackStorage = if (predrawDstReadbackEnabled) {
            context.device.createBuffer(
                BufferDescriptor(
                    size = M60_F16_AA_STENCIL_COVER_POST_PASS_READBACK_BUFFER_SIZE,
                    usage = GPUBufferUsage.Storage or GPUBufferUsage.CopySrc or GPUBufferUsage.CopyDst,
                    label = "SkWebGpuDevice.m60F16AaStencilCoverPredrawDstReadback.storage.diagnosticOnly",
                ),
            )
        } else {
            null
        }
        val predrawDstReadbackStaging = if (predrawDstReadbackEnabled) {
            context.device.createBuffer(
                BufferDescriptor(
                    size = M60_F16_AA_STENCIL_COVER_POST_PASS_READBACK_BUFFER_SIZE,
                    usage = GPUBufferUsage.MapRead or GPUBufferUsage.CopyDst,
                    label = "SkWebGpuDevice.m60F16AaStencilCoverPredrawDstReadback.staging.diagnosticOnly",
                ),
            )
        } else {
            null
        }
        val predrawDstReadbackBindGroup = if (predrawDstReadbackEnabled) {
            context.device.createBindGroup(
                BindGroupDescriptor(
                    layout = for258ShaderSideProbeBindGroupLayout,
                    entries = listOf(
                        BindGroupEntry(binding = 0u, resource = intermediateView),
                        BindGroupEntry(binding = 1u, resource = BufferBinding(buffer = predrawDstReadbackStorage!!)),
                    ),
                ),
            )
        } else {
    ...
    override fun close() {
        ...
        m60F16BoundedRuntimeCorrectionPipelineCache.values.forEach { it.close() }
        ...
        if (m60F16BoundedRuntimeCorrectionShaderLazy.isInitialized()) {
            m60F16BoundedRuntimeCorrectionShader.close()
        }
        ...
        if (m60F16AaStencilCoverContributionIsolationDiagnosticShaderLazy.isInitialized()) {
            m60F16AaStencilCoverContributionIsolationDiagnosticShader.close()
        }
        if (m60F16AaStencilCoverShaderReturnDiagnosticShaderLazy.isInitialized()) {
            m60F16AaStencilCoverShaderReturnDiagnosticShader.close()
        }
    ...
    private companion object {
        ...
        const val M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_SHADER: String =
            "diagnostic in-memory FOR-408 per-subdraw contribution-isolation variant of shaders/aa_stencil_cover.wgsl"
        const val M60_F16_AA_STENCIL_COVER_SHADER_RETURN_DIAGNOSTIC_SHADER: String =
        ...
        const val M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_SUBDRAW_COUNT: Int = 2
        const val M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_SAMPLE_COUNT: Int =
            16 * M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_SUBDRAW_COUNT
        const val M60_F16_AA_STENCIL_COVER_CONTRIBUTION_ISOLATION_VEC4S: Int =
            M60_F16_BOUNDED_CORRECTION_APPLICATION_POINT_VEC4S_PER_SAMPLE *
