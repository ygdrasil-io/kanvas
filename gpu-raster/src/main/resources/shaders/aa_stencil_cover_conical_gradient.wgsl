...
// with a conical (two-point) gradient, **kRadial** sub-case (concentric
// circles `c0 == c1`). Mirror of `aa_stencil_cover_radial_gradient.wgsl`
...
// the full AA profile at the path boundary ; only the gradient formula
// changes -- here `t_raw = (length(p - c) - r0) / (r1 - r0)` (mirror of
// `conical_gradient.wgsl` from G4.4 / G4.4.2).
//
...
struct Uniforms {
    ...
    centerRadii:  vec4f,                       // offset   32 : (cx, cy, r0, r1)
...
// kRadial conical : t_raw = (length(p - c) - r0) / (r1 - r0).
fn compute_t_raw(p: vec2f) -> f32 {
    let c = uniforms.centerRadii.xy;
...
// Tile-mode mapping helpers : same formulas as `conical_gradient.wgsl`.
