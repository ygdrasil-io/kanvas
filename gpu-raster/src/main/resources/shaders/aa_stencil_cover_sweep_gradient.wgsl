// G4.3.2 -- AA stencil-and-cover cover-pass for non-rect paths filled
// with a sweep gradient. Mirror of `aa_stencil_cover_gradient.wgsl`
// (linear-on-non-rect) and `aa_stencil_cover_radial_gradient.wgsl`
// (radial-on-non-rect) : same per-fragment minimum-distance edge AA
...
// parameterised by `(center, tBias, tScale)` instead of `(start, end)`
// or `(center, radius)`.
//
...
//
// `compute_t_raw` matches `sweep_gradient.wgsl` byte-for-byte :
//   angle = atan2(p.y - cy, p.x - cx)         // in [-pi, pi]
//   u     = angle / (2 * pi)                   // in [-0.5, 0.5]
//   u_cw  = if (u < 0) u + 1 else u            // in [0, 1) CW from +X
//   t_raw = (u_cw + tBias) * tScale
// where `tBias = -startAngle / 360`, `tScale = 360 / (endAngle - startAngle)`.
// Tile-mode mapping helpers are identical to the linear / radial variants.
// `sample_stops_at` is copied verbatim ; cross-shader helper extraction
// is deferred as in G4.1.2 / G4.2.2.
//
...
struct Uniforms {
    ...
    centerBiasScale: vec4f,                       // offset   32 : (cx, cy, tBias, tScale)
...
// Two-pi literal -- matches `sweep_gradient.wgsl`.
const TWO_PI: f32 = 6.2831853071795864;

// Mirrors `sweep_gradient.wgsl::compute_t_raw`. Singularity at the
// centre collapses to t_raw = tBias * tScale (matches the CPU
// `unitAngle(0, 0) -> 0` convention).
...
// Tile-mode mapping helpers : same formulas as `sweep_gradient.wgsl`.
