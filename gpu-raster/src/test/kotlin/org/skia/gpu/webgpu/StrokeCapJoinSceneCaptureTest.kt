...
class StrokeCapJoinSceneCaptureTest {
    ...
    private fun writeEvidence(
        ...
        if (aaStencilCoverFinalWgslDiagnosticSnapshot.enabled &&
            ...
            if (System.getProperty(FOR429_CPU_SPAN_QUANTIZATION_PROPERTY, "false").toBoolean()) {
                writeM60F16CpuSpanQuantizationFor429(
                    finalWgslSnapshot = aaStencilCoverFinalWgslDiagnosticSnapshot,
                    storageSnapshot = aaStencilCoverShaderReturnStorageZeroCauseSnapshot,
                    adapter = adapter,
                )
    ...
    private fun writeM60F16AaStencilCoverSubsampleMaskFor427(
        finalWgslSnapshot: SkWebGpuDevice.M60F16AaStencilCoverFinalWgslDiagnosticSnapshot,
        storageSnapshot: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnStorageZeroCauseSnapshot,
        adapter: String,
    ) {
        val sceneId = "m60-f16-aa-stencil-cover-subsample-mask-for427"
        val dir = repoFile("reports/wgsl-pipeline/scenes/artifacts/$sceneId").apply { mkdirs() }
        File(dir, "$sceneId.json").writeText(
            m60F16AaStencilCoverSubsampleMaskFor427Json(
                sceneId = sceneId,
                finalWgslSnapshot = finalWgslSnapshot,
                storageSnapshot = storageSnapshot,
                adapter = adapter,
    ...
    private fun m60F16AaStencilCoverSubsampleMaskFor427Json(
        sceneId: String,
        finalWgslSnapshot: SkWebGpuDevice.M60F16AaStencilCoverFinalWgslDiagnosticSnapshot,
        storageSnapshot: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnStorageZeroCauseSnapshot,
        adapter: String,
        ...
        val records = storageByKey.keys
            ...
            .map { key ->
                ...
                val expectedCoverageByte = (coverageMask.getPixel(key.x, key.y) ushr 24) and 0xFF
                val expectedCpuCoveredSubsamples = m60F16ExpectedCpuCoveredSubsamples4x4(expectedCoverageByte)
                val wgslMask = sample?.wgslSubsampleMask4x4
                val classification = m60F16SubsampleMaskClassification(
                    expectedCoverageByte = expectedCoverageByte,
                    cpuMask = null,
                    sample = sample,
                    wgslMask = wgslMask,
                ...
                M60F16SubsampleMaskFor427Record(
                    key = key,
                    event = event,
                    sample = sample,
                    expectedCoverageByte = expectedCoverageByte,
                    expectedCpuCoveredSubsamples4x4 = expectedCpuCoveredSubsamples,
                    cpuMask = null,
                    wgslMask = wgslMask,
                    classification = classification,
                )
            }
        val classificationOrder = M60_F16_FOR427_ALLOWED_CLASSIFICATIONS
        val classificationCounts = records.groupingBy { it.classification }.eachCount()
        val majority = classificationOrder.maxWithOrNull(
            compareBy<String> { classificationCounts[it] ?: 0 }.thenByDescending { classificationOrder.indexOf(it) },
        ) ?: "subsample-mask-stage-incomplete"
        val cpuOnlyTotal = records.mapNotNull { it.cpuOnlySubsamples }.sum().takeIf { records.all { it.cpuOnlySubsamples != null } }
        val wgslOnlyTotal = records.mapNotNull { it.wgslOnlySubsamples }.sum().takeIf { records.all { it.wgslOnlySubsamples != null } }
        val matchingTotal = records.mapNotNull { it.matchingSubsamples }.sum().takeIf { records.all { it.matchingSubsamples != null } }
        val recordsJson = records.joinToString(",\n") { record ->
            m60F16SubsampleMaskFor427RecordJson(record).prependIndent("    ")
        }
        return """
            {
              "schemaVersion": 1,
              "linear": "FOR-427",
              "sceneId": ${sceneId.jsonString()},
              "sourceSceneId": "m60-f16-aa-stencil-cover-coverage-input-stage-for426",
              "sourceDraftMemory": "global/kanvas/tickets/drafts/brouillon-ticket-for-427-m60-f16-comparer-les-sous-echantillons-cpu-et-wgsl-sample-covered",
              "sourceFindingMemory": "global/kanvas/findings/for-426-raw-path-coverage-already-96-before-clip",
              "sourceArtifacts": {
            ...
            ${classificationOrder.joinToString(",\n") { it.jsonString().prependIndent("    ") }}
              ...
              "comparisonPolicy": {
                ...
                "cpuMaskSource": "not-exported: the checked-in CPU reference reports alpha 160/255, which preserves the expected 10-of-16 count but does not expose the internal scanFillPath subsample identities.",
                "cpuMaskLimitation": "The CPU path does not currently export its internal scanFillPath subsample identity; this artifact refuses cell-by-cell CPU/WGSL comparison with subsample-mask-stage-incomplete instead of using a synthetic representative mask.",
              ...
              "summary": {
                ...
                "cpuCoveredSubsamples4x4Total": ${records.mapNotNull { it.cpuCoveredSubsamples4x4 }.sum()},
                "wgslCoveredSubsamples4x4Total": ${records.sumOf { it.wgslCoveredSubsamples4x4 ?: 0 }},
                "matchingSubsamplesTotal": ${matchingTotal ?: "null"},
                "cpuOnlySubsamplesTotal": ${cpuOnlyTotal ?: "null"},
                "wgslOnlySubsamplesTotal": ${wgslOnlyTotal ?: "null"},
            ...
            $recordsJson
              ...
              "nextStep": "Export the CPU scanFillPath 4x4 subsample identity before comparing cell-by-cell against the captured WGSL sample_covered mask.",
              "validationCommands": [
    ...
    private fun m60F16SubsampleMaskFor427RecordJson(record: M60F16SubsampleMaskFor427Record): String {
        ...
        return """
            {
              ...
              "cpuCoveredSubsamples4x4": ${record.cpuCoveredSubsamples4x4},
              "wgslCoveredSubsamples4x4": ${record.wgslCoveredSubsamples4x4 ?: "null"},
              "matchingSubsamples": ${record.matchingSubsamples},
              "cpuOnlySubsamples": ${record.cpuOnlySubsamples},
              "wgslOnlySubsamples": ${record.wgslOnlySubsamples},
              "cpuSubsampleMask4x4": ${record.cpuMask ?: "null"},
              "wgslSubsampleMask4x4": ${record.wgslMask ?: "null"},
              "cpuGrid4x4": null,
              "wgslGrid4x4": ${m60F16WgslSubsampleMaskGridJson(record.key, record.wgslMask).prependIndent("  ").trimStart()}
            }
        """.trimIndent()
    }

    private fun m60F16WgslSubsampleMaskGridJson(
    ...
    private fun m60F16ExpectedCpuCoveredSubsamples4x4(expectedCoverageByte: Int): Int? =
        if (expectedCoverageByte == 160) 10 else null

    private fun m60F16SubsampleMaskClassification(
        expectedCoverageByte: Int,
        cpuMask: Int?,
        sample: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnDiagnosticSample?,
        wgslMask: Int?,
        ...
        if (!diagnosticReturnPathVerified || !maskInstrumentationPresent || sample == null || wgslMask == null || cpuMask == null) {
            return "subsample-mask-stage-incomplete"
        }
        if (!sample.shaderObserved || sample.captureSynthetic || sample.subdrawRole != "inside") {
            return "subsample-mask-stage-incomplete"
        }
        if (expectedCoverageByte != 160 || cpuMask.countOneBits() != 10 || sample.coveredSubsamples4x4 != 6) {
            return "subsample-mask-stage-incomplete"
        }
        val cpuOnly = (cpuMask and wgslMask.inv()).countOneBits()
        val wgslOnly = (wgslMask and cpuMask.inv()).countOneBits()
        return when {
            cpuOnly > 0 && wgslOnly == 0 -> "wgsl-misses-cpu-covered-subsamples"
            wgslOnly > 0 && cpuOnly == 0 -> "wgsl-adds-extra-subsamples"
            cpuOnly == wgslOnly && cpuOnly > 0 -> "sample-grid-coordinate-shift"
            cpuOnly > wgslOnly -> "edge-evaluation-disagreement"
            else -> "winding-rule-disagreement"
        }
    }
    ...
    private fun m60F16SubsampleMaskReason(classification: String): String = when (classification) {
        "wgsl-misses-cpu-covered-subsamples" ->
            "The WGSL sample_covered mask covers fewer subsamples than the CPU 10/16 reference mask at the same pixel-local grid coordinates."
        "wgsl-adds-extra-subsamples" ->
            "The WGSL sample_covered mask covers subsamples that are not covered by the CPU 10/16 reference mask."
        "sample-grid-coordinate-shift" ->
            "CPU-only and WGSL-only subsamples are balanced, which is most consistent with a sample-grid coordinate shift."
    ...
    private fun writeM60F16CpuSpanQuantizationFor429(
        finalWgslSnapshot: SkWebGpuDevice.M60F16AaStencilCoverFinalWgslDiagnosticSnapshot,
        storageSnapshot: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnStorageZeroCauseSnapshot,
        adapter: String,
    ) {
        val sceneId = "m60-f16-cpu-span-quantization-for429"
        val dir = repoFile("reports/wgsl-pipeline/scenes/artifacts/$sceneId").apply { mkdirs() }
        File(dir, "$sceneId.json").writeText(
            m60F16CpuSpanQuantizationFor429Json(
                sceneId = sceneId,
                finalWgslSnapshot = finalWgslSnapshot,
                storageSnapshot = storageSnapshot,
                adapter = adapter,
    ...
    private fun m60F16CpuSpanQuantizationFor429Json(
        sceneId: String,
        finalWgslSnapshot: SkWebGpuDevice.M60F16AaStencilCoverFinalWgslDiagnosticSnapshot,
        storageSnapshot: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnStorageZeroCauseSnapshot,
        adapter: String,
        ...
        val classificationOrder = M60_F16_FOR429_ALLOWED_CLASSIFICATIONS
        val classificationCounts = records.groupingBy { it.classification }.eachCount()
        val majority = classificationOrder.maxWithOrNull(
            compareBy<String> { classificationCounts[it] ?: 0 }.thenByDescending { classificationOrder.indexOf(it) },
        ) ?: "cpu-span-quantization-trace-unavailable"
        val recordsJson = records.joinToString(",\n") { record ->
            m60F16CpuSpanQuantizationFor429RecordJson(record).prependIndent("    ")
        }
        return """
            {
              "schemaVersion": 1,
              "linear": "FOR-429",
              "sceneId": ${sceneId.jsonString()},
              "sourceSceneId": "m60-f16-cpu-scanfill-subsample-mask-for428",
              "sourceDraftMemory": "global/kanvas/tickets/drafts/brouillon-ticket-for-429-m60-f16-diagnostiquer-la-quantification-cpu-add-span-coverage",
              "sourceFindingMemory": "global/kanvas/findings/for-428-cpu-scan-fill-path-span-count-exceeds-center-mask",
              "previousFindingMemory": "global/kanvas/findings/for-427-subsample-mask-stage-incomplete-because-cpu-scan-fill-path-identity-is-not-exported",
              "sourceArtifacts": {
            ...
            ${classificationOrder.joinToString(",\n") { it.jsonString().prependIndent("    ") }}
              ...
              "scoringChanged": false,
              "comparisonPolicy": {
                "scope": "Exactly the 6 FOR-426/FOR-427/FOR-428 partial pixels whose CPU scanFillPath span count is 10/16 while CPU center and WGSL masks are 0x0137.",
                "cpuSpanSource": "real scanFillPath execution: SkScanFillPathSubsampleTrace records selected pixels inside SkBitmapDevice.scanFillPath.addSpanCoverage while BoundedStrokeCapJoinCoverageMaskGM renders.",
                "spanQuantizationRule": "For each pixel intersection on a subrow, addSpanCoverage computes roundedSamples = (intersectionWidth * supers + 0.5f).toInt().coerceIn(0, supers).",
                "centerComparisonRule": "The center count marks the 4 sample centers whose x coordinate lies inside [intersectionLeft, intersectionRight).",
                "wgslMaskSource": "m60_f16_subsample_mask_4x4 captured from sample_covered in the shader-return-storage-zero-cause diagnostic.",
                "noSyntheticCpuMask": true,
                "noRenderingFixApplied": true
              },
              "summary": {
                ...
                "expectedPartialPixelCount": 6,
                "expectedCoverage160Count": ${records.count { it.expectedCoverageByte == 160 }},
                "cpuScanFillPathSamplesTotal": ${records.sumOf { it.cpuScanFillPathSamples ?: 0 }},
                "spanQuantizedCoveredTotal": ${records.sumOf { it.spanQuantizedCoveredCount ?: 0 }},
                "cpuCenterCoveredTotal": ${records.sumOf { it.cpuCenterCoveredCount ?: 0 }},
                "wgslCoveredTotal": ${records.sumOf { it.wgslCoveredCount ?: 0 }},
                "roundedMinusCenterTotal": ${records.sumOf { it.roundedMinusCenterTotal ?: 0 }},
                "majorityClassification": ${majority.jsonString()},
                "majorityClassificationCount": ${classificationCounts[majority] ?: 0},
                "classificationCounts": ${m60F16ClassificationCountsJson(classificationOrder, classificationCounts).prependIndent("  ").trimStart()}
              },
            ...
            $recordsJson
              ...
              "classificationReason": ${m60F16CpuSpanQuantizationFor429Reason(majority).jsonString()},
              "nextStep": "Decide whether GPU M60 F16 should align with CPU span/area-width quantization or keep center-sample semantics with an explicit CPU-reference exception.",
              "validationCommands": [
    ...
    private fun m60F16CpuSpanQuantizationFor429Classification(
        expectedCoverageByte: Int,
        cpuTrace: SkScanFillPathSubsampleTrace.PixelMask?,
        sample: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnDiagnosticSample?,
        wgslMask: Int?,
        ...
        if (cpuTrace == null || cpuTrace.spanQuantizationRows.isEmpty()) {
            return "cpu-span-quantization-trace-unavailable"
        }
        if (!diagnosticReturnPathVerified || !maskInstrumentationPresent || sample == null || wgslMask == null) {
            return "subsample-mask-stage-incomplete"
        }
        ...
        val cpuCenterCount = cpuTrace.mask4x4.countOneBits()
        val wgslCount = wgslMask.countOneBits()
        val spanRounded = cpuTrace.spanQuantizationRows.sumOf { it.roundedSamples }
        val centerFromRows = cpuTrace.spanQuantizationRows.sumOf { it.centerCoveredCount }
        val rowDelta = cpuTrace.spanQuantizationRows.sumOf { it.roundedMinusCenter }
        val explainedByRows = spanRounded == cpuTrace.scanFillPathSamples &&
            centerFromRows == cpuCenterCount &&
            spanRounded - centerFromRows == rowDelta
        if (expectedCoverageByte == 160 &&
            cpuTrace.scanFillPathSamples == 10 &&
            spanRounded == 10 &&
            cpuTrace.mask4x4 == 0x0137 &&
            cpuCenterCount == 6 &&
            wgslMask == 0x0137 &&
            wgslCount == 6 &&
            sample.coveredSubsamples4x4 == 6 &&
            ...
            return "scanfill-rounded-width-exceeds-center-samples"
        }
        return "span-quantization-not-explained"
    }

    private fun m60F16CpuSpanQuantizationFor429Reason(classification: String): String = when (classification) {
        "scanfill-rounded-width-exceeds-center-samples" ->
            "The CPU span path reaches 10/16 because addSpanCoverage rounds fractional intersection widths per subrow, while the CPU center mask and WGSL sample_covered mask both remain 0x0137 at 6/16."
        "cpu-span-quantization-trace-unavailable" ->
            "The CPU span quantization trace was not captured for this pixel."
        "subsample-mask-stage-incomplete" ->
            "The CPU or WGSL diagnostic path was not fully captured."
        else ->
            "The captured span rows do not yet explain the CPU span count versus center-mask count."
    }

    private fun m60F16CpuSpanQuantizationFor429RecordJson(
        ...
        return """
            {
              ...
              "fillType": ${record.event?.fillType?.jsonString() ?: "null"},
              "classification": ${record.classification.jsonString()},
              "classificationReason": ${m60F16CpuSpanQuantizationFor429Reason(record.classification).jsonString()},
              "expectedCoverageByte": ${record.expectedCoverageByte},
              ...
              "roundedMinusCenterTotal": ${record.roundedMinusCenterTotal ?: "null"},
              "cpuTraceSpanCount": ${record.cpuTraceSpanCount ?: "null"},
              "cpuTraceSource": ${record.cpuTrace?.source?.jsonString() ?: "null"},
    ...
    private fun m60F16SpanQuantizationRowsFor429Json(
        key: M60F16DrawPixelKey,
        rows: List<SkScanFillPathSubsampleTrace.SpanQuantizationRow>,
    ): String {
        val rowsJson = rows.sortedWith(compareBy<SkScanFillPathSubsampleTrace.SpanQuantizationRow> { it.subRow }.thenBy { it.intersectionLeft })
            .joinToString(",\n") { row ->
                """
                    {
                      "subRow": ${row.subRow},
                      "spanLeft": ${m60F16JsonFloat(row.spanLeft)},
                      "spanRight": ${m60F16JsonFloat(row.spanRight)},
                      "cellLeft": ${m60F16JsonFloat(row.cellLeft)},
                      "cellRight": ${m60F16JsonFloat(row.cellRight)},
                      "intersectionLeft": ${m60F16JsonFloat(row.intersectionLeft)},
                      "intersectionRight": ${m60F16JsonFloat(row.intersectionRight)},
                      "intersectionWidth": ${m60F16JsonFloat(row.intersectionWidth)},
                      "widthTimesSupers": ${m60F16JsonFloat(row.widthTimesSupers)},
                      "roundedSamples": ${row.roundedSamples},
                      "centerCoveredCount": ${row.centerCoveredCount},
                      "roundedMinusCenter": ${row.roundedMinusCenter},
                      "sampleCenters": ${m60F16SpanSampleCentersFor429Json(key, row).prependIndent("  ").trimStart()}
                    }
                """.trimIndent()
            }
        return """
            [
        ${rowsJson.prependIndent("    ")}
            ]
        """.trimIndent()
    }

    private fun m60F16SpanSampleCentersFor429Json(
        key: M60F16DrawPixelKey,
        row: SkScanFillPathSubsampleTrace.SpanQuantizationRow,
    ): String {
        val centers = row.sampleCenters.joinToString(",\n") { center ->
            """
                {
                  "sx": ${center.sx},
                  "localX": ${m60F16JsonFloat(center.deviceX - key.x.toFloat())},
                  "deviceX": ${m60F16JsonFloat(center.deviceX)},
                  "deviceY": ${m60F16JsonFloat(key.y.toFloat() + (row.subRow + 0.5f) * 0.25f)},
                  "covered": ${center.covered}
                }
            """.trimIndent()
        }
        return """
            [
        ${centers.prependIndent("    ")}
            ]
        """.trimIndent()
    }
    ...
    private fun writeM60F16StencilSubdrawSourceColorFor433(
        reference: SkBitmap,
        currentGpu: SkBitmap,
        optInGpu: SkBitmap,
        shaderReturnSnapshot: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnDiagnosticSnapshot,
        ...
        val sceneId = "m60-f16-stencil-subdraw-source-color-for433"
        val dir = repoFile("reports/wgsl-pipeline/scenes/artifacts/$sceneId").apply { mkdirs() }
        File(dir, "$sceneId.json").writeText(
            m60F16StencilSubdrawSourceColorFor433Json(
                sceneId = sceneId,
                reference = reference,
                currentGpu = currentGpu,
                optInGpu = optInGpu,
    ...
    private fun m60F16StencilSubdrawSourceColorFor433Json(
        sceneId: String,
        reference: SkBitmap,
        currentGpu: SkBitmap,
        optInGpu: SkBitmap,
        ...
        val records = partialPoints
            ...
            .map { key ->
                ...
                val capturedSource = selected?.second?.sourceColorSentToBlend
                val capturedAlpha = selected?.second?.coverageOrAaAlpha
                val referenceFloat = rgbaByteArrayToFloat(rgbaArray(reference.getPixel(key.x, key.y)))
                val dstBefore = predraw?.second?.dstBeforeRgbaFloat
                val requiredSource = dstBefore?.let {
                    inverseSourceOverPremul(referenceFloat, it, coverageAlpha)
                }
                val reconstructedCpu = requiredSource?.let { source ->
                    sourceOverPremul(source, requireNotNull(dstBefore))
                }
                val sourceDelta = if (capturedSource != null && requiredSource != null) {
                    rgbaDelta(capturedSource, requiredSource, 1f / 255f)
                } else {
                    null
                }
                val reconstructedDelta = reconstructedCpu?.let {
                    rgbaDelta(it, referenceFloat, 1f / 255f)
                }
                val requiredSourcePremulConsistent = requiredSource?.let { source ->
                    source[3] in -0.000001f..1.000001f &&
                        source.sliceArray(0..2).all { channel -> channel >= -0.000001f && channel <= source[3] + 0.000001f }
                }
                val missingFields = buildList {
                    if (targetDrawSources.isEmpty()) add("for432WidthQuantizedShaderReturnSubdraw")
                    if (selected == null) add("selectedSingleStencilCandidate")
                    if (capturedSource == null) add("sourceColorSentToBlend")
                    if (capturedAlpha == null) add("coverageOrAaAlpha")
                    if (dstBefore == null) add("dstBeforeRgbaFloat")
                    if (requiredSource == null) add("requiredPremulSourceByInverseSrcOver")
                    if (reconstructedCpu == null) add("reconstructedCpuReferenceByRequiredSource")
                }
                val classification = when {
                    missingFields.isNotEmpty() -> "trace-incomplete"
                    requiredSourcePremulConsistent != true ||
                        reconstructedDelta?.withinTolerance != true -> "cpu-reference-inversion-inconsistent"
                    capturedAlpha == null ||
                        kotlin.math.abs(capturedAlpha - coverageAlpha) > (1f / 255f) -> "coverage-alpha-mismatch"
                    targetDrawSources.size > 1 && !alternateSingleCandidatesShareSource -> "stencil-side-ambiguous"
                    sourceDelta?.withinTolerance != true -> "source-payload-mismatch"
                    else -> "source-payload-mismatch"
                }
                M60F16StencilSubdrawSourceColorRecord(
                    key = key,
                    effectiveRenderDrawIndex = effectiveRenderDrawIndex,
                    ...
                    reconstructedCpuReference = reconstructedCpu,
                    sourceDelta = sourceDelta,
                    reconstructedDelta = reconstructedDelta,
                    requiredSourcePremulConsistent = requiredSourcePremulConsistent,
        ...
        val globalClassification = when {
            records.any { it.classification == "trace-incomplete" } -> "trace-incomplete"
            records.any { it.classification == "cpu-reference-inversion-inconsistent" } ->
                "cpu-reference-inversion-inconsistent"
            records.any { it.classification == "coverage-alpha-mismatch" } -> "coverage-alpha-mismatch"
            records.any { it.classification == "stencil-side-ambiguous" } -> "stencil-side-ambiguous"
            else -> "source-payload-mismatch"
        }
        val maxSourceDelta = records.mapNotNull { it.sourceDelta?.maxChannel }.maxOrNull() ?: 0f
        val maxCapturedAlphaDelta = records.mapNotNull { record ->
            record.selectedSource?.second?.coverageOrAaAlpha?.let { kotlin.math.abs(it - coverageAlpha) }
        }.maxOrNull() ?: 0f
        val recordsJson = records.joinToString(",\n") { record ->
            m60F16StencilSubdrawSourceColorFor433RecordJson(record).prependIndent("    ")
        }
        return """
            {
              "schemaVersion": 1,
              "linear": "FOR-433",
              "sceneId": ${sceneId.jsonString()},
              "sourceSceneId": "m60-f16-width-quantized-color-reconstruction-for432",
              "sourceDraftMemory": "global/kanvas/tickets/drafts/brouillon-ticket-m60-f16-isoler-source-couleur-du-sous-passage-stencil-regresse",
              "sourceFindingMemory": "global/kanvas/findings/for-432-web-gpu-width-quantized-color-reconstruction-matches-single-stencil-gated-subdraw",
              "sourceArtifact": "reports/wgsl-pipeline/scenes/artifacts/m60-f16-width-quantized-color-reconstruction-for432/m60-f16-width-quantized-color-reconstruction-for432.json",
              "adapter": ${adapter.jsonString()},
              "producer": "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/StrokeCapJoinSceneCaptureTest.kt",
              "runtimeOwner": "gpu-raster/src/main/kotlin/org/skia/gpu/webgpu/SkWebGpuDevice.kt",
              "classification": ${globalClassification.jsonString()},
              "allowedClassifications": [
            ${M60_F16_FOR433_ALLOWED_CLASSIFICATIONS.joinToString(",\n") { it.jsonString().prependIndent("    ") }}
              ],
              "diagnosticFlag": ${FOR433_STENCIL_SUBDRAW_SOURCE_COLOR_PROPERTY.jsonString()},
              ...
              "renderingFixApplied": false,
              "comparisonPolicy": {
                "scope": "Exactly the six FOR-432 width-quantized regression pixels.",
                "source": "FOR-432 selected single stencil-gated sourceColorSentToBlend.",
                "inverseMath": "premultiplied SrcOver inversion: requiredSource.rgb = cpuReference.rgb - dstBefore.rgb * (1 - 10/16), requiredSource.a = 10/16.",
              ...
              "summary": {
                "partialPixelCount": ${records.size},
                "expectedPartialPixelCount": 6,
                "sourcePayloadMismatchCount": ${classificationCounts["source-payload-mismatch"] ?: 0},
                "coverageAlphaMismatchCount": ${classificationCounts["coverage-alpha-mismatch"] ?: 0},
                "stencilSideAmbiguousCount": ${classificationCounts["stencil-side-ambiguous"] ?: 0},
                "cpuReferenceInversionInconsistentCount": ${classificationCounts["cpu-reference-inversion-inconsistent"] ?: 0},
                "traceIncompletePixelCount": ${classificationCounts["trace-incomplete"] ?: 0},
                "maxSourceVsRequiredChannelDelta": ${String.format(Locale.US, "%.9f", maxSourceDelta)},
                "maxCapturedCoverageAlphaDelta": ${String.format(Locale.US, "%.9f", maxCapturedAlphaDelta)}
              },
              "partialPixels": [
            $recordsJson
              ],
              ...
              "classificationReason": ${m60F16StencilSubdrawSourceColorFor433GlobalReason(globalClassification).jsonString()},
              "nextStep": ${m60F16StencilSubdrawSourceColorFor433NextStep(globalClassification).jsonString()},
              "validationCommands": [
    ...
    private fun m60F16StencilSubdrawSourceColorFor433RecordJson(
        record: M60F16StencilSubdrawSourceColorRecord,
    ): String {
        val selected = record.selectedSource?.second
        val targetCandidatesJson = record.targetDrawSources.joinToString(",\n") { (event, sample) ->
            """
                {
                  "drawIndex": ${event.drawIndex},
                  "subdrawOrdinal": ${sample.subdrawOrdinal},
        ...
        return """
            {
              ...
              "effectiveRenderDrawIndex": ${record.effectiveRenderDrawIndex},
              "classification": ${record.classification.jsonString()},
              "classificationReason": ${m60F16StencilSubdrawSourceColorFor433LocalReason(record).jsonString()},
              "referenceCpuRgba": ${rgbaArrayJson(record.referenceRgba)},
              "currentWebGpuRgba": ${rgbaArrayJson(record.currentGpuRgba)},
              ...
              "coverage": {
                "numerator": 10,
                "denominator": 16,
                "alpha": ${m60F16JsonFloat(10f / 16f)},
                "capturedCoverageOrAaAlpha": ${selected?.coverageOrAaAlpha?.let { String.format(Locale.US, "%.9f", it) } ?: "null"},
                "capturedCoverageAlphaDelta": ${selected?.coverageOrAaAlpha?.let { String.format(Locale.US, "%.9f", kotlin.math.abs(it - 10f / 16f)) } ?: "null"}
              },
              "destination": {
                "dstBeforeRgbaFloat": ${record.predraw?.second?.dstBeforeRgbaFloat.floatArrayOrNullJson()},
                "dstBeforeRgba8": ${record.predraw?.second?.dstBeforeRgba8.intArrayOrNullJson()}
              ...
              "source": {
                "capturedWebGpuPremulRgbaFloat": ${selected?.sourceColorSentToBlend.floatArrayOrNullJson()},
                "capturedWebGpuPremulRgba8Approx": ${selected?.sourceColorSentToBlend?.let { floatRgbaToByteArrayJson(it) } ?: "null"},
                "sourceColorBeforeQuantization": ${selected?.sourceColorBeforeQuantization.floatArrayOrNullJson()}
              },
              "requiredSourcePremul": {
                ...
                "premulConsistent": ${record.requiredSourcePremulConsistent ?: "null"},
                "reconstructedCpuReferenceRgbaFloat": ${record.reconstructedCpuReference.floatArrayOrNullJson()},
                "reconstructedVsCpuReferenceDelta": ${record.reconstructedDelta?.let { rgbaDeltaJson(it) } ?: "null"}
              },
              "webGpuSourceVsRequiredDelta": ${record.sourceDelta?.let { rgbaDeltaJson(it) } ?: "null"},
              "candidateSubdraws": [
            $targetCandidatesJson
              ],
              "missingFields": [$missingFieldsJson]
            }
        """.trimIndent()
    ...
    private fun inverseSourceOverPremul(out: FloatArray, dst: FloatArray, sourceAlpha: Float): FloatArray =
        FloatArray(4) { index ->
            if (index == 3) {
                sourceAlpha
            } else {
                out[index] - dst[index] * (1f - sourceAlpha)
            }
        }

    private fun floatRgbaToByteArrayJson(rgba: FloatArray): String =
        rgba.joinToString(prefix = "[", postfix = "]") { value ->
            ((value.coerceIn(0f, 1f) * 255f) + 0.5f).toInt().toString()
        }

    private fun m60F16StencilSubdrawSourceColorFor433LocalReason(
        record: M60F16StencilSubdrawSourceColorRecord,
    ): String = when (record.classification) {
        "source-payload-mismatch" ->
            "The selected FOR-432 stencil-gated source differs from the premultiplied source required to reach the CPU reference at 10/16 coverage."
        "coverage-alpha-mismatch" ->
            "The selected FOR-432 source does not carry the expected 10/16 coverage alpha."
        "stencil-side-ambiguous" ->
            "Multiple single-stencil candidates carry distinct source payloads, so the selected stencil side is ambiguous."
        "cpu-reference-inversion-inconsistent" ->
            "The inverse SrcOver source is outside premultiplied bounds or cannot reconstruct the CPU reference."
        else ->
            "The source-color comparison is incomplete; missing fields: ${record.missingFields.joinToString()}."
    }

    private fun m60F16StencilSubdrawSourceColorFor433GlobalReason(classification: String): String =
        when (classification) {
            "source-payload-mismatch" ->
                "All six pixels have matching 10/16 coverage and complete FOR-432 trace data, but the WebGPU source payload is not the premultiplied source required by the CPU reference."
            "coverage-alpha-mismatch" ->
                "At least one selected source does not preserve the expected 10/16 coverage alpha."
            "stencil-side-ambiguous" ->
                "The trace cannot isolate one stencil side because candidate subdraws disagree on the source payload."
            "cpu-reference-inversion-inconsistent" ->
                "The CPU reference cannot be reconstructed by a valid premultiplied source under the captured destination and 10/16 coverage."
            else ->
                "At least one selected pixel is missing a FOR-432 source, destination, or inversion field."
        }

    private fun m60F16StencilSubdrawSourceColorFor433NextStep(classification: String): String =
        when (classification) {
            "source-payload-mismatch" ->
                "Open the next correction ticket on the paint/source payload feeding the M60 F16 AA stencil-cover width-quantized subpass, without changing coverage thresholds or promoting FOR-431."
            "coverage-alpha-mismatch" ->
                "Inspect the coverage-to-alpha transfer before adjusting source color payload."
            "stencil-side-ambiguous" ->
                "Add a narrower stencil-side isolation trace before changing source or coverage logic."
            "cpu-reference-inversion-inconsistent" ->
                "Recheck the CPU reference and destination capture before deriving a rendering fix."
            else ->
    ...
    private data class M60F16SubsampleMaskFor427Record(
        val key: M60F16DrawPixelKey,
        ...
        val expectedCoverageByte: Int,
        val expectedCpuCoveredSubsamples4x4: Int?,
        val cpuMask: Int?,
        val wgslMask: Int?,
        val classification: String,
    ) {
    ...
    private data class M60F16CpuSpanQuantizationFor429Record(
        val key: M60F16DrawPixelKey,
        val event: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnDiagnosticEvent?,
        val sample: SkWebGpuDevice.M60F16AaStencilCoverShaderReturnDiagnosticSample?,
        val expectedCoverageByte: Int,
        ...
        val cpuCenterMask4x4: Int? = cpuTrace?.mask4x4
        val wgslMask4x4: Int? = wgslMask
        val cpuScanFillPathSamples: Int? = cpuTrace?.scanFillPathSamples
        val cpuTraceSpanCount: Int? = cpuTrace?.tracedSpanCount
        val cpuCenterCoveredCount: Int? = cpuCenterMask4x4?.countOneBits()
        val wgslCoveredCount: Int? = wgslMask4x4?.countOneBits()
    ...
    private data class M60F16StencilSubdrawSourceColorRecord(
        val key: M60F16DrawPixelKey,
        val effectiveRenderDrawIndex: Int,
        val selectedSource: Pair<
            SkWebGpuDevice.M60F16AaStencilCoverShaderReturnDiagnosticEvent,
        ...
        val reconstructedCpuReference: FloatArray?,
    ...
    private companion object {
        ...
        private val M60_F16_FOR429_ALLOWED_CLASSIFICATIONS = listOf(
            "scanfill-rounded-width-exceeds-center-samples",
        ...
        private val M60_F16_FOR433_ALLOWED_CLASSIFICATIONS = listOf(
            "source-payload-mismatch",
            "coverage-alpha-mismatch",
            "stencil-side-ambiguous",
            "cpu-reference-inversion-inconsistent",
