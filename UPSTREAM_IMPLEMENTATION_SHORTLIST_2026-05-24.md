...
|---:|---|---:|---|---|
| 1 | `STUB.SURFACE_SNAPSHOT_SUBSET` | 1 cpp (`surface`) | S | Narrow API, likely pure raster copy/subset, easy to test. |
| 2 | `STUB.IMAGE_MAKE_SCALED` | 1 cpp (`crbug_404394639`, currently `missing-mapping`) | S/M | Isolated image helper; can reuse existing sampling paths. |
| 3 | `STUB.SRC_RECT_CONSTRAINT` | 1 cpp (`bleed`) | S/M | Adds an overload/constraint path around existing `drawImageRect`; bounded blast radius. |
| 4 | `STUB.MAKE_WITH_COLOR_FILTER` | 1 cpp (`colorfilterimagefilter`) | M | Useful shader composition primitive; test surface is small. |
| 5 | `STUB.COLOR4F_BLEND_CF` | 1 cpp (`color4f`) | M | Pure color-filter behavior; likely contained in foundation/effects. |
| 6 | `STUB.POLY_TO_POLY` / `STUB.PERSPECTIVE_ADDPATH` | 2 cpps (`complexclip`, `patharcto`) | M/L | Core matrix/path correctness; useful but more geometry risk. |
| 7 | `STUB.PATH_EFFECT_CTM` | 1 cpp (`patheffects`) | M/L | Needs CTM-aware path-effect behavior; moderate correctness risk. |
| 8 | `STUB.GAUSSIAN_COLOR_FILTER` | 1 cpp (`shadowutils`) | M/L | Private color-filter behavior; useful but less central. |
| 9 | `STUB.IFX.MULTIPLE_FILTERS_SPAN` | 1 cpp (`imagefilters`) | L | Image-filter/layer integration can touch saveLayer behavior. |
| 10 | `STUB.EDGE_AA_IMAGE_SET` / `STUB.EDGE_AA_QUAD` | 3+ cpps (`drawimageset`, `savelayer`, `drawquadset`) | L | Higher impact, but batched image draw semantics are broad. |
| 11 | `STUB.DRAW_VERTICES` | 1 cpp (`vertices`) | L | Shared drawing primitive; likely useful, but broad rendering surface. |
| 12 | `STUB.RSXBLOB` / `STUB.DF_TEXT_RASTER` | 3 cpps (`drawatlas`, `dftext_blob_persp`, `textblobmixedsizes`) | L/XL | Text/glyph transform work; defer if font delivery may change internals. |
