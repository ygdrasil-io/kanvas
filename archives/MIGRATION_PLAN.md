...
| D2.6 | ~226     | **[D2.6] — DM pipeline missing-hash logging — 🎉 D2 CLOSE**. `Report.missingRuntimeEffectHashes(): Set<String>` parse les error messages des records failed et extrait les hashes `0x<16-hex>` non enregistrés via regex. `DmCli.listMissingEffects: Boolean` + parser support pour `--list-missing-effects` boolean flag. `DmMain.runFromArgs` print les hashes à `System.err` après le run quand le flag est set. Devs utilisent la liste pour prioriser le retro-port (chaque hash = un effet à hand-porter). 3 unit tests pinning end-to-end : (1) GM avec SkSL non enregistré → graceful failure dans Report.failed ; (2) hash extrait via `missingRuntimeEffectHashes()` ; (3) CLI flag se propage. Suite à 3214 verts. **🎉 D2 (SkRuntimeEffect) entièrement shipped — D2.0 / 2.1 / 2.2 / 2.3 / 2.4.a / 2.4.b / 2.4.c (6 sub-slices) / 2.4.d / 2.5 / 2.6 ✅ ; ~80 DEF_GMs upstream débloquées via la dispatch table SkSL → impl Kotlin.** | ✅ |
