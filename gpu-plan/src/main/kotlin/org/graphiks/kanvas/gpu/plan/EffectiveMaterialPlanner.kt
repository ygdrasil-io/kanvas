...
public object EffectiveMaterialPlanner {
    ...
    private fun normalizeW5d(draw: DrawNode, blend: BlendPlan, capture: GradientAddressingCaptureV2.Ready,
        ...
        val stops = when (val normalized = normalizeGradientStopsV2(inputStops, tileGraph.effectiveMode, preserveValidityMask = true)) {
        ...
        val binding: MaterialBindingPlan.GradientV2 = when {
            linear != null -> {
                ...
                val numeric = GradientNumericAuthorityV2.sealLinear(program, coordinates, linear.start, linear.end,
                    degeneracy, stops, magnitudeF64, magnitude(listOf(linear.start.x, linear.start.y, linear.end.x, linear.end.y)))
                    ?: return Normalization.Refused(W5cPlanDiagnostics.NumericDomainUnbounded)
                MaterialBindingPlan.LinearGradientV2(linear.start, linear.end, range, degeneracy, numeric)
            }
            radial != null -> {
                val degeneracy = RadialGradientDegeneracyV1(radial.radius, radial.radius <= 0.000030517578125f)
                val numeric = GradientNumericAuthorityV2.sealRadial(program, coordinates, radial.center, radial.radius,
                    degeneracy, stops, magnitudeF64, magnitude(listOf(radial.center.x, radial.center.y, radial.radius)))
                    ?: return Normalization.Refused(W5cPlanDiagnostics.NumericDomainUnbounded)
                MaterialBindingPlan.RadialGradientV2(radial.center, radial.radius, range, degeneracy, numeric)
            }
            sweep != null -> {
                val degeneracy = requireNotNull(sweepDegeneracy)
                val numeric = GradientNumericAuthorityV2.sealSweep(program, coordinates, sweep.center, degeneracy,
                    stops, magnitudeF64, magnitude(listOf(sweep.center.x, sweep.center.y, sweep.startAngle,
                        sweep.endAngle, degeneracy.sweepSpanDegreesF32)))
                    ?: return Normalization.Refused(W5cPlanDiagnostics.NumericDomainUnbounded)
                MaterialBindingPlan.SweepGradientV2(sweep.center, range, degeneracy, numeric)
            }
            else -> {
                requireNotNull(conical)
                val degeneracy = ConicalGradientDegeneracyV1.of(conical.start, conical.startRadius, conical.end, conical.endRadius)
                val numeric = GradientNumericAuthorityV2.sealConical(program, coordinates, conical.start, conical.end,
                    degeneracy, stops, magnitudeF64, magnitude(listOf(conical.start.x, conical.start.y,
                        conical.end.x, conical.end.y) + degeneracy.copyScalarsF32()))
                    ?: return Normalization.Refused(W5cPlanDiagnostics.NumericDomainUnbounded)
                MaterialBindingPlan.ConicalGradientV2(conical.start, conical.end, range, degeneracy, numeric)
            }
        }
        val entries = mutableListOf(MaterialPlanEntry(program, binding, stops))
