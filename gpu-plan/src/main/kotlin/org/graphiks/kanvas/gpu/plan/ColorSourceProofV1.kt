...
public class ColorSourceProofV1 private constructor(
    ...
    internal val uniformWordCountI64: Long = if (parentSource == null) bindingOwners.size * 4L else
    ...
    internal companion object {
        ...
        fun compose(source: ColorSourceProofV1, execution: ColorFilterExecutionPlanV1): ColorSourceProofV1? {
            ...
            repeat(20) { words[source.uniformWordCountI64 + it] = execution.coefficientF32(it) }
            val proof = ColorRoundedGraphProofV1.prove(graph, words) ?: return null
            val identity = "color-composed-proof-v1:${source.canonicalIdentity}:${execution.canonicalIdentity}:${graph.canonicalIdentity}:$proof"
            return ColorSourceProofV1(identity, filteredIdentity(source.sourceIdentity,execution), source.coordinates,
                graph, source.bindingOwners, java.util.Collections.unmodifiableMap(words), immutableList(proof),
                source.deviceBoundsF32, source, execution)
        }
        fun issue(table: MaterialPlanTable, root: MaterialPlanRef, coordinates: SourceCoordinatesV4,
            ...
            val identity = "color-source-proof-v1:$source:${coordinates.identityV4()}:" +
                ...
                ":${graph.canonicalIdentity}:${words.entries.joinToString { "${it.key}=${it.value.toRawBits()}" }}:$proof"
...
internal object ColorRoundedGraphProofV1 {
    ...
    fun prove(graph: ColorOperationGraphV1, words: Map<Long, Float>): List<ColorBranchFactV1>? = try {
        ...
        val cache = mutableMapOf<Pair<ColorOperationGraphV1.Scalar, Map<ColorOperationGraphV1.Scalar, ColorBoundsV1>>, ColorBoundsV1>()
            ...
            val result = when (node) {
                ...
                is ColorOperationGraphV1.Scalar.LazyBranch -> {
                    ...
                    fun branch(taken: Boolean): ColorBoundsV1 {
                        ...
                        val restricted = if (taken && equal) conditions + (a to ColorBoundsV1(maxOf(left.lowerF64,right.lowerF64),minOf(left.upperF64,right.upperF64)))
                        else if (!equal) conditions + (a to if (taken) ColorBoundsV1(left.lowerF64,minOf(left.upperF64,right.upperF64))
                            else ColorBoundsV1(maxOf(left.lowerF64,right.lowerF64),left.upperF64)) else conditions
