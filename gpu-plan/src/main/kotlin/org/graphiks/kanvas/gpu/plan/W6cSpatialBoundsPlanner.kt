...
internal object W6cSpatialBoundsPlanner {
    ...
    internal data class OffsetPlan(val bounds: FilterBoundsPlanV1, val offsetF64X: Double, val offsetF64Y: Double)
    ...
    internal fun crop(source: W6bFilterGraphConstruction.SourceBinding, node: CapturedFilterNodeV1.Crop): CropPlan {
        ...
        val produced = source.copyKnownContentDeviceI32()?.let { known ->
            if (node.tileMode == TileMode.DECAL) intersect(known, desired) else desired.copy()
        }
    ...
    internal fun offset(source: W6bFilterGraphConstruction.SourceBinding, node: CapturedFilterNodeV1.Offset): OffsetPlan {
        ...
        val translated = rectF64(input).translateF64OrNull(node.dx.toDouble(), node.dy.toDouble())
            ?: refuse("Offset bounds are non-finite.")
        val desired = seal(translated)
        val produced = source.copyKnownContentDeviceI32()?.let { known ->
            seal(rectF64(known).translateF64OrNull(node.dx.toDouble(), node.dy.toDouble())
                ?: refuse("Offset produced bounds are non-finite."))
        }
        return OffsetPlan(bounds(source, desired, input, produced), node.dx.toDouble(), node.dy.toDouble())
    }
    ...
    internal fun tile(source: W6bFilterGraphConstruction.SourceBinding, node: CapturedFilterNodeV1.Tile): TilePlan {
        ...
        val inputDomain = source.copyDeviceBoundsI32()
        ...
        val required = intersect(inputDomain, seal(sourceDevice)) ?: source.copyDeviceBoundsI32()
        return TilePlan(bounds(source, desired, required, source.copyKnownContentDeviceI32()?.let { desired.copy() }),
    ...
    internal fun requiredInputBounds(
        ...
        is CapturedFilterNodeV1.Offset -> seal(rectF64(desiredOutput).translateF64OrNull(
            -operation.dx.toDouble(), -operation.dy.toDouble()) ?: return null)
    ...
    internal fun producedOutputBounds(
        ...
        is CapturedFilterNodeV1.Offset -> seal(rectF64(known).translateF64OrNull(operation.dx.toDouble(), operation.dy.toDouble()) ?: return null)
        is CapturedFilterNodeV1.Tile -> map(operation.copyDestination(), mapping)?.let(::seal)
        else -> null
    } }
