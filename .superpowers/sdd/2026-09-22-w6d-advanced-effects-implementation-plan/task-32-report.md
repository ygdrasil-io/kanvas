...
Le prototype Surface à coordonnées spot finies `±1.8e38` n’est pas retenu comme témoin pixel : avant le correctif F64, il échouait à l’admission ; après le correctif, il atteignait le shader mais donnait `[0,0,0,255]` au lieu de l’oracle `[222,222,222,255]`. Il confond donc le défaut de normalisation visé avec le comportement des littéraux F32 extrêmes du backend et ne constitue pas une preuve publique stable. Le test math causal reste le témoin fini, isolé et discriminant de cette soustraction F32 overflowée.
