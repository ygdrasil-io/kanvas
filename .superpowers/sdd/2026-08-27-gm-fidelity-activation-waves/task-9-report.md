...
matrice 4×5 finie et le packing natif de 96 octets. L'oracle Kotlin applique
explicitement `encoded sRGB -> linear sRGB -> matrix -> encoded sRGB -> premul`.
Le shader `color-matrix-v1` suit la même séquence et est parser-validé par
`wgsl4k`.
...
multiplie R par 0.5 en linéaire, l'oracle indépendant et le readback GPU sont
`[46,32,96,128]` sur les 16 pixels : 64 canaux, `differentChannels=0`,
`maxDelta=0`, `meanDelta=0.0`, un submit et une copie de readback.
...
- Aucun Ganesh, Graphite, SkSL dynamique ou `gpu-renderer-scenes` n'est utilisé.
