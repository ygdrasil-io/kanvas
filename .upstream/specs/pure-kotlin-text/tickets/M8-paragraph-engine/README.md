...
- `KFONT-M8-004` is in `review` with bounded ellipsis insertion evidence: `TextStackSurfaceTest` plus `paragraph-layout.json` now prove one-line and placeholder-tail ellipsis insertion, `text.paragraph.ellipsis-no-room`, `text.paragraph.ellipsis-glyph-missing`, mixed-style trailing-style provenance, and shaped-cluster-safe truncation, while bidi visual-order preservation remains the remaining explicit gate.
