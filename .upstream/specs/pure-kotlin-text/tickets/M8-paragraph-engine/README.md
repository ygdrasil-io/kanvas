...
| [KFONT-M8-004 - Implement ellipsis and max-lines policy](KFONT-M8-004-implement-ellipsis-and-max-lines-policy.md) | `review` | `P1` | `tracked-gap` | `paragraph` | `KFONT-M8-002`, `KFONT-M8-003` | - |
...
- `KFONT-M8-004` is in `review` with bounded ellipsis insertion evidence: `TextStackSurfaceTest` plus `paragraph-layout.json` now prove one-line and placeholder-tail ellipsis insertion, ellipsis-only fallback when no visible cluster fits but the ellipsis does, retry-on-earlier-visible-style behavior when the current trailing style cannot shape the ellipsis, `text.paragraph.ellipsis-no-room`, `text.paragraph.ellipsis-glyph-missing`, mixed-style trailing-style provenance, and shaped-cluster-safe truncation, while bidi visual-order preservation remains the remaining explicit gate.
...
- `KFONT-M8-004` remains the active ellipsis/max-lines gate for M8 closeout because bidi truncation ordering evidence is still missing.
