...
- The runtime `BasicOpenTypeShapingEngine` now resolves per-run script policy before GSUB, GPOS anchor lookup, and pair-kerning gating, while preserving legacy enable-unless-disabled behavior for scripts that still have no explicit policy row.
...
- `review`: Contract-level feature policy rows, shaping-plan serialization, and bounded runtime GSUB/GPOS-anchor/pair-kerning policy gating are implemented and freshly validated. Remaining gate: land the per-script shaping fixture families owned by `KFONT-M6-007`, `KFONT-M6-008`, and `KFONT-M6-009`, finish the remaining GPOS single feature-routing work, and attach OpenType-specific `drawString` non-enablement evidence before `done`.
