...
- Current blocker audit (2026-06-18): `KFONT-M6-002` is `done`, but the required fixture set `gsub-context-format1.otf`, `gsub-context-format2-class.otf`, `gsub-context-format3-coverage.otf`, `gsub-context-nested-cycle.otf`, and `gsub-context-malformed-classdef.otf` is still not present in-repo. Do not substitute synthetic-only coverage for these fixtures; remaining gate is add reviewed fixture provenance and expected dump coverage for the contextual rows.
