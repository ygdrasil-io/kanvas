...
- `done`: reviewed fixture provenance is now checked in for `gsub-single-substitution.otf`, `gsub-multiple-substitution.otf`, `gsub-ligature-fi.otf`, `gsub-coverage-malformed.otf`, and `gsub-ligature-bad-component.otf`; parser/runtime tests cover both the positive and malformed fixture set; and `gsub-trace.json` / `shaped-glyph-run.json` are now fixture-backed Latin evidence rather than M6-001 contract placeholders. Contextual lookups and script-policy runtime adoption remain separate tickets.
