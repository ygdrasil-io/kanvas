...
- `KFONT-M6-010` is now `blocked` after bounded GSUB extension evidence landed: `ExtensionLookupFixtureTest` plus `extension-lookup-report.json` now prove generated-memory-font parsing and shaping for extension lookups that resolve to single and ligature substitutions without promoting chaining, reverse-chaining, GPOS contextual/chaining/extension, or variation/device adjustment support.
...
- 2026-06-19 audit: `KFONT-M6-007` remains blocked on positive `lam-alef`, vendored positive cursive, and Arabic-specific refusal fixtures/codes; `KFONT-M6-008` remains blocked on ticket-local syllable-plan or phase evidence, the full required `deva` / `dev2` feature set, dedicated unsupported-syllable and phase refusal fixtures/codes, and ticket-local `gsub-trace.json` / `gpos-trace.json` / `shaped-glyph-run.json` / `unicode-segments.json` dump families; `KFONT-M6-009` remains gated by its own named fixture families and paragraph-owned Thai/CJK rows; `KFONT-M6-010` remains blocked on GSUB extension targets beyond single/ligature, GSUB chaining/reverse chaining, GPOS contextual/chaining/extension positioning, device/variation adjustments, and ticket-local `gsub-trace.json` / `gpos-trace.json` / `variation-adjustment-trace.json` evidence.
