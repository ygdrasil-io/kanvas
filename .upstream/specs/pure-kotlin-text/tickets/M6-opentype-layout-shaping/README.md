...
| [KFONT-M6-007 - Add Arabic shaping fixtures](KFONT-M6-007-add-arabic-shaping-fixtures.md) | `review` | `P0` | `fixture-gated` | `shaping` | `KFONT-M5-003`, `KFONT-M6-003`, `KFONT-M6-005`, `KFONT-M6-006` | - |
...
- `KFONT-M6-007` is now in `review` on bounded vendored-font evidence only: `ArabicShapingFixtureTest`, `arabic-shaped-glyph-run.json`, `arabic-shaping-plan.json`, and `arabic-shaping-report.json` prove joining-form behavior beyond pure RTL reordering, pin the ticket-local Arabic glyph/cluster facts for joining and marks, record the required Arabic default feature set and refusal-on-missing expectations for those rows, keep `lam-alef` on a bounded runtime-divergence row rather than a feature-local positive claim, preserve the paragraph-owned mixed-bidi diagnostic on `NotoNaskhArabic-Regular.ttf`, and keep a reviewed generic `text.shaping.gdef-required` refusal row on `gpos-missing-gdef.otf` for Arabic base+mark input, while positive cursive-attachment evidence on the ticket-local Arabic fixture wave, Arabic-specific missing-mark/missing-cursive refusal fixtures, and ticket-local `gsub-trace` / `gpos-trace` dump families remain separate gates.

## Current Blockers

- 2026-06-18 audit: `KFONT-M6-006` still depends on absent per-script shaping fixture families from `KFONT-M6-007`, `KFONT-M6-008`, and `KFONT-M6-009`.
- 2026-06-18 audit: `KFONT-M6-007` still needs explicit `lam-alef` positive evidence, ticket-local positive cursive, Arabic-specific refusal fixtures/codes, and ticket-local `gsub-trace` / `gpos-trace` dumps before `done`, while `KFONT-M6-008`, `KFONT-M6-009`, and `KFONT-M6-010` remain gated by their own named fixture families, required-script rows, or advanced lookup dependencies.
