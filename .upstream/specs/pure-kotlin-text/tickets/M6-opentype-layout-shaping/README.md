...
- 2026-06-19 audit: `KFONT-M6-007` remains blocked on positive `lam-alef`, vendored positive cursive, and Arabic-specific refusal fixtures/codes; `KFONT-M6-008` remains blocked on ticket-local syllable-plan or phase evidence, the full required `deva` / `dev2` feature set, dedicated unsupported-syllable and phase refusal fixtures/codes, and ticket-local `gsub-trace.json` / `gpos-trace.json` / `shaped-glyph-run.json` / `unicode-segments.json` dump families; `KFONT-M6-009` remains blocked on paragraph-owned Thai dictionary diagnostics, Thai refusal fixtures/codes, `cmap` format 14 variation-selector evidence, Han/Hangul rows, paragraph-owned ruby/line-break diagnostics, and ticket-local `shaping-plan.json` / `gsub-trace.json` / `gpos-trace.json` / `shaped-glyph-run.json` / `cmap-map.json` / `unicode-segments.json` dump families; `KFONT-M6-010` remains gated by its own named fixture families, required-script rows, or advanced lookup dependencies.
