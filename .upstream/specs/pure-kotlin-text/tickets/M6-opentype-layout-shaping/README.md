...
- `KFONT-M6-007` is now in `review` on bounded vendored-font evidence only: `ArabicShapingFixtureTest` plus `arabic-shaping-report.json` prove joining-form behavior beyond pure RTL reordering, mark positioning tied to the mark cluster itself, and the paragraph-owned mixed-bidi diagnostic on `NotoNaskhArabic-Regular.ttf`, while `lam-alef` still has only a bounded non-promotional runtime check, positive cursive-attachment evidence on the ticket-local Arabic fixture wave, Arabic-specific missing-mark/missing-cursive refusal fixtures, and ticket-local shaping/trace dump families remain separate gates.
...
- 2026-06-18 audit: `KFONT-M6-007` still needs explicit `lam-alef`, ticket-local positive cursive, Arabic-specific refusal fixtures/codes, and ticket-local shaping/trace dump families before `done`, while `KFONT-M6-008`, `KFONT-M6-009`, and `KFONT-M6-010` remain gated by their own named fixture families, required-script rows, or advanced lookup dependencies.
