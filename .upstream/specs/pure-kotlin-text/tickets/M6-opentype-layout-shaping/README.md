...
- `KFONT-M6-007` is now in `review` on bounded vendored-font evidence only: `ArabicShapingFixtureTest`, `arabic-shaped-glyph-run.json`, and `arabic-shaping-report.json` prove joining-form behavior beyond pure RTL reordering, pin the ticket-local Arabic glyph/cluster facts for joining and marks, keep `lam-alef` on a bounded runtime-divergence row rather than a feature-local positive claim, preserve the paragraph-owned mixed-bidi diagnostic on `NotoNaskhArabic-Regular.ttf`, and keep a reviewed generic `text.shaping.gdef-required` refusal row on `gpos-missing-gdef.otf` for Arabic base+mark input, while positive cursive-attachment evidence on the ticket-local Arabic fixture wave, Arabic-specific missing-mark/missing-cursive refusal fixtures, and ticket-local `shaping-plan` / `gsub-trace` / `gpos-trace` dump families remain separate gates.
