...
- 2026-06-17 closeout: `KFONT-M6-003` is now a freshly validated `done` slice with checked-in GSUB contextual fixture provenance, refreshed `gsub-trace.json` / `shaped-glyph-run.json` evidence, and remediated independent-review findings around format 2 coverage/subtable handling, missing nested lookup refusals, and atomic contextual cluster-invariant rollback including nested contextual outer-cluster escapes.
...
- 2026-06-17 audit: `KFONT-M6-006` is blocked on absent per-script shaping fixture families from `KFONT-M6-007`, `KFONT-M6-008`, and `KFONT-M6-009`, plus runtime adoption of `ResolvedFeatureSet`.
- 2026-06-17 audit: `KFONT-M6-005`, `KFONT-M6-007`, `KFONT-M6-008`, `KFONT-M6-009`, and `KFONT-M6-010` remain gated by their own named fixture families and later runtime slices; the simple and contextual GSUB plus base GPOS tickets are no longer open blockers.
