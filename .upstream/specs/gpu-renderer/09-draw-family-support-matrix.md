...
| Path fill | `TargetPrepared` plus future native | `CPUPreparedGPU` initially, `GPUNative` when proven | Render pass sampling coverage or stencil/cover route | `MaterialKey` | `PathAtlasArtifact`, `CoverageMaskArtifact`, or future GPU stencil/compute coverage. |
| Path stroke | `TargetPrepared` plus future native | `CPUPreparedGPU` initially, `GPUNative` when proven | Render pass with prepared geometry/mask or future stencil/cover | `MaterialKey` | `PrecomputedGeometryArtifact`, `PathAtlasArtifact`, `CoverageMaskArtifact`. |
...
| Clip rrect/path | `TargetPrepared` | `CPUPreparedGPU` or `GPUNative` by strategy | Stencil/depth, coverage mask, or path atlas | none or `GPULayerPlan` context | `CoverageMaskArtifact` or `PathAtlasArtifact`; stable refusal for unsupported stack interactions. |
...
- atlas or geometry budget diagnostics;
