...
| Plan D2 | n/a   | **[RASTER_COMPLETION D2] — `SkRuntimeEffect` compatibility shim** : doc-only — décision de substituer le moteur SkSL par un registry de fonctions Kotlin pré-compilées (iso-fidelity exception, justifiée par le choix WGSL côté GPU). API publique conservée iso. | 📋 |
