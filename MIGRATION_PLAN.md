...
| D2.5 | ~226     | **[D2.5] — `SkRuntimeImageFilter` + `SkImageFilters.RuntimeShader` factory**. Bridge runtime-effect shaders into le `SkImageFilter` DAG. Nouveau fichier `SkRuntimeImageFilter.kt` (`org.skia.foundation`) + 2 factory overloads dans `SkImageFilters.kt` : (1) single-child `RuntimeShader(builder, sampleRadius, childShaderName, input)` ; (2) multi-child `RuntimeShader(builder, childShaderNames, inputs)`. Le `filterImage(src, ctm)` resolve chaque child binding (filter chaîné ou layer source direct), wrap chaque résultat en `ChildResolver.Shader`, run l'effect par-pixel sur la bbox de l'input, output un nouvelle bitmap. Helper `SkRuntimeEffectBuilder.snapshotUniforms(): SkData` ajouté pour exposer les uniforms bound. Tests : 3 unit tests pinning end-to-end fade-to-blue stub + multi-child variant + arity mismatch guard. **GM port deferred** : `rtif_unsharp` / `rtif_distort` (necessitent SkSL enregistrés + text rendering hors scope D2.5). Suite à 3211 verts. | ✅ |
| D2.4.d | ~226     | **[D2.4.d] — Specialised one-off effects (3 GMs)**. `SkBuiltinSpecialisedEffects` cluster — 4 SkSL programs hand-portés : (1) **InvertBlender** (`gm/destcolor.cpp`) — 1-line blender `(half4(1) - dst).rgb1` ; (2) **StretchColorsBlender** (`gm/imagedither.cpp`) — `((dst.rgb - 0.25) * 16).rgb1`, amplifie les variations sub-LSB ; (3) **KawaseBlurShader** + (4) **KawaseMixShader** (`gm/kawase_blur_rt.cpp`) — 5-tap Kawase blur + cross-fade mix avec `uniform shader src` / `in_blur` / `in_original` children. 3 GM ports : `DestColorGM` (35.75 %), `ImageDitherGM` (14.44 %), `KawaseBlurRtGM` (70.83 %). Faibles scores explicables : (a) on substitue `mandrill_*.png` upstream par un gradient synthétique RGB ; (b) `image_dither` ne supporte pas `setDither` (les 3 cellules dithérées rendent identiques à la non-dithérée) ; (c) `kawase_blur_rt` skip les DEBUG stage draws upstream (qui produisent l'essentiel de la structure visuelle). **L'objectif du slice est d'exercer la machinery SkSL multipass + blender + uniform shader children**, pas la pixel parity. Reportés à follow-up D2.4.d.2 : `RippleShaderGM` (~100 LOC SkSL avec 9 helper functions : turbulence, sparkles, softCircle…) et `FpSampleChainingGM` (~244 LOC upstream). 6 unit tests + 3 GM ratchets. Suite à 3208 verts. | ✅ |
| D2.4.c.6 | ~223     | **[D2.4.c.6] — Relational intrinsics — clôture D2.4.c**. `SkBuiltinShaderEffectsIntrinsicsRelational` cluster — **3e SkSL template distinct** : `make_bvec_sksl(type, fn)` avec `uniform <float,int>2 v1` (= `(-2, -2)`). p remappé en bins `{-3, -2, -1}` ; le shader évalue le bvec relational et output rouge / vert / jaune / noir selon `(cmp.x, cmp.y)`. 18 hashes registered (lessThan/lessThanEqual/greaterThan/greaterThanEqual/equal/notEqual × {float, int} + 4 bvec compositions / not() inverses + any/all reductions broadcast). Single impl `BvecRelationalImpl(useInt, cmp)` paramétré par useInt + lambda `(px, py, v1x, v1y) → Pair<Boolean, Boolean>`. GM port `RuntimeIntrinsicsRelationalGM` (4×6 grid). **Score : 97.53 %** vs `runtime_intrinsics_relational.png` (le plus haut de la série D2.4.c — cellules colour-discrètes plus simples à matcher pixel-iso). 8 unit tests + 1 GM ratchet. Suite à 3202 verts. **🎉 D2.4.c close (6 / 6 sub-slices ✅).** | ✅ |
| D2.4.c.5 | ~222     | **[D2.4.c.5] — Matrix intrinsics**. `SkBuiltinShaderEffectsIntrinsicsMatrix` cluster — **distinct SkSL template** (différent de `make_unary_sksl_1d`) : `make_matrix_comp_mult_sksl(N)` (uniforms `m1, m2 : floatNxN`, output `matrixCompMult(m1, m2)[k][l]` per cell) et `make_matrix_inverse_sksl(N)` (uniforms `scale, bias : float, m : floatNxN`, output `inverse(m)[k][l] * scale + bias`). 6 hashes registered (compMult × 3 dims + inverse × 3 dims). 2 nouveaux impls : `MatrixCompMultImpl(dim)` lit la cellule `m1[k][l] * m2[k][l]` ; `MatrixInverseImpl(dim)` calcule `inverse(m)` à chaque pixel via Gauss-Jordan avec partial pivoting (en `Double` interne pour la stabilité numérique, output `Float`). Helper `RuntimeIntrinsicsPlotHelper.drawShaderCell` ajouté pour le rendu sans polyline overlay (les matrix GMs montrent un colour map 2D, pas un plot 1D). GM port `RuntimeIntrinsicsMatrixGM` (3×2 grid). **Score : 96.61 %** vs `runtime_intrinsics_matrix.png`. 7 unit tests (hash résolution + selN partitioning + matrixInverse correctness 2×2/3×3 + identity/singular guards + end-to-end shader sampling). Suite à 3192 verts. | ✅ |
| D2.4.c.4 | ~221     | **[D2.4.c.4] — Geometric intrinsics**. `SkBuiltinShaderEffectsIntrinsicsGeometric` cluster — 16 SkSL hashes : `length(x)/length(p)`, `distance(x, 0)/distance(p, v1)`, `dot(x, 2)/dot(p, p.y1)`, `cross(p.xy1, p.y1x).{x,y,z}`, `normalize(x)/normalize(p).{x,y}`, `faceforward(v1, p.x0, v1.x0).x`, `reflect(p.x1, v1.0x).x`, `reflect(p.x1, normalize(v1)).y`, `refract(v1.x0, v1.0x, x).{x,y}`. Math : closed-form de chaque entry après expansion des swizzles + substitution `v1 = (1, 1)`. Particularités : `cross` est 3D (a×b ∈ ℝ³, on plotte chaque composante séparément), `refract` retombe sur `(0, 0)` quand `k = 1 − eta²·(1 − dot(N, I)²) < 0` (total internal reflection), `faceforward` flip le signe du normal selon le sign de `dot(Nref, I)`. GM port `RuntimeIntrinsicsGeometricGM` (4×5 grid). **Score : 95.84 %** vs `runtime_intrinsics_geometric.png`. 11 unit tests (1 batch + 10 spot-checks dont total-internal-reflection cas). Suite à 3184 verts. | ✅ |
