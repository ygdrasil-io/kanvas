...
class GPUPreparedSurfaceFrameExecutorTest {
    ...
    fun `destination read core frame executes ready with copy then formula evidence`() {
        ...
        val operations = listOf(
            DisplayOp.DrawRect(
                ...
                Paint.fill(ColorARGB.Blue).copy(antiAlias = false, blendMode = BlendMode.DARKEN),
                Matrix3x3F32.Identity,
                ClipStack.WideOpen,
            ),
        )
    ...
    fun `deduplicated destination snapshot evidence allows multiple copy consumers`() {
        val operations = listOf(
            DisplayOp.DrawRect(
                RectF32.ofLTRB(0f, 0f, 32f, 24f),
                Paint.fill(ColorARGB.Red).copy(antiAlias = false, blendMode = BlendMode.DARKEN),
                Matrix3x3F32.Identity,
                ClipStack.WideOpen,
            ),
            DisplayOp.DrawRect(
    ...
    private fun nonEmptyText(
        ...
        x = 4f,
        y = 24f,
        paint = Paint.fill(ColorARGB.White).copy(blendMode = blendMode),
        transform = Matrix3x3F32.Identity,
        clip = clip,
    )
