...
class GPUClipCoverageSurfaceTest {
    ...
    @Test
    fun `public positive translated drrect renders inside one identity hard path clip stencil scope`() {
        requireWebGpu()
        val fill = ColorARGB.of(255, 242, 135, 46)
        val surface = Surface(64, 64)
        surface.canvas {
        ...
        assertTrue(result.diagnostics.isEmpty, result.diagnostics.entries.toString())
        assertEquals(0, result.stats.opsRefused)
        assertRgbaNear(result.pixels, 64, 20, 20, fill)
        assertRgbaNear(result.pixels, 64, 32, 30, ColorARGB.Transparent)
        assertRgbaNear(result.pixels, 64, 50, 14, ColorARGB.Transparent)
    }
