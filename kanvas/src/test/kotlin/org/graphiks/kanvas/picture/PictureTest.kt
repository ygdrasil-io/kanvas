...
class PictureTest {
    @Test
    fun `legacy path data keeps its wire shape across semantic migration`() {
        val legacy = Base64.getDecoder().decode(LEGACY_PATH_PICTURE_DATA)

        val restored = requireNotNull(Picture.fromByteArray(legacy))
        val path = assertIs<DisplayOp.DrawPath>(restored.ops.single()).path
        assertEquals(
            listOf(
        ...
        assertContentEquals(legacy, restored.toByteArray())
    }
...
private const val LEGACY_PATH_PICTURE_DATA =
    ""
