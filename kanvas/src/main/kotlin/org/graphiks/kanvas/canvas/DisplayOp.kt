 ...
 *
 * Each drawing op carries the [Matrix3x3F32] transform and [ClipStack] that were
...
sealed interface DisplayOp {
    ...
    data class SetTransform(val matrix: Matrix3x3F32) : DisplayOp
    ...
    data class DrawPicture(val picture: Picture, val paint: Paint?, val transform: Matrix3x3F32, val clip: ClipStack) : DisplayOp

    /** Draw a triangle mesh. */
    data class DrawVertices(val vertices: Vertices, val paint: Paint, val transform: Matrix3x3F32, val clip: ClipStack) : DisplayOp

    /** Draw a mesh with optional fragment program and children. */
    data class DrawMesh(
        val mesh: Mesh, val paint: Paint,
        val blendMode: BlendMode?, val transform: Matrix3x3F32, val clip: ClipStack,
    ) : DisplayOp
