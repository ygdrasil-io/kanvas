...
private fun GPUMaterialDescriptor.LinearGradient.filteredColors(filter: ColorFilter): FilteredGradientColors? =
    filteredGradientColors(startR, startG, startB, startA, endR, endG, endB, endA, allStopColors, filter)
...
private fun filteredGradientColors(
    ...
    val start = filter.applyTo(GradientRgba(startR, startG, startB, startA)) ?: return null
    val end = filter.applyTo(GradientRgba(endR, endG, endB, endA)) ?: return null
...
private fun FloatArray.filteredBy(filter: ColorFilter): FloatArray? {
    ...
    var offset = 0
    while (offset < size) {
        val filtered = filter.applyTo(GradientRgba(this[offset], this[offset + 1], this[offset + 2], this[offset + 3]))
            ?: return null
...
private fun ColorFilter.applyTo(input: GradientRgba): GradientRgba? = when (this) {
    is ColorFilter.Matrix -> values.applyMatrix(input)
...
private fun FloatArray.applyMatrix(input: GradientRgba): GradientRgba? {
