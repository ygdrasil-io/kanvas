 ...
 * across the pathops pipeline.
 *
 * Phase D1.1.a — foundation slice. Only the helpers needed by SkDPoint,
 * SkDLine, SkDRect are ported here ; conditional debug helpers
 * (`SkOpGlobalState`, `SkOpPhase`) and ULPs-Pin variants used only
 * by the curve-curve intersection machinery are deferred to later
 * D1.1 / D1.2 slices.
 */
...
// ─── Epsilon constants ────────────────────────────────────────────────
//
// Mirrors the constants block in `SkPathOpsTypes.h:303-321`.
...
/**
 * 17-digit constant for `sqrt(FLT_EPSILON)` — matches the upstream
 * compile-time literal so we don't introduce drift via runtime sqrt
 * rounding.
 */
...
// ─── ULPs comparisons ─────────────────────────────────────────────────
//
// Mirrors `SkPathOpsTypes.cpp:18-201`. The 2's-complement transform
// turns `Float.toRawBits()` into a monotonically-ordered integer so
// the ULP distance between two floats is `|aBits - bBits|`.

/**
 * Mirrors Skia's `SkSignBitTo2sCompliment` : if the sign bit is set,
 * strip it and negate so that `−small` and `+small` become close
 * integers. Result : the int is monotonic in the float's order.
 */
...
/**
 * Number of representable floats between [a] and [b]. Mirrors
 * `SkPathOpsTypes.cpp:190` `UlpsDistance`. Returns [Int.MAX_VALUE]
 * if the floats have different signs (special-cases `+0 == -0`).
 */
...
//
// Mirrors `SkPathOpsTypes.h:323-547`.
...
/** True iff `(a ≤ b ≤ c) || (a ≥ b ≥ c)`. Mirrors `SkPathOpsTypes.h:530`. */
