 ...
 *
 * Iso-aligned port of Skia's `SkIPoint` (which doubles as `SkIVector`)
 * ([include/core/SkPoint.h](https://github.com/google/skia/blob/main/include/core/SkPoint.h)).
 *
 * Arithmetic operators (`+`, `-`, `+=`, `-=`) use saturating 32-bit
 * add/subtract to mirror upstream `Sk32_sat_add` / `Sk32_sat_sub`.
 */
public data class Vector2I32(public var x: Int = 0, public var y: Int = 0) {
    ...
    public companion object {
        /** Creates a [Vector2I32] from components. Mirrors Skia's `SkIPoint::Make`. */
        ...
        /** Saturating 32-bit addition. Mirrors Skia's `Sk32_sat_add`. */
        public fun sk32SatAdd(a: Int, b: Int): Int = sk64PinToS32(a.toLong() + b.toLong())

        /** Saturating 32-bit subtraction. Mirrors Skia's `Sk32_sat_sub`. */
 ...
 * vectors, mirroring Skia's convention (`SkIPoint` and `SkIVector` are the
 * same struct).
 */
