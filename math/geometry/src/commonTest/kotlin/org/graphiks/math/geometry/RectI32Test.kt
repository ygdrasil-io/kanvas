...
class RectI32Test {
    ...
    fun `MakeLTRB stores corners`() {
        val r = RectI32.MakeLTRB(1, 2, 3, 4)
    ...
    fun `isEmpty checks 64-bit dimensions and int32 fit`() {
        assertTrue(RectI32.MakeEmpty().isEmpty)
        assertTrue(RectI32.MakeLTRB(5, 5, 5, 10).isEmpty)
        assertTrue(RectI32.MakeLTRB(5, 10, 10, 5).isEmpty)
        assertFalse(RectI32.MakeLTRB(0, 0, 1, 1).isEmpty)
        val overflow = RectI32.MakeLTRB(Int.MIN_VALUE, 0, Int.MAX_VALUE, 1)
    ...
    fun `MakeXYWH saturates instead of wrapping on int32 overflow`() {
        val r = RectI32.MakeXYWH(Int.MAX_VALUE - 1, 0, 10, 10)
    ...
    fun `MakePtSize delegates to MakeXYWH saturation semantics`() {
        val max = RectI32.MakePtSize(Vector2I32(Int.MAX_VALUE - 1, Int.MAX_VALUE - 2), SizeI32(10, 20))
        ...
        val min = RectI32.MakePtSize(Vector2I32(Int.MIN_VALUE + 1, Int.MIN_VALUE + 2), SizeI32(-10, -20))
    ...
    fun `setXYWH offset inset adjust all saturate`() {
        val r = RectI32.MakeEmpty()
        r.setXYWH(Int.MAX_VALUE - 5, 0, 100, 1)
        assertEquals(Int.MAX_VALUE, r.right)

        val o = RectI32.MakeLTRB(Int.MIN_VALUE + 1, 0, Int.MAX_VALUE - 1, 10)
        o.offset(-5, 0)
    ...
    fun `makeOffset makeInset makeOutset saturate`() {
        val r = RectI32.MakeLTRB(0, 0, 10, 10)
    ...
    fun `offsetTo uses 64-bit pinned arithmetic`() {
        val r = RectI32.MakeLTRB(Int.MIN_VALUE, 0, Int.MIN_VALUE + 10, 5)
    ...
    fun `offset offsetTo inset outset adjust`() {
        val r = RectI32.MakeXYWH(10, 20, 5, 6)
        r.offset(2, 3)
    ...
    fun `sort and makeSorted swap reversed edges`() {
        val r = RectI32.MakeLTRB(10, 20, 0, 5)
        r.sort()
    ...
    fun `contains rect requires non-empty inner`() {
        val outer = RectI32.MakeLTRB(0, 0, 10, 10)
        assertTrue(outer.contains(RectI32.MakeLTRB(2, 2, 8, 8)))
        assertTrue(outer.contains(RectI32.MakeLTRB(0, 0, 10, 10)))
        assertFalse(outer.contains(RectI32.MakeEmpty()))
        assertFalse(outer.contains(RectI32.MakeLTRB(5, 5, 11, 8)))
    }

    @Test
    fun `intersect overlapping clips to overlap`() {
        val a = RectI32.MakeLTRB(0, 0, 10, 10)
        assertTrue(a.intersect(RectI32.MakeLTRB(5, 5, 15, 15)))
        assertEquals(RectI32(5, 5, 10, 10), a)
    }

    @Test
    fun `intersect non-overlapping returns false unchanged`() {
        val a = RectI32.MakeLTRB(0, 0, 5, 5)
        assertFalse(a.intersect(RectI32.MakeLTRB(10, 10, 20, 20)))
        assertEquals(RectI32(0, 0, 5, 5), a)
    }

    @Test
    fun `Intersects companion`() {
        assertTrue(RectI32.Intersects(RectI32.MakeLTRB(0, 0, 10, 10), RectI32.MakeLTRB(5, 5, 15, 15)))
        assertFalse(RectI32.Intersects(RectI32.MakeLTRB(0, 0, 5, 5), RectI32.MakeLTRB(10, 10, 20, 20)))
    }

    @Test
    fun `join expands to encompass r and skips empty args`() {
        val a = RectI32.MakeLTRB(0, 0, 5, 5)
        a.join(RectI32.MakeLTRB(3, 3, 10, 10))
        assertEquals(RectI32(0, 0, 10, 10), a)

        val b = RectI32.MakeLTRB(0, 0, 5, 5)
        b.join(RectI32.MakeEmpty())
        assertEquals(RectI32(0, 0, 5, 5), b)

        val c = RectI32.MakeEmpty()
        c.join(RectI32.MakeLTRB(1, 2, 3, 4))
        assertEquals(RectI32(1, 2, 3, 4), c)
    }

    @Test
    fun `topLeft returns Vector2I32`() {
        val r = RectI32.MakeLTRB(3, 5, 10, 20)
