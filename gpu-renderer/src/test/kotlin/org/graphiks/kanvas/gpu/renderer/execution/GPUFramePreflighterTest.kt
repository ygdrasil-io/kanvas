...
class GPUFramePreflighterTest {
    ...
    @Test
    fun `gradient core semantic envelopes admit only matching radial and sweep steps`() {
        val accepted = listOf(
            "radial.gradient.fill" to radialGradientMaterial(),
            "sweep.gradient.fill" to sweepGradientMaterial(),
        )
        accepted.forEach { (renderStepIdentity, material) ->
            val result = preflightPreparedGradientEnvelope(
                material = material,
                renderStepIdentity = renderStepIdentity,
            )
            assertIs<GPUFramePreflightResult.Prepared>(
                result,
                (result as? GPUFramePreflightResult.Refused)?.diagnostic.toString(),
            )
        }

        val radial = coreSemantic(material = radialGradientMaterial())
        val sweep = coreSemantic(material = sweepGradientMaterial())
        listOf(
            "rect.fill.coverage" to radial,
            "linear.gradient.fill" to radial,
            "radial.gradient.fill" to sweep,
            "sweep.gradient.fill" to radial,
            "radial.gradient.fill" to radial.withPayloadRenderStepIdentity("forged.gradient.step"),
        ).forEach { (renderStepIdentity, semantic) ->
            val result = preflightGradientEnvelope(semantic, renderStepIdentity)
            assertEquals(
                "invalid.preflight.core_primitive_semantic_integrity",
                assertIs<GPUFramePreflightResult.Refused>(result).diagnostic.code.value,
            )
    ...
    @Test
    fun `radial and sweep gradient steps require core semantic envelopes before side effects`() {
        val gradients = listOf(
            "radial.gradient.fill" to radialGradientMaterial(),
            "sweep.gradient.fill" to sweepGradientMaterial(),
        )
        gradients.forEach { (renderStepIdentity, material) ->
            val missingEvents = mutableListOf<String>()
            val missing = preflighter(
                RecordingResourceProvider(missingEvents),
                RecordingCompletionProvider(missingEvents),
                RecordingSurfaceProvider(missingEvents),
            ).preflight(
                framePlan(listOf(prepareScene(), coreRenderStep(null, renderStepIdentity = renderStepIdentity))),
            )
            assertEquals(
                "invalid.preflight.core_primitive_semantic_payload_missing",
                assertIs<GPUFramePreflightResult.Refused>(missing).diagnostic.code.value,
            )
            assertTrue(missingEvents.isEmpty(), "missing $renderStepIdentity semantic produced side effects")

            val render = coreRenderStep(
                coreSemantic(material = material),
                renderStepIdentity = renderStepIdentity,
            )
            val original = render.drawPackets.single()
            val wrongType = cloneCorePacket(
                original,
                semanticPayload = solidSemantic(commandId = original.commandIdValue),
            )
            val wrongTypeEvents = mutableListOf<String>()
            val wrongTypeResult = preflighter(
                RecordingResourceProvider(wrongTypeEvents),
                RecordingCompletionProvider(wrongTypeEvents),
                RecordingSurfaceProvider(wrongTypeEvents),
            ).preflight(
                framePlan(listOf(prepareScene(), render)).replacingCorePacket(original, wrongType),
            )
            assertEquals(
                "invalid.preflight.core_primitive_semantic_integrity",
                assertIs<GPUFramePreflightResult.Refused>(wrongTypeResult).diagnostic.code.value,
            )
            assertTrue(wrongTypeEvents.isEmpty(), "wrong $renderStepIdentity semantic produced side effects")
        }
    }

    @Test
    fun `split radial and sweep direct gradients retain one global slab then exact scope slices`() {
        val capabilities = gradientCapabilities(radialGradientMaterial(), sweepGradientMaterial())
        val plan = preparedSplitGradientFramePlan(capabilities)
        ...
        assertEquals(2, frameSeal.plan.slots.size)
        assertEquals(packets.map { it.commandIdValue }, frameSeal.commandIds)
        assertTrue(frameSeal.plan.slots.all { it.payloadBytes == CORE_PRIMITIVE_GRADIENT_UNIFORM_BYTES.toLong() })
        packets.forEach { packet ->
            assertSame(frameSeal, packet.corePrimitivePreparedAuthority?.uniformSlabSeal)
        }

        val result = preflightGradientPlan(plan, capabilities)
        val prepared = assertIs<GPUFramePreflightResult.Prepared>(
            result,
            (result as? GPUFramePreflightResult.Refused)?.diagnostic?.let { diagnostic ->
                "${diagnostic.code.value}: ${diagnostic.message}"
            },
        ).frame
        renders.forEach { render ->
            val scope = prepared.encoderPlan.scopes.single { scope ->
                scope.sourceStepIndex == plan.steps.indexOf(render)
            }
            val scopeSeal = requireNotNull(
            ...
            assertEquals(render.drawPackets.map { it.commandIdValue }, scopeSeal.commandIds)
            assertEquals(1, scopeSeal.plan.slots.size)
            assertEquals(0L, scopeSeal.plan.slots.single().alignedOffset)
            assertEquals(
                CORE_PRIMITIVE_GRADIENT_UNIFORM_BYTES.toLong(),
                scopeSeal.plan.slots.single().payloadBytes,
            )
        }
        ...
        val forged = listOf(
            GPUCorePrimitiveUniformSlabSeal(
                frameSeal.plan,
                frameSeal.commandIds.reversed(),
                frameSeal.packedBytesSnapshot(),
            ),
            GPUCorePrimitiveUniformSlabSeal(
                frameSeal.plan.copyForTest(
                    slots = frameSeal.plan.slots.mapIndexed { index, slot ->
                        slot.copy(slotLabel = "forged-slot-$index")
                    },
                ),
                frameSeal.commandIds,
                frameSeal.packedBytesSnapshot(),
            ),
            GPUCorePrimitiveUniformSlabSeal(
                frameSeal.plan.copyForTest(sourceLabel = "forged-gradient-plan"),
                frameSeal.commandIds,
                frameSeal.packedBytesSnapshot(),
            ),
            GPUCorePrimitiveUniformSlabSeal(
                frameSeal.plan,
                frameSeal.commandIds,
                frameSeal.packedBytesSnapshot().apply { this[0] = (this[0].toInt() xor 1).toByte() },
            ),
        )
        forged.forEach { seal ->
            val forgedPlan = packets.fold(plan) { current, packet ->
                current.replacingCorePacket(packet, cloneCorePacket(packet, uniformSlabSeal = seal))
            }
    ...
    @Test
    fun `mixed direct solid and radial retain one global legacy slab then exact scope slices`() {
        ...
        assertEquals(packets.map { it.commandIdValue }, frameSeal.commandIds)
        assertEquals(2, frameSeal.plan.slots.size)
        assertEquals(32L, frameSeal.plan.slots[0].payloadBytes)
        assertEquals(CORE_PRIMITIVE_GRADIENT_UNIFORM_BYTES.toLong(), frameSeal.plan.slots[1].payloadBytes)
        packets.forEach { packet ->
            assertSame(frameSeal, packet.corePrimitivePreparedAuthority?.uniformSlabSeal)
        }

        val prepared = assertIs<GPUFramePreflightResult.Prepared>(
            preflightGradientPlan(plan, capabilities),
        ).frame
        renders.forEach { render ->
            val scope = prepared.encoderPlan.scopes.single { scope ->
                scope.sourceStepIndex == plan.steps.indexOf(render)
        ...
        val forged = GPUCorePrimitiveUniformSlabSeal(
            frameSeal.plan,
            frameSeal.commandIds.reversed(),
            frameSeal.packedBytesSnapshot(),
        )
        val forgedPlan = packets.fold(plan) { current, packet ->
            current.replacingCorePacket(packet, cloneCorePacket(packet, uniformSlabSeal = forged))
        }
        assertIs<GPUFramePreflightResult.Refused>(
            preflightGradientPlan(forgedPlan, capabilities),
        )
    ...
    @Test
    fun `two render dst copy retains zero based per step slab seals`() {
        val fixture = preparedSurfacePreflightFixture(PreparedSurfaceFixtureShape.DstReadCore)
        val events = mutableListOf<String>()
        ...
        fixture.framePlan.steps.filterIsInstance<GPUFrameStep.RenderPassStep>()
            .forEach { render ->
                val scope = prepared.encoderPlan.scopes.single {
                    it.sourceStepIndex == fixture.framePlan.steps.indexOf(render)
                }
                val routes = assertIs<GPUCorePrimitiveDirectNativeRouteSeal.Routes>(
                    scope.corePrimitiveDirectNativeRouteSeal,
                )
                val seal = requireNotNull(routes.preparedPassSeal?.uniformSlabSeal)
                assertEquals(render.drawPackets.map { it.commandIdValue }, seal.commandIds)
                assertEquals(render.drawPackets.size, seal.plan.slots.size)
                assertEquals(0L, seal.plan.slots.first().alignedOffset)
                assertEquals(256L, seal.plan.totalBytes)
            }
    }
    ...
    fun `uniform slab slice rebases one step to zero based offsets and exact packed bytes`() {
        ...
        val sliced = sliceUniformSlabSealToCommands(frameSlab, listOf(2))

        assertEquals(listOf(2), sliced.commandIds)
        assertEquals(1, sliced.plan.slots.size)
        assertEquals("draw-2", sliced.plan.slots[0].slotLabel)
        assertEquals(0L, sliced.plan.slots[0].alignedOffset)
        assertEquals(256L, sliced.plan.totalBytes)
        val packed = sliced.packedBytesForUpload()
        assertEquals(256, packed.size)
        assertContentEquals(
            framePacked.copyOfRange(256, 256 + 32),
            packed.copyOfRange(0, 32),
        )
    }

    @Test
    fun `analytic shape uniform slab slice rebases one step to zero based offsets`() {
        val plan = preparedAnalyticFramePlan(
            plans = mapOf(
            ...
            geometries = mapOf(
                181 to GPUCorePrimitiveGeometryInput.Rect(0.75f, 1f, 2.5f, 3.25f),
                182 to GPUCorePrimitiveGeometryInput.Rect(1.5f, 0.5f, 3.25f, 4f),
            ),
            coverageModes = mapOf(
                181 to GPUCorePrimitiveCoverageMode.ScalarAA,
                182 to GPUCorePrimitiveCoverageMode.ScalarAA,
            ),
        )
        ...
        val semanticsByCommand = packets.associate { packet ->
            packet.commandIdValue to (packet.semanticPayload as GPUDrawSemanticPayload.CorePrimitive)
        }
        assertEquals(2, sealsByCommand.size)

        val sliced = sliceAnalyticShapeUniformSealsToCommands(
    ...
    fun `two render dst copy admission requires every core packet accepted`() {
        ...
        val consumer = renders.last()
        val dstPacket = consumer.drawPackets.single()
        // A foreign solid-lane packet (non-core render-step identity and SolidRect semantic) is
        // envelope-clean and solid-ABI-exact, but has no direct CorePrimitive route, so it can
        // never be accepted by the direct lane.
        val foreignSemantic = solidSemantic(99)
        val foreignSolid = packet(
            id = "packet.foreign-solid",
            commandId = 99,
            renderStepIdentity = "rect.fill.coverage",
            semanticPayload = foreignSemantic,
            uniformSlot = foreignSemantic.payloadRef.uniformSlot,
            targetStateHash = dstPacket.targetStateHash,
            resourceGeneration = dstPacket.resourceGeneration,
        )
        val consumerWithForeign = GPUFrameStep.RenderPassStep(
            target = consumer.target,
            loadStore = consumer.loadStore,
            samplePlan = consumer.samplePlan,
            resourceUses = consumer.resourceUses,
            drawPackets = listOf(dstPacket, foreignSolid),
            sourceTaskIds = consumer.sourceTaskIds,
            batches = listOf(
                GPUFrameRenderBatch(
                    "batch.foreign",
                    GPUPassBatchKind.Isolated,
                    listOf(dstPacket, foreignSolid),
                    consumer.sourceTaskIds,
                ),
            ),
        )
        val mutated = fixture.framePlan.replacingRender(consumer, consumerWithForeign)
        val events = mutableListOf<String>()
        ...
        val refused = assertIs<GPUFramePreflightResult.Refused>(
            result,
            (result as? GPUFramePreflightResult.Prepared)?.let { "unexpectedly classified Prepared" },
        )
        assertEquals("invalid.preflight.core_primitive_direct_geometry_resources", refused.diagnostic.code.value)
        assertTrue(events.isEmpty(), "pure validation side effects: $events")
    }
    ...
    fun `malformed third direct render refuses the two render dst copy admission at preflight`() {
        ...
        val third = GPUFrameStep.RenderPassStep(
            target = producer.target,
            loadStore = GPULoadStorePlan("load", GPUStorePlan.Store),
            samplePlan = GPUSamplePlan.SingleSampleFrame,
            resourceUses = producer.resourceUses,
            drawPackets = listOf(
                cloneCorePacket(
                    producer.drawPackets.single(),
                    packetId = GPUDrawPacketID("packet.third"),
                ),
            ),
            sourceTaskIds = listOf(GPUTaskID("task.third")),
        )
        val mutated = fixture.framePlan.withSteps(fixture.framePlan.steps + third)
        val events = mutableListOf<String>()
    ...
    fun `color only 4x core primitive preflight accepts exact continuation and refuses substitution before effects`() {
        ...
        val continuation = org.graphiks.kanvas.gpu.renderer.passes.GPUSampleContinuationRequest(
            key = org.graphiks.kanvas.gpu.renderer.passes.GPUSampleContinuationKey(
                target = org.graphiks.kanvas.gpu.renderer.state.GPUTargetIdentity("target.scene"),
                targetGeneration = 1L,
                ...
                colorAttachment = org.graphiks.kanvas.gpu.renderer.state.GPUTargetIdentity(
                    "msaa-color:target.scene:1",
                ),
                depthStencilAttachment = null,
            ),
            loadTransition = org.graphiks.kanvas.gpu.renderer.passes.GPUSampleLoadTransition.FreshClear,
            storeAction = org.graphiks.kanvas.gpu.renderer.passes.GPUSampleStoreAction.Store,
            resolveAction = org.graphiks.kanvas.gpu.renderer.passes.GPUSampleResolveAction.ResolveCanonical,
        )
        val semantic = coreSemantic()
        val render = coreRenderStep(
            semantic,
            samplePlan = samplePlan,
            targetStateHash = corePrimitiveTargetStateHash(4),
            sampleContinuation = continuation,
        )
        val capabilities = msaaCapabilities(depthStencil = false)
        val plan = framePlan(listOf(coreDirectPrepare(), render), capabilities = capabilities)
        val acceptedEvents = mutableListOf<String>()

        val acceptedResult = preflighter(
                RecordingResourceProvider(acceptedEvents),
                RecordingCompletionProvider(acceptedEvents),
                RecordingSurfaceProvider(acceptedEvents),
        ...
        assertIs<GPUFramePreflightResult.Prepared>(
            acceptedResult,
            (acceptedResult as? GPUFramePreflightResult.Refused)?.diagnostic.toString(),
        )
        assertTrue(acceptedEvents.isNotEmpty())

        val mutations = listOf(
            render.copyCoreRenderForTest(sampleContinuation = null) to
                "unsupported.msaa.continuation_attachment_not_stored",
            render.copyCoreRenderForTest(
            ...
            ) to "unsupported.preflight.core_primitive_msaa_attachment_authority",
            render.copyCoreRenderForTest(
                depthStencilLoadStore = GPUDepthStencilLoadStorePlan.ReadOnlyKeep,
            ) to "unsupported.preflight.core_primitive_msaa_depth_stencil",
            render.copyCoreRenderForTest(targetStateHash = CORE_PRIMITIVE_TARGET_STATE_HASH) to
                "invalid.preflight.core_primitive_packet_authority",
        )
        mutations.forEach { (mutation, expectedCode) ->
            val events = mutableListOf<String>()
            val result = preflighter(
                RecordingResourceProvider(events),
                RecordingCompletionProvider(events),
                ...
                framePlan(
                    listOf(coreDirectPrepare(), mutation),
                    capabilities = capabilities,
                ),
            )
    ...
    @Test
    fun `analytic shape preflight accepts two exact uniform80 seals in one pass`() {
        val plan = preparedAnalyticFramePlan(
            plans = mapOf(
                181 to GPUClipExecutionPlan.NoClip,
                182 to GPUClipExecutionPlan.NoClip,
            ...
            geometries = mapOf(
                181 to GPUCorePrimitiveGeometryInput.Rect(0.75f, 1f, 2.5f, 3.25f),
                182 to GPUCorePrimitiveGeometryInput.RRect(
                    0.5f,
                    0.5f,
                    3.5f,
                    3.5f,
                    listOf(0.25f, 0.5f, 0.75f, 0.25f, 0.5f, 0.75f, 0.25f, 0.5f),
                ),
            ),
            coverageModes = mapOf(
                181 to GPUCorePrimitiveCoverageMode.ScalarAA,
                182 to GPUCorePrimitiveCoverageMode.FullOrScissor,
            ),
        )
        val events = mutableListOf<String>()
        ...
        val routes = assertIs<GPUCorePrimitiveDirectNativeRouteSeal.Routes>(
            prepared.encoderPlan.scopes.single().corePrimitiveDirectNativeRouteSeal,
        )
        val passSeal = requireNotNull(routes.preparedPassSeal)
        assertEquals(2, passSeal.analyticShapeUniformSeals.size)
        assertEquals(listOf(0, 1), passSeal.analyticShapeUniformSeals.map { it.slotIndex })
    ...
    @Test
    fun `analytic shape uniform80 corruption matrix refuses before every preflight side effect`() {
        val validPlan = preparedAnalyticFramePlan(
            plans = mapOf(183 to GPUClipExecutionPlan.NoClip),
            geometries = mapOf(
                183 to GPUCorePrimitiveGeometryInput.Rect(0.75f, 1f, 2.5f, 3.25f),
            ),
            coverageModes = mapOf(183 to GPUCorePrimitiveCoverageMode.ScalarAA),
        )
        val packet = validPlan.steps.filterIsInstance<GPUFrameStep.RenderPassStep>()
            .single().drawPackets.single()
        val semantic = assertIs<GPUDrawSemanticPayload.CorePrimitive>(packet.semanticPayload)
        val validSeal = requireNotNull(packet.corePrimitivePreparedAuthority?.analyticShapeUniformSeal)
        val substitutedSemantic = semantic.withTargetBounds(semantic.targetBounds)
        val corruptedBytes = validSeal.payloadBytesSnapshot().also { bytes ->
            bytes[20] = (bytes[20] + 1).toByte()
        }
        val movedSlotPlan = validSeal.plan.copyForTest(
        ...
        val cases = listOf(
            "semantic substitution" to cloneCorePacket(
                packet,
                semanticPayload = substitutedSemantic,
            ),
            "payload bytes" to cloneCorePacket(
                packet,
                analyticShapeUniformSeal = validSeal.copyForTest(
                    semantic,
                    payloadBytes = corruptedBytes,
                ),
            ),
            "slot plan" to cloneCorePacket(
                packet,
                analyticShapeUniformSeal = validSeal.copyForTest(
                    semantic,
                    plan = movedSlotPlan,
            ...
            "missing seal" to cloneCorePacket(
                packet,
                analyticShapeUniformSeal = null,
            ),
            "render scissor" to cloneCorePacket(
                packet,
                analyticShapeUniformSeal = validSeal.copyForTest(
                    semantic,
                    renderScissor = GPUPixelBounds(1, 1, 2, 2),
                ),
            ),
        )

        cases.forEach { (label, replacement) ->
            val plan = validPlan.replacingCorePacket(packet, replacement)
            val events = mutableListOf<String>()
            val resources = RecordingResourceProvider(events)
            val result = preflighter(
                resources,
        ...
        // The route lane is reclassified from semantic input, never injected from packet state.
        // Forging the packet structural lane must therefore stop at packet authority first.
        val laneEvents = mutableListOf<String>()
        val laneResources = RecordingResourceProvider(laneEvents)
        val laneReplacement = cloneCorePacket(
            packet,
            structuralPipelineKey = requireNotNull(packet.corePrimitivePreparedAuthority)
                .structuralPipelineKey.copy(
                    shader = GPUCorePrimitiveRenderPipelineStructuralKey.Shader.DirectGeometry,
                ),
        )
        val lanePlan = validPlan.replacingCorePacket(packet, laneReplacement)
        val laneResult = preflighter(
            laneResources,
            RecordingCompletionProvider(laneEvents),
            RecordingSurfaceProvider(laneEvents),
            context = clipPreflightContext(lanePlan),
            capabilities = pathCapabilities(),
        ).preflight(lanePlan)
        assertEquals(
            "invalid.preflight.core_primitive_packet_authority",
            assertIs<GPUFramePreflightResult.Refused>(laneResult).diagnostic.code.value,
        )
        assertEquals(0, laneResources.beginFramePreparationCount)
        assertTrue(laneEvents.isEmpty(), "lane authority produced preflight side effects: $laneEvents")
    }
    ...
    fun `path stencil AA 4x preflight seals exact paired attachments and operand keys`() {
        ...
        val events = mutableListOf<String>()
        val capabilities = pathMsaaCapabilities()
        ...
        assertEquals(
            org.graphiks.kanvas.gpu.renderer.state.GPUTargetIdentity(depthUse.resource.value),
            continuation.key.depthStencilAttachment,
        )
    ...
    fun `path stencil AA 4x authority corruption refuses before provider side effects`() {
        ...
        val depthPreparation = prepare.requests.single { it.role == GPUFrameResourceRole.PathDepthStencil }
        fun depthPreparation(
            descriptor: org.graphiks.kanvas.gpu.renderer.resources.GPUFrameResourceDescriptor =
                depthPreparation.descriptor,
            byteSize: Long = depthPreparation.byteSize,
        ) = GPUResourcePreparationRequest(
            depthPreparation.resource,
            descriptor,
            depthPreparation.role,
            depthPreparation.usages,
            depthPreparation.lifetime,
            byteSize,
            depthPreparation.diagnosticLabel,
        )
        fun withDepthPreparation(
            replacement: GPUResourcePreparationRequest,
        ): GPUFramePlan = valid.withSteps(valid.steps.map { step ->
            if (step !== prepare) step else GPUFrameStep.PrepareResourcesStep(
                prepare.requests.map { request ->
                    if (request === depthPreparation) replacement else request
                },
                prepare.sourceTaskIds,
            )
        })
        val depthDescriptor = assertIs<GPUFrameTextureDescriptor>(depthPreparation.descriptor)
        fun withBudgetDelta(category: GPUFrameMemoryCategory, delta: Long): GPUFramePlan {
            val categories = valid.memoryBudget.categoryTotals.toMutableMap()
            categories[category] = Math.addExact(categories.getValue(category), delta)
            return GPUFramePlan(
                valid.frameId,
                valid.capabilitySeal,
                valid.recordingSeals,
                valid.steps,
                valid.memoryBudget.copy(categoryTotals = categories),
                valid.diagnostics,
                valid.dependencies,
            )
        }
        val firstPacket = render.drawPackets.first()
        val oneXStructural = requireNotNull(firstPacket.corePrimitivePreparedAuthority)
            .structuralPipelineKey.copy(sampleCount = 1)
        val cases = listOf(
            "missing-depth-key" to replaceRender(
                valid,
                renderWith(
                    render,
                    sampleContinuation = continuation.copy(
                        key = continuation.key.copy(depthStencilAttachment = null),
                    ),
                ),
            ),
            "foreign-depth-key" to replaceRender(
                valid,
                renderWith(
                    render,
                    sampleContinuation = continuation.copy(
                        key = continuation.key.copy(
                            depthStencilAttachment = org.graphiks.kanvas.gpu.renderer.state.GPUTargetIdentity(
                                "texture.foreign.path-depth-stencil",
                            ),
                        ),
                    ),
                ),
            ),
            "descriptor-1x" to withDepthPreparation(
                depthPreparation(
                    descriptor = GPUFrameTextureDescriptor(
                        depthDescriptor.logicalBounds,
                        depthDescriptor.format,
                        1,
                    ),
                ),
            ),
            "bytes-1x" to withDepthPreparation(
                depthPreparation(byteSize = depthPreparation.byteSize / 4L),
            ),
            "extent" to withDepthPreparation(
                depthPreparation(
                    descriptor = GPUFrameTextureDescriptor(
                        GPUPixelBounds(
                            depthDescriptor.logicalBounds.left,
                            depthDescriptor.logicalBounds.top,
                            depthDescriptor.logicalBounds.right - 1,
                            depthDescriptor.logicalBounds.bottom,
                        ),
                        depthDescriptor.format,
                        4,
                    ),
            ...
            "resolve-absent" to replaceRender(
                valid,
                renderWith(
                    render,
                    sampleContinuation = continuation.copy(
                        resolveAction = GPUSampleResolveAction.Skip,
                    ),
                ),
            ),
            "color-discard" to replaceRender(
                valid,
                renderWith(
                    render,
                    loadStore = render.loadStore.copy(storePlan = GPUStorePlan.Discard),
                ),
            ),
            "target-state-1x" to replaceRender(
                valid,
                renderWith(
                    render,
                    drawPackets = render.drawPackets.map { packet ->
                        cloneCorePacket(packet, targetStateHash = corePrimitiveTargetStateHash(1))
                    },
                ),
            ),
            "pipeline-key-1x" to replaceRender(
                valid,
                renderWith(
                    render,
                    drawPackets = listOf(
                        cloneCorePacket(
                            firstPacket,
                            structuralPipelineKey = oneXStructural,
                            renderPipelineKey = oneXStructural.stableRenderPipelineKey(
                                CORE_PRIMITIVE_RENDER_PIPELINE_KEY,
                            ),
                        ),
                    ) + render.drawPackets.drop(1),
                ),
            ),
            "budget-color" to withBudgetDelta(GPUFrameMemoryCategory.FrameLocalMsaaColor, -1L),
            "budget-depth" to withBudgetDelta(
                GPUFrameMemoryCategory.FrameLocalMsaaDepthStencil,
                -1L,
            ),
            "second-scope" to valid.withSteps(
                valid.steps + renderWith(
                    render,
                    drawPackets = listOf(
                        cloneCorePacket(
                            firstPacket,
            ...
            "clip-depth-injected" to replaceRender(
                valid,
                renderWith(
                    render,
                    resourceUses = render.resourceUses + depthUse.copy(
                        role = GPUFrameResourceRole.ClipDepthStencil,
                    ),
                ),
            ),
        )

        cases.forEach { (label, plan) ->
            val events = mutableListOf<String>()
            val resources = RecordingResourceProvider(events)
            val result = preflighter(
                resources,
        ...
        val staleResources = RecordingResourceProvider(staleEvents)
        val validContext = clipPreflightContext(valid)
        val staleResult = preflighter(
            staleResources,
            RecordingCompletionProvider(staleEvents),
            RecordingSurfaceProvider(staleEvents),
            context = GPUFramePreflightContext(
                targetId = validContext.targetId,
                deviceGeneration = validContext.deviceGeneration,
                targetGeneration = validContext.targetGeneration,
                resourceGenerations = validContext.resourceGenerations - depthPreparation.resource,
                surfaceGeneration = validContext.surfaceGeneration,
            ),
            capabilities = pathMsaaCapabilities(),
        ).preflight(valid)
        assertEquals(
            "invalid.preflight.core_primitive_path_stencil_msaa_authority",
            assertIs<GPUFramePreflightResult.Refused>(staleResult).diagnostic.code.value,
        )
        assertEquals(0, staleResources.beginFramePreparationCount)
        assertTrue(staleEvents.isEmpty(), "stale-generation escaped pure validation: $staleEvents")
    }
    ...
    fun `core primitive hot path reuses builder pipeline and uniform slab seals`() {
        ...
        assertEquals(
            ...
            "recording owns uniform32, analytic-shape uniform80, analytic-clip uniform64/uniform160, " +
                "and coverage-mask uniform64 sites",
        )
        assertEquals(
            0,
        ...
        val directPreflight = preflight
            .substringAfter("private fun validateCorePrimitiveDirectGeometryResourcesDiagnostic(")
        ...
        val perPacketStructuralChecks = directPreflight
            ...
            .substringBefore("val structuralPipelineKey = sceneStructuralKeys.first()")
        assertFalse(
        ...
        assertEquals(
            1,
            Regex(
                """\.stableRenderPipelineKey\(\s*CORE_PRIMITIVE_RENDER_PIPELINE_KEY,?\s*\)""",
            ).findAll(directPreflight).count(),
            "pure preflight must derive the stable render-pipeline keys exactly once per distinct structural key",
        )
        assertFalse(
            directPreflight.contains("val analyticShapePackedBytes ="),
            "pure preflight must delegate owned uniform80 slab construction to the pass-seal factory",
        )
        assertTrue(
            directPreflight.contains("GPUCorePrimitiveDirectPreparedPassSeal.analyticShape("),
            "uniform80 preflight must retain the one factory-owned packed slab",
        )
        val uniform80PreflightValidation = directPreflight
            ...
            .substringBefore("val legacyUniformLayouts =")
        assertFalse(
            uniform80PreflightValidation.contains("GPUUniformSlabPayload("),
            "uniform80 preflight must not create a second payload snapshot per draw",
        )
        assertFalse(
            uniform80PreflightValidation.contains(".hasExactPayloads("),
            "uniform80 preflight must validate exact bytes without repeating planner SHA-256 per draw",
        )
        val legacyPreflightValidation = directPreflight
            .substringAfter("val legacyUniformLayouts =")
            .substringBefore("val gradientAcceptedIndices =")
        assertFalse(
            legacyPreflightValidation.contains(".map {"),
            "legacy slab preflight must not allocate a per-draw command or payload list",
        )
        assertFalse(
            legacyPreflightValidation.contains(".toByteArray()"),
            "legacy slab preflight must not rebuild packed uniform bytes",
        )
        assertFalse(
            legacyPreflightValidation.contains("GPUUniformSlabPayload("),
            "legacy slab preflight must not reconstruct planner payloads",
        )
        assertFalse(
            legacyPreflightValidation.contains(".hasExactPayloads("),
            "legacy slab preflight must not rehash a planner payload sequence",
        )
        assertFalse(
            legacyPreflightValidation.contains(".indexOf("),
            "legacy slab preflight must preindex accepted packets before gradient scope validation",
        )
        assertTrue(
            legacyPreflightValidation.contains("legacyUniformAcceptedIndices"),
            "legacy direct uniform validation must cover solid32 and gradient592 packets in one frame order",
        )
        assertTrue(
            legacyPreflightValidation.contains("legacyUniformSlotByAcceptedIndex"),
            "legacy direct uniform validation must preindex accepted packet slots once",
        )
        assertTrue(
            legacyPreflightValidation.contains("seal.hasExactPayload("),
            "legacy direct uniform validation must compare each immutable payload against its sealed packed range",
        )
        assertTrue(
            legacyPreflightValidation.contains("seal.commandIds[slotIndex]"),
            "legacy direct uniform validation must authenticate the complete frame command order before scope slicing",
        )
        val legacyAndGradientPreflightValidation = directPreflight
            .substringAfter("val legacyUniformLayouts =")
            .substringBefore("val analyticGradientSteps =")
        assertEquals(
            1,
            Regex("""seal\.hasExactPayload\(""")
                .findAll(legacyAndGradientPreflightValidation)
                .count(),
            "legacy gradient scope validation must reuse the frame seal authentication without rescanning payload bytes",
        )
        assertEquals(
            2,
            Regex("""hasCorePrimitiveSemanticEnvelopeIntegrity\(""")
                .findAll(preflight)
                .count(),
            "one declaration and the global envelope gate must be the only full CorePrimitive semantic authentications",
        )
        val coverageMatrixValidation = preflight
            .substringAfter("private fun validateCorePrimitiveCoverageSampleMatrix(")
            .substringBefore("private fun validateCorePrimitiveSemanticEnvelopes(")
        assertFalse(
            coverageMatrixValidation.contains("hasCorePrimitiveSemanticEnvelopeIntegrity("),
            "coverage validation must reuse the immutable envelope authority already authenticated for this frame",
        )
        val perPacketSemanticValidation = preflight
            .substringAfter("private fun validateCorePrimitiveSemanticPayload(")
            .substringBefore("private fun validateSolidRectSemanticPayload(")
        assertFalse(
            perPacketSemanticValidation.contains("hasCorePrimitiveSemanticEnvelopeIntegrity("),
            "per-packet semantic validation must reuse the immutable envelope authority already authenticated for this frame",
        )
        assertTrue(
    ...
    private fun preparedAnalyticFrameResult(
        ...
        val semantics = packets.associate { packet ->
            ...
            packet.commandIdValue to coreSemantic(
                ...
                geometry = geometry,
                coverageMode = coverageModes[packet.commandIdValue]
    ...
    private fun rrectBuilderCommand(
        commandId: Int,
        paintOrder: Int,
    ...
    private fun pathCapabilities(includeRRect: Boolean = false): GPUCapabilities = GPUCapabilities(
        ...
        snapshotId = if (includeRRect) "preflight-path-rrect" else "preflight-path",
        limits = GPULimits(
            8192,
            256,
            256,
    ...
    private fun GPUTaskList.withCoreSamplePlan(samplePlan: GPUSamplePlan): GPUTaskList = GPUTaskList(
        frameId = frameId,
        capabilitySeal = capabilitySeal,
        recordingSeals = recordingSeals,
        expectedReplayKeyHash = expectedReplayKeyHash,
        tasks = tasks.map { task ->
            ...
            GPUTask.Render(
                ...
                task.phase,
                task.target,
                if (samplePlan == GPUSamplePlan.MultisampleFrame(4)) {
                    GPULoadStorePlan("clear", GPUStorePlan.Store)
                } else {
                    task.loadStore
                },
    ...
    private fun GPUTaskList.withCorePacketRenderStep(
        commandId: Int,
        ...
        tasks = tasks.map { task ->
            ...
            val packets = task.drawPackets.map { packet ->
                if (packet.commandIdValue == commandId) {
                    cloneCorePacket(packet, renderStepId = GPURenderStepID(renderStepIdentity))
                } else {
                    packet
                }
            }
    ...
    private fun renderWith(
        ...
        sampleContinuation: GPUSampleContinuationRequest? = source.sampleContinuation,
    ...
    private fun pathMsaaCapabilities(): GPUCapabilities = pathCapabilities().copy(
        textureFormatSampleSupport = msaaCapabilities(depthStencil = true).textureFormatSampleSupport,
    )
    ...
    private fun cloneCorePacket(
        ...
        renderStepId: GPURenderStepID = packet.renderStepId,
        ...
        dropPreparedAuthority: Boolean = false,
        ...
        targetStateHash: String = packet.targetStateHash,
    ): GPUDrawPacket {
        ...
        if (authority != null && !dropPreparedAuthority) {
    ...
    private fun GPUFrameStep.RenderPassStep.copyCoreRenderForTest(
        sampleContinuation: org.graphiks.kanvas.gpu.renderer.passes.GPUSampleContinuationRequest? =
            this.sampleContinuation,
        depthStencilLoadStore: GPUDepthStencilLoadStorePlan? = this.depthStencilLoadStore,
        targetStateHash: String? = null,
    ): GPUFrameStep.RenderPassStep {
        val replacementPackets = drawPackets.map { packet ->
            targetStateHash?.let { cloneCorePacket(packet, targetStateHash = it) } ?: packet
        }
        return GPUFrameStep.RenderPassStep(
            target,
            loadStore,
            ...
            batches.map { batch ->
                GPUFrameRenderBatch(
                    batch.batchId,
                    batch.kind,
                    batch.packets.mapIndexed { index, _ -> replacementPackets[index] },
                    batch.sourceTaskIds,
                )
            },
            sampleContinuation,
    ...
    private fun GPUCorePrimitiveAnalyticShapeUniformSeal.copyForTest(
        ...
        renderScissor: GPUPixelBounds = this.renderScissor,
    ...
    private fun coreRenderStep(
        ...
        sampleContinuation: org.graphiks.kanvas.gpu.renderer.passes.GPUSampleContinuationRequest? = null,
        depthStencilLoadStore: GPUDepthStencilLoadStorePlan? = null,
        renderStepIdentity: String = CORE_PRIMITIVE_RENDER_STEP_IDENTITY,
    ): GPUFrameStep.RenderPassStep {
        ...
        return GPUFrameStep.RenderPassStep(
            ...
            samplePlan = samplePlan,
            resourceUses = if (
                semantic != null && org.graphiks.kanvas.gpu.renderer.recording.classifyCorePrimitiveDirectNativeRoute(
                    semantic,
                    clipExecutionPlan,
                    blendPlan,
                    samplePlan,
    ...
    private fun preflightGradientEnvelope(
        semantic: GPUDrawSemanticPayload.CorePrimitive,
        renderStepIdentity: String,
    ): GPUFramePreflightResult {
        val structuralKey = corePrimitiveRenderPipelineStructuralKey(
            ...
            coreBlend(GPUBlendMode.SRC_OVER),
            1,
            GPUColorFormat.RGBA8Unorm.corePrimitiveStructuralColorFormat(),
        )
        val events = mutableListOf<String>()
        return preflighter(
            RecordingResourceProvider(events),
            ...
            framePlan(
                listOf(
                    coreDirectPrepare(uniformBytes = 1024L),
                    coreRenderStep(
                        semantic = semantic,
                        bindingLayoutHash = requireNotNull(
                            corePrimitiveGradientBindingLayoutHash(structuralKey.shader),
                        ),
                        renderStepIdentity = renderStepIdentity,
                    ),
                ),
    ...
    private fun preflightPreparedGradientEnvelope(
        material: GPUCorePrimitiveMaterialPayload,
        renderStepIdentity: String,
    ): GPUFramePreflightResult {
        val plan = preparedGradientFramePlan(material, renderStepIdentity)
        val packet = plan.steps.filterIsInstance<GPUFrameStep.RenderPassStep>()
            .single()
            .drawPackets
            .single()
        val seal = requireNotNull(packet.corePrimitivePreparedAuthority?.uniformSlabSeal)
        assertEquals(CORE_PRIMITIVE_GRADIENT_UNIFORM_BYTES.toLong(), seal.plan.slots.single().payloadBytes)
        return preflightGradientPlan(plan, gradientCapabilities(material))
    }

    private fun preflightGradientPlan(
        plan: GPUFramePlan,
        capabilities: GPUCapabilities,
    ): GPUFramePreflightResult {
        val events = mutableListOf<String>()
    ...
    private fun preparedGradientFramePlan(
        material: GPUCorePrimitiveMaterialPayload,
        renderStepIdentity: String,
    ): GPUFramePlan {
        val commandId = 142
        val capabilities = gradientCapabilities(material)
        val base = GPURecorder(
            GPURecordingID("recording.preflight.gradient.$commandId"),
            GPUFrameID(142),
            capabilities,
        ).apply {
            record(pathBuilderCommand(commandId, 0))
        }.close().taskList
            .withCoreClipPlans(mapOf(commandId to GPUClipExecutionPlan.NoClip))
            .withCorePacketRenderStep(commandId, renderStepIdentity)
        val packet = base.tasks.filterIsInstance<GPUTask.Render>().single().drawPackets.single()
        val prepared = GPUCorePrimitivePreparedFrameTaskListBuilder().build(
            GPUCorePrimitivePreparedFrameRequest(
                baseTaskList = base,
                capabilities = capabilities,
                target = GPUFrameTargetRef("target.scene"),
                ...
                semanticsByCommandId = mapOf(
                    commandId to coreSemantic(commandIdValue = commandId, material = material),
                ),
            ),
        )
        val taskList = assertIs<GPUCorePrimitivePreparedFrameResult.Recorded>(
    ...
    private fun preparedSplitGradientFramePlan(capabilities: GPUCapabilities): GPUFramePlan {
        val commands = listOf(
            142 to ("radial.gradient.fill" to radialGradientMaterial()),
            143 to ("sweep.gradient.fill" to sweepGradientMaterial()),
        )
        val base = GPURecorder(
            GPURecordingID("recording.preflight.gradient-split"),
            GPUFrameID(143),
            capabilities,
        ).apply {
            commands.forEachIndexed { paintOrder, (commandId, _) ->
                record(pathBuilderCommand(commandId, paintOrder))
            }
        }.close().taskList
            .withCoreClipPlans(commands.associate { (commandId, _) -> commandId to GPUClipExecutionPlan.NoClip })
            .let { taskList ->
                commands.fold(taskList) { current, (commandId, routeAndMaterial) ->
                    current.withCorePacketRenderStep(commandId, routeAndMaterial.first)
                }
            }
        val prepared = GPUCorePrimitivePreparedFrameTaskListBuilder().build(
            GPUCorePrimitivePreparedFrameRequest(
                ...
                target = GPUFrameTargetRef("target.scene"),
                targetBounds = GPUPixelBounds(0, 0, 4, 4),
                semanticsByCommandId = commands.associate { (commandId, routeAndMaterial) ->
                    commandId to coreSemantic(
                        commandIdValue = commandId,
                        material = routeAndMaterial.second,
                    )
                },
            ),
        )
    ...
    private fun preparedMixedSolidRadialFramePlan(capabilities: GPUCapabilities): GPUFramePlan {
        val commands = listOf(
            142 to (CORE_PRIMITIVE_FILL_RECT_STEP_IDENTITY to GPUCorePrimitiveMaterialPayload.SolidColor(
                listOf(0.25f, 0.5f, 0.75f, 1f),
            )),
            143 to ("radial.gradient.fill" to radialGradientMaterial()),
        )
        val base = GPURecorder(
            GPURecordingID("recording.preflight.solid-radial-split"),
            GPUFrameID(144),
            capabilities,
        ).apply {
            commands.forEachIndexed { paintOrder, (commandId, _) ->
    ...
    private fun gradientCapabilities(vararg materials: GPUCorePrimitiveMaterialPayload): GPUCapabilities =
    ...
    private fun sweepGradientMaterial() = GPUCorePrimitiveMaterialPayload.SweepGradient(
        centerX = 2f,
        centerY = 2f,
        startAngle = 0f,
        endAngle = 360f,
    ...
    private fun GPUDrawSemanticPayload.CorePrimitive.withPayloadRenderStepIdentity(
        renderStepIdentity: String,
    ): GPUDrawSemanticPayload.CorePrimitive = GPUDrawSemanticPayload.CorePrimitive(
        payloadRef = payloadRef.copy(renderStepIdentity = renderStepIdentity),
        sourceFamily = sourceFamily,
        geometry = geometry,
        premultipliedRgba = premultipliedRgba,
        targetBounds = targetBounds,
