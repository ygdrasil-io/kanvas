...
class GPUIntermediatePlanContractsTest {
    @Test
    fun `plan dump exposes ordered destination copy bind render and composite steps`() {
        ...
        val plan = GPUIntermediatePlan(
            ...
            steps = listOf(
                ...
                GPUIntermediatePlanStep.CompositeIntermediate(
                    ...
                    tokenLabel = "compose-token:cmd-7",
                ),
            ),
        ...
        assertEquals(
            listOf(
                "intermediate.plan id=intermediate-plan:screen-blend target=target:main steps=5 diagnostics=none",
                ...
                "intermediate.composite source=intermediate:dst-copy:cmd-7 parent=surface:root blend=srcOver route=composite:layer token=compose-token:cmd-7",
                "intermediate.telemetry destinationReadCopies=1 destinationReadIntermediateBinds=0 copiedBytes=8192 passSplits=1 intermediatesCreated=1 intermediatesReused=0 intermediatesRefused=0 liveIntermediateBytes=8192 layerTargets=0 layerComposites=1 msaaTargets=0 msaaResolves=0",
            ),
            plan.dumpLines(),
        )
    }
    ...
    fun `refusal-only plan dump derives terminal diagnostic in header`() {
        ...
        assertEquals(
            listOf(
                "intermediate.plan id=intermediate-plan:refused target=target:main steps=1 diagnostics=unsupported.destination_read.active_attachment_sampled",
                "intermediate.refused scope=cmd-9 reason=unsupported.destination_read.active_attachment_sampled",
                "intermediate.telemetry destinationReadCopies=0 destinationReadIntermediateBinds=0 copiedBytes=0 passSplits=0 intermediatesCreated=0 intermediatesReused=0 intermediatesRefused=1 liveIntermediateBytes=0 layerTargets=0 layerComposites=0 msaaTargets=0 msaaResolves=0",
            ),
            plan.dumpLines(),
        )
    }
    ...
    @Test
    fun `refusal-only plans require exactly one terminal diagnostic source`() {
        assertFailsWith<IllegalArgumentException> {
            GPUIntermediatePlan(
                planId = "intermediate-plan:duplicate-refusal-signal",
                targetId = "target:main",
                steps = listOf(
                    GPUIntermediatePlanStep.Refuse(
                        scopeLabel = "cmd-10",
                ...
                diagnostics = listOf(
                    GPUIntermediateDiagnostic(
                        code = "unsupported.destination_read.budget",
                        scopeLabel = "cmd-10",
    ...
    @Test
    fun `step payloads and telemetry reject blank labels and negative counters`() {
        assertFailsWith<IllegalArgumentException> {
            GPUIntermediatePlanStep.RenderToTarget(
                commandId = "",
                targetLabel = "surface:main",
    ...
    private fun descriptor(): GPUIntermediateTextureDescriptor =
        GPUIntermediateTextureDescriptor(
            label = "intermediate:dst-copy:cmd-1",
            purpose = GPUIntermediatePurpose.DestinationCopy,
            descriptorHash = "sha256:dst-copy",
            sourceTargetLabel = "surface:main",
            boundsLabel = "bounds:cmd-1",
