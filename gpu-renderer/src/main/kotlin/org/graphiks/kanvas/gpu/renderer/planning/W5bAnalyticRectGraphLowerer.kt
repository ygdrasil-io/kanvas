...
/** W4a supplies immutable geometry and reservations; W5b supplies the ordered color tail. */
internal class W5bAnalyticRectGraphLowerer {
    fun lower(request: GpuPlanLoweringRequest): GpuPlanLoweringResult = try {
        ...
        val draws = graph.passes().filterIsInstance<PlanPass.RenderPass>().flatMap { it.draws() }
            .map { it as AnalyticRectDraw }
        val footprint = (AnalyticRectPlanBudget.calculate(graph.targetExtent, draws.size,
        ...
        val identity = "w5b.w4a.session.${request.deviceGeneration.value}.${bounds.width}x${bounds.height}.rgba8unorm-srgb"
        ...
        val replay = "w5b.w4a:${graph.id.value}"
        ...
        val witness = W5bPreparedFrameWitnessV3(graph, W5bGeometryScratchV3.AnalyticRect(scratch, packets, keys, payloads),
        ...
        val render = GPUTask.Render(GPUTaskID("task.w5b.w4a.${graph.id.value}.packing"), request.recordingId,
