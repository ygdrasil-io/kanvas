...
internal class PdfDocument(
     ...
     * operators :
     *
     *  - [SkPath.Verb.kMoveVerb] → `x y m`
     *  - [SkPath.Verb.kLineVerb] → `x y l`
     *  - [SkPath.Verb.kQuadVerb] → degree-elevated to a cubic
     *    `c1 c2 e c` ; standard formula
     *    `cubic = (p0, p0 + 2/3·(p1−p0), p2 + 2/3·(p1−p2), p2)`.
     *  - [SkPath.Verb.kCubicVerb] → `c1 c2 e c`
     *  - [SkPath.Verb.kConicVerb] → subdivided into 4 quads via De
    ...
    private fun appendPathVerbs(sb: StringBuilder, path: SkPath, pageHeight: Float) {
        ...
        while (true) {
            ...
            when (verb) {
                ...
                SkPath.Verb.kCloseVerb -> sb.append("h\n")
                SkPath.Verb.kDoneVerb -> return
            }
        }
    }
