...
/**
 * Port of Skia's `gm/anisotropic.cpp::AnisotropicGM` (the `kMip`
 * registration — `anisotropic_image_scale_mip`).
 *
 * Mirror of [AnisotropicImageScaleLinearGM]; the only deltas are :
 *  - the source [SkImage] is wrapped via [SkImage.withDefaultMipmaps]
 *    so the LOD-selection plumbing in [org.skia.core.SkBitmapDevice]
 *    can pick a pre-rendered level when the destination is small ;
 *  - the sampler is `SkSamplingOptions(kLinear, kLinear)` — bilinear
 *    in-level filter, kLinear mip-pyramid LOD interpolation.
 *
 * C++ original (`gm/anisotropic.cpp:156`):
 * ```cpp
 * DEF_GM(return new AnisotropicGM(AnisotropicGM::Mode::kMip);)
 * ```
 */
public class AnisotropicImageScaleMipGM : GM() {
    ...
    override fun getName(): String = "anisotropic_image_scale_mip"
    ...
    override fun onOnceBeforeDraw() {
        ...
        // Build the box-filtered mip pyramid up-front so LOD selection
        // in [org.skia.core.SkBitmapDevice.drawImageRect] picks a pre-
        // averaged level under minification.
    ...
    private companion object {
        ...
        val kMipSampling: SkSamplingOptions = SkSamplingOptions(SkFilterMode.kLinear, SkMipmapMode.kLinear)
    }
}
