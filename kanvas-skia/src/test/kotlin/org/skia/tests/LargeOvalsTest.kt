...
class LargeOvalsTest {
    @Test
    fun `LargeOvalsGM matches largeovals_png within tolerance`() {
        val gm = LargeOvalsGM()
        val rendered = TestUtils.runGmTest(gm)
        val reference = TestUtils.loadReferenceBitmap(gm.name())
        assertNotNull(reference, "Missing reference image largeovals.png")
        val comparison = TestUtils.compareBitmapsDetailed(rendered, reference!!, tolerance = 1)
        TestReport.recordDetailed("LargeOvalsGM", comparison)
