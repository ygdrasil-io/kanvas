...
/**
 * R-suivi.19 verification suite for [BmpEncoder].
 *
 * Asserts :
 ...
 *  - the [BmpEncoder.BmpFormat.kBGR_888] code path drops alpha and
...
class BmpEncoderTest {
    ...
    fun `Encode returns non-null bytes starting with 'BM' signature`() {
        ...
        val data = BmpEncoder.Encode(bitmap)
        assertNotNull(data)
    ...
    fun `Encode reports a valid file size in the file header`() {
        val bitmap = makeGradient(4, 4)
        val bytes = BmpEncoder.Encode(bitmap)!!.toByteArray()
    ...
    fun `BGRA round-trip via pure Kotlin BMP codec preserves RGB channels byte-identical`() {
        ...
        val bytes = BmpEncoder.Encode(src)!!.toByteArray()
    ...
    fun `real BMP fixtures re-encode with valid headers and round-trip pixels`() {
        ...
        for (fixture in fixtures) {
            ...
            val encoded = BmpEncoder.Encode(src)!!.toByteArray()
    ...
    fun `BGR_888 format drops alpha and uses 24-bit pixels`() {
        ...
        val bytes = BmpEncoder.Encode(
            src,
            BmpEncoder.Options(format = BmpEncoder.BmpFormat.kBGR_888),
        )!!.toByteArray()
        // Bits per pixel = 24
    ...
    fun `Encode to OutputStream agrees with Encode to SkData`() {
        val src = makeGradient(4, 4)
        val viaData = BmpEncoder.Encode(src)!!.toByteArray()
