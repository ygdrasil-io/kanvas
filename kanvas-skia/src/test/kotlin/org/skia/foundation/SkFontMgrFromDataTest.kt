 ...
 * as `LiberationFontMgr`. The bundled TTFs are valid TrueType and AWT's
 * `Font.createFont(TRUETYPE_FONT, …)` should accept them.
 */
class SkFontMgrFromDataTest {
    ...
    fun `makeFromData with garbage bytes returns null`() {
        val mgr = SkFontMgr.RefDefault()
        // Random bytes that don't look like a TrueType file — AWT's
        // FontFormatException should be caught and surfaced as null.
