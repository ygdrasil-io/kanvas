...
def update_readme(readme: Path) -> None:
    ...
    note = "- M89 registry counters live in `manifest.json` under `m89GmRegistry`; policy-only visibility rows do not count as support and threshold-only misses remain fidelity burn-down scope."
...
def build_manifest_entry(registry: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "notice": "M89 normalizes generated dashboard and policy-only GM visibility rows into support/refusal registry evidence. It does not promote policy-only rows, weaken thresholds, or change render paths.",
    }
