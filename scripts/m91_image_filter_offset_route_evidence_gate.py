#!/usr/bin/env python3
"""Generate and validate M91-IF-3A-ROUTE OffsetImageFilterGM route evidence gate."""
...
ROUTE_DIAG_CPU_JSON = ROOT / "reports/wgsl-pipeline/m91-image-filter-route-diagnostics/routes/skia-gm-offsetimagefilter/route-cpu.json"
ROUTE_DIAG_GPU_JSON = ROOT / "reports/wgsl-pipeline/m91-image-filter-route-diagnostics/routes/skia-gm-offsetimagefilter/route-gpu.json"
ARTIFACT_DIR = ROOT / "reports/wgsl-pipeline/scenes/artifacts/skia-gm-offsetimagefilter"
OUTPUT_DIR = ROOT / "reports/wgsl-pipeline/m91-image-filter-offset-route-evidence-gate"
...
def validate_route_diag(route: dict[str, Any], backend: str) -> None:
    expected_route = (
        "cpu.image-filter.offset-image-filter.expected-unsupported"
        if backend == "CPU"
        else "webgpu.image-filter.offset-image-filter.expected-unsupported"
    )
...
def validate_provenance_gate(gate: dict[str, Any]) -> None:
    require(gate.get("classification") == "image-filter-offset-reference-provenance-gate-no-new-rendering-support", "provenance gate classification changed")
...
def build_summary(route_cpu: dict[str, Any], route_gpu: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schemaVersion": 1,
        "generatedBy": "scripts/m91_image_filter_offset_route_evidence_gate.py",
        "milestone": "M91",
        "ticket": "M91-IF-3A-ROUTE",
...
def main() -> int:
    try:
        ...
        print(f"m91_image_filter_offset_route_evidence_gate: FAIL: {error}", file=sys.stderr)
        return 1
    print(
        "M91 OffsetImageFilterGM route evidence gate validation passed: "
