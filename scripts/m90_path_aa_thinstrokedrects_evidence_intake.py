#!/usr/bin/env python3
"""Generate and validate M90-PAA-3C ThinStrokedRectsGM evidence intake."""
...
ROUTE_CPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-thinstrokedrects/route-cpu.json"
ROUTE_GPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-thinstrokedrects/route-gpu.json"
...
HISTORICAL_WEBGPU_TEST = ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/ThinStrokedRectsWebGpuTest.kt"
HISTORICAL_CROSSBACKEND_TEST = ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/crossbackend/ThinStrokedRectsCrossBackendTest.kt"
OUTPUT_DIR = ROOT / "reports/wgsl-pipeline/m90-path-aa-thinstrokedrects-evidence-intake"
...
EXPECTED_CPU_ROUTE = "cpu.path.thin-stroked-rects.expected-unsupported"
EXPECTED_GPU_ROUTE = "webgpu.path.thin-stroked-rects.expected-unsupported"
EXPECTED_VISIBILITY_CPU_ROUTE = "cpu.path-coverage.thinstrokedrects.expected-unsupported"
EXPECTED_VISIBILITY_GPU_ROUTE = "webgpu.coverage.thinstrokedrects.expected-unsupported"
EXPECTED_VISIBILITY_PIPELINE_KEY = "pathAA=thinStrokedRects source=ThinStrokedRectsGM visibility=policy-only"
...
REQUIRED_EVIDENCE = [
    ("row-specific Skia reference", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/skia.png"),
    ("CPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/route-cpu.json"),
    ("WebGPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/route-gpu.json"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/cpu.png"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/gpu.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/cpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/gpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/stats.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/cpu-performance.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-thinstrokedrects/gpu-performance.json"),
]
NON_CLAIMS = {
...
def candidate_row(readiness: dict[str, Any]) -> dict[str, Any]:
    ...
    require(candidate.get("promotionTicket") == "M90-PAA-3C", f"{ROW_ID}: candidate ticket changed")
...
def historical_signals() -> list[dict[str, Any]]:
    ...
    require("ThinStrokedRectsGM()" in webgpu_test, "ThinStrokedRects WebGPU test no longer instantiates the GM")
    require("ThinStrokedRectsGM()" in crossbackend_test, "ThinStrokedRects crossbackend test no longer instantiates the GM")
    require("94.0" in crossbackend_test, "ThinStrokedRects crossbackend GPU floor changed or missing")
    require("80.0" in crossbackend_test, "ThinStrokedRects crossbackend raster floor changed or missing")
    require(load_property_value(GPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "94.21", "ThinStrokedRectsGM GPU historical score changed or missing")
    require(load_property_value(CPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "88.86848958333333", "ThinStrokedRectsGM CPU historical score changed or missing")
    return [
        {
            "kind": "historical-webgpu-test",
            "path": rel(HISTORICAL_WEBGPU_TEST),
            "promotional": False,
            "reason": "Historical WebGPU ThinStrokedRectsGM test is not a M90 row-specific artifact bundle and does not update the M89 registry row.",
        },
        {
        ...
        },
        {
            ...
            "observed": "ThinStrokedRectsGM=94.21",
            "promotional": False,
        ...
        },
        {
            ...
            "observed": "ThinStrokedRectsGM=88.86848958333333",
            "promotional": False,
...
def build_summary(
    ...
    require(all(item["present"] is False for item in required), "M90-PAA-3C intake found unexpected row-specific artifacts")
    signals = historical_signals()
    return {
        "schemaVersion": 1,
        "generatedBy": "scripts/m90_path_aa_thinstrokedrects_evidence_intake.py",
        "milestone": "M90",
        "ticket": "M90-PAA-3C",
        "classification": "path-aa-thinstrokedrects-evidence-intake-no-new-rendering-support",
...
def validate_summary(summary: dict[str, Any]) -> None:
    require(summary.get("classification") == "path-aa-thinstrokedrects-evidence-intake-no-new-rendering-support", "classification mismatch")
    ...
    require(isinstance(signals, list) and len(signals) == 4, "historicalSignals list changed")
...
def main() -> int:
    try:
        ...
        print(f"m90_path_aa_thinstrokedrects_evidence_intake: FAIL: {error}", file=sys.stderr)
        return 1
    print("M90 Path AA ThinStrokedRects evidence intake validation passed: missingEvidenceItems=10 newSupportClaims=0 readinessDelta=0.0")
    return 0
