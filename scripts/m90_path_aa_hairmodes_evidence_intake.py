#!/usr/bin/env python3
"""Generate and validate M90-PAA-3F HairModesGM evidence intake."""
...
ROUTE_CPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-hairmodes/route-cpu.json"
ROUTE_GPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-hairmodes/route-gpu.json"
...
HISTORICAL_CPU_TEST = ROOT / "skia-integration-tests/src/test/kotlin/org/skia/tests/Round13Test.kt"
HISTORICAL_GM_SOURCE = ROOT / "skia-integration-tests/src/main/kotlin/org/skia/tests/HairModesGM.kt"
OUTPUT_DIR = ROOT / "reports/wgsl-pipeline/m90-path-aa-hairmodes-evidence-intake"
SUMMARY_JSON = OUTPUT_DIR / "summary.json"
SUMMARY_MD = OUTPUT_DIR / "summary.md"

ROW_ID = "skia-gm-hairmodes"
SOURCE_GM = "HairModesGM"
FALLBACK = "coverage.hairmode.row-specific-artifacts-required"
EXPECTED_CPU_ROUTE = "cpu.path.hairmode.expected-unsupported"
EXPECTED_GPU_ROUTE = "webgpu.path.hairmode.expected-unsupported"
EXPECTED_VISIBILITY_CPU_ROUTE = "cpu.path-coverage.hairmodes.expected-unsupported"
EXPECTED_VISIBILITY_GPU_ROUTE = "webgpu.coverage.hairmodes.expected-unsupported"
EXPECTED_VISIBILITY_PIPELINE_KEY = "pathAA=hairModes source=HairModesGM visibility=policy-only"
EXPECTED_SHAPE_FACTS = [
    "hairline stroke",
    "blend/paint mode interaction",
    "AA edge facts",
]
...
REQUIRED_EVIDENCE = [
    ("row-specific Skia reference", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/skia.png"),
    ("CPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/route-cpu.json"),
    ("WebGPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/route-gpu.json"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/cpu.png"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/gpu.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/cpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/gpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/stats.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/cpu-performance.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-hairmodes/gpu-performance.json"),
]
NON_CLAIMS = {
...
def candidate_row(readiness: dict[str, Any]) -> dict[str, Any]:
    ...
    require(candidate.get("promotionTicket") == "M90-PAA-3F", f"{ROW_ID}: candidate ticket changed")
...
def visibility_row(visibility: dict[str, Any]) -> dict[str, Any]:
    ...
    require("feature.blend" in scene.get("tags", []), "visibility row missing blend tag")
    return scene
...
def historical_signals() -> list[dict[str, Any]]:
    ...
    require('runGm(HairModesGM(), "HairModesGM", 60.0)' in cpu_test, "HairModes CPU test no longer instantiates the GM")
    require("private fun runGm" in cpu_test, "HairModes CPU helper changed or missing")
    require("TestUtils.compareBitmapsDetailed(rendered, reference!!, tolerance = 1)" in cpu_test, "HairModes CPU tolerance changed or missing")
    require("public class HairModesGM" in gm_source, "HairModesGM source class changed or missing")
    require('override fun getName(): String = "hairmodes"' in gm_source, "HairModesGM name changed")
    require("SkBlendMode.kSrcOver" in gm_source, "HairModesGM blend-mode table changed or missing")
    require("canvas.drawLine" in gm_source, "HairModesGM hairline draw path changed or missing")
    require(load_property_value(CPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "72.19368489583333", "HairModesGM CPU historical score changed or missing")
    require(load_property_value(CPU_RASTER_SIMILARITY_PROPERTIES, SOURCE_GM) == "72.19368489583333", "HairModesGM cpu-raster score changed or missing")
    require(load_property_value(KANVAS_SKIA_SIMILARITY_PROPERTIES, SOURCE_GM) == "72.19368489583333", "HairModesGM kanvas-skia score changed or missing")
    return [
        {
            "kind": "historical-cpu-test",
            "path": rel(HISTORICAL_CPU_TEST),
            "promotional": False,
            "reason": "Historical CPU HairModesGM test is not a M90 row-specific CPU/WebGPU artifact bundle and does not update the M89 registry row.",
        },
        {
        ...
        },
        {
            ...
            "observed": "HairModesGM=72.19368489583333",
            "promotional": False,
...
def build_summary(
    ...
    require(all(item["present"] is False for item in required), "M90-PAA-3F intake found unexpected row-specific artifacts")
    signals = historical_signals()
    return {
        "schemaVersion": 1,
        "generatedBy": "scripts/m90_path_aa_hairmodes_evidence_intake.py",
        "milestone": "M90",
        "ticket": "M90-PAA-3F",
        "classification": "path-aa-hairmodes-evidence-intake-no-new-rendering-support",
...
def validate_summary(summary: dict[str, Any]) -> None:
    require(summary.get("classification") == "path-aa-hairmodes-evidence-intake-no-new-rendering-support", "classification mismatch")
    ...
    require(counters.get("historicalSignals") == 5, "historical signal count changed")
    ...
    require(isinstance(signals, list) and len(signals) == 5, "historicalSignals list changed")
...
def main() -> int:
    try:
        ...
        print(f"m90_path_aa_hairmodes_evidence_intake: FAIL: {error}", file=sys.stderr)
        return 1
    print("M90 Path AA HairModes evidence intake validation passed: missingEvidenceItems=10 newSupportClaims=0 readinessDelta=0.0")
    return 0
