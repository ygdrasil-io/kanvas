#!/usr/bin/env python3
"""Generate and validate M90-PAA-3G ScaledStrokesGM evidence intake."""
...
ROUTE_CPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-scaledstrokes/route-cpu.json"
ROUTE_GPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-scaledstrokes/route-gpu.json"
...
HISTORICAL_CPU_TEST = ROOT / "skia-integration-tests/src/test/kotlin/org/skia/tests/ScaledStrokesTest.kt"
HISTORICAL_GM_SOURCE = ROOT / "skia-integration-tests/src/main/kotlin/org/skia/tests/ScaledStrokesGM.kt"
HISTORICAL_WEBGPU_TEST = ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/ScaledStrokesWebGpuTest.kt"
HISTORICAL_CROSSBACKEND_TEST = ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/crossbackend/ScaledStrokesCrossBackendTest.kt"
OUTPUT_DIR = ROOT / "reports/wgsl-pipeline/m90-path-aa-scaledstrokes-evidence-intake"
SUMMARY_JSON = OUTPUT_DIR / "summary.json"
SUMMARY_MD = OUTPUT_DIR / "summary.md"

ROW_ID = "skia-gm-scaledstrokes"
SOURCE_GM = "ScaledStrokesGM"
FALLBACK = "coverage.scaled-stroke.row-specific-artifacts-required"
EXPECTED_CPU_ROUTE = "cpu.path.scaled-stroke.expected-unsupported"
EXPECTED_GPU_ROUTE = "webgpu.path.scaled-stroke.expected-unsupported"
EXPECTED_VISIBILITY_CPU_ROUTE = "cpu.path-coverage.scaledstrokes.expected-unsupported"
EXPECTED_VISIBILITY_GPU_ROUTE = "webgpu.coverage.scaledstrokes.expected-unsupported"
EXPECTED_VISIBILITY_PIPELINE_KEY = "pathAA=scaledStrokes source=ScaledStrokesGM visibility=policy-only"
...
REQUIRED_EVIDENCE = [
    ("row-specific Skia reference", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/skia.png"),
    ("CPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/route-cpu.json"),
    ("WebGPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/route-gpu.json"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/cpu.png"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/gpu.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/cpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/gpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/stats.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/cpu-performance.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-scaledstrokes/gpu-performance.json"),
]
NON_CLAIMS = {
...
def candidate_row(readiness: dict[str, Any]) -> dict[str, Any]:
    ...
    require(candidate.get("promotionTicket") == "M90-PAA-3G", f"{ROW_ID}: candidate ticket changed")
...
def visibility_row(visibility: dict[str, Any]) -> dict[str, Any]:
    ...
    require("feature.transform" in scene.get("tags", []), "visibility row missing transform tag")
    return scene
...
def historical_signals() -> list[dict[str, Any]]:
    ...
    webgpu_test = HISTORICAL_WEBGPU_TEST.read_text(encoding="utf-8")
    crossbackend_test = HISTORICAL_CROSSBACKEND_TEST.read_text(encoding="utf-8")
    require("val gm = ScaledStrokesGM()" in cpu_test, "ScaledStrokes CPU test no longer instantiates the GM")
    require('TestReport.recordDetailed("ScaledStrokesGM", comparison)' in cpu_test, "ScaledStrokes CPU report key changed")
    require("comparison.similarity >= 85.0" in cpu_test, "ScaledStrokes CPU floor changed or missing")
    require("public class ScaledStrokesGM" in gm_source, "ScaledStrokesGM source class changed or missing")
    require('override fun getName(): String = "scaledstrokes"' in gm_source, "ScaledStrokesGM name changed")
    require('runGpuCrossTest(ScaledStrokesGM(), floor = 96.44, logTag = "ScaledStrokesWebGpu")' in webgpu_test, "ScaledStrokes WebGPU floor changed or missing")
    require("gm = ScaledStrokesGM()" in crossbackend_test, "ScaledStrokes crossbackend GM changed or missing")
    require("rasterFloor = 96.36" in crossbackend_test, "ScaledStrokes crossbackend raster floor changed or missing")
    require("gpuFloor = 96.44" in crossbackend_test, "ScaledStrokes crossbackend GPU floor changed or missing")
    require(load_property_value(GPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "96.49", "ScaledStrokesGM GPU historical score changed or missing")
    require(load_property_value(CPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "96.02783203125", "ScaledStrokesGM CPU historical score changed or missing")
    require(load_property_value(CPU_RASTER_SIMILARITY_PROPERTIES, SOURCE_GM) == "96.02783203125", "ScaledStrokesGM cpu-raster score changed or missing")
    require(load_property_value(KANVAS_SKIA_SIMILARITY_PROPERTIES, SOURCE_GM) == "96.05859375", "ScaledStrokesGM kanvas-skia score changed or missing")
    return [
        {
            "kind": "historical-cpu-test",
            "path": rel(HISTORICAL_CPU_TEST),
            "promotional": False,
            "reason": "Historical CPU ScaledStrokesGM test is not a M90 row-specific CPU/WebGPU artifact bundle and does not update the M89 registry row.",
        },
        {
        ...
        },
        {
            ...
            "promotional": False,
            "reason": "Historical WebGPU ScaledStrokesGM test is not a M90 row-specific artifact bundle and does not update route status.",
        },
        {
        ...
        },
        {
            ...
            "observed": "ScaledStrokesGM=96.49",
            "promotional": False,
        ...
        },
        {
            ...
            "observed": "ScaledStrokesGM=96.02783203125",
            "promotional": False,
        ...
        },
        {
            ...
            "observed": "ScaledStrokesGM=96.05859375",
            "promotional": False,
...
def build_summary(
    ...
    require(all(item["present"] is False for item in required), "M90-PAA-3G intake found unexpected row-specific artifacts")
    signals = historical_signals()
    return {
        "schemaVersion": 1,
        "generatedBy": "scripts/m90_path_aa_scaledstrokes_evidence_intake.py",
        "milestone": "M90",
        "ticket": "M90-PAA-3G",
        "classification": "path-aa-scaledstrokes-evidence-intake-no-new-rendering-support",
...
def validate_summary(summary: dict[str, Any]) -> None:
    require(summary.get("classification") == "path-aa-scaledstrokes-evidence-intake-no-new-rendering-support", "classification mismatch")
...
def main() -> int:
    try:
        ...
        print(f"m90_path_aa_scaledstrokes_evidence_intake: FAIL: {error}", file=sys.stderr)
        return 1
    print("M90 Path AA ScaledStrokes evidence intake validation passed: missingEvidenceItems=10 newSupportClaims=0 readinessDelta=0.0")
    return 0
