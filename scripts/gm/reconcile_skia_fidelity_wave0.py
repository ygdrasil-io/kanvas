...
def _dashboard_classification(row):
    ...
    if row.get("renderFailed"):
        return "failure"
    if row.get("noReference") or cause == "reference-missing":
        return "missing-reference"
    if row.get("sizeMismatch") or cause == "size-mismatch":
        return "size-mismatch"
    if row.get("referenceUntrustable") or cause == "reference-untrustable":
        return "reference-untrustable"
...
def render_markdown(delta: dict) -> str:
    ...
    file_metadata = delta.get("inputs", {}).get("fileMetadata", {})
    metadata_rows = (
        sorted(file_metadata.items())
        if isinstance(file_metadata, dict)
        else []
    )
    metadata_rows = [
        (name, metadata)
        for name, metadata in metadata_rows
    ...
    lines.extend(
        [
            ...
            "## CLI Provenance",
            "",
            "- generatedBy: `%s`" % _markdown_value(delta.get("generatedBy", "")),
            "- sourceCommit: `%s`" % _markdown_value(delta.get("sourceCommit", "")),
            "",
            "| Input | Path | SHA-256 |",
            "| --- | --- | --- |",
    ...
    lines.extend(
        [
            ...
            "",
            "- Content above this marker is CLI-generated by `reconcile_skia_fidelity_wave0.py`.",
            "- Evidence-owner additions are outside the CLI output and must remain below this marker.",
            "<!-- evidence-owner additions begin below this marker; not CLI-generated -->",
        ]
    )
    return "\n".join(lines) + "\n"
...
def _check_violations(runner, svg, missing):
    ...
    for lane, result in (("skia", runner), ("svg", svg)):
        ...
        terminal_rows = [
            ...
            if row.get("outcome") in {"failure", "error"} and row.get("terminal")
        ]
        if terminal_rows:
            terminal_codes = sorted(
                {row.get("failureCode") or "unknown" for row in terminal_rows}
            )
            violations.append(
                "%s has %s unexpected terminal failure/error testcase(s): %s"
                % (lane, len(terminal_rows), ", ".join(terminal_codes))
            )
