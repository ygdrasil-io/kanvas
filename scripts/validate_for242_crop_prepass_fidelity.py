#!/usr/bin/env python3
"""Validate FOR-242 crop image-filter pre-pass fidelity classification."""
...
def fail(message: str) -> None:
    raise SystemExit(f"FOR-242 validation failed: {message}")
...
def require_text(owner: dict[str, Any], field: str, expected: str | None = None) -> str:
    value = owner.get(field)
    if not isinstance(value, str) or not value:
        fail(f"missing text field `{field}`")
...
def main() -> None:
    ...
    if gpu_similarity < 98.0:
        fail(f"GPU route regressed below the FOR-242 floor: {gpu_similarity:.2f}%")
    if cpu_similarity > gpu_similarity:
        fail("CPU/reference similarity unexpectedly exceeds GPU/reference similarity")
    ...
    require_text(prepass, "destination", "SkWebGpuDevice.cropNonNullOffsetChildPrePassScratch")
    ...
    if gpu_similarity < STRICT_THRESHOLD or cpu_similarity < STRICT_THRESHOLD:
        ...
        if scene.get("status") != "pass":
            fail("bounded pre-pass route should remain pass while the route itself is supported")

    report = PROJECT_ROOT / "reports/wgsl-pipeline/2026-06-03-for-242-crop-image-filter-prepass-fidelity.md"
    if not report.is_file():
        fail("missing FOR-242 fidelity report")

    print(
        "FOR-242 crop pre-pass fidelity classification OK: "
