...
def require_row_evidence() -> None:
    ...
    require(
        evidence.get("statusCountsAfterFor467") == {"diagnostic-only": 2, "expected-unsupported": 3, "supported": 7},
        "FOR-467 snapshot status counts mismatch",
    )
    ...
    require(not any("For468" in key for key in row.get("nonClaims", {})), "FOR-467 evidence must not include future-ticket non-claims")
    for key in FEATURE_NON_CLAIMS:
