#!/usr/bin/env python3
"""Validate the FOR-471 skia-gm-rectpolystroke row-specific refusal evidence."""
...
LINEAR = "FOR-471"
ROW_ID = "skia-gm-rectpolystroke"
FALLBACK_REASON = "coverage.rectpolystroke.row-specific-artifacts-required"
VISIBILITY = ROOT / "reports/wgsl-pipeline/scenes/generated/d50-gm-dashboard-visibility.json"
ROW_EVIDENCE = ROOT / "reports/wgsl-pipeline/scenes/generated/for471-skia-gm-rectpolystroke-evidence.json"
ROW_REPORT = ROOT / "reports/wgsl-pipeline/2026-06-08-for-471-skia-gm-rectpolystroke-evidence.md"

FEATURE_NON_CLAIMS = (
    "broadPathAaSupportClaimAddedByFor471",
    "broadStrokeSupportClaimAddedByFor471",
    "capJoinDashSupportClaimAddedByFor471",
    "edgeBudgetSupportClaimAddedByFor471",
    "pathStrokerSupportClaimAddedByFor471",
    "rectPolyStrokeEvidenceInherited",
)
GLOBAL_NON_CLAIMS = (
    "dashboardRowAddedByFor471",
    "dashboardStatusChangedByFor471",
    "fallbackPolicyChanged",
    "pipelineKeyChanged",
    "productionCodeChanged",
    "scoringChanged",
    "skiaComparableClaimAddedByFor471",
    "supportClaimAddedByFor471",
    "thresholdChanged",
...
def require_visibility_row() -> None:
    ...
    require(row.get("cpuRoute") == "cpu.path.rectpolystroke.expected-unsupported", "D50 CPU route mismatch")
    require(row.get("gpuRoute") == "webgpu.path.rectpolystroke.expected-unsupported", "D50 GPU route mismatch")
    require("does not claim RectPolyStrokeGM" in row.get("nonClaim", ""), "D50 non-claim must name RectPolyStrokeGM")
    require("Path AA support" in row.get("nonClaim", ""), "D50 non-claim must preserve Path AA refusal")
...
def require_row_evidence() -> None:
    ...
    require(row.get("cpu", {}).get("route") == "cpu.path.rectpolystroke.expected-unsupported", "CPU route mismatch")
    require(row.get("gpu", {}).get("status") == "expected-unsupported", "GPU must remain expected-unsupported")
    require(row.get("gpu", {}).get("route") == "webgpu.path.rectpolystroke.expected-unsupported", "GPU route mismatch")
    require(row.get("diffStats", {}).get("status") == "not-computed", "diff/stat must remain not-computed")

    provenance = row.get("rectPolyStrokeProvenance")
    require(isinstance(provenance, dict), "RectPolyStroke provenance must be recorded")
    require(provenance.get("scene") == "RectPolyStrokeGM", "scene provenance mismatch")
    require(provenance.get("kotlinSource") == "skia-integration-tests/src/main/kotlin/org/skia/tests/RectPolyStrokeGM.kt", "Kotlin source provenance mismatch")
    require(provenance.get("upstreamSource") == "gm/rect_poly_stroke.cpp", "upstream source provenance mismatch")
    require("48 cells" in provenance.get("cellMatrix", ""), "cell matrix provenance mismatch")
    require("97.4320415879017" in provenance.get("historicalSimilarity", ""), "historical similarity provenance mismatch")
    ...
    cross_backend_tests = set(provenance.get("crossBackendTests", []))
    require(
        {
            "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/RectPolyStrokeWebGpuTest.kt",
            "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/crossbackend/RectPolyStrokeCrossBackendTest.kt",
        }
        <= cross_backend_tests,
    ...
    require(
        {
            ...
            "historical WebGPU/cross-backend evidence inheritance",
        }
        <= unsupported,
    ...
    require(route_diagnostics.get("cpu") == "cpu.path.rectpolystroke.expected-unsupported", "route diagnostics CPU mismatch")
    require(route_diagnostics.get("gpu") == "webgpu.path.rectpolystroke.expected-unsupported", "route diagnostics GPU mismatch")
    ...
    require("without threshold, scoring, edge-budget, or fallback policy changes" in route_policy, "route policy must preserve threshold/scoring/edge budget")
    for key in FEATURE_NON_CLAIMS + GLOBAL_NON_CLAIMS:
...
def require_report() -> None:
    ...
    for required in (
        ...
        "ne sont pas herites",
        "48 cellules",
        "RectPolyStrokeGM=97.4320415879017",
        ...
        "0 support ajoute",
        "Aucun support large Path AA, stroke large, cap/join/dash, path stroker parity, edge-budget promotion ou heritage de preuve WebGPU/cross-backend",
        "sans augmenter l'edge budget",
