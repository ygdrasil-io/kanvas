#!/usr/bin/env python3
"""Generate and validate M90-PAA-3E StrokeRectsGM evidence intake."""
...
ROUTE_CPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-strokerects/route-cpu.json"
ROUTE_GPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-strokerects/route-gpu.json"
...
HISTORICAL_CPU_TEST = ROOT / "skia-integration-tests/src/test/kotlin/org/skia/tests/StrokeRectsTest.kt"
HISTORICAL_GM_SOURCE = ROOT / "skia-integration-tests/src/main/kotlin/org/skia/tests/StrokeRectsGM.kt"
HISTORICAL_WEBGPU_TEST = ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/StrokeRectsWebGpuTest.kt"
HISTORICAL_CROSSBACKEND_TEST = ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/crossbackend/StrokeRectsCrossBackendTest.kt"
OUTPUT_DIR = ROOT / "reports/wgsl-pipeline/m90-path-aa-strokerects-evidence-intake"
...
EXPECTED_CPU_ROUTE = "cpu.path.stroke-rects.expected-unsupported"
EXPECTED_GPU_ROUTE = "webgpu.path.stroke-rects.expected-unsupported"
EXPECTED_VISIBILITY_CPU_ROUTE = "cpu.path-coverage.strokerects.expected-unsupported"
EXPECTED_VISIBILITY_GPU_ROUTE = "webgpu.coverage.strokerects.expected-unsupported"
EXPECTED_VISIBILITY_PIPELINE_KEY = "pathAA=strokeRects source=StrokeRectsGM visibility=policy-only"
EXPECTED_SHAPE_FACTS = [
    "multiple rect strokes",
    "join facts",
    "stroke width",
]
...
REQUIRED_EVIDENCE = [
    ("row-specific Skia reference", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/skia.png"),
    ("CPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/route-cpu.json"),
    ("WebGPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/route-gpu.json"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/cpu.png"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/gpu.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/cpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/gpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/stats.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/cpu-performance.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokerects/gpu-performance.json"),
]
NON_CLAIMS = {
...
def candidate_row(readiness: dict[str, Any]) -> dict[str, Any]:
    ...
    require(candidate.get("promotionTicket") == "M90-PAA-3E", f"{ROW_ID}: candidate ticket changed")
...
def historical_signals() -> list[dict[str, Any]]:
    ...
    require("val gm = StrokeRectsGM()" in cpu_test, "StrokeRects CPU test no longer instantiates the GM")
    require('TestReport.recordDetailed("StrokeRectsGM", comparison)' in cpu_test, "StrokeRects CPU report key changed")
    require("comparison.similarity >= 80.0" in cpu_test, "StrokeRects CPU floor changed or missing")
    require("public class StrokeRectsGM" in gm_source, "StrokeRectsGM source class changed or missing")
    require('override fun getName(): String = "strokerects"' in gm_source, "StrokeRectsGM name changed")
    require("runGpuCrossTest(StrokeRectsGM(), floor = 99.81)" in webgpu_test, "StrokeRects WebGPU floor changed or missing")
    require("gm = StrokeRectsGM()" in crossbackend_test, "StrokeRects crossbackend GM changed or missing")
    require("rasterFloor = 80.0" in crossbackend_test, "StrokeRects crossbackend raster floor changed or missing")
    require("gpuFloor = 99.81" in crossbackend_test, "StrokeRects crossbackend GPU floor changed or missing")
    require("rasterTolerance = 1" in crossbackend_test, "StrokeRects crossbackend raster tolerance changed or missing")
    require(load_property_value(GPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "99.86", "StrokeRectsGM GPU historical score changed or missing")
    require(load_property_value(CPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "92.0928125", "StrokeRectsGM CPU historical score changed or missing")
    require(load_property_value(CPU_RASTER_SIMILARITY_PROPERTIES, SOURCE_GM) == "92.0928125", "StrokeRectsGM cpu-raster score changed or missing")
    require(load_property_value(KANVAS_SKIA_SIMILARITY_PROPERTIES, SOURCE_GM) == "92.0928125", "StrokeRectsGM kanvas-skia score changed or missing")
    return [
        {
            "kind": "historical-cpu-test",
            "path": rel(HISTORICAL_CPU_TEST),
            "promotional": False,
            "reason": "Historical CPU StrokeRectsGM test is not a M90 row-specific CPU/WebGPU artifact bundle and does not update the M89 registry row.",
        },
        {
        ...
        },
        {
            ...
            "promotional": False,
            "reason": "Historical WebGPU StrokeRectsGM test is not a M90 row-specific artifact bundle and does not update route status.",
        },
        {
        ...
        },
        {
            ...
            "observed": "StrokeRectsGM=92.0928125",
            "promotional": False,
...
def build_summary(
    ...
    require(all(item["present"] is False for item in required), "M90-PAA-3E intake found unexpected row-specific artifacts")
    signals = historical_signals()
    return {
        "schemaVersion": 1,
        "generatedBy": "scripts/m90_path_aa_strokerects_evidence_intake.py",
        "milestone": "M90",
        "ticket": "M90-PAA-3E",
        "classification": "path-aa-strokerects-evidence-intake-no-new-rendering-support",
...
def validate_summary(summary: dict[str, Any]) -> None:
    require(summary.get("classification") == "path-aa-strokerects-evidence-intake-no-new-rendering-support", "classification mismatch")
...
def main() -> int:
    try:
        ...
        print(f"m90_path_aa_strokerects_evidence_intake: FAIL: {error}", file=sys.stderr)
        return 1
    print("M90 Path AA StrokeRects evidence intake validation passed: missingEvidenceItems=10 newSupportClaims=0 readinessDelta=0.0")
    return 0
