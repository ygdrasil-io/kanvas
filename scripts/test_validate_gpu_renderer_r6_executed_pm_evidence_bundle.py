...
class GpuRendererR6ExecutedPmEvidenceValidatorTest(unittest.TestCase):
    ...
    def test_root_build_keeps_retired_pm_bundle_absent(self) -> None:
        retired_task_name = "pipeline" + "PmBundle"
        build_gradle = (PROJECT_ROOT / "build.gradle.kts").read_text(encoding="utf-8")

        self.assertNotIn(retired_task_name, build_gradle)
        validator.validate_retired_root_bundle(PROJECT_ROOT)

        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            (root / "build.gradle.kts").write_text(
                f'tasks.register("{retired_task_name}") {{}}\n',
                encoding="utf-8",
            )

            with self.assertRaisesRegex(validator.ValidationError, "retired root PM bundle"):
                validator.validate_retired_root_bundle(root)
