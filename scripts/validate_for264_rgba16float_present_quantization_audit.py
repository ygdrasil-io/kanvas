#!/usr/bin/env python3
"""Validate FOR-264 RGBA16Float present byte quantization audit evidence."""
...
PROBE_NAME = "rgba16float-present-quantization-audit-for264.json"
ARTIFACT_ROOT = (
    PROJECT_ROOT
...
TEST = PROJECT_ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/For264Rgba16FloatPresentQuantizationAuditTest.kt"
REPORT = PROJECT_ROOT / "reports/wgsl-pipeline/2026-06-03-for-264-rgba16float-present-quantization-audit.md"
...
def fail(message: str) -> None:
    raise SystemExit(f"FOR-264 validation failed: {message}")
...
def validate_probe(path: Path) -> None:
    ...
    require_text(probe, "linear", "FOR-264")
    require_text(probe, "probe", "rgba16float-present-byte-quantization-audit")
...
def main() -> None:
    ...
    if generated_probe != static_probe:
        fail("generated and static FOR-264 probe JSON differ")
    ...
    for forbidden in (
        "FOR264",
        "for264",
        "kanvas.webgpu.for264",
        "targetColorSpaceBlend: Boolean = true",
        "correctionApplied = true",
    ):
        if forbidden in device_text:
            fail(f"FOR-264 must not add production switches or corrections: `{forbidden}`")

    require_file_text(
        TEST,
        [
            "FOR-264 RGBA16Float present byte quantization audit stays diagnostic",
    ...
    print(
        "FOR-264 validation passed: RGBA16Float store-to-present byte "
        "quantization is audited for FOR-261 residuals, an exact control, and a "
        "targetColorSpaceBlend-sensitive case; the result stays diagnostic, no "
        "production policy changed, and Crop diagnostics are preserved."
    )
