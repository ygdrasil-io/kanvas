#!/usr/bin/env python3
"""Generate M70-M72 Kadre live runtime PM evidence."""
...
def status_from_demo(native_demo: dict[str, Any]) -> tuple[str, str]:
    ...
    if (
        ...
        return "native-runnable", "m72.kadre-single-scene-replay-presented-frames"
    if (
        ...
        return "degraded", "m72.kadre-single-scene-replay-present-call-completed-timeout-only"
...
def build_route(project_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claimLevel": "single-scene-replay-contract" if status == "native-runnable" else "selected-present-call-demo" if status == "degraded" else "blocked",
...
def write_markdown(path: Path, route: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "The demo is still deliberately narrow: it renders one selected replay scene contract in a Kadre native WebGPU window and emits reporting-only telemetry.",
        "",
        "## PM Outcome",
        "",
