#!/usr/bin/env python3
"""Generate and validate M90-PAA-3H DashingGM evidence intake."""
...
ROUTE_CPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-dashing/route-cpu.json"
ROUTE_GPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-dashing/route-gpu.json"
...
HISTORICAL_CPU_TEST = ROOT / "skia-integration-tests/src/test/kotlin/org/skia/tests/DashingTest.kt"
HISTORICAL_GM_SOURCE = ROOT / "skia-integration-tests/src/main/kotlin/org/skia/tests/DashingGM.kt"
HISTORICAL_CROSSBACKEND_TEST = ROOT / "gpu-raster/src/test/kotlin/org/skia/gpu/webgpu/crossbackend/DashingCrossBackendTest.kt"
OUTPUT_DIR = ROOT / "reports/wgsl-pipeline/m90-path-aa-dashing-evidence-intake"
...
EXPECTED_CPU_ROUTE = "cpu.path.dashing.expected-unsupported"
EXPECTED_GPU_ROUTE = "webgpu.path.dashing.expected-unsupported"
EXPECTED_VISIBILITY_CPU_ROUTE = "cpu.path-coverage.dashing.expected-unsupported"
EXPECTED_VISIBILITY_GPU_ROUTE = "webgpu.coverage.dashing.expected-unsupported"
EXPECTED_VISIBILITY_PIPELINE_KEY = "pathAA=dashing source=DashingGM visibility=policy-only"
...
REQUIRED_EVIDENCE = [
    ("row-specific Skia reference", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/skia.png"),
    ("CPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/route-cpu.json"),
    ("WebGPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/route-gpu.json"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/cpu.png"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/gpu.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/cpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/gpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/stats.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/cpu-performance.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-dashing/gpu-performance.json"),
]
NON_CLAIMS = {
...
def candidate_row(readiness: dict[str, Any]) -> dict[str, Any]:
    ...
    require(candidate.get("promotionTicket") == "M90-PAA-3H", f"{ROW_ID}: candidate ticket changed")
...
def historical_signals() -> list[dict[str, Any]]:
    ...
    require("val gm = DashingGM()" in cpu_test, "Dashing CPU test no longer instantiates the GM")
    require('TestReport.recordDetailed("DashingGM", comparison)' in cpu_test, "Dashing CPU report key changed")
    require("comparison.similarity >= 50.0" in cpu_test, "Dashing CPU floor changed or missing")
    require("public class DashingGM" in gm_source, "DashingGM source class changed or missing")
    require('override fun getName(): String = "dashing"' in gm_source, "DashingGM name changed")
    require("SkDashPathEffect.Make" in gm_source, "DashingGM dash path effect source changed or missing")
    require("finalX = 20_000f" in gm_source, "DashingGM giant dash case changed or missing")
    require("gm = DashingGM()" in crossbackend_test, "Dashing crossbackend GM changed or missing")
    require("rasterFloor = 96.03" in crossbackend_test, "Dashing crossbackend raster floor changed or missing")
    require("gpuFloor = 96.75" in crossbackend_test, "Dashing crossbackend GPU floor changed or missing")
    require(load_property_value(GPU_SIMILARITY_PROPERTIES, "DashingGM-gpu") == "96.80", "DashingGM GPU historical score changed or missing")
    require(load_property_value(GPU_SIMILARITY_PROPERTIES, "DashingGM-raster") == "96.08", "DashingGM raster historical score changed or missing")
    require(load_property_value(CPU_SIMILARITY_PROPERTIES, SOURCE_GM) == "96.06985294117646", "DashingGM CPU historical score changed or missing")
    require(load_property_value(CPU_RASTER_SIMILARITY_PROPERTIES, SOURCE_GM) == "96.06985294117646", "DashingGM cpu-raster score changed or missing")
    require(load_property_value(KANVAS_SKIA_SIMILARITY_PROPERTIES, SOURCE_GM) == "96.06985294117646", "DashingGM kanvas-skia score changed or missing")
    return [
        {
            "kind": "historical-cpu-test",
            "path": rel(HISTORICAL_CPU_TEST),
            "promotional": False,
            "reason": "Historical CPU DashingGM test is not a M90 row-specific CPU/WebGPU artifact bundle and does not update the M89 registry row.",
        },
        {
        ...
        },
        {
            ...
            "observed": "DashingGM-gpu=96.80; DashingGM-raster=96.08",
            "promotional": False,
        ...
        },
        {
            ...
            "observed": "DashingGM=96.06985294117646",
            "promotional": False,
...
def build_summary(
    ...
    require(all(item["present"] is False for item in required), "M90-PAA-3H intake found unexpected row-specific artifacts")
    signals = historical_signals()
    return {
        "schemaVersion": 1,
        "generatedBy": "scripts/m90_path_aa_dashing_evidence_intake.py",
        "milestone": "M90",
        "ticket": "M90-PAA-3H",
        "classification": "path-aa-dashing-evidence-intake-no-new-rendering-support",
...
def validate_summary(summary: dict[str, Any]) -> None:
    require(summary.get("classification") == "path-aa-dashing-evidence-intake-no-new-rendering-support", "classification mismatch")
...
def main() -> int:
    try:
        ...
        print(f"m90_path_aa_dashing_evidence_intake: FAIL: {error}", file=sys.stderr)
        return 1
    print("M90 Path AA Dashing evidence intake validation passed: missingEvidenceItems=10 newSupportClaims=0 readinessDelta=0.0")
    return 0
