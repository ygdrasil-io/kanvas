#!/usr/bin/env python3
"""Generate and validate M90-PAA-3D StrokedLinesGM evidence intake."""
...
ROUTE_CPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-strokedlines/route-cpu.json"
ROUTE_GPU_JSON = ROOT / "reports/wgsl-pipeline/m90-path-aa-route-diagnostics/routes/skia-gm-strokedlines/route-gpu.json"
...
HISTORICAL_CPU_TEST = ROOT / "skia-integration-tests/src/test/kotlin/org/skia/tests/StrokedLinesTest.kt"
HISTORICAL_GM_SOURCE = ROOT / "skia-integration-tests/src/main/kotlin/org/skia/tests/StrokedLinesGM.kt"
OUTPUT_DIR = ROOT / "reports/wgsl-pipeline/m90-path-aa-strokedlines-evidence-intake"
SUMMARY_JSON = OUTPUT_DIR / "summary.json"
SUMMARY_MD = OUTPUT_DIR / "summary.md"

ROW_ID = "skia-gm-strokedlines"
SOURCE_GM = "StrokedLinesGM"
FALLBACK = "coverage.stroked-lines.row-specific-artifacts-required"
EXPECTED_CPU_ROUTE = "cpu.path.stroked-lines.expected-unsupported"
EXPECTED_GPU_ROUTE = "webgpu.path.stroked-lines.expected-unsupported"
EXPECTED_VISIBILITY_CPU_ROUTE = "cpu.path-coverage.strokedlines.expected-unsupported"
EXPECTED_VISIBILITY_GPU_ROUTE = "webgpu.coverage.strokedlines.expected-unsupported"
EXPECTED_VISIBILITY_PIPELINE_KEY = "pathAA=strokedLines source=StrokedLinesGM visibility=policy-only"
...
REQUIRED_EVIDENCE = [
    ("row-specific Skia reference", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/skia.png"),
    ("CPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/route-cpu.json"),
    ("WebGPU route evidence with fallbackReason=none", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/route-gpu.json"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/cpu.png"),
    ("CPU/GPU rendered artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/gpu.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/cpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/gpu-diff.png"),
    ("CPU/GPU diff/stat artifacts", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/stats.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/cpu-performance.json"),
    ("performance impact evidence", "reports/wgsl-pipeline/scenes/artifacts/skia-gm-strokedlines/gpu-performance.json"),
]
NON_CLAIMS = {
...
def candidate_row(readiness: dict[str, Any]) -> dict[str, Any]:
    ...
    require(candidate.get("promotionTicket") == "M90-PAA-3D", f"{ROW_ID}: candidate ticket changed")
...
def visibility_row(visibility: dict[str, Any]) -> dict[str, Any]:
    ...
    require("feature.line" in scene.get("tags", []), "visibility row missing line tag")
    return scene
...
def historical_signals() -> list[dict[str, Any]]:
    ...
    require("StrokedLinesGM(useDrawPath = true)" in cpu_test, "StrokedLines drawPath test no longer instantiates the GM")
    require("StrokedLinesGM(useDrawPath = false)" in cpu_test, "StrokedLines drawLine test no longer instantiates the GM")
    require('TestReport.recordDetailed("StrokedLinesGM_drawPath", comparison)' in cpu_test, "StrokedLines drawPath report key changed")
    require('TestReport.recordDetailed("StrokedLinesGM_drawLine", comparison)' in cpu_test, "StrokedLines drawLine report key changed")
    require(cpu_test.count("val floor = 68.1") == 2, "StrokedLines historical floors changed or missing")
    require("public class StrokedLinesGM" in gm_source, "StrokedLinesGM source class changed or missing")
    require('if (useDrawPath) "strokedlines" else "strokedlines_drawPoints"' in gm_source, "StrokedLinesGM variant names changed")
    require(load_property_value(CPU_SIMILARITY_PROPERTIES, "StrokedLinesGM_drawLine") == "68.22145061728395", "StrokedLinesGM_drawLine historical score changed or missing")
    require(load_property_value(CPU_SIMILARITY_PROPERTIES, "StrokedLinesGM_drawPath") == "68.22145061728395", "StrokedLinesGM_drawPath historical score changed or missing")
    require(load_property_value(CPU_RASTER_SIMILARITY_PROPERTIES, "StrokedLinesGM_drawLine") == "68.22145061728395", "StrokedLinesGM_drawLine cpu-raster score changed or missing")
    require(load_property_value(CPU_RASTER_SIMILARITY_PROPERTIES, "StrokedLinesGM_drawPath") == "68.22145061728395", "StrokedLinesGM_drawPath cpu-raster score changed or missing")
    require(load_property_value(KANVAS_SKIA_SIMILARITY_PROPERTIES, "StrokedLinesGM_drawLine") == "68.22145061728395", "StrokedLinesGM_drawLine kanvas-skia score changed or missing")
    require(load_property_value(KANVAS_SKIA_SIMILARITY_PROPERTIES, "StrokedLinesGM_drawPath") == "68.22145061728395", "StrokedLinesGM_drawPath kanvas-skia score changed or missing")
    return [
        {
            "kind": "historical-cpu-test",
            "path": rel(HISTORICAL_CPU_TEST),
            "promotional": False,
            "reason": "Historical CPU StrokedLinesGM tests are not a M90 row-specific CPU/WebGPU artifact bundle and do not update the M89 registry row.",
        },
        {
        ...
        {
            "kind": "historical-cpu-similarity-floor-drawLine",
            "path": rel(CPU_SIMILARITY_PROPERTIES),
            "observed": "StrokedLinesGM_drawLine=68.22145061728395",
            "promotional": False,
            "reason": "A historical CPU score is not a support claim; tolerance-only status is not counted as a production missing feature.",
        },
        {
            "kind": "historical-cpu-similarity-floor-drawPath",
            "path": rel(CPU_SIMILARITY_PROPERTIES),
            "observed": "StrokedLinesGM_drawPath=68.22145061728395",
            "promotional": False,
        ...
        },
        {
            ...
            "observed": "StrokedLinesGM_drawLine=68.22145061728395; StrokedLinesGM_drawPath=68.22145061728395",
            "promotional": False,
...
def build_summary(
    ...
    require(all(item["present"] is False for item in required), "M90-PAA-3D intake found unexpected row-specific artifacts")
    signals = historical_signals()
    return {
        "schemaVersion": 1,
        "generatedBy": "scripts/m90_path_aa_strokedlines_evidence_intake.py",
        "milestone": "M90",
        "ticket": "M90-PAA-3D",
        "classification": "path-aa-strokedlines-evidence-intake-no-new-rendering-support",
...
def validate_summary(summary: dict[str, Any]) -> None:
    require(summary.get("classification") == "path-aa-strokedlines-evidence-intake-no-new-rendering-support", "classification mismatch")
...
def main() -> int:
    try:
        ...
        print(f"m90_path_aa_strokedlines_evidence_intake: FAIL: {error}", file=sys.stderr)
        return 1
    print("M90 Path AA StrokedLines evidence intake validation passed: missingEvidenceItems=10 newSupportClaims=0 readinessDelta=0.0")
    return 0
