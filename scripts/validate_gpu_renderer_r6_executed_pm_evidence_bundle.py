...
def build_summary(fields: dict[str, str], rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "retiredRootBundleDependency": False,
        ...
        "nonClaims": [
            ...
            "No retired root bundle dependency on adapter-backed evidence.",
        ...
        "notice": "The executed GPU renderer R6 bundle is adapter-backed diagnostic PM evidence. It proves one opt-in WebGPU first-route submit/readback path for review, but it is not root packaging and not product support activation.",
    }
...
def validate_summary_payload(summary: dict[str, Any], rows: list[dict[str, Any]] | None = None) -> None:
    ...
    require(
        summary.get("retiredRootBundleDependency") is False,
        "summary sidecar retiredRootBundleDependency must be false",
    )
...
def validate_retired_root_bundle(root: Path) -> None:
    build_gradle_path = root / "build.gradle.kts"
    require(build_gradle_path.is_file(), f"missing root build file: {build_gradle_path}")
    retired_task_name = "pipeline" + "PmBundle"
    build_gradle = build_gradle_path.read_text(encoding="utf-8")
    require(retired_task_name not in build_gradle, "retired root PM bundle task is still declared")
...
def main(argv: list[str]) -> int:
    ...
    validate_retired_root_bundle(root)
    ...
    print(
        ...
        f"status={summary['status']}, retiredRootBundleDependency={summary['retiredRootBundleDependency']}"
    )
    return 0
