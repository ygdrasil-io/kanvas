...
/**
 * Kotlin Test version of AaRectModesGM
 * Tests anti-aliased rectangle drawing with different modes
 */
class AaRectModesTest {
    
    @Test
    fun `AaRectModes test should run without errors`() {
        ...
        println("✅ AaRectModes test executed successfully")
        println("   Size: ${size.width}x${size.height}")
        println("   Result: ${result.getWidth()}x${result.getHeight()} bitmap")
    }
    
    @Test
    fun `AaRectModes test should match reference image`() {
        val gm = AaRectModesGM()
        val size = gm.getSize()
        ...
        val referenceImage = TestUtils.loadReferenceImage("aarectmodes")
        
        if (referenceImage != null) {
            // Compare images
            val similarity = TestUtils.compareBitmaps(testResult, referenceImage)
            println("🔍 AaRectModes similarity with Skia reference: ${String.format("%.2f", similarity)}%")
            
            // Track similarity scores over time and fail if similarity drops significantly
            val scoreAcceptable = SimilarityTracker.updateScore("AaRectModesGM", similarity)
            ...
            if (similarity < 80.0) {
                TestUtils.saveDebugImage(testResult, "aarectmodes_test_output")
                println("⚠️  Low similarity - debug image saved")
            }
        } else {
            println("⚠️  No reference image found for aarectmodes")
